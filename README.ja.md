...
**`continuo doctor` が17の検査を通します** — 設定ファイル / 片付けの状態 / **未記入の項目** / claude / **hook の置き場所** / **ロックの場所** / **ボードのロック** / Claude の設定 / worktree の場所 / herdr / gh の認証 / ボード / Status の名前 / 対応表のキー / clone / 信頼登録 / 資格情報（定額プランの枠を読むためのもの）。**OS と Go の版は調べないので、そこは自分で確認してください。**
