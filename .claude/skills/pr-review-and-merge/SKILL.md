...
**そこにあるもの。**指摘1件ごとに「直すか / 直さないか」と「合理的理由」を先に書いて段3のコメントへ入れること、
**掛け直した回数を数えること**、**3・6・9回目に通す6段の手順**、**「収まっている」の定義と、何周回すか**
（どれも [CLAUDE.md](../../../CLAUDE.md) の「回数を数える」にある）。

**そこの表が「そこで終わり」と決めた周のあとで、段5へ進む。それまでは進まない。**
**3・6・9回目で収まらないときは、そこにある6段を通してから次の周を回す。人間の判断は待たない。**
**段5へ進んだら、段6 も飛ばさない。**段6 で `MERGEABLE/CLEAN` が出なければマージしない。
**ただし `MERGEABLE/CLEAN` は、必須の検査が全部通っているかを見るだけである。**
**その1つの `review-result` は、目印が貼ってあるかしか数えない。**
**だから、収まっているかは誰も見ていない。**上の門を通っていることが前提である。
