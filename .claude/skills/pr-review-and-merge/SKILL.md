...
段3のコメントへ入れること、**掛け直した回数を数えること**（上限3回）、
**3回で通らなかったときに入る4段の手順**（目的の確認役・経緯の説明・敵対的レビュー・削除の検討）。

**上限まで回っても、段5と段6は飛ばさない。**3回目で収まらないときは、
CLAUDE.md の「敵対的レビューが判定したあと」の手順を終えてから段5へ進む。**人間の判断は待たない。**
