...
**数え方を自分で書き直してはならない。**数える条件は2箇所の実装が持っていて
...
**2-7 と 2-8 は、受け取った結果を検査する側（このスキルの段4）で効かせる。**

**ただし `/code-review` の出力へ「検査で落とすもの」をそのまま当ててはならない。**
**レビュワーへ 2-7 と 2-8 を渡せていない以上、根拠や分類が無いのはレビュワーの落ち度ではない。**
...
**`--body` に直接書くと、quote の取り違えで中身が黙って変わる。**
**レビュー本文には `` ` `` と `$` が普通に出てくるので、二重引用符で囲むと shell がコマンドの実行と変数の展開として解釈する。**
（単引用符で囲めば展開されないが、**本文に単引用符が1つでもあると、そこで囲みが切れる。**）
...
**[CLAUDE.md](../../../CLAUDE.md) の「コードレビュー記録フロー」に従う。ここには写さない。**
**写すと食い違う。**

**そこにあるもの。**指摘1件ごとに「直すか / 直さないか」と「合理的理由」を先に書いて
段3のコメントへ入れること、**掛け直した回数を数えること**（毎周 issue と実装を突き合わせ直し、連続10回で止まる）、
**毎周通す6段の手順**（[CLAUDE.md](../../../CLAUDE.md) の「回数を数える」にある）。
...
（[CLAUDE.md](../../../CLAUDE.md) の「絶対条件：毎周 issue と実装を突き合わせ直す。連続10回で完全に止まる」）。
**そのとき、Medium と Low は直さない。放置する。**


**収まるまで何回回しても、このスキルの段5と段6は飛ばさない。**収まらないときは、
...
| **`--body` で貼る** | **quote の取り違えで、本文の `` ` `` と `$` を shell が解釈し、貼った中身が変わる。**Write ツールで書き出して `--body-file` を使う |
...
| **対応表を書かずに直す** | **どれをなぜ直さなかったのかが誰にも分からない。**先に表を書いて段3のコメントへ入れる |

## 判定の条件を変えるとき

**同じ判定を2箇所が持っている。**一覧と条件は
