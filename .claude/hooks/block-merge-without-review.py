...
「今実装しているものは特別に PR 作ったらそのままマージして良い」という許可は、
**「ほかの規則も免除された」と読み替えられる。**そう読み替えると、レビューを通さないまま
マージが並び、**リリース前の検査が落として、マージ後にレビューを回し直すことになる。**
...
- `gh pr merge --repo <他所>`。**このリポジトリの PR でなければ見ない**
  （**このリポジトリを指す `--repo` は止める。**止めないと、`--repo <このリポジトリ>` を
  付けるだけでマージが素通しになる）
...
# **`--repo` が付いていれば無条件に素通しすると、
# `gh pr merge <番号> --repo <このリポジトリ>` が hook を通らずにマージされる。**
