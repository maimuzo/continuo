...
| **本番のカンバンへの書き込み**（GitHub Projects v2 の project #3。104件の実データが入っている） | [CLAUDE.md](../../CLAUDE.md) の「絶対に守る制約」の2。とくに `updateProjectV2Field` は Status の値を全部消す |
...
[.claude/hooks/check-reply-clarity.py:73](../hooks/check-reply-clarity.py#L73) が
`MIN_LEN_FOR_CHECK = 200` を置き、
[.claude/hooks/check-reply-clarity.py:635](../hooks/check-reply-clarity.py#L635) が
...
（[.claude/hooks/check-reply-clarity.py:641-645](../hooks/check-reply-clarity.py#L641-L645)）。
...
**同じ返答の中で3回出てきたら、3回とも内容を添える。**
**「さっき書いたから」は通らない。**読む人は表だけを見ることがある。

**そのうえで、3回とも1文字も違わない同じ文字列を添える。**
...
- **同じ誤りが他に無いかを数えていない指摘**（[.claude/skills/worker-briefing/SKILL.md](../skills/worker-briefing/SKILL.md) の 2-5）。**数えた件数と、叩いた検索パターンと範囲と対象コミットを書いていないものは、数えていないものとして扱う。**
