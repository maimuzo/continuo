...
- **2周目以降で、「前の周に既に在ったもの」か「直しが持ち込んだもの」かの分類が無い指摘**（同じ 2-6）。**ただし「前の周の対応表を渡されていないので分類できない。理由は〜」の1行があるものは落とさない**（`/code-review` は前の周のコメントを受け取らない）
- **「直しが持ち込んだ」と分類しながら、前の周の commit でその行が違っていたことを示していない指摘**（同じ 2-6）。**これは落とさない。**「前の周に既に在った」として扱い直す（[CLAUDE.md](../../CLAUDE.md) の対応表の「分類」の列と同じ扱い）。**示せないものは、レビュワーの見落としと区別が付かないためである**
