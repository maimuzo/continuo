...
**読み手は、このプロジェクトを今日はじめて聞いた人である。**
continuo が何をするソフトウェアかも、issue や PR の番号が何を指すかも、
設定のキーが何を変えるものかも、知らない。

**だから、次の1段落を毎回いちから書く。**

> **continuo は、GitHub のカンバン（Projects v2 のボード）1枚を見張り、issue ごとに git worktree を用意して、
> その中で Claude Code を対話モードで起動し、完了までを面倒見る常駐プロセスである。**
...
**合格の線は「前提知識のない人が、書いてあることだけで、こちらの出した問いに全部答えられること」である。**
**自分が読んで分かるかどうかでは測らない。**

**実際に測った（2026-09-02）。**前提知識のない読み手3人に説明を読ませ、15問を出した。
**線には届かなかった。**7問が「文章だけでは答えられない」で、3人の理解度は 42 / 55 / 45 だった。
...
**その続きから書き始めてしまう。**
**下の5段の枠を埋めることと、初見の人に通じることは別である。**枠は埋まっても通じない。
...
**冒頭の「初見の人が分かる形で、問題の定義から書く」を、三行まとめで具体化したものである。**
**「前に説明した」は効かない。同じセッションで何度説明していても、毎回いちから名乗り直す。**

**三行の中に、次の3つを入れる。**

| 何を | 例 |
| --- | --- |
| **主語が何であるか**（1行の定義） | continuo は、GitHub のカンバン1枚を見張り、issue ごとに worktree を用意して Claude Code を起動し、完了まで面倒を見る常駐プロセスです |
| **話題にしているものが、その中の何か** | WORKFLOW.md は、その Claude Code が読む作業手順書で、`continuo init` が最初に1回置きます |
| **決めてほしいことの数と急ぎ順**（あるなら） | 決めてほしいことが3つあります。急ぐのは2つ目です |

実際に書いてしまった悪い例（2026-09-02）。

> **WORKFLOW.md は continuo init が置いたきり、continuo が勝手に書き足すことはありません。**

**continuo が何なのかも、WORKFLOW.md が何なのかも、読み手が知っている前提で書いている。**
...
| **PR #112（1台で continuo を複数動かす）** | PR #112 |
| **1台で複数動かす（PR #112）** | 番号を先に出しても、内容が無ければ同じ |
...
> PR #112（1台で continuo を複数動かす）は、1台のマシンで continuo を2つ動かせるようにする変更です。
...
**冒頭の「読み手をどう仮定するか」と同じ文なので、直すときは両方直す。**
