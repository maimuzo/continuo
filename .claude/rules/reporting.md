...
| 測らずに書いたこと | 測ると分かること |
| --- | --- |
| **「この worktree は消してよいと思います」** | **消すと失われる行とファイルが残っている** |
| **「失われるのは3ファイルだけ」** | **行数で数えると、見積もりと桁が違う** |
| **「このテストは flaky でした」** | **再実行で通っただけで、条件が揃えば必ず起きる競合である** |
| **「マージした」** | **`gh pr merge` が受け付けただけで、マージはされていない** |
| **「送信が途中で止まる」** | **大きな入力でも通り、実害が無い** |
...
**判断票は7つで書く。**目印を先頭に置き、引用はその下から始め、対応表は `### 詳細` の中に置く。
...
| 3 | **既存の構造。**いまどう作られていて、何がどの順で起きるか。シーケンス図で、具体的な値を載せて描く |
...
- **自分で訳語を作らない。**英語の技術用語は英語のまま書く（上の「技術用語は英語のまま書く」が正である）
