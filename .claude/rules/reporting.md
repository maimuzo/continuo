...
## 絶対条件：issue と PR は、番号だけで書かない

**24時間で60回やり直しになり、うち39回がこれである**（指摘は143件）。**いちばん多い。**

### 守る形

**`#NNN（内容）` を、1つの塊として扱う。**番号だけを書ける場面は無い。
...
**表の1列目に番号だけを書いてしまう。**列が狭いので短く書きたくなる。

**対策。****表の1列目には番号を書かない。**中身の名前を書き、番号は同じセルに括弧で添える。
...
**送る前に、本文から `#` で始まる番号を全部探す。**
**括弧が付いていないものが1つでもあれば、やり直しになる。**
