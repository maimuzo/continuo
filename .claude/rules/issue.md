...
（載せることと Status は [docs/plans/continuo_design.md:9070](../../docs/plans/continuo_design.md#L9071)、
並べ替えは [docs/plans/continuo_design.md:9257-9261](../../docs/plans/continuo_design.md#L9258-L9262) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:9076](../../docs/plans/continuo_design.md#L9077)）。
**組み込みの指示書は、それを勧めてはいない**
（[internal/prompt/builtin.md:326](../../internal/prompt/builtin.md#L326) は「あなたが `gh` を叩く必要はありません」）。
そちらは応答の最後に `CONTINUO-STATUS:` の1行を書くだけで、Status を動かすのは continuo である。

**カンバンの操作は AI が行う。**ただし 4-1 の遷移表で「誰が」の欄が「人間」だけの3つは人間である
（[docs/plans/continuo_design.md:9068-9080](../../docs/plans/continuo_design.md#L9069-L9081)）。
...
（[docs/plans/continuo_design.md:9071](../../docs/plans/continuo_design.md#L9072) の 4-1 の遷移表）。
**AI は、人間に名指しで頼まれない限り、ここを代行しない。**理由は3つある。

| 何が | なぜ |
| --- | --- |
| **利用者への約束である** | [SECURITY.md:52](../../SECURITY.md#L52) が「**`Ready` へ動かすのは人間です。知らない issue を動かさないでください**」を、公開された安全対策として書いている。**AI が上げられるようにすると、この約束が空文になる** |
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9114](../../docs/plans/continuo_design.md#L9115) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:9116](../../docs/plans/continuo_design.md#L9117)）。104件の全並べ替えで約2分かかる |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9072](../../docs/plans/continuo_design.md#L9073) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9116](../../docs/plans/continuo_design.md#L9117)）。
...
組み込みの指示書（[internal/prompt/builtin.md:679-694](../../internal/prompt/builtin.md#L679-L694) の 7-2）が、
...
（[internal/config/types.go:319-320](../../internal/config/types.go#L319-L320)）、
