...
（載せることと Status は [docs/plans/continuo_design.md:9295](../../docs/plans/continuo_design.md#L9295)、
並べ替えは [docs/plans/continuo_design.md:9482-9486](../../docs/plans/continuo_design.md#L9482-L9486) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:9301](../../docs/plans/continuo_design.md#L9301)）。
**組み込みの指示書は、それを勧めてはいない**
（[internal/prompt/builtin.md:301](../../internal/prompt/builtin.md#L301) は「あなたが `gh` を叩く必要はありません」）。
そちらは応答の最後に `CONTINUO-STATUS:` の1行を書くだけで、Status を動かすのは continuo である。

**カンバンの操作は AI が行う。**ただし 4-1 の遷移表で「誰が」の欄が「人間」だけの3つは人間である
（[docs/plans/continuo_design.md:9293-9305](../../docs/plans/continuo_design.md#L9293-L9305)）。
...
`Ready` と `In Progress` の2つで（[internal/config/default.go:153](../../internal/config/default.go#L153)）、
**`Ice Box` は入っていない。**段3 で issue を `Ice Box` へ置く以上、上げる段が要る。
**上げるのは人間で、GitHub の画面から行う**
（[docs/plans/continuo_design.md:9296](../../docs/plans/continuo_design.md#L9296) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9339](../../docs/plans/continuo_design.md#L9339) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:9341](../../docs/plans/continuo_design.md#L9341)）。104件の全並べ替えで約2分かかる |
...
| **グループごとの計画** | **そのグループの代表の issue のコメントへ残す**（[docs/plans/continuo_design.md:3581](../../docs/plans/continuo_design.md#L3581) の 3-26） |
...
（[internal/config/default.go:153](../../internal/config/default.go#L153)）、**continuo はこの issue を dispatch しない。**
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9297](../../docs/plans/continuo_design.md#L9297) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9341](../../docs/plans/continuo_design.md#L9341)）。
...
（`Ready` と `In Progress`。[internal/config/default.go:153](../../internal/config/default.go#L153)）に入っていないためである。
...
組み込みの指示書（[internal/prompt/builtin.md:703-718](../../internal/prompt/builtin.md#L703-L718) の 7-2）が、
