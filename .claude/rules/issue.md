...
（載せることと Status は [docs/plans/continuo_design.md:9307](../../docs/plans/continuo_design.md#L9307)、
並べ替えは [docs/plans/continuo_design.md:9494-9498](../../docs/plans/continuo_design.md#L9494-L9498) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:9313](../../docs/plans/continuo_design.md#L9313)）。
...
（[docs/plans/continuo_design.md:9305-9317](../../docs/plans/continuo_design.md#L9305-L9317)）。
...
（[docs/plans/continuo_design.md:9308](../../docs/plans/continuo_design.md#L9308) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9351](../../docs/plans/continuo_design.md#L9351) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:9353](../../docs/plans/continuo_design.md#L9353)）。104件の全並べ替えで約2分かかる |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9309](../../docs/plans/continuo_design.md#L9309) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9353](../../docs/plans/continuo_design.md#L9353)）。
...
組み込みの指示書（[internal/prompt/builtin.md:719-734](../../internal/prompt/builtin.md#L719-L734) の 7-2）が、
