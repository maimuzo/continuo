...
（載せることと Status は [docs/plans/continuo_design.md:8722](../../docs/plans/continuo_design.md#L8722)、
並べ替えは [docs/plans/continuo_design.md:8907-8914](../../docs/plans/continuo_design.md#L8907-L8914) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:8728](../../docs/plans/continuo_design.md#L8728)）。
...
（[docs/plans/continuo_design.md:8720-8732](../../docs/plans/continuo_design.md#L8720-L8732)）。
...
（[docs/plans/continuo_design.md:8723](../../docs/plans/continuo_design.md#L8723) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:8766](../../docs/plans/continuo_design.md#L8766) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:3934](../../docs/plans/continuo_design.md#L3934)）。104件の全並べ替えで約2分かかる |
...
| **動かすのは `Ice Box` の item だけにする** | **並び順は project 全体で1本しかない**（[docs/plans/continuo_design.md:8799](../../docs/plans/continuo_design.md#L8799)）。「先頭へ送る」はボード全体の先頭へ送る。**`Ready` や `In Progress` の item を動かすと、走っている continuo が次に dispatch する issue が変わる**（[internal/orchestrator/dispatch.go:442-443](../../internal/orchestrator/dispatch.go#L442-L443) が「返ってきた配列の順序をそのまま使う」と書いている。**同じ行のコメントは「並び順を決めるのは人間である」と続くが、それは 3-30 の旧い見出しのままで、正は [docs/plans/continuo_design.md:3874-3875](../../docs/plans/continuo_design.md#L3874-L3875) の本文である**） |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:8724](../../docs/plans/continuo_design.md#L8724) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:3934](../../docs/plans/continuo_design.md#L3934)）。
