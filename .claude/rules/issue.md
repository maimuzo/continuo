...
**issue を作ることと、その issue をすぐ進めることは違う。**
**優先順位を計画して人間の確認を受けてから着手する。**
...
（載せることと Status は [docs/plans/continuo_design.md:9145](../../docs/plans/continuo_design.md#L9145)、
並べ替えは [docs/plans/continuo_design.md:9332-9336](../../docs/plans/continuo_design.md#L9332-L9336) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:9151](../../docs/plans/continuo_design.md#L9151)）。
**組み込みの指示書は、それを勧めてはいない**
（[internal/prompt/builtin.md:314](../../internal/prompt/builtin.md#L314) は「あなたが `gh` を叩く必要はありません」）。
そちらは応答の最後に `CONTINUO-STATUS:` の1行を書くだけで、Status を動かすのは continuo である。

**カンバンの操作は AI が行う。**ただし 4-1 の遷移表で「誰が」の欄が「人間」だけの3つは人間である
（[docs/plans/continuo_design.md:9143-9155](../../docs/plans/continuo_design.md#L9143-L9155)）。
...
（[docs/plans/continuo_design.md:9146](../../docs/plans/continuo_design.md#L9146) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9189](../../docs/plans/continuo_design.md#L9189) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:9191](../../docs/plans/continuo_design.md#L9191)）。104件の全並べ替えで約2分かかる |
...
| **グループごとの計画** | **そのグループの代表の issue のコメントへ残す**（[docs/plans/continuo_design.md:3638](../../docs/plans/continuo_design.md#L3638) の 3-26） |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9147](../../docs/plans/continuo_design.md#L9147) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
[docs/plans/continuo_design.md:3661-3662](../../docs/plans/continuo_design.md#L3661-L3662) の 3-26 が
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9191](../../docs/plans/continuo_design.md#L9191)）。
...
組み込みの指示書（[internal/prompt/builtin.md:681-684](../../internal/prompt/builtin.md#L681-L684) の 7-2）が、
