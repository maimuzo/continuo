...
（載せることと Status は [docs/plans/continuo_design.md:9217](../../docs/plans/continuo_design.md#L9217)、
並べ替えは [docs/plans/continuo_design.md:9404-9408](../../docs/plans/continuo_design.md#L9404-L9408) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:9223](../../docs/plans/continuo_design.md#L9223)）。
...
（[docs/plans/continuo_design.md:9215-9227](../../docs/plans/continuo_design.md#L9215-L9227)）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9261](../../docs/plans/continuo_design.md#L9261) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:9263](../../docs/plans/continuo_design.md#L9263)）。104件の全並べ替えで約2分かかる |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9219](../../docs/plans/continuo_design.md#L9219) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9263](../../docs/plans/continuo_design.md#L9263)）。
