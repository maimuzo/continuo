...
（載せることと Status は [docs/plans/continuo_design.md:9071](../../docs/plans/continuo_design.md#L9071)、
並べ替えは [docs/plans/continuo_design.md:9258-9262](../../docs/plans/continuo_design.md#L9258-L9262) の 4-4 の表に
...
（[internal/prompt/builtin.md:431](../../internal/prompt/builtin.md#L431) は「あなたが `gh` を叩く必要はありません」）。
そちらは応答の最後に `CONTINUO-STATUS:` の1行を書くだけで、Status を動かすのは continuo である。

**カンバンの操作は AI が行う。**ただし 4-1 の遷移表で「誰が」の欄が「人間」だけの3つは人間である
（[docs/plans/continuo_design.md:9069-9081](../../docs/plans/continuo_design.md#L9069-L9081)）。
...
（[docs/plans/continuo_design.md:9072](../../docs/plans/continuo_design.md#L9072) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9115](../../docs/plans/continuo_design.md#L9115) の 4-2）。
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9073](../../docs/plans/continuo_design.md#L9073) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9117](../../docs/plans/continuo_design.md#L9117)）。
...
組み込みの指示書（[internal/prompt/builtin.md:1138-1160](../../internal/prompt/builtin.md#L1138-L1160) の 7-2）が、
