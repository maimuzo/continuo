...
（載せることと Status は [docs/plans/continuo_design.md:8834](../../docs/plans/continuo_design.md#L8834)、
並べ替えは [docs/plans/continuo_design.md:9019-9026](../../docs/plans/continuo_design.md#L9019-L9026) の 4-4 の表に
「continuo が起動したエージェント」の行が無いこと）。
**ただし「1バイトも触らない」ではない。**設計は、そのエージェントが自分で `gh` を叩いて
`In Progress` → `Blocked` を動かす経路を認めている
（[docs/plans/continuo_design.md:8840](../../docs/plans/continuo_design.md#L8840)）。
...
（[docs/plans/continuo_design.md:8832-8844](../../docs/plans/continuo_design.md#L8832-L8844)）。
...
（[docs/plans/continuo_design.md:8835](../../docs/plans/continuo_design.md#L8835) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:8878](../../docs/plans/continuo_design.md#L8878) の 4-2）。
...
| 何を | なぜ |
| --- | --- |
| **書き込みの間は1秒空ける** | GitHub が変更を伴うリクエストに求めている（[docs/plans/continuo_design.md:4039](../../docs/plans/continuo_design.md#L4039)）。104件の全並べ替えで約2分かかる |
...
| **動かすのは `Ice Box` の item だけにする** | **並び順は project 全体で1本しかない**（[docs/plans/continuo_design.md:8911](../../docs/plans/continuo_design.md#L8911)）。「先頭へ送る」はボード全体の先頭へ送る。**`Ready` や `In Progress` の item を動かすと、走っている continuo が次に dispatch する issue が変わる**（[internal/orchestrator/dispatch.go:361-365](../../internal/orchestrator/dispatch.go#L361-L365) が「返ってきた配列の順序をそのまま使う」と書いている。**同じ行のコメントは「並び順を決めるのは人間である」と続くが、それは 3-30 の旧い見出しのままで、正は [docs/plans/continuo_design.md:3979-3980](../../docs/plans/continuo_design.md#L3979-L3980) の本文である**） |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:8836](../../docs/plans/continuo_design.md#L8836) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:4039](../../docs/plans/continuo_design.md#L4039)）。
