...
（載せることと Status は [docs/plans/continuo_design.md:9066](../../docs/plans/continuo_design.md#L9066)、
並べ替えは [docs/plans/continuo_design.md:9253-9257](../../docs/plans/continuo_design.md#L9253-L9257) の 4-4 の表に
...
（[docs/plans/continuo_design.md:9064-9076](../../docs/plans/continuo_design.md#L9064-L9076)）。
...
（[docs/plans/continuo_design.md:9067](../../docs/plans/continuo_design.md#L9067) の 4-1 の遷移表）。
...
**段4 の並び順は、`updateProjectV2ItemPosition` で動かす**（[docs/plans/continuo_design.md:9110](../../docs/plans/continuo_design.md#L9110) の 4-2）。
...
**この書き込みは worker へ渡さない**（[CLAUDE.md:328](../../CLAUDE.md#L328) の「**不可逆な操作**…**は worker に渡さない**」）。**メインエージェントが自分で叩く。**
...
| **`updateProjectV2Field` は絶対に呼ばない** | [CLAUDE.md](../../CLAUDE.md) の「GitHub Projects v2 の project #3 は本番のボードである」。**Status の値が全部消える** |
...
| **3** | **AI** | **グループの代表以外のうち、`Ready` か `In Progress` に在るものを `Ice Box` へ落とす**（[docs/plans/continuo_design.md:9068](../../docs/plans/continuo_design.md#L9068) の 4-1 の遷移表）。`updateProjectV2ItemFieldValue` を叩く |
...
**書き込みの間は1秒空ける**（[docs/plans/continuo_design.md:9112](../../docs/plans/continuo_design.md#L9112)）。
