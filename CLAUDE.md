...
- **消す前に、commit していない変更・未マージの commit・開いている PR・走っている作業の4つを確かめる**
...
[.claude/rules/reporting.md](.claude/rules/reporting.md) に従うこと。とくに次の9点。
...
- **話題ごとに題名つきの節を立て、節ごとに引用を置く。chat の返答は、節の中を引用 → 三行まとめ → 何が言いたいのか → 結果 → 詳細の順で書き（話題を変えるときは `-----` で区切る）、mermaid を使わない。issue と PR のコメントは、節の中を7つの見出しで書く**
...
- **質問を表で並べない。**1つの質問につき1つの節を立て、前提・単語の説明・既存の構造・何が問題なのかを書いてから訊く
...
**急ぎで通したいときは、`enforce_admins` を一時的に戻すしかない。**
**その操作は人間が行う。AI が自分で戻してはならない。**
...
**Critical と High が潰れているのに、Medium と Low の修正のたびに再レビューするのは、時間とコストの無駄である。**

**避けたいのは、次の形である。**2周目で Critical も High も0件になったのに、
**そこから4周ぶん、Medium と Low のためだけに回している。**
