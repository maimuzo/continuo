...
- **ボードの操作は AI が行う。**ボードへ載せて `Ice Box` を付けるのも、**代表以外を代表の sub-issue にする**のも、**並び順を着手順序へ並べ替える**のも AI である。**人間がやるのは `Ice Box` から `Ready` へ上げることだけ**（人間が名指しで依頼したときだけ AI が代行してよい）**。****代表以外の Status を外してはならない**（未設定の item は continuo から見えなくなり、グループの表明が1件も通らない）
