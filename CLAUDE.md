...
### 数えた件数の表（判定の表とは別に、もう1つ作る）

**列を6つにしない**（[.claude/rules/reporting.md](.claude/rules/reporting.md) の「列数を増やしすぎない。列が多い場合は表を分割する」）。
**判定の表の下に、同じ短縮名でもう1つ表を置く。**

| 列 | 中身 |
| --- | --- |
| **短縮名** | 判定の表と同じもの |
| **何件あったか** | 1件なら「1件だけ」。**文字列で数えられないものは、その理由を1行**（[.claude/skills/worker-briefing/SKILL.md](.claude/skills/worker-briefing/SKILL.md) の 2-5 の段3） |
| **叩いた検索パターンと範囲** | **範囲は絶対パスで書く**（別の checkout を数えると0件と1件に割れる） |
...
| 順 | 何をするか |
| --- | --- |
| **1** | **対になる issue を読み、何を求めているかを取り出す役を立てる。**その役には **diff を見せない**（下の「3回で通らなかったとき」の段1）。**実装した内容は、実装担当者が説明する**（同じ4段の段2） |
| **2** | **issue に無い内容が入っているなら、それが必要となる合理的理由をまとめる。**1件も無ければ段3と段4を飛ばし、判定は「いる」として段6へ進む |
| **3** | **その理由で敵対的レビュワーを説得する** |
| **4** | **説得できなかった内容は削除する。****削除する内容を、対になる issue のコメントへまとめてから消す** |
...
**別の手順ではない。**段1から段3が4段の段1から段4に当たる。
**段4は「敵対的レビューが判定したあと」の「いらない」の行の1と2を先取りしたものである。**
**段5と段6がそのあとの始末である。**
...
**下の「3回で通らなかったとき」の4段は、上の6段の段1から段4そのものである。**
**そこへ入っても、この決まりは消えない。**
...
**入るのは3回目だけではない。6回目・9回目でも同じ段へ入る**（上の6段の段1から段4がこれである）。
