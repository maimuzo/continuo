...
- **レビューは最大3回。**3回で収まらなければ、下の「3回で通らなかったとき」へ入る。**どんな理由があっても、掛け直しは10回まで。10回で収まらなければ止まって人間に方針を確認する**
...
**4段のあとに再度レビューを回すので、その回数も同じ勘定に入れる。**

| 何回目 | どうするか |
| --- | --- |
| 3回目まで | 上の表のとおり |
| **4〜10回目** | **「3回で通らなかったとき」の4段を通し、設計から直して回し直す** |
| **10回目で収まらなかったら** | **止まる。人間に方針を確認する。**そこから先は、人間が決めるまで1回も回さない |
...
**数えるのは「1つの issue に対する作業」の単位である。**
**種類を分けない。**設計レビュー・code-review・敵対的レビューを、同じ勘定に入れる。

**なぜ分けないか。**人間が止めたいのは「同じところを何度も回している」ことであって、
**回している道具の種類ではない。**分けると、設計で5回・コードで5回して「どちらも上限以内」になる。

**pull request が立つ前の設計レビューも数える。**
**その回数を、pull request を作ったときの本文か、最初のレビュー結果のコメントへ書き写す。**
**書き写さないと、pull request から先だけを見た人が数え直せない。**

**別の issue へ切り出したら、そちらは1回目から数え直す。**
**同じ issue のまま pull request を作り直した場合は、数え直さない。**
