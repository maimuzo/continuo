...
4. **指摘に対応する**（下の「コードレビュー記録フロー」に従う）。
   ...
   **その周の結果も、段3 のとおりに貼る**
5. **収まっていることを確かめてから、`gh pr ready` で draft を外す**（「収まっている」の定義は下の「回数を数える」）
...
**収まっていて、最後の1周も終えていたら、`gh pr ready <番号>` を打つ。**
`ready_for_review` が飛んで CI の検査が回り直し、緑になる。
**収まっていないうちは打たない**（上の段5。何をもって収まったとするかは、下の「回数を数える」）。
...
| **Critical か High が1件以上** | それ以外 | **直して次を回す。**人間に訊かない |
| **収まった。この周では何も直さない**（指摘が0件のときもここ） | どの回でも | **そこで終わり** |
| **収まった。この周で Medium か Low を直した** | **10回目** | **そこで終わり。11回目は無い** |
| **収まった。この周で Medium か Low を直した** | それ以外 | **最後に1回だけ回す。**その周も10回に数える |
...
PR #215（ボードの操作を AI が行う形へ規則と設計を直す（並び順の段は、そもそも書かれていなかった））では、
