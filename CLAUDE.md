...
- **3回ごとに人間へ相談する。**3・6・9回目で収まらなければ、下の「3回で通らなかったとき」の4段を通し、その結果を持って人間へ相談して返事を待つ。**設計レビューと実装レビューをそれぞれ別に数え、どちらかが連続10回に達したら完全に止まる。人間が方針を変えたら数え直す**
...
| **3回目のあと** | **収まっていれば進む。収まっていなければ、下の4段を通したうえで人間へ相談し、返事を待つ** |
| 4・5回目のあと | **直して次を回す。**人間に訊かない |
| **6回目のあと** | **3回目と同じ。収まっていなければ人間へ相談し、返事を待つ** |
| 7・8回目のあと | **直して次を回す。**人間に訊かない |
| **9回目のあと** | **3回目と同じ。収まっていなければ人間へ相談し、返事を待つ** |
| **10回目** | **上限。**下の絶対条件を見よ |

**回数そのものが目的ではない。**人間が見たいのは「方向がずれていないか」である。
**それでも数える。**書いていないと、ずれ始めた地点が分からず、次のセッションが数え直せない。

### 絶対条件：3回ごとに人間へ相談する。連続10回で完全に止まる

**設計レビューと実装レビューを、それぞれ別に数える。**
**3回・6回・9回で収まらなかったら、そのつど止めて人間に方針を確認する。**
**返事を待たずに4回目・7回目・10回目を回してはならない。**
**どちらかが連続10回に達したら、そこで完全に止める。**
**人間が方針を変えたら、そこから数え直す。**
...
**3の倍数で1回ずつ止まる。**そこで相談していれば、方向がずれたまま6回も回すことは起きない。
...
| **3・6・9回目のあと** | **収まっていれば進む。****収まっていなければ、「3回で通らなかったとき」の4段を通し、その結果を持って人間へ相談する。返事を待つ** |
...
| 何を数えるか | 相談する回 | 完全に止まる回 |
| --- | --- | --- |
| **設計レビューの連続した回数** | **3・6・9回目** | **10回** |
| **実装レビューの連続した回数** | **3・6・9回目** | **10回** |

**足して20回まで、という意味ではない。**
**どちらか一方が3・6・9回に達した時点で相談し、10回に達した時点で完全に止まる。**
