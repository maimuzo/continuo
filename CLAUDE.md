...
- **Critical と High が0件になったら、そこから先は最大1周で終える**（何周で終えるかは、下の「回数を数える」が決める）
...
**Critical と High が0件になったら、そこから先は最大1周で終える。**
...
**Medium と Low と Info が何件あっても、収まっている。**

**この定義はここにしか書かない。**他の文書は、この節を指すこと。写すと食い違う。

**収まったあと、何周回すか。**

**Critical と High が0件になったら、そこから先は最大1周である。**
**この表は、実装レビューにも設計レビューにも同じく当たる。**
**別なのは、数える単位が2つに分かれていることと、終わったあとに進む先だけである**（単位は下の「設計レビューと実装レビューは、別々に数える」）。

| 状態 | 次に何をするか |
| --- | --- |
| **Critical か High が1件以上** | **直して次の周を回す。**回数の数え方と、止まる回は、下の「収まらないまま回すときの数え方」に従う |
| **収まった。Medium と Low と Info を直さないと決めた** | **そこで終わり。**下の「終わったあと、どこへ進むか」へ |
| **収まった。Medium か Low か Info を直した** | **最後に1回だけ回す** |
| **その最後の周でも収まっている** | **指摘があっても、もう直さない。**下の「終わったあと、どこへ進むか」へ |
| **その最後の周で Critical か High が出た** | **直して、また回す。**この表の1行目へ戻る。**その周が3・6・9回目に当たるなら、先に下の6段を通す** |

**終わったあと、どこへ進むか。**

| どのレビューか | 次にどこへ |
| --- | --- |
| **実装レビュー** | [.claude/skills/pr-review-and-merge/SKILL.md](.claude/skills/pr-review-and-merge/SKILL.md) の段5（検査を回し直す） |
| **設計レビュー** | [.claude/rules/design-review.md](.claude/rules/design-review.md) の段5（実装する） |

**どちらの場合も、最後の周のレビュー結果と対応表を残してから進む。**
**残す先は、実装レビューなら pull request のコメント、設計レビューなら issue のコメントである**
（上の「手順」の段2 と、[.claude/rules/design-review.md](.claude/rules/design-review.md) の段2）。

**設計レビューの結果に `<!-- code-review-result -->` の目印を付けてはならない。**
**あの目印は、pull request の CI が「コードのレビューを通した」と数えるためのものである。**
**設計だけを見たレビューで付けると、コードを1行も見ていないのに関門が開く。**

**この表は、下の「収まらないまま回すときの数え方」が決めた10回の上限の中で回る。**
**10回目で初めて収まったなら、そこで終わる。**「最後に1回だけ回す」は使わない。

**Medium と Low と Info を直すかどうかの判断は、上の「Critical と High は直す」に従う。**
**ただし最後の周では、この表の「もう直さない」が優先する。**
**そのとき対応表の「合理的理由」の欄には、「最後の周なので直さない」と書けばよい。**
...
**毎回、まず収まっているかを見る。**
**収まっていたら、何回目であっても上の「収まったあと、何周回すか」へ行く。**
**下の表は、収まっていないときだけ効く。**

| 回 | どうするか |
| --- | --- |
| 1・2・4・5・7・8回目のあと | **直して次を回す。**人間に訊かない |
| **3・6・9回目のあと** | **下の6段を通す** |
| **10回目のあと** | **止まる。**下の絶対条件を見よ |

**収まったあとの「最後の1回」も、この10回に数える。**
**10回目で初めて収まったなら、Medium と Low と Info を直さずにそこで終える**（11回目は無い）**。**
**そのときも、上の「終わったあと、どこへ進むか」へ行く。**
...
| **3・6・9回目のあと** | **上の6段を通す。**判定は敵対的レビュワーが行い、人間へは経緯を報告する |
...
| 何を | 中身 |
| --- | --- |
| **何回回したか** | 各回の critical / high / medium / low / info の件数 |
