...
| 順 | 何をするか |
| --- | --- |
| **1** | **対になる issue を読み、何を求めているかを取り出す役を立てる。**その役には **diff を見せない**（下の「収まらなかったとき」の段1）。**実装した内容は、実装担当者が敵対的レビュワーへ説明する**（同じ4段の段2）。**対になる issue が無いときは、人間がチャットで出した指示の原文を issue の代わりに使う。**原文が残っていなければ、そのことを段5の報告に書く |
...
| **6** | **段4で削除が起きた周だけ、下の「敵対的レビューが判定したあと」の表の「実装を止める」以降へ進む。**実装を止めて設計内容を敵対的レビューし、実装し直してから次のレビューを回す。**削除と issue への記録は段4で済んでいるので、繰り返さない。****削除が1件も無かった周は、設計へ戻らずにそのまま直す** |

**この6段は、下の「収まらなかったとき」の4段の前半に、削除の実行・記録・人間への報告・設計レビューへの復帰を足したものである。**
...
**下の「収まらなかったとき」の4段へ入っても、この決まりは消えない。**
...
| --- | --- |
| **1〜9回目のあと** | **上の6段を通す**（収まっていれば、そもそもこの表を読まない）。判定は敵対的レビュワーが行い、人間へは経緯を報告する |
...
| --- | --- | --- |
| **設計レビューの連続した回数** | **毎周** | **10回** |
| **実装レビューの連続した回数** | **毎周** | **10回** |
