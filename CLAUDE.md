...
- **3回ごとに issue と実装を突き合わせ直す。**3・6・9回目で収まらなければ、issue に無い内容の合理的理由を敵対的レビュワーへ説明し、説得できないものを削除して次のレビューを回す。**経緯は人間へ報告するが、返事は待たない。設計レビューと実装レビューをそれぞれ別に数え、どちらかが連続10回に達したら完全に止まる**
...
| **3** | **表のとおりに直す。**表に無いものを直さない。**直す前に、同じ誤りが他に無いかを数える**（[.claude/skills/worker-briefing/SKILL.md](.claude/skills/worker-briefing/SKILL.md) の 2-5。worker だけでなく、自分で直すときも通す） |
...
| **数えた件数** | **同じ誤りが他に無いかを数えた結果。**叩いた検索パターンと範囲も書く。1件なら「1件だけ」。文字列で数えられないものは、その理由を1行（[.claude/skills/worker-briefing/SKILL.md](.claude/skills/worker-briefing/SKILL.md) の 2-5） |
...
| 順 | 何をするか |
| --- | --- |
| **1** | **issue に書いてある内容と、実装した内容を並べて比べる。**比べる役には **diff を見せない**（下の「3回で通らなかったとき」の段1） |
| **2** | **issue に無い内容が入っているなら、それが必要となる合理的理由をまとめる** |
| **3** | **その理由で敵対的レビュワーを説得する** |
| **4** | **説得できなかった内容は削除する。****削除する内容を、対になる issue のコメントへまとめてから消す** |
| **5** | **この経緯を人間に報告する。返答は待たない** |
| **6** | **下の「敵対的レビューが判定したあと」の表のとおりに進む。**いる / いらないのどちらでも、実装を止めて設計内容を敵対的レビューし、実装し直してから次のレビューを回す |

**この6段は、下の「3回で通らなかったとき」の4段を、3・6・9回目に必ず通す形にしたものである。**
**別の手順ではない。**段1から段4が4段そのもので、段5と段6がそのあとの始末である。
...
**どちらかが連続10回に達したら、そこで完全に止めて人間の返事を待つ。**
**人間が方針を変えたら、そこから数え直す。**
...
**足して20回まで、という意味ではない。**
**どちらか一方が3・6・9回に達した時点で6段を通し、10回に達した時点で完全に止まる。**
