...
4. **指摘に対応する**（下の「コードレビュー記録フロー」に従う）。
   **収まったあと Medium か Low を直したなら、最後に1回だけ回す**（下の「収まったあと、何周回すか」）。
   **その周の結果も、段3 のとおりに貼る**
5. **収まっていることを確かめてから、`gh pr ready` で draft を外す**（Critical と High が0件。定義は下の「回数を数える」）
...
**すぐ下の表は、実装レビューにも設計レビューにも同じく当たる。**
**別なのは、数える単位が2つに分かれていることだけである**（下の「設計レビューと実装レビューは、別々に数える」）。
...
**上限は、この表のどの行よりも優先する。**10回目で Critical か High が出ても、11回目は無い。
...
**毎回、まず収まっているかを見る。収まっていたら、上の「収まったあと、何周回すか」へ行く。**
**収まっていなければ、そのとき下の表を見る。**

| 回 | どうするか |
| --- | --- |
| 1・2・4・5・7・8回目のあと | **直して次を回す。**人間に訊かない |
| **3・6・9回目のあと** | **収まっていないので、下の6段を通す** |
| **10回目のあと** | **収まっていないので、止まる。**下の絶対条件を見よ |

**収まったあとの「最後の1回」も、この10回に数える。**
**10回目で初めて収まったなら、Medium と Low を直さずにそこで終える。11回目は無い。**
...
**何回目で何をするかは、上の「収まらないまま回すときの数え方」にある。ここには写さない。写すと食い違う。**

**この節が足すのは、誰が判定するかだけである。**
