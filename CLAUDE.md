...
- **ボードの操作は AI が行う。**ボードへ載せて代表に `Ice Box` を付けるのも、**代表以外の Status を外す**のも、**`Ice Box` の並び順を着手順序へ並べ替える**のも AI である。**人間がやるのは `Ice Box` から `Ready` へ上げることだけ**（外さないと continuo が代表とは別に dispatch する）
