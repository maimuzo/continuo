...
4. **指摘に対応する**（下の「コードレビュー記録フロー」に従う）。
   **収まったあと Medium か Low を直したなら、最後に1回だけ回す**（下の「何周回すか」）。
   **その周の結果も、段3 のとおりに貼る**
5. **下の「何周回すか」の表が「そこで終わり」と言う状態になってから、`gh pr ready` で draft を外す**
...
**下の「何周回すか」の表が「そこで終わり」と言う状態になったら、`gh pr ready <番号>` を打つ。**
`ready_for_review` が飛んで CI の検査が回り直し、緑になる。
**表がそう言うまでは打たない**（上の段5）。
...
**毎周、この表を1回見る。どの周も、当てはまる行はちょうど1つである。**
**実装レビューにも設計レビューにも同じく当たる**（数える単位が2つに分かれていることだけが別。下の「絶対条件：3回ごとに issue と実装を突き合わせ直す。連続10回で完全に止まる」にある）。

| この周の結果 | 何回目か | 次に何をするか |
| --- | --- | --- |
| **Critical か High が1件以上** | **3・6・9回目** | **下の6段を通す。**次に何をするかは、その段6 が決める |
| **Critical か High が1件以上** | **10回目** | **止まる。人間に方針を確認する** |
| **Critical か High が1件以上** | **1・2・4・5・7・8回目** | **直して次を回す。**人間に訊かない |
| **収まった。この周では何も直さない**（指摘が0件のときもここ） | どの回でも | **そこで終わり** |
| **収まった。この周で Medium か Low を直した** | **10回目** | **そこで終わり。11回目は無い** |
| **収まった。この周で Medium か Low を直した** | **1〜9回目** | **最後に1回だけ回す。**その周も10回に数える |

**最後の1回でも収まっていれば、指摘があってももう直さない。**
そのとき何も直さないので、上の表の「この周では何も直さない」に当たる。

**収まったまま終わるなら、3・6・9回目でも6段は通らない。**
**6段へ行くのは、表の1行目（収まっておらず、かつ3・6・9回目のとき）だけである。**

**Medium と Low を直すかどうかの判断は、上の「対応表の列」の下にある「Critical と High は直す」の2行に従う。**
**ただし最後の周では、この表の「この周では何も直さない」が優先する。**
...
PR #215（ボードの操作を AI が行う形へ規則と設計を直す（並び順の段は、そもそも書かれていなかった））
——対になる issue は無い——では、
**8周目の Critical が7周目の直しから生まれた。**
...
**返事を待って止まるのは、連続10回で収まらなかったときだけである。**
...
**何回目で何をするかは、上の「何周回すか」にある。ここには写さない。写すと食い違う。**

**3・6・9回目に通す6段では、判定は敵対的レビュワーが行い、人間へは経緯を報告する。**
**10回目で止まったときは、人間に方針を確認する。そこから先は、人間が決めるまで1回も回さない。**
...
| --- | --- | --- |
| **設計レビューの連続した回数** | **3・6・9回目** | **10回**（収まらなかったとき） |
| **実装レビューの連続した回数** | **3・6・9回目** | **10回**（収まらなかったとき） |
