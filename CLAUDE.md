...
- **Critical と High が0件になったら、そこから先は最大1周で終える。**Medium と Low を直さないなら、そこで終わり。直したなら、最後に1回だけ回す。**その最後の周でも収まっていれば、Medium と Low の指摘があってももう直さない**
...
**この節は、実装レビューにも設計レビューにも同じく当たる。**
**別なのは数え方だけである**（下の「設計レビューと実装レビューは、別々に数える」）。

| 状態 | 次に何をするか |
| --- | --- |
| **Critical か High が1件以上** | **直して次の周を回す** |
| **収まった。Medium と Low を直さないと決めた** | **そこで終わり。**[.claude/skills/pr-review-and-merge/SKILL.md](.claude/skills/pr-review-and-merge/SKILL.md) の段5（検査を回し直す）へ進んでよい |
| **収まった。Medium か Low を直した** | **最後に1回だけ回す** |
| **その最後の周でも収まっている** | **Medium と Low の指摘があっても、もう直さない。**そこで終わり |
| **その最後の周で Critical か High が出た** | **直して、また回す。**この表の1行目へ戻る |
