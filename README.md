...
**If continuo is not running, it still checks the panes before deleting.** The lock file lives at `~/.continuo/continuo.lock` and no environment variable moves it (if you set `runtime.lock_file`, it lives there instead). **If you run a second continuo with `--id <name>`, pass the same name — and the same `WORKFLOW.md` path — to `abandon`.** **A live pane on that worktree stops the deletion**, whatever the lock file says.
