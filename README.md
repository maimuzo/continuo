...
`continuo doctor` runs nineteen checks: config, cleanup states, **settings missing from your `WORKFLOW.md`**, **prompt variables**, Claude Code, **agent teams**, **the hook socket location**, the Claude settings directory, the worktree root, herdr, `gh` auth, kanban board, Status names, the rewrite table's keys, **kanban automations**, clones, trust, credentials (used to read your plan's usage window), and **the GitHub App** (its credentials, when `github_app_attribution` is `true`). It does **not** check your OS or Go version — that part is on you.
