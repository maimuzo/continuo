...
**設定のキーは1つも増えていません。****本文（front matter より下）に足すものが2つあります。**
...
| **名前が変わったキー** | **ありません** |
| **本文に当てるもの** | **2つ。**「worktree と branch は切り替えないこと」と、「まとめて対応する issue のグループ」を書いた人の立場を確かめさせる段落です（どちらも下にあります） |
| **変わった振る舞い** | **ロックが `~/.continuo/continuo.lock` に固定されました。**設定でも環境変数でも動きません。**同じカンバンを2つの continuo が見ると、2つ目は起動を止められます。****枠を読めないときは、新しい issue に着手しなくなりました** |
