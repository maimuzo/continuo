...
| 決まり | 破ると |
| --- | --- |
| **1以上にすること** | 起動しません。**0 にすると「間隔0分ごとに書け」という文面が送られます** |
| **`idle_timeout_ms`（既定18時間）より短くすること** | 起動しません。**長いと、エージェントが指示どおりに書いていても、書く前に担当が外れます** |
...
**使える変数は9つです。**一覧は [FAQ.md](FAQ.md) にあります。
