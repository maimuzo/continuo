...
| **`idle_timeout_ms` を1時間以下にしていると起動しません** | 担当を外すまでの待ち時間を短くしていた人（下の節） | **`progress_interval_ms` を足せば起動します。**ただし `idle_timeout_ms` より短い値しか書けないので、**エージェントが進捗を書く間隔は、v0.1.13 の1時間より短くなります** |
...
**v0.1.14 で PR を作らせるようにすると、issue 1件につき1回は動きます。**
**上の節の1行を消していないなら、いままでどおり動きません。**
...
**標準エラーに出る内訳の `WORKFLOW.md body` の行数が、あなたの期待と合っているかを見てください。**
**本文が案内のコメントだけになった場合は、`no body in WORKFLOW.md` と出ます。**
