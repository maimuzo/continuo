...
| 3 | **2台が同じ巡回で締め切りを跨いだときは、両方が担当者を書きます。アカウントが別なので、担当者が2人になります。**片方が先に hold を書き終えていれば、もう片方はそこで止まります（「他人が担当していて、期限内である」と読むためです） |
| 4 | **担当者が2人になったときは、両方が hold を書き、両方が着手します。**branch 名は issue から決まるので同じ名前になり、**片方の push が通りません** |
| 5 | 次の巡回では両方が「担当者が2人以上」で止まります。**担当者に自分が混じっているので、issue へは案内も書きません** |
| 6 | **あなたが担当者を外すまで、その issue は止まったままになります。**issue には何も書かれず、ログとダッシュボードにしか出ません |
...
#### 画面とログの言い方が「機械」から「アカウント」に変わりました — 手を動かすものはありません

**見分ける単位が機械からアカウントに変わったので、そう呼んでいた文言も揃えました。**
**設定に足すものも消すものもありません。**この節より下にある古い版の案内は、
**その版で実際に出ていた文言のまま残してあります。**

| どこで見えるか | v0.1.14 まで | v0.1.15 から |
| --- | --- | --- |
| **ダッシュボードの印** | 別の機械の担当かどうかを切り分けられません | **別のアカウントの continuo の担当かどうかを切り分けられません** |
| **ダッシュボードの直し方** | 別の機械が担当していないかを先に確かめてください | **別のアカウントの continuo が担当していないかを先に確かめてください** |
| **入札のコメント** | `<機械の名前>` がこの issue の担当に立候補しています | **`<アカウント名>` がこの issue の担当に立候補しています** |
| **担当が移ったときの案内の【対処】** | 続きは新しい担当の機械が同じ branch で進めます | **新しい担当が continuo なら、続きは同じ branch で進みます。人間が担当を取った場合は、その人が続けます** |
