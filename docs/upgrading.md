...
**破壊的変更はありません。****当てるものもありません。**

| 何が変わったか | 当てる必要 |
| --- | --- |
| **機械が書いた issue と pull request のコメントに、`<!-- continuo:ai -->` の印が付くようになりました** | **要りません。**設定にも `WORKFLOW.md` にも足すものはありません。**ただし、自分で動かした Claude Code にも付けさせたいなら、`CLAUDE.md` に1行書きます**（下の節） |
