...
| **担当しているリポジトリでも、免除はそこで止まります** | **免除の文が、こう言い切っています。**「リポジトリ X の GitHub 上のもの（issue、pull request、ref、release、設定）と、GitHub Projects v2 のカンバン（project）への書き込みのうち、いま肯定した形に当たらないものは、**いま担当している issue に関係していても断る**」。**種類（コードや配布物を変える操作、など）では閉じていません。**承認や close は、その瞬間にコードも配布物も1バイトも変えないので、種類では捕まらないためです。**「担当先だからという理由では通さない」とも書きません。**それは理由を1本封じるだけで、**「この pull request はこの issue を閉じるものだ」という別の理由が残るためです** |
