...
| **担当しているリポジトリでは、push が加わります** | **担当している worktree の branch への push と、コメントを `gh api` で直に書く形が、免除の文に名指しで書いてあります。**`git push -u origin HEAD` と、進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` が、これに当たります |
| **担当しているリポジトリでも、免除はそこで止まります** | **上に挙げた以外で**コードや配布物を変える操作と、リポジトリやカンバンの設定を変える操作には及びません。**そのうえで3つを名指しで断ります。**既定の branch へ直に送る push、branch や tag を消す push、pull request を通さずにファイルを書き込む形（`gh api` の contents など）です。**push を肯定の形で通しているぶん、この3つは種類では閉じられないためです。****既定の branch の名前は、その issue のリポジトリの実物を使います**（`main` とは限りません） |
