...
| **エージェントに許可を出す方法** | **issue のコメントで、あなたの言葉で許可を出せば通ります。**判定役が会話の流れを読みます（決まった言い回しはありません）。**公開リポジトリの issue では、同じことを第三者も書けます**（減らし方は [SECURITY.md](../SECURITY.md) の危険の表）。`dontAsk` では設定ファイルを書き換えるしかありませんでした |
...
**いちばん変わるのは、引用の置き方です。**
**v0.1.15 までは、コメントの先頭に引用が1箇所ありました。**
**この版からは、返信したい内容ごとに節が立ち、節ごとに引用が付きます。**
...
**組み込みの指示書を差し替えている場合**（`agent.prompt_file`）**だけ、自分の写しを直す必要があります。**
