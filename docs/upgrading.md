...
| **担当しているリポジトリでは、その一部を名指しで断ります** | merge・close・approve・release の作成・パッケージの公開・既定の branch へ直に送る push です。**担当している worktree の branch への push と、コメントを `gh api` で直に書く形は、免除の文に名指しで書いてあります** |
