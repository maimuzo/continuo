...
| **push の扱いは、この変更の前と同じです** | **免除の文は、push について1文字も書いていません。**断る条件の3つ目が挙げている「他のリポジトリへの push」で、これまでどおり判定されます。**担当しているリポジトリへの push は、そこに当たりません。****`git push -u origin HEAD` は、送り先の branch 名がコマンドの文字列に無いので、外れる3つのどれにも当たりません。****肯定の側を「既定の branch 以外へ」と限ると、判定役はそれに当たるかを確かめられず、断る側へ落ちます。**落ちると pull request が1本も出ません。**既定の branch の名前は、その issue のリポジトリの実物を使います**（`main` とは限りません）。**タグを作る push を外すのは、`v*` のタグで release を公開する仕掛けを持つリポジトリでは `gh release create` と同じ結果へ届いてしまうためです** |
