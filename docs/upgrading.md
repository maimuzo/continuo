...
| --- | --- | --- |
| **`runtime:` を書いていると起動しません** | front matter に `runtime` の節を書いている人（下の節） | **2行消せば起動します。**ただしロックの置き場所は `~/.continuo/continuo.lock` に固定されたままです |
| **`idle_timeout_ms` を1時間以下にしていると起動しません** | 担当を外すまでの待ち時間を短くしていた人（下の節） | **`progress_interval_ms` を足せば起動し、振る舞いも元どおりです** |
...
**v0.1.12 までは、そのまま送られていました。**下の節に、使える変数の一覧と直し方があります。
...
**`continuo init` が置く本文には、次の節が入っています。**要らない節は消してかまいません。
...
```bash
cd ~/continuo-work && continuo prompt --show | grep -c '^## 自分で作った worktree は、自分で消すこと'
```
...
```bash
cd ~/continuo-work && continuo prompt --show | grep -c '^## 長くかかるときは、途中でも状況を書くこと'
```
