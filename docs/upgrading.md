...
```
<!-- continuo:self -->
Status を **Sub Issues → In Review** へ動かしました。
...
| **このコメントだけを止めるつまみはありません** | `WORKFLOW.md` の本文から `### まとめて直してよい範囲` を消すと、まとめて直す指示は届かなくなります。**ただし組み込みの手順は残り、計画は代表の issue のコメント側にもあります。**節を消したときの振る舞いは測っていません |
