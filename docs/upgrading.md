...
| --- | --- | --- |
| **`runtime:` を書いていると起動しません** | **v0.1.13 の `continuo init` を使った人は全員。**雛形が `runtime:` を書いていました（下の節） | **2行消せば起動します。**ただしロックの置き場所は `~/.continuo/continuo.lock` に固定されたままです |
...
| **本文に「PR を作らない」と書いても、PR が作られます** | 調査だけ・レビューだけを頼んでいる人（下の節） | **戻せません。**打ち消しの口そのものがありません |
| **本文に書いた `<!-- … -->` が、エージェントへ送られません** | 決まりを HTML のコメントで書いていた人（下の節） | **コメントを外して、ふつうの文として書けば届きます** |

| 何が変わったか | 当てる必要 |
| --- | --- |
| **エージェントが PR を作るようになりました** | **止められません。**本文に「PR を作らない」と書いてあっても作ります（下の節） |
...
**止める口はありません。**本文に「PR を作らない」と書いてあっても作ります。
...
**60000（1分）以上にすること。**59999 までは、送る文面が「0分以上黙らない」になります。
...
**`1` なら当たっています。**`2` が出たら、本文にも同じ節を書いています。本文の側を消してください。
...
`## 長くかかるときは、途中でも状況を書くこと` は、**もうどの版の組み込みにもありません。**
