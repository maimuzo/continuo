...
| **`continuo init` が置く雛形から「すべて日本語で書いてください」が消えました** | **要りません。**既にある `WORKFLOW.md` は書き換わりません（下の節） |
...
| `## 書く言語` | **v0.1.14 までの雛形は「すべて日本語で書いてください」を既定にしていました**（v0.1.14 の雛形での見出しは `### 書く言語` です）。**v0.1.15 の雛形からは既定が消え、空になります**（上の「v0.1.14 から v0.1.15 へ」）。**あなたの `WORKFLOW.md` は書き換わりません** |
