...
### コメントと pull request の本文を、ファイルから渡すようになりました — 設定に足すものはありません

**エージェントは、issue のコメントと pull request の本文を一時ファイルへ書き、`--body-file` で `gh` へ渡します。**
**v0.1.15 までの指示書は、本文を `--body "…"` の二重引用符の中へ直に書かせていました。**
