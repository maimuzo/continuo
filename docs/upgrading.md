...
**担当先へ免除が及ぶのは、issue と pull request を作ること、その本文とコメントを書くこと、
そして push の3つです。**担当先の外は、前の2つだけです。
...
| **担当しているリポジトリでは、push が加わります** | **既定の branch 以外の branch への push と、コメントを `gh api` で直に書く形が、免除の文に名指しで書いてあります。**`git push -u origin HEAD` と、進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` が、これに当たります。**既定の branch の名前は、その issue のリポジトリの実物を使います**（`main` とは限りません）。**タグを作る push は入りません。**`v*` のタグで release を公開する仕掛けを持つリポジトリでは、`gh release create` と同じ結果へ届いてしまうためです |
| **担当しているリポジトリでも、免除はそこで止まります** | **免除の文が、こう言い切っています。**「リポジトリ X への書き込みのうち、いま挙げた形に当たらないものは、リポジトリ X が担当先であることを理由に『関係がある』として通してはならない」。**種類（コードや配布物を変える操作、など）では閉じていません。**承認や close は、その瞬間にコードも配布物も1バイトも変えないので、種類では捕まらないためです。**そのうえで、branch や tag を消す push と、pull request を通さずにファイルを書き込む形（`gh api` の contents など）を、名指しで外しています** |
| **pull request の承認と取り込みは、コメントに数えません** | `gh pr review --approve` は、道具の形としては pull request へのコメントの投稿です。**そのままだと、コメントの免除に入ってしまいます。**必須のレビューや自動マージを設定したリポジトリでは、**承認がそのままコードを入れます。**だから免除の文が、承認・取り込み・却下と、issue と pull request を閉じることを、コメントの書き込みから名指しで外しています |
