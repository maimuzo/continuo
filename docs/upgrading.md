...
**破壊的変更はありません。****何もしなければ、v0.1.13 と同じ振る舞いのままです。**

| 何が変わったか | 当てる必要 |
| --- | --- |
| **エージェントが PR を作るようになりました** | **要りません。**ただし**いまのままだと、あなたの手元では作られません**（下の節）。**作らせたくなったら本文の1行を消します** |
| **雛形の `## PR を作るか` が `## PR の決まり` に変わりました** | **要りません。**continuo は見出しの名前を見ていません。**中身だけが効きます** |
...
### エージェントが PR を作るようになりました — 作らせたくなったら1行消してください
...
**v0.1.14 から。**組み込みのプロンプトに `## PR を出すこと` が入りました。
**`CONTINUO-STATUS: review` を出す前に、エージェントが自分で PR を作ります。**

**ただし、本文に「PR を作らない」と書いてあるときは作りません。**
**組み込みがそう決めています。**
**だから、いま何もしなければ、あなたの手元では v0.1.13 と同じく push で止まります。**
**壊れません。**

| エージェントがすること | 中身 |
| --- | --- |
| **既にある PR を先に探す** | `OPEN` のものがあれば作り直さず、その branch へ push します |
| **無ければ作る** | `gh pr create` を叩きます |
| **本文へ `Closes #<番号>` を入れる** | この1行が PR と issue を結びつけます。**次の turn がレビューの指摘を読む先になります** |
| **作れなかったら `blocked`** | 理由をコメントに書いて、人間へ渡します |

**PR を作らせたくなったら、その1行を消します。**

```bash
cd ~/continuo-work && grep -n 'PR は作らないでください' WORKFLOW.md
```

**行番号が出たら、その行を消します。**`## PR を作るか` の節ごと消してかまいません。
**節の名前はそのままでも動きます。**continuo は見出しの名前を見ていません。

**消したかどうかは、送られる文面で確かめます。**

```bash
cd ~/continuo-work && continuo prompt --show | grep -c 'PR は作らないでください'
```

**`0` なら消えています。**次の着手から PR が作られます。

**逆に、これからも作らせたくない場合は、その1行をそのまま残してください。**
**調査だけ・レビューだけを頼んでいる運用では、そのほうが正しい状態です。**
commit する成果が無い run は PR を作れないので、作らせると `Blocked` で止まります。

**なぜ continuo が代わりに消さないのか。**
**continuo は `WORKFLOW.md` を1文字も書き換えません。**本文はあなたが書く場所であり、
**そこに「作らない」と書いてあることは、いまは正しい設定の1つです。**
**消してよいかを決められるのは、あなただけです。**
`continuo init --force` は本文ごと雛形で上書きするので、**これには使わないでください**（上の節）。
