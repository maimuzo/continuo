...
**破壊的変更はありません。**設定を足さなくても、そのまま起動します。
**ただし、何もしなくても振る舞いが2つ変わります。**

| 何が変わるか | 誰に効くか | 手を動かす必要 |
| --- | --- | --- |
| **枠に余裕が無いときの理由が、既定のログの水準で出るようになりました** | **全員** | **要りません。**`--log-level debug` で起動し直さなくても読めます |
| **1週間の枠が明けるのを5時間より長く待つことになると、その issue の担当を自分で外すようになりました** | **1週間の枠を使い切る人** | **要りません。**待ち続けたい人だけ、下の設定を書いてください |
...
**どちらの行も、止められている候補が1件でもあるときだけ出ます。**
**カンバンが空の機械は、止まっていても1行も出しません。**巡回は既定30秒なので、
出し続けると1時間で120行になり、本当に困っている行が pane の scrollback から押し出されるためです。

**ダッシュボードにも同じ理由が出ます**（`server.port` を設定している人だけ）。
