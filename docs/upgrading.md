...
**いちばん大きな変更は、エージェントへ送るプロンプトが `WORKFLOW.md` から出たことです。**
**いまの `WORKFLOW.md` はそのままでも動きます。**ただし、そのままだと
**これから continuo が直すプロンプトの改良が、1つも届かなくなります。**
...
| 何が変わったか | 当てる必要 |
| --- | --- |
| **送るプロンプトが `WORKFLOW.md` の外へ出ました** | **当てると、これから先のプロンプトの改良が自動で届きます**（下の節） |
...
**いままで、エージェントへ最初に送る指示書は `WORKFLOW.md` の本文そのものでした。**
**そこには2種類のものが混ざっていました。**

| 種類 | 例 | 誰のものか |
| --- | --- | --- |
| **仕組みの部分** | 表明の1行（`CONTINUO-STATUS:`）／worktree と branch を切り替えない | **continuo のもの** |
| **プロジェクト固有の部分** | テストの走らせ方／レビューの手順／書く言語 | **あなたのもの** |

**混ざっていたので、continuo が仕組みの部分を直しても、あなたの `WORKFLOW.md` には届きませんでした。**
**v0.1.13 からは、こう分かれます。**

| 何 | どこにあるか | 誰が直すか |
| --- | --- | --- |
| **組み込みのプロンプト** | continuo の実行ファイルの中 | **continuo。**版を上げれば自動で新しくなる |
| **固有のプロンプト** | `PROJECT_SPECIFIC_PROMPT.md`（`WORKFLOW.md` と同じディレクトリ） | **あなた** |

**固有のプロンプトは、組み込みの「## 終わったらやること」の直前に差し込まれます。**
**送られる全文はいつでも読めます。**
...
**壊れません。**`WORKFLOW.md` に本文が残っている限り、**いままでと1バイトも同じ文面が送られます。**
**組み込みのプロンプトは送られません。**

**そのかわり、次の2つが起こります。**

- **これから continuo が直すプロンプトの改良が、1つも届きません**
- **`continuo doctor` の `残った本文` が、毎回 `!` を出し続けます**（起動は止まりません）

### 送るプロンプトを移す

**4段です。**

**段1。`continuo init` を叩きます。`--force` は付けません。**
既にある `WORKFLOW.md` には触らず、**足りない `PROJECT_SPECIFIC_PROMPT.md` だけが置かれます。**
...
**段4。`WORKFLOW.md` の閉じの `---` より下を、全部消します。**
消したら `continuo doctor` を叩いて、`残った本文` が `✓` になったことを確かめてください。

```bash
cd ~/continuo-work && continuo doctor
```

### `PROJECT_SPECIFIC_PROMPT.md` の雛形に、はじめから入っているもの

**`continuo init` が置く雛形には、次の節が入っています。**要らない節は消してかまいません。
**ファイルごと消しても continuo は動きます。**
...
**起動できずに止まる場合は、`WORKFLOW.md` の `claude.permissions.allow` に
subagent を起動する道具を足してください**（雛形の HTML のコメントにも書いてあります）。

### プロンプトの誤りが、起動の時点で止まるようになりました

**`PROJECT_SPECIFIC_PROMPT.md` に、一覧に無い変数（`{{.issue.nope}}` など）を書くと、
continuo は起動しません。**いままでは着手のたびに1件ずつ失敗していたので、
**常駐プロセスは健康に見えたまま issue が全部落ちていました。**

**使える変数は9つです。**一覧は [FAQ.md](FAQ.md) にあります。

**`WORKFLOW.md` に残っている本文の誤りでは、起動は止まりません**（警告だけです）。
**版を上げた瞬間に起動しなくなることを避けるためです。**着手の時点では、いままでどおり失敗します。
