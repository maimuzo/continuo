...
### `tracker.automated_state_rewrite` の説明が「何のための設定か」から始まるようになりました

**雛形の説明は「どう動くか」から始まっていて、読んでも自分に要るのかが判断できませんでした。**
**この版から、1行目が「カンバンの自動化に Status を動かされても、エージェントを止めずに続けさせるための設定」になり、**
**カンバンの `Settings` → `Workflows` を見て要否を決められるようになりました。**

**設定そのものは1バイトも変わっていません。**書き写す必要はありません。
**どう決めればよいかは [FAQ.md](FAQ.md) の「使い方が分からないとき」の
...
**`"In Progress": "AI In Progress"` のような行が意味を持つのは、`tracker` の Status 名を
`AI Ready` / `AI In Progress` のように先に改名してある場合だけです。**
雛形の `active_states: ["Ready", "In Progress"]` のままで、この行だけを写しても起動しません。
...
**起動しない書き方は5つあります。**どれも `continuo doctor` の `設定ファイル` の行が `✗` になります。
