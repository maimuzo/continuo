...
| 順 | 何を見るか |
| --- | --- |
| 1 | **`WORKFLOW.md` の本文（`## 4-4. このプロジェクトの決まり` に入る部分）に指定があれば、それ** |
| 2 | **worktree の直下にある continuo の記録ファイル**（既定 `.continuo.json`。`workspace.identity_file` で名前を変えられます）**の `"base"` の値** |
| 3 | issue にリンクされた branch（`{{.push_branch}}`） |
| 4 | リポジトリの既定 branch |

**段2 がいちばん確かです。**continuo は worktree を作るときに、実際に使った分岐元をそこへ書いています。

**`herdr.worktree.base`（worktree を切る分岐元。既定 `null`）を設定している人へ。**
**段2 がその値を持っているので、何も書かなくても当たります。**
...
| `## 書く言語` | **v0.1.14 までの雛形は「すべて日本語で書いてください」を既定にしていました。****v0.1.15 の雛形からは既定が消え、空になります**（下の「v0.1.14 から v0.1.15 へ」）。**あなたの `WORKFLOW.md` は書き換わりません** |
