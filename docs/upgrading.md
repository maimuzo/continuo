...
**なぜか。****二重起動を止めるロックの置き場所が変わったので、古い版と新しい版は
互いを見つけられません。**古い版は hook を受ける socket と同じディレクトリの
`continuo.lock` を握り、新しい版は `~/.continuo/continuo.lock` を握ります。
...
| 何 | 中身 |
| --- | --- |
| **消えたキー** | **ありません。**`runtime.lock_file` は**そのまま読みます** |
| **増えたキー** | **1つ。**`tracker.provider.handoff.on_assignee_gate`。**`--id` は設定ではなくフラグです** |
| **名前が変わったキー** | **ありません** |
| **変わった振る舞い** | **`runtime.lock_file` を書いていないときのロックの置き場所が `~/.continuo/continuo.lock` になりました**（下を見てください） |

### ロックの既定の置き場所が `~/.continuo/continuo.lock` になりました

**ロックとは。**continuo は、1台で2つ動いてしまわないように、起動のときに
ファイルを1つ掴みます（`flock(2)`）。**このファイルの置き場所が変わりました。**

| 版 | `runtime.lock_file` を書いていないときのロック |
| --- | --- |
| v0.1.11 まで | **hook を受ける socket と同じディレクトリ**（`CONTINUO_RUNTIME_DIR` / `XDG_RUNTIME_DIR` / `TMPDIR` で動く） |
| v0.1.12 から | **`~/.continuo/continuo.lock`**（環境変数では動かない） |

**なぜ変えたか。**socket の置き場所は環境変数と `claude.hook_bridge.listen` で動きます。
**そこからロックを導いていると、socket を分けただけで2本目が黙って起動できてしまいます。**
**「1台で continuo は1本だけ」を、設定の書き換えで迂回できない形にしました。**

**当てるものはありません。**`WORKFLOW.md` は書き換えなくて構いません。

**`runtime.lock_file` を書いている人は、そのままで動きます。**
このキーは**引き続き読みます。**優先順位は次のとおりです。
...
**`runtime.lock_file` の親ディレクトリは continuo が作りません。**
パスを打ち間違えたときに黙って別の場所へロックを置くと、二重起動を検出できなくなるためです。
**打ち間違いは「ロックファイルを用意できません」で止まります。**

### `--id <名前>` — 1台で2本目を動かす

**何のためのものか。**本番を止めずに、検証用の continuo をもう1本動かすためです。
**開発者向けの使い方で、ふだんの運用では使いません。**
...
**書き換え忘れると何が起きるかは、[docs/FAQ.md](FAQ.md) の
「1台で continuo を2つ動かしたい」に一覧があります。**

**名前に書けるのは、小文字の英数字とハイフンだけです。**先頭は英数字、32文字まで。
**大文字・空白・`.`・`/` は起動する前に弾きます。**

**`continuo abandon` にも、同じ名前と検証用の `WORKFLOW.md` のパスを渡してください。**
渡さないと既定の1本のロックと本番の置き場所を見に行き、
**動いている continuo の worktree を消しに行きます。**

**当てるものはありません。**`--id` を使わないなら、いままでどおりです。
