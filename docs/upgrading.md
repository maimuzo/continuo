...
新規着手を止める門が2つあり、**先に効くほうが `Debug` の1行しか出さなかった**ためです。

**v0.1.15 からは、この帯でも `Info` で1行出ます。**

```
枠に余裕が無いので、入札の要る issue には着手しません（走行中の turn は止めません）。…
  理由=余裕値がマイナス 5時間の枠の使用率=30 1週間の枠の使用率=92
  5時間の枠の閾値="90% を超えたら止まります" 1週間の枠の閾値="90% を超えたら止まります"
```

**`rate_limit.pause_above_percent`（既定95）を超えたときの文面も変わりました。**
**止まる範囲が違う**ので、2つに分けています。

| 文面 | 何が止まるか |
| --- | --- |
| **枠に余裕が無いので、入札の要る issue には着手しません** | **担当者のいない issue だけ。**担当が既にこの機械にある issue は、そのまま着手します |
| **枠が閾値を超えているので、この巡回では1件も着手しません** | **担当が既に付いている issue も含めて全部** |

**ログを grep している人は、`枠が閾値を超えている` だけでは片方しか拾えません。**

```bash
grep -E '枠に余裕が無いので|枠が閾値を超えているので、この巡回では' <ログの出力先>
```

**どちらの行も、止まっているあいだは巡回のたびに出ます。**
