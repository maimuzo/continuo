...
`~/.continuo/continuo.lock` を握ります。**カンバンのロックも古い版は取りません**（この版で入ります）。
...
**設定のキーが1つ増えました**（`tracker.provider.handoff.on_assignee_gate`）。**本文（front matter より下）に足すものが3つあります。**

| 何 | 中身 |
| --- | --- |
| **消えたキー** | **ありません。**`runtime.lock_file` は**効かなくなりましたが、書いてあっても起動します** |
| **増えたキー** | **1つ。**`tracker.provider.handoff.on_assignee_gate`（着手できずに止まっている issue を issue のコメントにも書くかどうか。既定は `warn_and_comment` で、書かなくても動きます）。**`--id` は設定ではなくフラグです** |
| **名前が変わったキー** | **ありません** |
| **本文に当てるもの** | **3つ。**「worktree と branch は切り替えないこと」、「まとめて対応する issue のグループ」を書いた人の立場を確かめさせる段落、「push には `-u` を付けろ」の3つです（どれも下にあります） |
| **変わった振る舞い** | **ロックが `~/.continuo/continuo.lock` に固定されました。**設定でも環境変数でも動きません。**同じカンバンを2つの continuo が見ると、2つ目は起動を止められます。****枠を読めないときは、新しい issue に着手しなくなりました。****着手できずに止まっている issue が、ダッシュボードと issue のコメントに出るようになりました。****push 先を分けた worktree が片付くようになりました** |
...
### 着手されない理由が、1行で読めるようになりました

**何が変わったか。**枠に余裕が無くて issue を取らないとき、**その理由が `Info` で1行出ます。**
**走っている turn は止まりません。**

**前はどうだったか。**入札（持ち回り）の側と新規着手の側が**別々の判定**を持っており、
枠を読めないときに**逆を向いていました**（入札は黙る、新規着手は止めない）。
入札が先に効くので**結果は入札側のとおり**でしたが、
**出るのは `Debug` の1行だけ**で、なぜ進まないのかを知る手立てがありませんでした。

**いまは判定が1つで、巡回1回につき `Info` を1行出します。**

```
level=INFO msg="枠に余裕が無いので、入札の要る issue には着手しません（走行中の turn は止めません）。…" 理由=枠を読めない 新規着手が止まる使用率=90 巡回そのものを止めるか=false ...
```
...
| 値 | 何が起きているか |
| --- | --- |
| `true` | **枠の使い過ぎ。**その巡回では候補を1件も見ません |
| `false` | **枠を読めない、または余裕値がマイナス。****入札の要る issue（担当者がまだいないもの）だけを見送ります。**既にこの機械が担当者になっている issue には着手します |
