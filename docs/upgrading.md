...
| 何を足したか | 中身 |
| --- | --- |
| **`--url <issue の URL>`** | その issue の値で変数を展開する。**カンバンを1回読む**（2リクエスト・計8ポイント） |
| **`--attempt <回数>`** | 何回目の試行として展開するか。**省くと1回目**。やり直しのときの文面を見るのに使う |
...
別物で、連動しません。**`LANG` が英語の環境でも、**画面は英語になり、エージェントは日本語で書いていました。**

**v0.1.15 から。**雛形の `### 書く言語` は空になり、案内のコメントだけが入ります。
