...
**破壊的変更が1つあります。**`WORKFLOW.md` の**本文に、消さないと困る1行があります。**

| 何が変わったか | 当てる必要 |
| --- | --- |
| **エージェントが PR を作るようになりました** | **本文の「PR は作らないでください」を消してください**（下の節）。**消さないと、指示が2つ届きます** |
| **雛形の `## PR を作るか` が `## PR の決まり` に変わりました** | **要りません。**節の名前は送る文面に効きません。**中身だけ直してください** |
...
### エージェントが PR を作るようになりました — 本文の1行を消してください
...
**あなたがすること。****本文に「PR は作らないでください」が残っていたら、消してください。**
...
**節の名前はそのままでも動きます。**送る文面に効くのは中身だけです。

**消さないとどうなるか。**組み込みは「作れ」と言い、あなたの本文は「作るな」と言います。
**本文は組み込みの前半と後半のあいだに挟まるので、順番としては組み込みの「作れ」が後に来ます。**
**それでも、2つの指示が同じ文面に並ぶこと自体が事故のもとです。**どちらに従うかはエージェント次第になります。
...
**続きを書き足したり、節ごと入れ替えたりしているのが普通だからです。**
**消す相手を機械が言い当てる手がかりがありません。**
...
**GitHub Projects で新しく作ったカンバンは、`Pull request linked to issue` などが
有効な状態で作られます。**その自動化は、**PR が issue に紐づいた瞬間に Status を書き換えます。**
