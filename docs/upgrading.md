...
pull request の本文の書き込み、pull request の draft を外すこと、issue と pull request へのコメントの書き込みです。**
**push には1文字も触っていません。**この変更の前と同じ扱いです。
...
| **push の扱いは、この変更の前と同じです** | **免除の文は、push について1文字も書いていません。**断る条件の3つ目が挙げている「他のリポジトリへの push」で、これまでどおり判定されます。**担当しているリポジトリへの push は、そこに当たりません。****タグを作る push も、既定の branch へ直に送る push も、この変更では扱いが変わりません** |
