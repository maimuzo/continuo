...
**残る15件のうち、手を動かすものは1件だけです。**
**`continuo init` が2枚目を置くようになった件だけ、もう一度 `continuo init` を叩いてください。**
**残る14件は、上げるだけで届きます。**当てるものはありません。
...
| **機械が書いた issue と pull request のコメントに、`<!-- continuo:ai -->` の印が付くようになりました** | **要りません。**設定にも `WORKFLOW.md` にも足すものはありません。**ただし、自分で動かした Claude Code にも付けさせたいなら、`CLAUDE.md` に1行書きます**（下の節）。**あわせて、continuo が投稿する本文の先頭の空白が落ちるようになりました**（印を1行目に置くため。4桁字下げのコード片で始まる本文は、1行目だけ字下げが消えます） |
...
**`<!-- continuo:ai -->` の1行が入るようになりました。**

**なぜ要るか。**continuo を使うと、1つの issue に4種類の書き手が現れます。
**4種類とも、同じ GitHub アカウントで投稿します。**投稿者でも `author_association` でも見分けられません。
**「人間が前に出した指示を探して、その下から読み直す」という読み方が、いままでできませんでした。**

**印は、本文の先頭に並ぶ印の、いちばん後ろに入ります。**
...
**既にある目印は1つも動きません。**`<!-- continuo:agent -->` も
`<!-- code-review-result -->` も、いままでどおり本文の先頭のままです。
**あなたのリポジトリに置いた `continuo-ci.yaml`（`.github/workflows/` へ移したもの）は、そのままで通ります。**
**入れ替える必要はありません。**

**設定に足すものはありません。**印は固定で、`WORKFLOW.md` から変えられません。
**機械ごとに違う値にできると、別の機械が書いた印を読めなくなるためです。**

**v0.1.14 までのコメントには印がありません。**AI が書いたものにも付いていません。
**遡って付け直す仕組みはありません。**

**自分で動かした Claude Code にも付けさせたいときだけ、1行書きます。**
**continuo が送る指示書は、continuo が起動したエージェントにしか届きません。**
`CLAUDE.md` へ足す文面は [FAQ.md](FAQ.md) の
「continuo を入れたら、`CLAUDE.md` には何を書けばいい？」にあります。
