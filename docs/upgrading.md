...
| **担当しているリポジトリでも、免除は同じところで止まります** | コードや配布物を変える操作と、リポジトリの設定を変える操作には及びません。**そのうえで2つを名指しで断ります。**既定の branch へ直に送る push と、pull request を通さずにファイルを書き込む形（`gh api` の contents など）です。**担当しているリポジトリへの push を肯定の形で通しているので、この2つだけは種類では閉じられないためです。****既定の branch の名前は、その issue のリポジトリの実物を使います**（`main` とは限りません）。**担当している worktree の branch への push と、コメントを `gh api` で直に書く形は、免除の文に名指しで書いてあります** |
