...
**破壊的変更が3つあります。****どれも `WORKFLOW.md` を直さないと、直すまで元の振る舞いに戻せません。**

| 何が壊れるか | 誰に効くか |
| --- | --- |
| **`runtime:` を書いていると起動しません** | **front matter に `runtime` の節を書いている人**（下の節） |
| **`idle_timeout_ms` を1時間以下にしていると起動しません** | **担当を外すまでの待ち時間を短くしていた人**（下の節） |
| **本文に「PR を作らない」と書いても、PR が作られます** | **調査だけ・レビューだけを頼んでいる人**（下の節） |
...
**2つ目。**front matter の **`runtime` の節（`runtime.lock_file`）が無くなりました。**
**書いてあると起動しません。**2行消すだけで直ります（下の節）。

> **上げる前に、動いている continuo を止めてください。**
> **v0.1.13 で、二重起動を止めるロックの置き場所が変わります**（hook の socket と同じディレクトリ →
...
| **二重起動を止めるロックが `~/.continuo/continuo.lock` の1本に固定され、`runtime` の節が無くなりました** | **`runtime:` の2行を消してください**（下の節）。**消さないと起動しません** |
...
| `## PR を作るか` | **v0.1.13 にしかない節です。****v0.1.14 で `## PR の決まり` に変わり、「作らない」という既定は無くなりました**（上の「v0.1.13 から v0.1.14 へ」） |
