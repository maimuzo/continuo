...
| **エージェントが、担当している issue のリポジトリへ issue を立てられるようになりました** | **要りません。**危ない道具の呼び出しを断る検査に、担当先を伝えるようにしただけです（下の節） |
...
### エージェントが、担当している issue のリポジトリへ issue を立てられます — 設定に足すものはありません
...
**v0.1.15 から。**検査する側へ、担当している issue を伝えます。
...
| **別のリポジトリへの書き込みは、これまでどおり検査されます** | 通るようにしたのは、担当している issue と同じリポジトリだけです |
| **fork へ push して本家へ PR を出す形も、これまでどおりです** | 断る条件の「**関係のない**」という限定を残してあるので、その余地は変わりません |
| **他の断る条件は1文字も変えていません** | 取り消せない破壊（force push など）・資格情報の持ち出し・権限の昇格・検査そのものの無効化・判定を曲げる文字列の5つは、そのままです |
| **判定は最後まで LLM が行います** | 「担当先と同じなら必ず通る」という機械的な保証ではありません。**伝えているのは、判定に要る材料です** |
