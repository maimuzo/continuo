...
```
<!-- continuo:group -->
<owner>/<repo>#183 とまとめて直しました。この issue の分は次のとおりです。
...
| **`blocked` のときは書式が違います** | 直していないので、「まとめて直しました」とも pull request の URL とも書きません。**どこまで見て、なぜ止まったかを書きます** |
...
| **2件付くことがあります** | **経路は2つです。**一、書き換える先を読めなかったとき（**1件も足さないより、重ねて残すほうを選んでいます**）。二、同じカンバンを別のアカウントの機械で見張っていて**担当が引き継がれたとき**（新しい機械は、前の機械が書いたコメントを自分のものとして見つけられません）。**一は1台で動かしていても起きます** |
