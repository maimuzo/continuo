...
| --- | --- | --- |
| **`runtime:` を書いていると起動しません** | **v0.1.10 から v0.1.13 までの `continuo init` を使った人は全員。**どの版の雛形も `runtime:` を書いていました（下の節） | **2行消せば起動します。**ただしロックの置き場所は `~/.continuo/continuo.lock` に固定されたままです |
...
`2` が出たら、本文にも同じ節を書いています。本文の側を消してください。
