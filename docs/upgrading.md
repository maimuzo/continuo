...
### herdr 0.9.1 に合わせました。**`WORKFLOW.md` の `herdr.protocol` を手で `22` へ直してください**
...
**`continuo init` で `WORKFLOW.md` を作った人は、全員が手で直す必要があります。**
**雛形が `protocol: 20` を書いているためです。**書いてある値は既定値より優先されるので、continuo の既定値を 22 に上げただけでは効きません。
...
確かめてから閉じるまでの間に、同じリポジトリの別の issue が worktree を開いたときだけ断られます。そのときは閉じずに残し、
**あとから開いた worktree が片付くときに閉じます。**
