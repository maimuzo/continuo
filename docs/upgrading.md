...
**破壊的変更はありません。**`WORKFLOW.md` に足すキーも、消すキーもありません。

| 何が変わったか | 当てる必要 |
| --- | --- |
| **エージェントが、相手を問わず issue を立てられるようになりました** | **ありません。**上げるだけで届きます |

### エージェントが、相手を問わず issue を立てられます

**公開リポジトリの issue を continuo に処理させている人に関わる話です。**
**非公開リポジトリだけで使っているなら、たいていは見た目が変わりません**（既定の `public_only` では検査が掛からないためです）。
**ただし、公開か非公開かを読み取れなかった issue には掛かります。**
分からないものを「公開ではない」と決めない作りなので、**非公開のリポジトリしか使っていなくても、この節が当てはまることがあります。**

**v0.1.15 まで。**エージェントが `gh issue create` を叩くと、**担当している issue と同じリポジトリでも断られていました。**

```
$ gh issue create --repo <owner>/<repo> --title "…" --body "…"
断られました: いまの作業と関係のない外部のリポジトリ（<owner>/<repo>）に issue を作ろうとしている
```

**`<owner>/<repo>` は、そのエージェントがいま担当しているリポジトリそのものです。**
**検査する側に「いま担当しているのはどこか」を伝えていなかったので、自分が守っている場所を「外部」と読んでいました。**

**そのため、作業の途中で気づいた別の不具合を issue にできませんでした。**
エージェントは題名と本文をコメントへ置くしかなく、**その下書きはカンバンにも issue の一覧にも現れませんでした。**

**v0.1.16 から。**検査する側へ、担当している issue と、担当先の外への免除を伝えます。

**担当先へ免除が及ぶのは、issue と pull request を作ること、そのときに付ける本文、
pull request の本文の書き込み、issue と pull request へのコメントの書き込み、そして push です。**
**ただし push は、送り先が既定の branch だと読み取れるものと、タグだと読み取れるものと、ref を消すものが外れます。**
**コメントのほうも、pull request の取り込みと承認と、issue と pull request を閉じることが外れます。**
担当先の外は、issue と pull request を作ることと、その本文とコメントを書くことだけです。
...
| 何 | 中身 |
| --- | --- |
| **`WORKFLOW.md` に足すキーはありません** | `claude.tool_gate` は今までのままです。**書いていない人も、上げるだけで届きます** |
| **相手のコードや配布物を変える操作には、免除が及びません** | push、パッケージの公開、pull request の取り込みと却下と承認（merge・close・approve）、release の作成、ラベルや担当者の付け外しです。**いまの作業と関係があるかどうかで、これまでどおり判断します** |
| **担当しているリポジトリでは、push が加わります** | **push は丸ごと免除に入り、そこから3つが外れます。**送り先が既定の branch だと読み取れる push、branch でない ref（タグなど）への push、ref を消す push です。**`git push -u origin HEAD` は、送り先の branch 名がコマンドの文字列に無いので、外れる3つのどれにも当たりません。****肯定の側を「既定の branch 以外へ」と限ると、判定役はそれに当たるかを確かめられず、断る側へ落ちます。**落ちると pull request が1本も出ません。**既定の branch の名前は、その issue のリポジトリの実物を使います**（`main` とは限りません）。**タグを作る push を外すのは、`v*` のタグで release を公開する仕掛けを持つリポジトリでは `gh release create` と同じ結果へ届いてしまうためです** |
| **担当しているリポジトリでも、免除はそこで止まります** | **免除の文が、こう言い切っています。**「リポジトリ X の GitHub 上のもの（issue、pull request、ref、release、設定）への書き込みと削除、そして GitHub Projects v2 のカンバン（project）への書き込みと削除のうち、いま肯定した形に当たらないものは、**いま担当している issue に関係していても断る**」。**種類（コードや配布物を変える操作、など）では閉じていません。**承認や close は、その瞬間にコードも配布物も1バイトも変えないので、種類では捕まらないためです。**「担当先だからという理由では通さない」とも書きません。**それは理由を1本封じるだけで、**「この pull request はこの issue を閉じるものだ」という別の理由が残るためです** |
| **pull request の承認と取り込みは、コメントに数えません** | `gh pr review --approve` は、道具の形としては pull request へのコメントの投稿です。**そのままだと、コメントの免除に入ってしまいます。**必須のレビューや自動マージを設定したリポジトリでは、**承認がそのままコードを入れます。**だから免除の文が、承認・取り込みと、issue と pull request を閉じることを、コメントの書き込みから名指しで外しています。**「却下」の語は使っていません。**担当先の外への免除が「却下（close）」と書いているので、同じ語が2箇所で違うものを指してしまうためです |
| **fork へ push して本家へ PR を出す形は、これまでどおりです** | 断る条件の「**関係のない**」という限定を残してあるので、その余地は変わりません。**「fork への push は通す」とは書いていません**（そう書くと、任意のリポジトリへの push が「fork への push」と名乗るだけで通ってしまいます） |
| **他の断る条件は1文字も変えていません** | 取り消せない破壊（force push など）・資格情報の持ち出し・権限の昇格・検査そのものの無効化・判定を曲げる文字列の5つは、そのままです |
| **判定は最後まで LLM が行います** | 「必ず通る」「必ず断る」という機械的な保証ではありません。**伝えているのは、判定に要る材料です** |
