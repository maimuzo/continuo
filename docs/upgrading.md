...
**v0.1.12 から上げる人は、この節の例外です。**本文の意味が変わったので、
**`--force` で作り直すのが唯一の手順です**（下の「v0.1.12 から v0.1.13 へ」）。
...
| 何が変わったか | 当てる必要 |
| --- | --- |
| **本文の意味が変わりました**（全文の差し替え → 組み込みの真ん中へ挟む固有の指示） | **`continuo init` で作り直してください**（下の節） |
...
```bash
cd ~/continuo-work && continuo init --force
```

**`--force` は front matter も雛形に戻します。**`continuo setup` で決めた Status の割り当ても、
手で足した設定の行も、1つも残りません。**段1 の控えから、front matter を貼り直してください。**

**段3。段1 の控えから、自分で書き足した指示だけを本文へ貼り直します。**

**組み込みに同じことが書いてある部分は、貼りません。**二重に届きます。
