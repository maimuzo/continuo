...
| 何が変わったか | 当てる必要 |
| --- | --- |
| **エージェントが、相手を問わず issue を立てられるようになりました** | **設定に足すものはありませんが、下の節を読んでから上げてください。**引き換えに開くものがあります |

### エージェントが、相手を問わず issue を立てられます — **読んでから上げてください**
...
**免除が及ぶのは、issue と pull request を作ることと、その本文やコメントを書くことだけです。**
...
| **増える能力には裏があります** | **公開 issue に文を書ける人が、あなたのアカウントで任意のリポジトリへ issue を投稿させられます。**[FAQ.md](FAQ.md) の「エージェントが叩いたコマンドが「危ない」と断られる」の節を読んでください |
| **この能力だけを止めるつまみはありません** | `claude.tool_gate.mode: off` は判定を丸ごと消します。**確実に止めたいなら、公開リポジトリの issue を continuo に処理させないでください** |
...
| **担当しているリポジトリでは、その一部を名指しで断ります** | merge・close・approve・release の作成・パッケージの公開・ラベルや担当者の付け外し・リポジトリの設定の書き換え（公開と非公開の切り替えなど）・pull request を通さないファイルの書き込み・既定の branch へ直に送る push です。**既定の branch の名前は、その issue のリポジトリの実物を使います**（`main` とは限りません）。**担当している worktree の branch への push と、コメントを `gh api` で直に書く形は、免除の文に名指しで書いてあります** |
