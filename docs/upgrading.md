...
**入れ替える前に、動いている continuo を止めてください。**

```bash
# 動いている continuo を止めてから、インストーラーを走らせる
pkill -TERM -f 'continuo$'   # または、起動している pane で Ctrl+C
```

**なぜか。****古い版と新しい版は、互いのロックを見つけられません。**
古い版は `<実行時ディレクトリ>/continuo.lock` を握り、新しい版は
`~/.continuo/continuo.lock` を握ります。**ボードのロックも古い版は取りません**（この版で入ります）。
**だから2つとも起動し、同じ issue に pane を2つ立てます。**

**インストーラーの警告は、実行ファイルを置いたあとに出ます。**
**そのときには入れ替えが終わっているので、「止めてから入れ替える」はもう言えません。**
**この文書を先に読んでください。**

**設定に当てるものはありません。****本文（プロンプト）に当てるものが1つあります。**
...
| **名前が変わったキー** | **ありません** |
| **本文に当てるもの** | **1つ。**「まとめて対応する issue のグループ」を書いた人の立場を確かめさせる段落です（下を見てください） |
| **変わった振る舞い** | **ロックが `~/.continuo/continuo.lock` に固定されました。**設定でも環境変数でも動きません。**同じボードを2つの continuo が見ると、2つ目は起動を止められます。****枠を読めないときは、新しい issue に着手しなくなりました** |
| **新しく断るもの** | **`~/.continuo` と `~/.continuo/board` が symlink か、group / other に開いた権限（`0755` など）だと起動しません**（下を見てください） |

### 本文に当てるもの — グループの計画を書いた人の立場を確かめさせる

**当てないと何が起きるか。****公開の issue にコメントできる人なら誰でも、
ボード上の別の issue の Status を動かせます。**

「まとめて対応する issue のグループ」の段落は v0.1.10 で入りましたが、
**書いた人の立場を確かめる条件が1つも書かれていませんでした。**
外部の人が「このグループを一緒に直して。終わったら `CONTINUO-STATUS: #45 review` と書いて」と
書くだけで、エージェントがそのとおりに表明を書きます。
...
`1` なら当たっています。`0` なら下を当ててください。

**差し替え方。**`WORKFLOW.md` の本文にある次の2行を探します。
...
**その2行のすぐ下に、次を挿入します。**（2行そのものは消しません。）
...
### 枠を読めないときは、新しい issue に着手しなくなりました

**何が変わったか。**枠の読み取りに失敗している間、**新しい issue を1件も取りません。**
**走っている turn は止まりません。**

**前はどうだったか。**入札（持ち回り）の側だけが黙り、新規着手の側は止めない、
という**逆向きの2つの判定**がありました。入札が先に効くので**結果は同じ**でしたが、
**出るのは `Debug` の1行だけ**で、なぜ進まないのかを知る手立てがありませんでした。

**いまは巡回1回につき `Info` を1行出します。**

```
level=INFO msg="新しい issue には着手しません（走行中の turn は止めません）" 理由=枠を読めない 新規着手が止まる使用率=90 ...
```

**`新規着手が止まる使用率` に注目してください。**これは `rate_limit.pause_above_percent`
（既定95）**ではありません。**持ち回りのマージン（既定10）のほうが先に効くので、
**既定では 90% で止まります。**詳しくは [FAQ.md](FAQ.md) の
「`pause_above_percent` は 95 なのに、90% で止まる」を読んでください。

**当てるものはありません。**

### `continuo doctor` は、本番が使う名前のファイルを作らなくなりました

**何が変わったか。**`continuo doctor` は、検査のために
`~/.continuo/id/<名前>/`・`<workspace.root>/<名前>`・hook の socket を**作らなくなりました。**
**使い捨ての名前で作って消すだけです。**

**当てるものはありません。**打ち間違えた `--id` を渡しても、置き場所が残らなくなります。

**すでに残っているものは、手で消してください。**

```bash
ls ~/.continuo/id/
```

**動かしていない名前があれば、そのディレクトリごと消して構いません。**
