...
**原因。**枠の状態が次の3つのどれかに当たると、continuo は**その巡回で新しい issue を1件も取りません。**
...
**走っている turn は止まりません。**止まるのは新しい issue に着手することだけです。

**確かめ方。**continuo のログで「新しい issue には着手しません」を探してください。
**既定のログレベル（`info`）で、巡回1回につき1行出ます。**理由も一緒に出ます。

```
level=INFO msg="新しい issue には着手しません（走行中の turn は止めません）" 理由=枠の使い過ぎ 新規着手が止まる使用率=90 rate_limit.pause_above_percent=95 five_hour_margin_percent=10 weekly_margin_percent=10 候補=7
```
...
**止めるのは「新しい issue に着手すること」だけです。**

**この向きは、持ち回りの入札と揃えてあります。**
**「分からないなら止まる」**に倒しています。読めない枠を0%（＝いちばん暇）とみなして走り続けると、
**上限を超えてもそれに気づけないためです。**
