...
```bash
cat <worktree のパス>/.continuo.json | jq -r .base
```

**`workspace.identity_file`（記録ファイルの名前。既定 `.continuo.json`）を変えている人へ。**
**エージェントがその記録ファイルを見つけられないと、段3 か段4 へ落ちます。**
`herdr.worktree.base`（worktree を切る分岐元。既定 `null`）で `develop` などを指定していると、
**`develop` から切った worktree に `main` が混ざります。**
**心配なら、`WORKFLOW.md` の本文に1行書いてください。**
...
**取り込めなかったときは、エージェントは直さずに止まります**（Status が `Blocked` になります）。
衝突したまま作業を続けると、直したものがマージのときに捨てられるためです。
