...
**カンバンを1回読みます**（2リクエスト・計8ポイント）。`gh` の認証が要ります。
...
**front matter の `language`（画面に出す文言の言語。既定 `auto` で、環境変数 `LANG` から決めます）とは
別物で、連動しません。**`language` は continuo が画面へ出す文言だけを決めます。
**エージェントが何語で書くかは、本文にしか書けません。**

**なぜ、いまは連動しないのか。**組み込みの指示書が、まだ日本語しか持っていないためです。
連動させても、**指示書の大半が日本語のまま「英語で書け」とだけ言う状態になります。**

**これは「日本語だけにする」と決めたものではありません。**英語版をまだ作っていないだけです。
**組み込みを日本語と英語で持つ作業は、別に立ててあります**（issue #226。
「日本語を読まない利用者が、エージェントへ送られる指示書の大半を日本語のまま受け取る」）。
**そちらが入ったら、この節を `language` に連動させるかどうかを決め直します。**

**v0.1.14 までの雛形は「すべて日本語で書いてください」を既定にしていました。**
**v0.1.15 の雛形からは、この既定が消えて空になります。**
