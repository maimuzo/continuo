...
**担当しているリポジトリへは、push も丸ごと免除に入り、そこから3つが外れます。**送り先が既定の branch だと読み取れる push、branch でない ref（タグなど）への push、ref を消す push です。**`git push -u origin HEAD` は、送り先の branch 名がコマンドの文字列に無いので、外れる3つのどれにも当たりません。**進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` も、免除の文に名指しで書いてあります（既定の branch の名前は、その issue のリポジトリの実物を使います）。
...
**担当しているリポジトリでも、免除はそこで止まります。**免除の文が、こう言い切っています——**「リポジトリ X の GitHub 上のもの（issue、pull request、ref、release、設定）と、その issue が載っているカンバンへの書き込みのうち、いま挙げた形に当たらないものは、いま担当している issue に関係していても断る」。**
