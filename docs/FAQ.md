...
Status の名前 / 対応表のキー / clone / 信頼登録 / 資格情報の14個を調べます。
...
**先に知っておくこと。****いまの版では、何を書いても画面には日本語が出ます。**
英語の資源は、日本語の資源をそのまま複製したものだからです。**英語の文章はまだ1文字も存在しません。**
`language:` に書けるのは「どの言語を選ぶか」であって、いま画面が変わるわけではありません。
...
| `language:` の値 | 選ばれる言語 | いま画面に出る文言 |
| --- | --- | --- |
| `ja` | 日本語 | **日本語** |
| `en` | 英語 | **日本語**（訳がまだ無く、日本語へ落ちるため） |
| `auto`（雛形の値）または未記入 | 環境変数 `LANG` から決める。決まらなければ英語 | **日本語** |

**`ja` と書いておく意味。****英語の訳が入った版に上げても、日本語のままになります。**
書かずに上げると、`LANG` を持たない環境（CI・コンテナ・`env -i`）で英語に変わります。
...
**いま何語が選ばれているかは、画面からは見分けられません。**`ja` でも `en` でも日本語が出るためです。
**確かめられるのは「書いた値が読めているか」だけです。**`設定ファイル` の行が `✓` なら読めています。
