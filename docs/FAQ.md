...
**直し方。**どちらか1つを行い、ボードでその issue を `Ready` へ置き直してください。

| どうしたいか | 何をするか |
| --- | --- |
| **continuo に任せる** | GitHub の画面で、その担当者を外す |
| **continuo に任せるが、担当者は残したい** | **continuo が使うアカウントへ付け替える** |
| **人間が自分でやる** | そのままにする。**ボードから外すか、Status を `Ready` 以外へ動かしてください** |
