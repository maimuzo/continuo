...
| --- | --- | --- |
| **pane が動いている**（`agent.get` の `state_change_seq`（agent の状態が変わるたびに増える連番）が変わった、または `agent_status` が `idle` でも `done` でもない） | **止まるまで待ちます。**人間の指示が「今 pane の内容が動いていたら、それが止まるまで待って」です。**サブエージェントの出力も同じ pane に出ます** | **そのまま残ります。**次の巡回でまた見ます |
| **画面の状態を読めない**（herdr へ届かない） | **手放しません。**次の巡回でやり直します。**確かめられないのに pane を閉じて担当を外すのは取り返しがつきません** | **そのまま残ります** |
