...
**`after_run` が走らない場合。**次の4つのどれかに当たると、continuo は手放しそのものを見送ります。
**`after_run` も走りません。**
...
| **担当が既に別の機械へ移っている** | **担当者にもコメントにも触りません。**`after_run` も走らせません。**`git push` を書いていると、別の機械が作業中の branch へ飛ぶ**ためです。**ただし pane は閉じ、run は印から外します**（この機械の仕事ではなくなったためです） |
| **`gh` の持ち主を取れない** | **手放しません。**次の巡回でやり直します。**`after_run` の印を空撃ちしないため**に、外す相手を先に確かめています |
