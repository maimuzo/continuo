...
**相手のコードや配布物を変える操作は、これまでどおり検査の対象です**（他のリポジトリへの push、パッケージの公開、pull request の取り込みと却下と承認、release の作成、ラベルや担当者の付け外し）。**`git push -u origin HEAD` のように、いま居る branch をそのまま送る形は、免除の文に名指しで書いてあります。**
