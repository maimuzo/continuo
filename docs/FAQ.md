...
| --- | --- | --- |
| **画面が動いている**（`agent.get` の版が増えている、または `agent_status` が `idle` でも `done` でもない） | **止まるまで待ちます。**人間の指示が「今 pane の内容が動いていたら、それが止まるまで待って」です。**サブエージェントの出力も同じ pane に出ます** | **そのまま残ります。**次の巡回でまた見ます |
