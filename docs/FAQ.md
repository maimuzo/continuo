...
確かめ方は、このファイルの原文（GitHub なら Raw、手元ならエディタ）の、この行のすぐ下にあります。

<!--
目次の検算。2本あります。bash か zsh で叩いてください（どちらも <(…) を使っています）。

1. 手元だけで済むほう。push もネットワークも要らないので、commit する前に叩けます。
   (a) と (c) が1行も出さず、(b) が 0 でなければ合格です。

     ( export LC_ALL=C
       cd "$(git rev-parse --show-toplevel)" || exit 1

       # (a) 同じ文言の見出しが2つ無いこと。
       #     2つあると GitHub は後のほうの id に -1 を付けるので、
       #     目次の2本目のリンクが1本目の節へ飛びます。
       awk '/^```/{f=!f;next} !f && /^#{1,6} /' docs/FAQ.md \
         | sed -E 's/^#+ +//' | sort | uniq -d

       # (b) 目次を何行読めたか。0 なら ## 目次 の見出しの文言が変わっています。
       #     ここを見ないと、1行も読めていない状態が「合格」に見えます。
       awk '/^## 目次$/{t=1;next} t&&/^---$/{exit} t&&/^ *- \[/' docs/FAQ.md | wc -l

       # (c) 目次の項目と見出しが、入れ子の深さも文言も1対1であること。
       diff \
   ...
   LC_ALL=C をパイプライン全体へ効かせてください。sort にだけ付けると
   uniq が locale で照合し、(a) が偽の重複を6件出します（実測）。

2. push したあとに叩くほう。GitHub が見出しへ実際に付けた id を取り、
   目次のリンクの飛び先が、その中に在るかを見ます。1行も出なければ合格です。
...
ただし、見出しを足す人が、その節からこのコメントまで戻ってくるとは限りません。
