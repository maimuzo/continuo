...
| --- | --- |
| **画面が動いている**（`agent.get` の版が増えている） | **枠待ちと判定しません。**枠を待っているのではなく、**1回のツール呼び出しに時間が掛かっているだけ**だからです。この判定は枠待ちに入る前に行います |
| **issue かコメントを読めない** | **手放しません。**次の巡回でやり直します。**pane を閉じて担当を外すのは取り返しがつかない**ので、分からないときは動きません |
