...
| --- | --- | --- |
| **枠に余裕が無いので、入札の要る issue には着手しません** | 入札の余裕値（`100 − 使用率 − マージン`）がマイナスか、枠を読めていません。**担当が既に自分にある issue は、そのまま着手します** | **待てば自分で再開します。**すぐ動かしたいときは `five_hour_margin_percent` / `weekly_margin_percent`（どちらも既定10）を下げてください |
...
**枠の余裕が足りないとき、continuo は入札しません。**
**この判定が効き始める使用率は、`rate_limit.pause_above_percent` で止まる使用率より低いところにあります**（既定で91%と96%）。
**そのため、91〜95%の帯では入札の側だけが効きます。**

**コードの上での順序は逆です。**`rate_limit.pause_above_percent` の判定が先に走り、入札の判定はそのあとです。
**効き始める使用率の低さと、判定が走る順序は別のものです。**混ぜて読まないでください。
