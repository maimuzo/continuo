...
**書く決まりの例。**次の3つは、continuo が送る指示書も同じことを言っています。
**`CLAUDE.md` に書いておくと、人間が自分で Claude Code を動かすときにも効きます。**

**3つ目は、書かないと片手落ちになります。**continuo が送る指示書は、
**continuo が起動したエージェントにしか届きません。**
**あなたが自分で動かした Claude Code が書いたコメントには、印が付きません。**
...
## AI が書いたコメントには、機械が書いた印を付ける

issue と pull request へあなたが書くコメントには、<!-- continuo:ai --> の1行を入れてください。
人間と AI は同じ GitHub アカウントで投稿するので、これが無いと見分けが付きません。

置き場所は「本文の先頭に並ぶ印の、いちばん後ろ」です。
既にある目印（<!-- continuo:agent --> や <!-- code-review-result -->）を1つも動かさないでください。

<!-- design-review-skipped --> の断りにだけは付けないでください。
```

**3つ目に書いてあるのは決まりだけで、仕組みの説明は入れていません。**
**「なぜ先頭へ置いてはいけないか」と「なぜ断りだけ例外なのか」は continuo の側にあり、版を上げれば新しくなります。**
**理由を知りたくなったら、下の「issue のコメントを、人間が書いたのか AI が書いたのか見分けたい」を読んでください。**
**5行とも写してください。**とくに最後の1行（飛ばす断りには付けない）を落とすと、
**理由を1文字も書かない断りが CI を通ります。**

**レビュー結果の2つの目印を、機械が数えます**（`design-review-result` と `code-review-result`）。
**`<!-- continuo:ai -->` は、いま数えている機械がありません。**人間が読み分けるための印です。
`continuo init` が置いた `continuo-ci.yaml` を
...
**原因。**continuo を使うと、1つの issue に4種類の書き手が現れます。
**4種類とも、同じ GitHub アカウントで投稿します。**continuo 本体も、continuo が起動した Claude Code も、
あなた自身が動かした Claude Code も、あなた本人も、投稿者は同じ名前になります。
**`author_association` も全部 `OWNER` です。**

**v0.1.15 から、機械が書いたコメントには印が付きます。**

```
<!-- continuo:ai -->
```

**この印があれば、人間ではなく機械が書いた本文です。**
**無いことは、人間が打鍵したことの証拠にはなりません。**下の表の3行目と4行目が混ざります。

| 誰が書くか | 印が付くか |
| --- | --- |
| **continuo 本体**（引き渡しの通知・Status を動かした記録・入札・担当の決定） | **付く** |
| **continuo が起動した Claude Code** | **付く** |
| **あなた自身が動かした Claude Code**（continuo の外） | **付かない。**上の「`CLAUDE.md` には何を書けばいい？」の3つ目を書いてください |
| **人間** | **付かない** |
...
**印だけを見て、人間の決定を捨てないでください。**
**AI が人間の決定を記録するときは、本文の中で「人間がこう決めました」と名乗らせてください。**

**置く場所は「本文の先頭に並ぶ印の、いちばん後ろ」です。**
...
**先頭へ置いてはいけません。**continuo と CI は「本文の先頭がこの目印で始まっているか」で数えています。
**前へ1行入れると、その数え方が外れます。**

**v0.1.14 までのコメントには、印がありません。**AI が書いたものにも付いていません。
**遡って付け直す仕組みはありません。**
**古いコメントを「印が無いから人間が書いた」と読まないでください。**

**設定に足すものはありません。**印は固定で、`WORKFLOW.md` から変えられません。
**機械ごとに違う値にできると、別の機械や別の project が書いた印を読めなくなるためです。**
**綴りが固定であること自体が、この印の値打ちです。**あなたの project の設定を知らない読み手でも、機械の本文を当てられます。
