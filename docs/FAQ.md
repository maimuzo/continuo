...
**既定の水準（info）で出るのは、次の4つです。**まずこれを探してください。

```bash
grep -E '枠に余裕が無いので|枠が閾値を超えているので、この巡回では|空きスロットが尽きた|必須のラベルが揃っていない' <ログの出力先>
```

| 出ている行 | 何が起きているか | どうするか |
| --- | --- | --- |
| **枠に余裕が無いので、入札の要る issue には着手しません** | 入札の余裕値（`100 − 使用率 − マージン`）がマイナスか、枠を読めていません。**担当が既にこの機械にある issue は、そのまま着手します** | **待てば自分で再開します。**すぐ動かしたいときは `five_hour_margin_percent` / `weekly_margin_percent`（どちらも既定10）を下げてください |
| **枠が閾値を超えているので、この巡回では1件も着手しません** | 定額プランの枠が `rate_limit.pause_above_percent`（既定95%）を超えました。**担当が既に付いている issue も含めて止まります。走っている turn は止めません** | **待てば自分で再開します。**すぐ動かしたいときは `pause_above_percent` を上げてください |
...
**上の2つには、観測した使用率と、枠ごとの閾値が入っています。**

```
理由=余裕値がマイナス 5時間の枠の使用率=30 1週間の枠の使用率=92
5時間の枠の閾値="90% を超えたら止まります" 1週間の枠の閾値="90% を超えたら止まります"
```

**4つとも出ていないときは、担当者を見てください。**
...
| いちばん使っている枠の使用率 | 担当者のいない issue を取るか | 担当が自分の issue を取るか | 既定の水準で見えるか |
| --- | --- | --- | --- |
| 〜90% | 取る | 取る | 正常に着手します |
| **91〜95%** | **取らない** | **取る** | **`枠に余裕が無いので` の1行が出ます**（v0.1.15 から） |
| 96%〜 | 取らない | **取らない** | `枠が閾値を超えているので` の1行が出ます |

**どちらの行も、止まっているあいだは巡回のたびに出ます**（既定30秒に1回）。
**カンバンが空でも出ます。**候補の数で黙らせると、必須のラベルが足りない・
信頼していないリポジトリ・バックオフ中・空きスロットが尽きた、のどれかで候補が全部落ちる機械が、
**枠で止まっていても1行も出さなくなるためです。**
...
**v0.1.11 から出ます。**
**`枠に余裕が無いので` は v0.1.15 からです。**それまでは `Debug` でしか分かりませんでした。
**残る2つは前から INFO で出ています。**

**4つとも WARN ではなく INFO です。**どれも異常ではなく、
...
出ていなければ、**`枠に余裕が無いので` の行を探してください。理由と、観測した使用率がそこに出ます。**
**既定のログレベル（`info`）で出ます**（v0.1.15 から。v0.1.14 までは `debug` でしか出ませんでした）。

```bash
grep '枠に余裕が無いので' <ログの出力先>
```
...
| **枠の使い過ぎ** | 待つか、`rate_limit.pause_above_percent` を上げてください。**ただし、上げても入札のマージンの門は動きません。**`five_hour_margin_percent` / `weekly_margin_percent`（どちらも既定10）から決まる `100 − マージン` のほうが先に効きます。**既定では 91% から、担当者のいない issue を取らなくなります** |
...
### 1週間の枠を使い切ったあと、issue の担当が外れた

**原因。**1週間の枠を使い切った run が、`rate_limit.weekly_wait_limit_minutes`（既定300分＝5時間）を
超えて待つことになったので、**continuo が自分で担当を手放しました。**
...
**待ち続けると、その issue を抱えたまま何日も止まります。**
**利用者からは「動いているのに何も進まない」に見えます。**
...
| **herdr の pane** | **閉じます** |
| **`workspace_hooks.after_run`** | **担当を外す前に走ります。**ここに `git push` を書いている人は、手放す前に push されます。**ただし下の3つのどれかに当たると、1回も走らないまま手放しを見送ります** |
...
**`after_run` が走らない場合。**次の3つのどれかに当たると、continuo は手放しそのものを見送ります。
**`after_run` も走りません。**

| 何が起きているか | continuo はどうするか |
| --- | --- |
| **画面が動いている**（`agent.get` の版が `claude.turn_timeout_ms` のあいだに増えている） | **手放しません。**枠を待っているのではなく、**1回のツール呼び出しに時間が掛かっているだけ**だからです |
| **画面の版・issue・コメントのどれかを読めない** | **手放しません。**次の巡回でやり直します。**pane を閉じて担当を外すのは取り返しがつかない**ので、分からないときは動きません |
| **担当が既に別の機械へ移っている** | **手放しません。**`after_run` も走らせません。**`git push` を書いていると、別の機械が作業中の branch へ飛ぶ**ためです（下の「複数の機械で見張っているのに…」と同じ扱いです） |
| **その issue が draft か、`gh` の持ち主を取れない** | **手放しません。**次の巡回でやり直します。**`after_run` の印を空撃ちしないため**に、外す相手を先に確かめています |

**枠の使用率を直前に読めなかったときも手放しません。**資格情報が切れた機械は、
切れた時点の値を返し続けるので、**その古い値で pane を閉じて担当を外すと取り返しがつきません。**

**1台で動かしている場合。**拾う機械はこの1台だけです。
**担当が外れても、別の issue が進むようにはなりません。**
1週間の枠が満杯のあいだは `rate_limit.pause_above_percent` の門が、
**新しい issue の着手を全部やめさせている**ためです（**巡回そのものは続いています。**
走っている run の面倒は見ますし、枠の読み直しも続きます）。
**実際に変わるのは、pane が閉じることと、担当が外れることだけです。**
**枠が戻ったら、同じ機械が拾い直します。**

**5時間の枠では起きません。**この上限は1週間の枠にだけ効きます。
