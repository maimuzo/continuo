...
**相手のコードや配布物を変える操作は、これまでどおり検査の対象です**（push、パッケージの公開、pull request の取り込みと却下と承認、release の作成、ラベルや担当者の付け外し）。
**担当しているリポジトリでは、そのうち merge・close・approve・release の作成・パッケージの公開・ラベルや担当者の付け外し・リポジトリの設定の書き換え（公開と非公開の切り替えなど）・pull request を通さないファイルの書き込み・既定の branch へ直に送る push を、名指しで断ります**（既定の branch の名前は、その issue のリポジトリの実物を使います）。
**担当している worktree の branch への push と、コメントを `gh api` で直に書く形は、免除の文に名指しで書いてあります。**
...
**引き換えに、知っておいていただきたいことがあります。**

> **公開 issue に文を書ける人が、あなたの GitHub アカウントを使って、
> 任意のリポジトリへ任意の文の issue と pull request を投稿させられます。**

**なぜ止められないか。**エージェントは issue の本文とコメントを読んで作業します。
**そこに「〇〇へこう報告して」と書いてあると、それが作業の指示に見えます。**
**判定する側はコマンド1回ぶんしか見ないので、それが人間の依頼なのか、
外部の人が書いた注入なのかを、コマンドの文字列からは見分けられません。**
**1件でも成立します。量の問題ではありません。**

**塞ぐ道は4つ検討して、4つとも採りませんでした。**

| 切れる場所 | なぜ採らなかったか |
| --- | --- |
| **担当しているリポジトリだけ通す** | **不具合を、原因のあるリポジトリへ報告できません** |
| **本文と題名の中身を見る** | **まともなバグ報告は、コマンドの出力とファイルの抜粋でできています。**切ると報告そのものが止まります |
| **回数を数える** | **1件でも成立するので、回数を数えても防げません** |
| **作った issue の URL を報告に並べる** | 塞ぐ手立てではなく気づく手立てです。**別の作業として切り出しました** |

**止めたいときにできること。**

| どうするか | 何が起きるか |
| --- | --- |
| **公開リポジトリの issue を continuo に処理させない** | この経路は使われません。**いちばん確実です** |
| `claude.tool_gate.mode: off` | **逆効果です。**判定そのものが消えるので、取り消せない破壊・資格情報の持ち出し・権限の昇格を止める仕掛けまで無くなります |
| **自分のアカウントの投稿を見張る** | GitHub の通知に、自分の名前で立った issue が出ます |

**この能力だけを止めるつまみはありません。**

**残っている守り。**書き込む中身が鍵・トークン・資格情報・環境変数のときは、
...
| どうするか | 書く行 | 引き換えに失うもの |
| --- | --- | --- |
| **そのままにする** | なし | **上の「引き換えに、知っておいていただきたいこと」が残ります。**公開 issue に文を書ける人が、あなたのアカウントで任意のリポジトリへ issue を投稿させられます |
