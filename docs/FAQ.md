...
**issue と pull request を作ることと、その本文やコメントを書くことは、相手を問わず断られにくくなりました。**
**作業の中で気づいたことを、原因のあるリポジトリへ報告できるようにするためです。**
**fork へ push して本家のリポジトリへ pull request を出す形は、これまでどおりです**（断る条件の「関係のない」という限定が受け持ちます。**「fork への push は通す」とは書いていません。**そう書くと、任意のリポジトリへの push が「fork への push」と名乗るだけで通ってしまいます）。
**担当しているリポジトリへは、push も丸ごと免除に入り、そこから3つが外れます。**送り先が既定の branch だと読み取れる push、branch でない ref（タグなど）だと読み取れる push、ref を消す push です。**`git push -u origin HEAD` は、送り先の branch 名がコマンドの文字列に無いので、外れる3つのどれにも当たりません。**進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` も、免除の文に名指しで書いてあります（既定の branch の名前は、その issue のリポジトリの実物を使います）。
**タグを作る push は入りません。**`v*` のタグで release を公開する仕掛けを持つリポジトリでは、`gh release create` と同じ結果へ届いてしまうためです。
**担当先の外へは、push は入りません**（相手のコードや配布物を変える操作は、これまでどおり検査の対象です。push、パッケージの公開、pull request の取り込みと却下と承認、release の作成、ラベルや担当者の付け外し）。

**担当しているリポジトリでも、免除はそこで止まります。**免除の文が、こう言い切っています——**「リポジトリ X の GitHub 上のもの（issue、pull request、ref、release、設定）への書き込みと削除、そして GitHub Projects v2 のカンバン（project）への書き込みと削除のうち、いま肯定した形に当たらないものは、いま担当している issue に関係していても断る」。**
**種類（コードや配布物を変える操作、など）では閉じていません。**承認や close は、その瞬間にコードも配布物も1バイトも変えないので、種類では捕まらないためです。
**そのうえで、pull request を通さないファイルの書き込み（`gh api` の contents など）も、肯定から名指しで外しています。**直に push するのと同じ結果へ、別の道具で届いてしまうためです。

**pull request の承認と取り込みは、コメントに数えません。**`gh pr review --approve` は、道具の形としては pull request へのコメントの投稿なので、そのままだとコメントの免除に入ってしまいます。
**必須のレビューや自動マージを設定したリポジトリでは、承認がそのままコードを入れます。**だから免除の文が、承認・取り込みと、issue と pull request を閉じることを、コメントの書き込みから名指しで外しています。

**「必ず通る」ではありません。**判定するのは最後まで LLM で、**伝えているのは判定に要る材料です。**

**v0.1.15 まではそうではありませんでした。**検査する側に担当先を伝えていなかったので、
**担当しているリポジトリへの `gh issue create` まで「関係のない外部への書き込み」と読まれて断られていました。**

**中身による守りは、この変更でも外れていません。**書き込む中身が鍵・トークン・資格情報・環境変数のときは、
相手がどのリポジトリでも断ります。**ただし、エージェントが読んだ中身を文字列として貼る形は止まりません**
（`Read` は判定に回らないので、判定役から見ると普通の文章と区別が付きません）。
...
| どうするか | 書く行 | 引き換えに失うもの |
| --- | --- | --- |
| **そのままにする** | なし | **その呼び出しは通りません。**エージェントは別のやり方を試すので、多くはそのまま進みます |
