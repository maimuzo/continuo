...
**この issue のコメントで、あなたの言葉で許可を出してください。**判定役はそれを読みます。決まった言い回しはありません。
...
continuo は `--permission-mode auto`（既定）で起動します。**リード自身の許可は判定役が決めます**（`dontAsk` を選んだときは、許可の一覧の外がその場で拒否されます）。**どちらでも、リード自身が一歩ずつの承認を求めることはありません**（`auto` で遮断が続いたときに確認へ戻るかは、観測できていません）。
...
**いちばん変わるのは引用の置き方です。**返信したい内容ごとに節が立ち、**節ごとに引用が付きます。**
**コメントの先頭に引用を固める形ではなくなりました。**
...
**`1` 以上が返れば入っています。**`0` なら古い形のままです。
**件数そのものは見ないでください。**この見出しは新しい形にしか無いので、在るかどうかだけで見分けられます。
いくつ出るかは指示書の書き方で変わります。
