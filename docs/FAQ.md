...
**ただし、その担当者名の `<!-- continuo:hold -->` が残っていない場合に限ります。**
過去にその人の機械が入札で取ったことのある issue では、期限が働いて担当者が外れます。
**下の「チームで、issue ごとに担当の PC を決めたい」の「その PC が壊れたとき」に、確かめ方があります。**
...
**書くのは、その PC につき1回です。**同じカンバンを見張る PC が5台あれば、5件付きます。
...
| --- | --- | --- | --- |
| `warn_and_comment`（既定） | 出ます | 出ます | **その PC につき1回書きます** |
...
**書くのは、その PC につき1回です。**「もう書いた」の記録は、その PC の `gh` の持ち主が
...
**その issue を、あなたの PC が過去に入札で取ったことが無ければ、他の PC は拾いません。**
18時間の期限も働きません。

期限が働くのは、入札で担当を取ったときだけです。
そのとき continuo は `<!-- continuo:hold -->` のコメントを1件書き、それが期限を数える起点になります。
人間が付けた担当者には、このコメントがありません。

**ただし、このコメントは担当が移っても消えません。**
**過去に一度でもあなたの PC がその issue を入札で取っていると、あなたの名前の入った
`<!-- continuo:hold -->` が残ります。**その状態であなたを担当者に付け直すと、
**他のメンバーの PC が担当者を外し、入札をやり直します。**

**18時間を数える起点は、担当者を付け直した時刻ではありません。**
**残っている古い `<!-- continuo:hold -->` のコメントが作られた時刻です**
（[internal/handoff/assess.go:338](../internal/handoff/assess.go#L338) の `lastProgressOf`）。
**その時刻はたいてい既に18時間より前なので、付け直した直後の巡回で外れます**
（巡回の間隔は `polling.interval_ms` で決まり、既定は30秒です）。**猶予はありません。**
...
gh issue view <番号> --repo <owner>/<repo> --json comments \
  --jq '.comments[]
        | select((.body | sub("^[[:space:]]+";"")) | startswith("<!-- continuo:hold -->"))
        | .body | capture("\"assignee\"[[:space:]]*:[[:space:]]*\"(?<a>[^\"]+)\"").a'
```

**あなたのアカウント名が出たら、期限が働きます。**1行も出なければ働きません。

**`contains("continuo:hold")` で探してはいけません。**continuo が hold と認めるのは
**本文がその印で始まるコメントだけ**で（[internal/handoff/handoff.go:603-614](../internal/handoff/handoff.go#L603-L614)）、
**照合するのはコメントの投稿者ではなく、hold の中に書かれた `assignee` です**
（[internal/handoff/assess.go:400-423](../internal/handoff/assess.go#L400-L423)）。
**印の話をしただけの議論のコメントまで数えると、hold が1件も無い issue でも「ある」と出ます。**

**拾わせたいときは、GitHub の画面で担当者を外してください。**外れた瞬間から入札の対象に戻ります。

これは意図した守りです。人間が割り当てた issue を、機械が勝手に取り上げないためです。
