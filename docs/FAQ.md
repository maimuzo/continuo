...
**逆向きの落とし穴が1つあります。****成果の報告の本文に、この印そのものを書かないでください。**
continuo は印が本文のどこにあっても数えるので、
**「進捗報告の印は付けないでください」と説明するつもりで印を貼ると、その報告が途中経過と読まれます。**
**書かれているのに、continuo はセッションを復元してもう一度書かせます。**
説明したいときは、山括弧を外して `continuo:progress` と書いてください。
...
| 順 | 何を見るか |
| --- | --- |
| 1 | **`WORKFLOW.md` の本文に指定があれば、それ** |
| 2 | **worktree の直下にある continuo の記録ファイル**（既定 `.continuo.json`）**の `"base"` の値** |
| 3 | issue にリンクされた branch |
| 4 | リポジトリの既定 branch |

**段2 がいちばん確かです。**continuo は worktree を作るときに、実際に使った分岐元をそこへ書いています。
**確かめ方。**
