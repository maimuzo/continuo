...
| **3** | **切りがついたら、カードを `In Progress` か `Ready` へ戻す。**同じ pane・同じ会話のまま、続きの指示が飛びます（**最大2巡回、既定60秒かかります**） |
...
**「抱えている」には、やり直し待ちの issue も入ります**（GitHub の担当者とは関係ありません。下の「担当者は1バイトも見ません」）。
**着手に失敗して待っている issue は、pane が無くても抱えたままです。**
そこへ動かしても、その場では pane ができません。**そのまま待ってください。**
**待ちが明けると抱えている状態が自分で解け、次の巡回で pane ができます**（合わせて最大5分半）。
**ただし、リポジトリの信頼登録が外れている・branch を別の worktree が使っている、といった場合は、待っても直りません。**
その場合は下の段2 へ進んでください。

**5分半待っても pane が来ないときは、原因が別です。**herdr が落ちるなどして
**pane だけが消えた場合、continuo はそれに気づけません。**待っても永久に来ません。
**症状はどちらも同じ**（`Direct Chat` にカードがあるのに pane が無い）**なので、この順でやってください。**

| 順 | 何をするか |
| --- | --- |
| **1** | **最大5分半待つ** |
| **2** | それでも来なければ、**カードを1度 `In Review` か `Blocked` へ動かしてから `Direct Chat` へ戻す**（1巡回、既定30秒で効きます） |
...
| **18時間を超えるなら、コメントを1件書いてください** | **continuo が先に着手していた issue を direct chat へ動かした場合だけです**（そちらは continuo が自分を担当者に書いてあります）。`Direct Chat` のあいだは進捗の報告が書かれないので、18時間（`idle_timeout_ms`）を超えると別の PC が担当を引き取りにきます。本文の先頭に進捗の目印（`WORKFLOW.md` の雛形にある `continuo:progress` の HTML コメント）を置いたコメントを1件書けば、そこから数え直します。**continuo が何もしていない issue を direct chat へ動かした場合は、担当者を書いていないので当たりません** |
| **担当者は1バイトも見ません** | 誰が担当者に付いていても pane を用意します。**自分を担当者に付けたまま direct chat を使えます。****代償として、何台かで同じカンバンを見張っていると、機械の数だけ pane ができます**（人間が使うのは1つで、残りは枠を食ったまま残ります） |
| **continuo を再起動した場合** | **pane はそのまま残り、continuo が引き取ります。**戻したときも同じ pane に続きの指示が飛びます。**引き取れなかったとき**（herdr が agent を登録し直していない、などの場合）**も pane は閉じません**が、戻した最初の巡回で作り直します。そのときは同じ会話へ復帰したうえで、1回目の指示書が送られます |
...
担当者を書き換えた時点で、そのターンの終わりに気づいて止まります。

> **例外が1つあります。****その issue を1度でも `Direct Chat` へ動かした機械は、止まりません。**
> direct chat は担当者を1バイトも見ない機能なので、**確かめ直しも当てません**（「使い方」の節）。
> **この免除は解けません。**カードを戻しても、その run が終わるまで続きます。
> **止めたいときは、その機械でカードを `In Review` か `Blocked` へ動かしてください。**
