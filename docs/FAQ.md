...
```bash
grep -E '枠に余裕が無いので|枠を読めないので' <ログの出力先>
```
...
#### 1週間の枠に余裕が無くなったあと、issue の担当が外れた

**原因。**1週間の枠に余裕が無くなった run が、`rate_limit.weekly_wait_limit_minutes`（既定300分＝5時間）を
超えて待つことになったので、**continuo が自分で担当を手放しました。**

**「使い切った」ではありません。**判定は**余裕値**（`100 − 使用率 − マージン`）が0以下かどうかで、
**マージンが既定の10なら使用率90%で当たります。**claude.ai の画面が100%になるのを待つ必要はありません。
...
| --- | --- | --- |
| **pane が動いている**（`agent_status` が `working`） | **止まるまで待ちます。**人間の指示が「今 pane の内容が動いていたら、それが止まるまで待って」です。**サブエージェントの出力も同じ pane に出ます** | **そのまま残ります。**打ち切りの判定も `working` の run には手を出しません |
| **`agent_status` が `blocked` か `unknown`** | **手放しません。**止まったとは言えないためです | **打ち切りの判定へ回ります。**無音が `claude.turn_timeout_ms` を超えていれば、**pane を閉じてリトライを1つ積みます** |
| **`idle` だが、前の巡回から `state_change_seq`（agent の状態が変わるたびに増える連番）が動いた** | **手放しません。**2回続けて同じ連番でないと「止まった」と言えないためです | **1回目の観測の直後の1巡回だけ守ります。**そのあとも連番が動き続ける run は、**打ち切りの判定へ回ります** |
| **画面の状態を読めない**（herdr へ届かない） | **手放しません。**確かめられないのに pane を閉じて担当を外すのは取り返しがつきません | **打ち切りの判定へ回ります。**同じ巡回でもう1回 `agent.get` を叩き、また読めなければ**pane を閉じてリトライを1つ積みます** |
| **`gh` の持ち主を取れない・issue かコメントを読めない** | **手放しません。**次の巡回でやり直します。**`after_run` の印を空撃ちしないため**に、外す相手を先に確かめています | **そのまま残ります** |
| **担当が既に別の機械へ移っている** | **担当者にもコメントにも触りません。**`after_run` も走らせません。**`git push` を書いていると、別の機械が作業中の branch へ飛ぶ**ためです | **畳みます。**pane を閉じ、印から外します。**この機械の仕事ではなくなったためです。**見送りではありません |
...
### 走っている continuo の中身を見たいとき

#### `--port` で開く画面は何のためのものですか

**走っている continuo が、いま頭の中に持っているものを見るための画面です。**

**カンバン（GitHub Projects v2）を見れば、どの issue がどの Status にあるかは分かります。**
**分からないのは「その Status のまま、いま何が起きているのか」です。**
`In Progress` の issue が、**次の turn を待っているのか、レートリミットの回復を待っているのか、
失敗してやり直しを待っているのか、エージェントが固まっているのか**は、カンバンからは区別が付きません。
...
```bash
continuo --port 8080          # コマンドの引数で指定する（その場かぎり）
```

```yaml
# WORKFLOW.md の front matter に書く（毎回開く）
server:
  port: 8080
```

**`WORKFLOW.md` を書き換えたら continuo を再起動してください**（設定を読み直しても待ち受け先は動きません）。
**書かなければ画面は開きません**（既定では開きません）。

**`http://127.0.0.1:<番号>/` が人間向けの画面、`http://127.0.0.1:<番号>/api/v1/state` が JSON です。**
**127.0.0.1 からしか繋がりません。**認証を持たないので、外へ晒す作りにはしていません。
**画面は10秒ごとに自動で再読み込みします。**
...
| 表 | 何が並ぶか | これを見て分かること |
| --- | --- | --- |
| **着手できずに止まっているもの** | issue / 止めた理由 / いつから / 直し方 | **担当者が付いている issue など、continuo が「触らない」と決めたもの。**issue へ案内を書けなかったときは、その印も付きます |
| **実行中の run** | issue / Status / turn 数 / 最後に hook を受けた時刻 / トークン合計 | **いま走っているもの。**枠待ち・やり直し中・再 dispatch 待ちの印がここに付きます |
| **トークンの集計** | issue ごとの API 応答の件数 / 入力 / キャッシュ作成 / キャッシュ読み出し / 出力 | **どの issue が重いか。**いちばん下の行が、いま走っている run の合計です |
| **run をまたぐ累計** | 上と同じ5項目と、その合計を1行で | **終わった run のぶんも残ります**（下の「累計は何を指しますか」を見てください） |

**とくに、カンバンには出ないのが次の6つです。**

| 何 | 何が分かるか |
| --- | --- |
| **turn 数** | 何往復したか。`agent.max_dispatch_turns`（打ち切りまでの往復の上限）にどれだけ近いか |
| **最後に hook を受けた時刻** | **エージェントが生きているかどうか。**ここが何十分も止まっていたら、固まっている可能性があります |
| **枠待ちの印** | **レートリミットの回復を待って止まっている。**カンバンでは `In Progress` のままです |
| **再開する時刻** | 失敗してやり直しを待っている。**いつ動き出すか** |
| **やり直した回数** | 同じところで繰り返し失敗していないか |
| **トークン** | どの issue が枠を食っているか |

**JSON（`/api/v1/state`）には、画面に出ないものが1つあります。**
`stall_clock_at` は「無音の検知が数え始めた時刻」です。
**「最後に hook を受けた時刻」とは別です**（turn を送っただけでも進むので、生死の判断には使えません）。

#### どういうときに開けばいいですか

| こういうとき | 何を見るか |
| --- | --- |
| **`In Progress` のまま何時間も動かない** | **「最後に hook を受けた時刻」。**止まっていればエージェントが固まっています。**枠待ちの印**が付いていれば、単に待っているだけです |
| **issue を `Ready` にしたのに始まらない** | **「着手できずに止まっているもの」の表。**担当者が付いているなどの理由が並びます |
| **同じ issue が何度もやり直されている** | **「やり直した回数」と「再開する時刻」** |
| **枠をどれだけ使ったか知りたい** | **「run をまたぐ累計」の表** |
| **状況を機械で拾いたい** | `curl -s http://127.0.0.1:<番号>/api/v1/state \| jq .` |

**逆に、ここを見なくてよいとき。**どの issue がどこまで進んだかは**カンバンに出ます。**
**何が起きたかの記録は**ログにあります。**この画面が答えるのは「いま」だけです。**

#### 累計は何を指しますか

**「この continuo が起動してから、turn の終わりに読み取った transcript の合計」です。**

**3つ、気をつけてください。**

| 何 | どうなるか |
| --- | --- |
| **走行中の turn のぶんは入っていません** | 集計は turn が終わったときに1回だけ走ります。**turn が長いと、その run の値はしばらく 0 のままです** |
| **引き継いだ run では、起動より前のぶんも入ります** | continuo を再起動しても pane の Claude Code は生きたままなので、その transcript には前のぶんが残っています |
| **continuo を再起動すると 0 に戻ります** | メモリだけに持っているためです。**課金の明細の代わりにはなりません** |
...
**まず issue を開いて、入札のコメントを見てください。**ログより先にこちらです。
**着手する機械は入札で決まる**ので、**誰も入札していなければ、どの機械にも余裕がありません。**

| どう使っているか | 入札が1件も無いとき、何が起きているか |
| --- | --- |
| **1台で使っている** | **その1台に余裕がありません。**claude.ai の使用量を見てください |
| **何台かで分担している** | **他に continuo を動かしている機械が無いか、全部の機械に余裕がありません** |

**ログだけで全体を追うことはできません。**
**他の機械の状況を知る手立ては入札しかない**ので、**ログには自分の分しか出ません。**

**自分の機械が入札しなかったときは、既定の水準（info）で次の1行が出ます。**

```bash
grep -E '枠に余裕が無いので|空きスロットが尽きた|必須のラベルが揃っていない' <ログの出力先>
```

| 出ている行 | 何が起きているか | どうするか |
| --- | --- | --- |
| **枠に余裕が無いので、入札の要る issue には着手しません** | 入札の余裕値（`100 − 使用率 − マージン`）が0以下か、枠を読めていません。**担当が既に自分にある issue は、そのまま着手します。走っている turn も止めません** | **待てば自分で再開します。**すぐ動かしたいときは `five_hour_margin_percent` / `weekly_margin_percent`（どちらも既定10）を下げてください |
...
| --- | --- | --- |
| 〜89% | 取る | 取る |
| **90%〜** | **取らない**（`枠に余裕が無いので` の1行が出ます） | **取る** |

**走っている run の打ち切りは、この線では止まりません。**
**止まるのは使用率100のときだけです**（そこで初めて Claude Code が応答できなくなります）。
**90%で打ち切りを止めると、本当に固まった run が長く残ります。**

**境界の出どころ。**90% は `five_hour_margin_percent` と `weekly_margin_percent`（どちらも既定10）から決まります。
**余裕値 `100 − 使用率 − マージン` が0以下になる使用率です。**マージンを20にすれば80%から止まります。

**見るのは5時間の枠だけではありません。**1週間の枠も、それぞれ独立に同じ判定を受けます。
**どれか1つでも余裕値が0以下になれば、そこで止まります。**
**5時間の枠が30%でも、1週間の枠が92%なら止まります。**

**この行は、止まっているあいだ巡回のたびに出ます**（既定30秒に1回）。
...
#### エージェントのコメントが文章だけで、読むのに1往復かかる

**v0.1.15 で直りました。**上げてください。**設定に足すものはありません。**

**v0.1.14 まで何が起きていたか。**組み込みの指示書は、コメントごとに**印と外枠**を決めていました
（判断票なら表、進捗報告なら1行）。**その中に何を書くかは決めていませんでした。**
そのため届くのは文章だけのコメントで、**読む側は構造を頭の中で組み立て直すか、
「詳しく説明して」ともう1往復するかのどちらかになりました。**
