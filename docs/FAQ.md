...
### 版を上げたら「progress_interval_ms の値 3600000 は不正です」で起動しなくなった
...
```
front matter が不正です: 設定のキー tracker.provider.handoff.progress_interval_ms の値 3600000 は不正です:
tracker.provider.handoff.idle_timeout_ms（1800000 ミリ秒）より短くすること
```
...
**60000（1分）以上にしてください。**59999 までは、送る文面が「0分以上黙らない」になります。
