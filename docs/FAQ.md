...
**push の扱いは、この変更の前と同じです。**免除の文は、push について1文字も書いていません。断る条件の3つ目が挙げている「他のリポジトリへの push」で、これまでどおり判定されます。**担当しているリポジトリへの push は、そこに当たりません。****`git push -u origin HEAD` は、送り先の branch 名がコマンドの文字列に無いので、外れる3つのどれにも当たりません。**進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` も、免除の文に名指しで書いてあります（既定の branch の名前は、その issue のリポジトリの実物を使います）。
...
**担当しているリポジトリでも、免除はそこで止まります。**免除の文が、こう言い切っています——**「リポジトリ X の issue、pull request、release、リポジトリの設定への書き込みと削除、そして GitHub Projects v2 のカンバン（project）への書き込みと削除のうち、いま肯定した形に当たらないものは、いま担当している issue に関係していても断る」。**
