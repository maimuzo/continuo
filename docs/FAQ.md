...
**担当しているリポジトリへは、push も免除に入ります。**`git push -u origin HEAD` と、進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` が、免除の文に名指しで書いてあります。
**担当先の外へは、push は入りません**（相手のコードや配布物を変える操作は、これまでどおり検査の対象です。push、パッケージの公開、pull request の取り込みと却下と承認、release の作成、ラベルや担当者の付け外し）。

**担当しているリポジトリでも、免除はそこで止まります。****上に挙げた以外で**コードや配布物を変える操作と、リポジトリやカンバンの設定を変える操作には、この免除が及びません。
**そのうえで3つを名指しで断ります。**既定の branch へ直に送る push、branch や tag を消す push、pull request を通さないファイルの書き込み（`gh api` の contents など）です（既定の branch の名前は、その issue のリポジトリの実物を使います）。
**push を肯定の形で通しているぶん、この3つは種類では閉じられないためです。**
