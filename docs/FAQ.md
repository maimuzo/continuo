...
**担当している issue のリポジトリへの書き込みは、断られません。**
検査する側に「いま担当しているのは `<owner>/<repo>#<番号>` である」と伝えてあるので、
**そのリポジトリへの issue・pull request・コメントの作成と更新は、担当している作業として通ります。**
**別のリポジトリへの書き込みは、これまでどおり検査の対象です。**

**v0.1.14 まではそうではありませんでした。**検査する側に担当先を伝えていなかったので、
**担当しているリポジトリへの `gh issue create` まで「関係のない外部への書き込み」と読まれて断られていました。**
