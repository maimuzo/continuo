...
**相手のコードや配布物を変える操作は、これまでどおり検査の対象です**（他のリポジトリへの push、パッケージの公開、pull request の取り込みと却下と承認、release の作成、ラベルや担当者の付け外し）。**担当している worktree の branch への push と、コメントを `gh api` で直に書く形も、免除の文に名指しで書いてあります。**
