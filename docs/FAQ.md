...
**担当しているリポジトリへは、既定の branch 以外の branch への push も免除に入ります。**`git push -u origin HEAD` と、進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` が、免除の文に名指しで書いてあります（既定の branch の名前は、その issue のリポジトリの実物を使います）。
...
**担当しているリポジトリでも、免除はそこで止まります。**免除の文が、こう言い切っています——**「リポジトリ X への書き込みのうち、いま挙げた形に当たらないものは、リポジトリ X が担当先であることを理由に『関係がある』として通してはならない」。**
**種類（コードや配布物を変える操作、など）では閉じていません。**承認や close は、その瞬間にコードも配布物も1バイトも変えないので、種類では捕まらないためです。
**そのうえで2つを名指しで外します。**branch や tag を消す push と、pull request を通さないファイルの書き込み（`gh api` の contents など）です。

**pull request の承認と取り込みは、コメントに数えません。**`gh pr review --approve` は、道具の形としては pull request へのコメントの投稿なので、そのままだとコメントの免除に入ってしまいます。
**必須のレビューや自動マージを設定したリポジトリでは、承認がそのままコードを入れます。**だから免除の文が、承認・取り込み・却下と、issue と pull request を閉じることを、コメントの書き込みから名指しで外しています。
