...
**1枚の `WORKFLOW.md` を共有していれば、書き換えるのは1回で済みます。**
やり方は下の「1枚の WORKFLOW.md をチームで共有して、余裕がある機械に処理させたい」にあります。
...
> **共有している1枚に当てるときは、当てたあとに `rate_limit.token_source` の行を見てください。**
> OS が混ざるチームでは、その行をわざと消してあることがあります。
> **このコマンドは消した行を `claude_credentials` として書き戻します。**
> 理由は下の「1枚の WORKFLOW.md をチームで共有して、余裕がある機械に処理させたい」にあります。
