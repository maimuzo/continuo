...
**担当しているリポジトリが相手でも同じです。**いま挙げた以外でコードや配布物を変える操作と、リポジトリの設定を変える操作には、この免除が及びません。
**そのうえで2つを名指しで断ります。**既定の branch へ直に送る push と、pull request を通さないファイルの書き込み（`gh api` の contents など）です（既定の branch の名前は、その issue のリポジトリの実物を使います）。
**担当しているリポジトリへの push を肯定の形で通しているので、この2つだけは種類では閉じられないためです。**
