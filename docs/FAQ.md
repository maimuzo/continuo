...
**push の扱いは、この変更の前と同じです。**免除の文は、push について1文字も書いていません。断る条件の3つ目が挙げている「他のリポジトリへの push」で、これまでどおり判定されます。**担当しているリポジトリへの push は、そこに当たりません。****進捗報告を書き足す `gh api --method PATCH .../issues/comments/<ID>` は、免除の文に名指しで書いてあります。**
...
**pull request を通さないファイルの書き込み（`gh api` の contents など）は、この変更では扱いが変わりません。**push と同じく、断る条件の3つ目でこれまでどおり判定されます。
