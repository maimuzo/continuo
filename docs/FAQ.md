...
| **必須のラベルが揃っていないので飛ばします** | `tracker.required_labels` に書いたラベルが、その issue に付いていません。**足りないラベルの名前も同じ行に出ます** | **ラベルを付けるか、`required_labels` を見直してください**。この行は**1つの issue につき1回だけ**出ます |

**3つとも出ていないときは、まず入札を疑ってください。**

**枠の余裕が足りないとき、continuo は入札しません。**その判定は `pause_above_percent` より手前で働きます。

| 5時間の使用率 | dispatch を止めるか | 入札するか | 既定の水準で見えるか |
| --- | --- | --- | --- |
| 〜90% | 止めない | する | 正常に着手します |
