...
**例外は `continuo init` だけです**（そのときはまだ `WORKFLOW.md` がありません。下に書きました）。
...
**`continuo init` だけは `language:` を読みません。**`WORKFLOW.md` をこれから作るコマンドなので、
読む相手がまだありません。**環境変数 `LANG` から決めた言語で出ます**（決まらなければ英語です）。

**残りのコマンドは `language:` に従います。**`continuo setup` の対話も、`continuo doctor` の検査結果も、
`continuo abandon` も、`continuo prompt` も、常駐プロセスも同じです。
