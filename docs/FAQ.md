...
### 画面に出る文言が英語になった。日本語にしたい（画面の言葉を変えたい）

**原因。**`WORKFLOW.md` の `language:` が `auto` のままで、**環境変数 `LANG` から言語を決められなかった**からです。
**決められなかったときは英語になります。**`LANG` が空・`C`・`POSIX` の環境（CI・コンテナ・`env -i`）で起きます。

**直し方。**`WORKFLOW.md` の `language:` に `ja` と書きます。**設定は `LANG` より強いので、これで必ず日本語になります。**
...
**いま何語で出るかは `continuo doctor` の見出し語で分かります。**
