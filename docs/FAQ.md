...
**`--url` に issue の URL を渡してください**（v0.1.15 から）。**その issue の値で展開した全文が出ます。**

```bash
cd ~/continuo-work && continuo prompt --show --url https://github.com/octocat/hello-world/issues/42
```

**番号ではなく URL です。**1枚のカンバンに複数のリポジトリの issue が載るので、
**番号だけでは、どのリポジトリの issue か決まりません。**
...
**カンバンを1回読みます**（104件のカンバンで2リクエスト・計8ポイント）。`gh` の認証が要ります。
**費用はカンバンの大きさで変わります。**100件ずつ読み、探している issue が見つかった時点で止めます。
...
**変数を展開しないまま出すことはしません。**
**このコマンドの目的は「本当に送られる文面」を確かめることなので、
展開に失敗したものを出すと、`--url` を付けなかったときと同じものが出て、気づけなくなります。**

#### continuo を入れたら、`CLAUDE.md` には何を書けばいい？

**原因。**continuo が置くのは `WORKFLOW.md` と `continuo-ci.yaml` の2枚で、
**`CLAUDE.md` には1バイトも書きません。**そのため「進め方の決まりをどこに書くのか」が決まりません。

**直し方。****進め方の決まりは `CLAUDE.md` に書いてください。**
**continuo を使うかどうかに関わらず要る決まりだからです。**Claude Code がプロジェクトごとに
読む場所が `CLAUDE.md` であり、**人間が自分で Claude Code を動かすときにも同じ決まりが要ります。**

| どこへ書くか | 何を書くか | なぜそこか |
| --- | --- | --- |
| **`CLAUDE.md`** | **進め方の決まり。**設計のレビューを実装の前に置く、レビュー結果を貼ってからマージ可能にする、など | **continuo を使わないときにも効かせたい決まりです。**人間が自分で Claude Code を動かすときにも読まれます |
| **`WORKFLOW.md` の本文** | **continuo が起動したエージェントにだけ効く決まり。**テストの走らせ方、pull request の作り方、まとめて直してよい範囲 | 本文は、continuo が送る指示書の真ん中へ挟まります。**`continuo prompt --show` で全文が読めます** |
| **`.github/workflows/`** | **その決まりを機械で止める検査**（`continuo-ci.yaml` を移したもの） | 文書に書くだけでは守られません |

**`CLAUDE.md` に continuo の仕組みを書き写さないでください。**
仕組みの説明は continuo の実行ファイルの中にあり、**版を上げれば自動で新しくなります。**
書き写すと、continuo が説明を直しても、あなたの `CLAUDE.md` には二度と届きません。

```bash
# エージェントへ送られる仕組みの説明は、これで全部読めます
continuo prompt --show --builtin
```

**書く決まりの例。**次の2つは、continuo が送る指示書も同じことを言っています。
**`CLAUDE.md` に書いておくと、人間が自分で Claude Code を動かすときにも効きます。**

```markdown
## 設計のレビューを、実装の前に通す

1. 設計を固める
2. 決めた設計を、その issue のコメントに書く
...
pull request のコメントに、レビュー結果を貼ってください
（1行目を <!-- code-review-result --> にする）。
```

**この2つの目印を、機械が数えます。**`continuo init` が置いた `continuo-ci.yaml` を
`.github/workflows/` へ移すと、**貼っていない pull request の検査が赤くなります。**
...
**`continuo prompt --builtin` だけは読みません。**組み込みの指示書は実行ファイルの中にあるので、
設定が壊れている人でも読めるようにしてあります。

**ただし、下の表に並べたところは、どの言語を選んでも日本語のまま出ます。**
文言そのものがまだ訳の対象になっていないためで、言語の選び方とは別の話です。
...
**資源の無い言語（`fr` など）を書くと、常駐プロセス（`continuo`）は起動せずに止まります。**黙って別の言語で動くと、
書いたつもりの設定が効いていないことに、無人で動かしているあいだ誰も気づけないからです。

**`continuo setup` は止まりません。**読めなかった理由を1行出してから対話へ進みます。
**止めると、`continuo init` が `gh` から値を引けなかった人が、Status の割り当てを1回も終えられなくなるためです。**
...
**書いたあとは、あなたのファイルです。**書き換えても消してもかまいません。
**消すと、エージェントの既定に任されます。**

**あとから `language` を変えても、既にある `WORKFLOW.md` は変わりません。**
continuo は本文を書き換えないためです（`continuo setup` も Status の行しか触りません）。
**その1行を書き換えるのは、あなたです。**

**組み込みの指示書は、いまのところ日本語だけです。**
`language` が `en` でも、その大半は日本語のまま届きます。
**これは「日本語だけにする」と決めたものではありません。**英語版をまだ作っていないだけです
（issue #226。「日本語を読まない利用者が、エージェントへ送られる指示書の大半を日本語のまま受け取る」）。

**v0.1.14 までの雛形は「すべて日本語で書いてください」を、言語によらず持っていました。**
**v0.1.15 からは `language` に連動します。**
**既にある `WORKFLOW.md` は書き換わりません**（雛形が書かれるのは `continuo init` のときだけです）。
**いままでどおり日本語で書かせたいなら、何もしなくてかまいません。**

---

## 目的別使用例

### 会社の GitHub（organization）で使いたいとき
...
「continuo doctor が赤くなるとき」にもあります。**
...
そのときは「同じ GitHub アカウントで、複数の PC を動かしたい」を読んでください。
...
#### 1枚の WORKFLOW.md をチームで共有して、余裕がある機械に処理させたい

**メンバーそれぞれが continuo を1台ずつ動かし、同じカンバンを見張る使い方です。**
**担当者のいない issue には全員が入札し、レートリミットの枠にいちばん余裕がある機械が取ります。**
その設定を1枚の `WORKFLOW.md` にまとめて、共有するリポジトリへ commit して配ります。

##### 先に決める2つ

| 何を | 中身 |
| --- | --- |
| **機械ごとに別の GitHub アカウントを用意する** | 同じアカウントで複数台はサポートしていません。「同じ GitHub アカウントで、複数の PC を動かしたい」を読んでください |
| **`WORKFLOW.md` を置くリポジトリを決める** | **`.gitignore` に入れないでください。**入れると、この手順は成り立ちません |
...
| **7** | **各自** | **`~/continuo-work` として clone する** |
| **8** | **各自** | **Claude Code を1度起動する** |
| **9** | **各自** | `continuo trust --dry-run ~/continuo-work` で、何を許すことになるかを先に見る |
| **10** | **各自** | `continuo trust ~/continuo-work` を叩く |
| **11** | **各自** | `continuo doctor ~/continuo-work` を叩き、`✗` と `!` が無いことを確かめる |
| **12** | **各自** | そのディレクトリで `continuo` を起動する |

**段7 で名前を決めるのは、この文書のコマンドをそのまま使えるようにするためです。**
この文書は `~/continuo-work` を前提にしたコマンドを何十箇所も持っています。

**段8 を飛ばすと、段10 が止まります。**`continuo trust` は `~/.claude.json` が無いと
「`~/.claude.json` がありません」で止まります。**Claude Code が一度も起動していないと、このファイルはありません。**

**段9 と段10 を各自が叩くのは、`trust.repositories` の一覧が
「その機械で何をするか」を決めるからです。**`continuo trust` はその一覧を各自の機械で clone し
...
**共有すると、次が自動的に揃います。**それがこの使い方の効き目です。
...
**「どの機械が担当しているか」を書く設定は、`WORKFLOW.md` に1つもありません。**
...
| キー | なぜ |
| --- | --- |
| **`rate_limit.token_source`** | 走っている OS で既定が変わります（「OS が混ざるときの1行」を読んでください） |
...
**別の人が別の割り当てを選べます。**共有した1枚でそれが差し替わると、
**チーム全員の dispatch と片付けが一度に変わります。**

**各自が叩いてよいのは、割り当てが消えた `WORKFLOW.md` を直すときだけです。**

##### `trust.repositories` は、誰が叩いても同じ一覧になります

**`continuo init` はカンバンに載っている issue からリポジトリ名を集めます。**
生成した人の手元の clone には依りません。**要らない行を消すのは人間です。**

**重複した行があると、全員が起動しません。**共有した一覧を各自が手で削るときは、
マージの取り違えに注意してください。
...
**なぜか。**continuo が「どの continuo か」を見分ける値は、**その機械の `gh` がログインしている
GitHub アカウントだけ**だからです。**同じアカウントなら、2台は同じものに見えます。**

**その結果、こうなります。**

| 何 | 中身 |
| --- | --- |
| **2台目が入札を書かなくなる** | 1台目の入札を「自分のもの」と読みます |
| **2台とも勝者になる** | 2台とも担当者を書き、2台とも hold を書き、2台とも着手します |
| **同じ branch を押し合う** | branch 名は issue から決まるので、2台とも同じになります |

**機械の名前（`os.Hostname()`）で見分ける仕組みは、v0.1.15 でやめました。**
**重複するからです。**WSL のディストリは既定が Windows のホスト名で、`/etc/wsl.conf` に書かないかぎり
2つとも同じ名前になります。社内で命名規則に沿って配られた PC も衝突します。
**「重複しない名前を各自が付ける」に頼る仕組みは、書き忘れたら黙って壊れます。**
...
**同じ PC の2つは同じ `gh` の持ち主を使うので、上と同じことが起きます。**
**`--id <名前>` を付けても分かれません。**あれは二重起動を止めるロックを分けるだけで、
**持ち回りの見分けには効きません。**
...
**どちらの機械の入札かは、GitHub がそのコメントに付ける投稿者で分かります。**JSON には入りません。
**`score` でどちらが勝つ見込みかが分かります。**
...
#### コードが別のリポジトリにあるので、この worktree では commit させたくない

**`WORKFLOW.md` の本文に、成果の出し方を書いてください。**

**組み込みの指示書は「`review` または `blocked` を出す前に、必ず commit して push してください」を求めます。**
**v0.1.15 から、そこに例外が1つあります。**成果がこの worktree の外にあるときは、本文へ譲ります。
...
**worktree の片付けは止まりません。**指示どおり worktree の中で1つも commit しなければ、
`cleanup.require_pushed`（push していない commit が残っていたら消さない。既定 `true`）が数えるのは0件になります。
...
**`continuo setup` が「使うカンバンの番号が決まりませんでした」で止まるときは、「目的別使用例」の
「会社の GitHub（organization）で使いたいとき」にあります。**

**片付ける Status へ動かしたあとに起動しなくなったときは、「目的別使用例」の
...
**判定の中身は「複数の機械で見張っているのに、どの機械も issue を取らない」にあります**（「目的別使用例」の「何台かの PC で分担したいとき」）。
**1台で動かしていても当たります。**

**それでも出ないときは、担当者を見てください。**
**人間が担当者になっていると、continuo は触りません**
（「担当が外れる・別の PC とぶつかるとき」の
...
#### エージェントが、分岐元で既に直っているものを直し直す

**v0.1.15 で直りました。**上げてください。

**v0.1.14 まで何が起きていたか。**worktree は issue に着手したときの分岐元から切られます。
**その issue が時間をおいて再開されると、分岐元はその間に進んでいます。**
**組み込みの指示書に、分岐元を取り込む指示が1文字も無かったので**、
エージェントは古いコードの上で作業していました。

**v0.1.15 から。**読む前に取り込ませます。**分岐元の名前は4段で決まります。**

| 順 | 何を見るか |
| --- | --- |
| 1 | **worktree の直下にある continuo の記録ファイル**（既定 `.continuo.json`）**の `"base"` の値** |
| 2 | `WORKFLOW.md` の本文に指定があれば、それ |
| 3 | issue にリンクされた branch |
| 4 | リポジトリの既定 branch |

**段1 がいちばん確かです。**continuo は worktree を作るときに、実際に使った分岐元をそこへ書いています。
**本文の指定を上に置いていません。****本文は issue をまたいで同じ文言だからです。**
本文に「分岐元は develop です」と書くと、**既定 branch から切った worktree にも develop が混ざります。**
**衝突しないので、誰も気づきません。**

**確かめ方。**

```bash
jq -r .base <worktree のパス>/.continuo.json
```

**本文に分岐元を書く必要があるのは、次の2つを両方満たす人だけです。**

| 条件 | なぜ |
| --- | --- |
| **`workspace.identity_file`（記録ファイルの名前。既定 `.continuo.json`）を変えている** | 名前を変えていなければ、エージェントは段1 で当てられます |
| **`herdr.worktree.base`（worktree を切る分岐元。既定 `null`）を指定している** | 指定していなければ、段3 と段4 が当たります |

**両方に当てはまる人は、記録ファイルを見つけられなかったときに段3 か段4 へ落ち、
`develop` から切った worktree に `main` が混ざります。**
**そのときだけ、`WORKFLOW.md` の本文に1行書いてください。**

```markdown
### 分岐元

この worktree の分岐元は develop です。
```

**当てはまらない人は書かないでください。**本文は issue をまたいで同じ文言なので、
**分岐元が issue ごとに違う運用では、書いたほうが外れます。**

**落ちたときの扱いは、落ちた場所で分かれます。**

| どこで落ちたか | エージェントがどうするか |
| --- | --- |
| **取ってくるところ**（その名前が remote に無い） | **取り込むものが無いだけです。**そのまま作業を続けます |
| **マージを始める前**（commit していない変更がある） | **先に commit してから取り込み直します。**前の試行が残した作業なので、捨てません |
| **マージの途中**（衝突した） | **`git merge --abort` で取り込む前へ戻してから止まります**（Status が `Blocked` になります） |

**衝突のときだけ戻します。**組み込みの指示書は `blocked` の前に commit と push を求めるので、
**戻さないと、衝突の印が付いたままのファイルが branch へ push されます。**
...
#### 「まだ作業中です」しか書かれていないのに、issue が `In Review` に立った

**v0.1.15 で直りました。**上げてください。

**v0.1.14 まで何が起きていたか。**continuo は run の終わりに
「エージェントがこの run の報告を書いたか」を確かめ、書いていなければセッションを復元して書かせます。
**その判定が、途中経過の報告も「書いた」と数えていました。**

| 順 | 何が起きるか |
| --- | --- |
| 1 | エージェントが長い作業の途中で「まだ作業中です」と1回書く |
| 2 | **そのまま最後の報告を書かずに終える** |
| 3 | **continuo は「書かれている」と判定し、書き直させない** |
| 4 | **issue には「まだ作業中です」だけが残り、`In Review` に立つ** |

**v0.1.15 から。**途中経過の報告（`continuo:progress` の印が付いたもの）は、成果の報告として数えません。
**書かずに終えた run には、セッションを復元して書かせ直します。**

**設定に足すものはありません。**

**成果の報告の中で、この印について書くときは、山括弧を外した `continuo:progress` の形で書いてください。**
**成果の報告かどうかを見るときは、本文のいちばん上にある印の並びだけを見ます。**
本文の途中で引用しても、途中経過とは読まれません。

**ただし、囲み付きのまま（`<!` から始まる形で）書かないでください。**
**エージェント自身が「書き足す先」を探す問い合わせは、印が本文のどこにあっても拾います。**
**囲み付きで引用すると、次の途中経過の報告が、その成果の報告へ書き足されます。**

**ただし、持ち回りの死活の判定は別です。**そちらは印が本文のどこにあっても数えるので
（設計 5-3l。厳しくすると、書き足し続けているエージェントの担当が18時間で外れます）、
**印を引用した成果の報告は、そのぶん「まだ生きている」の合図として数えられます。**
**害はありません。**担当が外れにくくなるだけです。

**上げられないときの回避。**その issue のコメントを人間が読み、**エージェントに書かせ直したいなら
Status を `Ready` へ戻してください。**continuo が次の巡回で dispatch し直します。

### 担当が外れる・別の PC とぶつかるとき

**何台かで分担する設定そのものは、「目的別使用例」の
「何台かの PC で分担したいとき」にあります。**
...
**`assignee` にどのアカウントの担当かが、`at` にいつ書かれた hold かが入っています。**
...
（v0.1.14 から。「進捗のコメントが1件にまとまり、増えなくなりました」）。
...
（「トラブルシューティング」の「入れたばかり、または版を上げたあとに continuo が起動しないとき」にある
『版を上げたら「progress_interval_ms の値 3600000 が不正です」で起動しなくなった』）。
...
**エージェントは、いちばん下にある自分の進捗報告に書き足します**
...
**その使い方は「チームで、issue ごとに担当の PC を決めたい」にあります**（「目的別使用例」の「何台かの PC で分担したいとき」）。
...
`### まとめて直してよい範囲` の節を消してください。**まとめて直す指示が届かなくなります。

**ただし、確実に止まるとは言えません。**組み込みの 7-2 の手順は `WORKFLOW.md` と無関係に届き、
**グループの計画は代表の issue のコメント側にも書かれています**（`gh issue view --json comments` で読ませています）。
**節を消したときにエージェントが実際にどう振る舞うかは、測っていません。**
**このコメントだけを止めるつまみはありません。**

**2件以上並ぶことがあります。**同じカンバンを別のアカウントの機械で見張っていて担当が引き継がれると、
**新しい機械は前の機械が書いたコメントを自分のものとして見つけられません。**
**書き換える先が読めなかったときも、今回の分が1文字も残らないより、前の分と重ねて残るほうを選んでいます。**
