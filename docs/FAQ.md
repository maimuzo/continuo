...
5時間の枠の閾値="90% を超えたら止まります" 1週間の枠の閾値="90% を超えたら止まります" 担当者のいない候補=3
```
...
**どちらの行も、担当者のいない候補（96%〜のときは、まだ着手していない候補）が
1件でもあるときだけ出ます。**カンバンが空の機械は、止まっていても1行も出しません。
...
```
1週間の枠が明けるのを待つ上限を超えたので、担当を手放します（worktree は残します。カンバンへは書きません）
```
...
| **herdr の pane** | **閉じます** |
| **`workspace_hooks.after_run`** | **走ります。**ここに `git push` を書いている人は、手放す前に push されます |
...
1週間の枠が満杯のあいだは `rate_limit.pause_above_percent` の門が巡回を止めているためです。
