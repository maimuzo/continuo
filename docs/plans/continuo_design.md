...
| 自動化が動かしたが、対応表に無い | いままでどおり止める。**足す1行を issue のコメントに書く** |
| 自動化が動かしたが、戻す先が知らない Status か、書き戻す回数が上限（3回）に達した | 書き戻さずに止める。ログに1行出す |
...
```yaml
tracker:
  automated_state_rewrite: {"In Progress": "AI In Progress", "Done": "AI Done"}
```

**既定は空である。**書かなければ挙動は変わらないので、既存の `WORKFLOW.md` をそのまま使える。
**キーを書いても「知っている Status」にはならない**（`knownStates` はキーを見ない）。
入れてしまうと、書き戻しの分岐そのものが二度と通らなくなる。
**値（戻す先）は起動時にボードと照合する**（`requiredStatesForBootstrap`）。綴りの誤りをそこで落とす。
