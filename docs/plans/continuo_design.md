...
（macOS 15 で実測。inode が変わり、hard link の相手は元の内容のまま残りました）。**それでも Edit や Write を勧めるのは、直した範囲が見えるからです。**
...
**先に貼ると、あとから同じコメントへ足せません。**判断票は1周につき1つで、2つ目を貼ると、どれがその周の判断かが読めなくなります。
...
**例外は、下の「心配な場所があるときに足す3つ目の役」だけです。**その役には、名指しした場所と、そこを直した理由を渡します。
...
**3-2 では、前の周の判断票に同じ指摘が在るかどうかで分けさせます。**在れば「前の周に既に在った」、無ければ「前の周の直しが持ち込んだ」です。
...
**採る形。**[internal/prompt/builtin.md:298-331](../../internal/prompt/builtin.md#L298-L331) の
