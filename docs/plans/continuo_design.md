...
**綴りの打ち間違いは、起動を止めずに名前で知らせる**（`missingRewriteKeys`。起動時に1回）。
`In Progres` と書くと**その行は一度も効かないまま黙って死ぬ**ので、知らせる価値はある。
**だが「打ち間違えた」と「使わなくなったので選択肢を消した」は同じ形に見える。**
**見分けられない検査で起動を止めてはならない。**
**`continuo doctor` の見出し語 `Status の名前` は、いままでどおりキーも紛らわしさの検査に掛ける**（`configuredStates`）。

**採らなかった案。**照合は続け、失敗の文言に「対応表のその行を消してください」を出す。
文言が正しくなっても、**設定を書き換えて再起動するまで continuo は動かない。**
**continuo を入れることが、人間のボードの設定を変える理由になってはならない**（3-54 の同じ判断）。
...
| `config.BootstrapStates` | ボードの選択肢が設定に出てくるか（`unknownStatusOptions`） | **入れる。**キーの Status は書き戻されるので「知らない Status」に数えない |
