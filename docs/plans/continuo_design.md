...
**残る未達を埋める先は `CLAUDE.md` である**（文面は [docs/FAQ.md:201-258](../FAQ.md#L201-L258)）。
**このリポジトリでは、1行足すだけでは埋まらない。**印の並びを具体形で決めている
[.claude/rules/design-review.md:12](../../.claude/rules/design-review.md#L12) と
[.claude/rules/design-review.md:23](../../.claude/rules/design-review.md#L23) が勝つためで、3箇所を同時に直すことになる。
...
**足す位置は「本文の先頭の印の直後」である**（`withAIMarker`）。**見るのは、先頭の空行を飛ばした最初の1行だけである。**
**印と数えるのは、空白を落としたものが `<!--` で始まり、同じ行に `-->` がある行だけである**（`isMarkerLine`）。
**1行目が印でなければ、本文の先頭へ入れる。**

**走査は持たない。**先頭に並ぶ印を辿る走査と、空行の扱いと、末尾の改行の扱いは、いったん入れて捨てた
（2026-09-08、実装レビュー6周目のあとの突き合わせ）。**投稿する本文を作る12箇所を1つずつ開いて数えたところ、
散文が7件、1行で閉じた印が1本だけのものが5件で、印が2本以上並ぶ本文は1件も無かった。**
**本番の入力では1度も通らない枝から、Critical が4件出ていた。**
**関門の案内だけは印が2行並ぶが、2行目の `self_marker` はあとから前へ足すので、
`withAIMarker` へ来る本文は1行目が印である。**
...
**印が何を言い、何を言わないかは、組み込みの 5-6 が表で持っている**（同じ表を2つ置かない）。
**要点は2つ。**人間の決定を AI が記録したコメントにも、この印は付く。**誰でも書ける文字列であり、認証ではない**（3-65）。
...
### 3-82e. continuo は、印の落ちたコメントを書き換えない

**言いたいこと。**2026-09-08、人間が「真実を知ってるなら直接コメントを書き換えるか」と書いた。
**その前提が成り立たない。**continuo が直せる集合は、既に見分けの付く集合と一致する。
**印を落としたときの直しは、組み込みの指示書に置かない。**置くと、守る対象より守り方のほうが重くなる。
...
**エージェントに直させる案も採らない。**2026-09-08 の5度目の突き合わせで、敵対的レビュワーが
「いらない」と判定した。**守る対象は「機械が1つも読んでいない印」で、落ちても何も止まらない。**
一方、直すときに本文を確かめ損ねると、**成果報告が `gh` のエラーの JSON に置き換わり、
continuo が「成果が書かれていない」と判定して run を人間へ渡す。**
**そもそも構造が自己矛盾している。**出発点は「エージェントは印を落とすことがある」——
**指示を守り切れないという前提**なのに、**別の指示（書く前に本文を確かめろ）を確実に守ることに依存する。**
**2つの指示の信頼度は同じで、外れたときの損失だけが桁違いである。**

**代わりに置いているもの。**組み込みの指示書が、**8箇所の見本で印を本文の型そのものへ埋め込んでいる**
（3-2 の2つ / 3-6 / 3-7 / 5-3 / 5-5 / 7-2 の2つ）。**人間が却下した「検知してログに出すだけ」の形ではない。**
...
**1行目より前へ置かないでください。**そこだけは、置くと
**あなた自身が、次の途中経過を書き足す先を見つけられなくなります**（5-3 の段1 は
`startswith("<!-- continuo:agent -->")` で探します）。**新しいコメントが1時間ごとに増えます。**
