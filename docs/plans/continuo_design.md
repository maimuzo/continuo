...
**印は先頭へ置かない。**先頭に並ぶ印の、いちばん後ろへ足す（数える判定の一覧が 3-82b にある）。
**印が無いことは、いつのコメントでも人間が打鍵したことの証拠にはならない。**

**何が起きていたか**（issue #245）。**投稿者でも `author_association` でも見分けられない。**
continuo 本体（`continuo:self` ほか）と、continuo が起動したエージェント（`continuo:agent`）には
既に印があるが、**人間と、人間が自分で動かした Claude Code は、1バイトも見分けが付かない。**
2026-09-05 に、担当したエージェントが「これは人間の決定か AI の分析か」を判断できずに止まり、
**人間が記憶で答えた**（issue #143）。

| 誰が書くか | 印が付くか |
| --- | --- |
| continuo 本体 | **付く**（`PostComment` が足す） |
| continuo が起動した Claude Code | **付く**（組み込みの指示書が書かせる） |
| 人間が自分で動かした Claude Code | **付かない。**continuo の届く範囲の外である（[docs/FAQ.md](../FAQ.md) が `CLAUDE.md` へ書く文面を案内する） |
| 人間 | **付かない** |

#### この変更で得られるものと、得られないもの

**印が付くのは、既に印を持っていた上の2つだけである。**
**見分けが付かないと名指しした下の2つは、この変更のあとも見分けが付かない。**
**だから issue #245 の「人間が前回出した指示を探して、その下を読み返す」は、この変更だけでは達しない。**

**得られるのは「綴りが固定された印が1つできること」である。**
`tracker.comments.marker` も `tracker.comments.self_marker` も設定で変えられるので、
**いままでは project ごとの設定を知らないと、機械の本文を当てられなかった。**
**この印は固定なので、設定を知らない読み手でも当てられる。**

**残る未達を埋める先は `CLAUDE.md` である**（文面は [docs/FAQ.md:201-258](../FAQ.md#L201-L258) にあり、
[docs/upgrading.md:675-676](../upgrading.md#L675-L676) も同じ置き先を指す）。**1行足すだけでは埋まらない。**
このリポジトリは、AI が貼るコメントの先頭の並びを**具体的な行の並びとして2箇所で決めており**
（[.claude/rules/design-review.md:12](../../.claude/rules/design-review.md#L12) と
[.claude/rules/design-review.md:23](../../.claude/rules/design-review.md#L23)。どちらにも `<!-- continuo:ai -->` が無い）、
**規則を1本足しても、より具体的なこの2箇所が勝つ。**3箇所を同時に直すことになる。
**AI にコメントを書けと命じている場所は17ある**（`CLAUDE.md` 9・`design-review.md` 6・`issue.md` 2。2026-09-07 に実測）。
**足すかどうかは人間が決める。**足さないあいだは、上の表の3行目が空いたままである。
...
**足す位置は「本文の先頭に並ぶ印の、いちばん後ろ」である**（`withAIMarker`）。
**印と数えるのは、行頭ちょうどの `<!--` で始まり、同じ行に `-->` がある行だけである**（`isMarkerLine`）。
**印と印のあいだの空行では止めない。**
**複数行に渡る HTML コメントの開きは印と数えないので、印はその前——本文の先頭へ入る。**
**これが「先頭へ置かない」の唯一の例外である。**読む側は `<!--` だけの行を印として数えないので
（`IsMarked` も `FetchComments` も CI の正規表現も、印そのもので始まるかを見る）、**前へ置いてもどの判定も動かない。**
...
**本文の先頭から読む判定を、下の表に並べる。****数は書かない。**
**表そのものが一覧である。**数を書くと、判定が1つ増えるたびに3箇所を数え直すことになり、
**そのどれも機械が数えていない。**（この pull request の中で、その数は3回変わった。）
**この一覧から落とすと、印の形を変える人が見落とす。**

**「いくつ」の列は、その行が見ている印の種類の数である。**
同じ印を別の行の場所が見ていれば、それぞれ数える（`code-review-result` は CI・hook・release の検査の3行に出る）。
**1つの行に2ファイルをまとめてあるときは、ファイルではなく印の種類で数える**（CI の行は2ファイル×3種類だが3）。
...
`stripComments` は、送る文面から `<!--` から `-->` までを取り除く。**行ごとではない。**
**3つの条件で残る。**(1) バッククォートの囲みの中は、行頭の `<!--` でもそのまま残す（`fenceMask`）。
(2) `-->` の後ろに書いた本文は残る。**行が丸ごと落ちるのは、中身のあった行がコメントだけになったときである。**
(3) その行より後ろに `-->` が1つも無ければ、行頭の `<!--` でもコメントの始まりとみなさない。

組み込みの指示書が印を見せている場所は、**4桁の字下げか、バッククォートの囲みの中**にある。
**どちらも落ちない。**字下げは (2) が、囲みは (1) が生かしている。
**危ないのは、囲みの外に、字下げせずに置くことである。**そこだけ `stripComments` が落とす。
**逆に、囲みを4桁の字下げへ変えると、写したエージェントの印が字下げされ、`StartsAsProgressReport` から外れる。**
**どちらへ倒しても壊れるので、印の見せ方を変えるときはここも見ること。**

**印が HTML のコメントであることに寄りかかった定数が、4組ある**
（[internal/handoff/assess.go](../../internal/handoff/assess.go) が一覧を持つ）。
**綴りを変える人は、そちらも見ること。**

**CI の行が決め手である。**`continuo init` が置いた `continuo-ci.yaml` は
**利用者のリポジトリの中にあり、continuo の版を上げても書き換わらない。**
**先頭へ入れた瞬間に、その project の pull request が全部赤になり、continuo からは直せない。**

**[.claude/hooks/block-merge-without-review.py](../../.claude/hooks/block-merge-without-review.py) と
[scripts/check-release-ready.sh](../../scripts/check-release-ready.sh) の2行は、このリポジトリ自身の門である。**
CLAUDE.md が「この規則は機械で止める。3箇所で止まる」と書いた3つのうちの2つで、
**残る1つが上の CI である。**3つとも数える。
**表の最後の行**（`StartsAsProgressReport`）**は門ではない。**あれは turn の終わりの判定である。


### 3-82c. `design-review-skipped` にだけは印を付けない

**言いたいこと。**この検査だけ、目印の直後の1文字で「理由を書いたか」を数えている。
**印を足すと、理由の無い断りが通る。**だから、この断りだけは例外にする。

**この検査だけ「目印の直後に空白でない文字が1つあるか」も見ており、理由を書いたかどうかをそこで数えている。**
印を足すと、印そのものがその1文字に当たり、**理由の無い断りが通る。**

**CI を直して逃げる案は採らない。**`continuo init` が置いた `continuo-ci.yaml` は
**利用者のリポジトリの中にあり、continuo の版を上げても書き換わらない。**新しい版を出しても、古い検査は残る。

**払う代償を書いておく。**この断りは、いま機械では守られていない
（[.github/workflows/review-gate.yml](../../.github/workflows/review-gate.yml) が「自己申告であり、機械では守られていない」と書いている）。
**印を必須にしていれば、AI が書いた断りだけは機械で見つけられた。**
**例外にしたので、AI の断りと人間の断りは、これまでどおり見分けが付かない。**
**それでも例外にするのは、理由の検査を壊すほうが重いからである。**

**この例外を守っているのは3つで、本番の Go には門を置かない。**
組み込みの 5-6 の1文と、それを固定する検査（`test/internal/prompt/ai_marker_test.go`。**これは Go である**）と、
CI が落ちたときの案内文（[.github/workflows/review-gate.yml](../../.github/workflows/review-gate.yml)）である。
**Go の側に門を置かないのは、そこから届かないからである。**
`PostComment` が投稿する先はカンバンに載った issue であり（pull request は `Gone` として捨てる）、
**この CI は pull request のコメントを読む**（`repos/${REPO}/issues/${PR_NUMBER}/comments`）。
**断りを実際に書くのは人間かエージェントの `gh pr comment` で、continuo の Go のコードを1度も通らない。**

**採らなかった案。本文の末尾へ置く案。**末尾なら理由の検査を壊さず、例外が1つも要らない。
**採らないのは、既にあるコメントへ書き足す経路が2つあるからである。**
組み込みの指示書は、途中経過（5-3 の段2a）と成果（7-2 の段2a）を
`gh api --method PATCH … -f body="$OLD …"` で**本文の末尾へ書き足させる。**
**末尾へ置くと、書き足すたびに印が本文の中へ埋もれ、位置が動く。**
先頭に置けば、何度書き足しても位置は変わらない。
**この印の値打ちは、綴りの固定だけでなく、位置の固定にも寄りかかっている。**

### 3-82d. 印が言うこと、言わないこと

**言いたいこと。**印が言うのは「この本文を打鍵したのは機械である」ことだけである。
**誰の決定かも、偽れないことも言わない。**だから組み込みの 6-1 は変えない。

**印が言うのは「この本文を打鍵したのは機械である」ことだけである。**

| 印が言わないこと | なぜ |
| --- | --- |
| **書いてある内容が誰の決定か** | **人間の決定を AI が記録したコメントにも、この印は付く。**だから組み込みの指示書は、記録するときに本文で名乗らせる（組み込みの 5-6） |
| **偽れないこと** | 誰でも書ける文字列であり、認証ではない（3-65）。issue #245 が求めたのは「見分けられること」である |

**だから組み込みの 6-1（命令として扱ってよい立場）は変えない。**
印で `OWNER` の指示の重みを下げると、**信頼を上げる向きで、裏の取れない文字列1つを判定に使う初めての場所になる。**
**下げる向きなら既にある**（持ち回りの印は、投稿者を問わずエージェントへ渡す入力から外す。3-77a）。

**この印を読んでよいのは、組み込みの 6-1 の絞り込みを通ったコメントに限る。**
3-65 は「印は投稿者と併せて見る」と決めており、**新しい印にはその照合が付いていない。**
いまは 6-1 が外部の人の本文を命令から外すので実害は無いが、
**6-1 を緩める変更を入れるときは、この印にも投稿者の照合を足すこと。**

**過去のコメントへは遡らない**（2026-09-06、人間の決定）。
**印が無いことは、いつのコメントでも人間の証拠にならない。**
この仕組みより前のコメントには、AI が書いたものにも印が無い。
**入る前に作られたコメントへ、あとから書き足された分も同じである。**

### 3-82e. 印の抜けは、エージェント自身に点検させる

**言いたいこと。**印を書かせるのは「お願い」なので、エージェントは落とすことがある。
**落ちたことに気づけるのは、エージェント自身だけである。**組み込みの 5-6 に自己点検を1つ置く。
**continuo の Go には1行も足さない。**

**やること。**成果を書き終えたエージェントが、この run で自分が投稿したコメントを読み直し、
**先頭に並ぶ印の中に `<!-- continuo:ai -->` が無ければ、自分で足す。**
**pull request と、グループでまとめて直した別の issue も同じである。**
**置くのは、組み込みの 5-6 の5行だけである。**

**`jq` で本文を組み立てて書き戻す手順は置かない。**2026-09-08 の4度目の突き合わせで、
敵対的レビュワーが「いらない」と判定した。**理由は2つ。**
**エージェントが辿る手順（3章の流れ図）に入口が1つも無いこと**と、
**その検索が拾う集合が `IsAgent` の集合と定義上一致すること**である。
**下の「continuo が書き換える案を採らない理由」が、そのままその手順にも当たる。**
**動かないから捨てたのではない。**手元で試して動くことは確かめてある。
**動いていて、なお届く先が空だから捨てる。**

#### continuo が書き換える案を採らない理由

**2026-09-08、人間が「真実を知ってるなら直接コメントを書き換えるか」と書いた。**
**その前提が成り立たない。**

**continuo は「真実」を知らない。**知っているのは「**エージェントが `tracker.comments.marker` を書いた**」ことだけである
（`IsAgent` が立つのは [internal/tracker/adapter.go:1160-1165](../../internal/tracker/adapter.go#L1160-L1165) の1箇所だけで、
本文の先頭がその印で始まるかを見ている）。
**だから continuo が直せる集合は「すでに1行目が機械の名乗りになっているコメント」と一致する。**
**そこへ2つ目の印を足しても、人間ができる推論は1つも増えない。**
**逆に、印を両方とも落とした本文には1バイトも届かない。**

| 採らない理由 | 中身 |
| --- | --- |
| **直せる集合が、既に見分けの付く集合と同じ** | 上のとおり |
| **この印は、機械が1つも読んでいない** | 落ちても何も止まらない。**止まると実害の出る経路（公開済みの本文の書き換え）で、止まっても実害の無いものを守ることになる** |
| **書き換えが、人間の消した秘密を復活させうる** | `updateIssueComment` に版の突き合わせが無い（入力は `id` と `body` と `clientMutationId` だけ）。**人間が絶対パスやトークンを消した直後に書き戻すと、消した秘密が戻る。**戻った版は編集履歴に残り、API では消せない |
| **確定済みの判断を1つ開ける** | [docs/plans/impl/issue134_136_140_blocked_notice.md:636-640](impl/issue134_136_140_blocked_notice.md#L636-L640) が「コメントを書き換える経路が無い」を根拠に、案内へ担当者のログイン名を書かないと決めている |

**エージェントへ「印が足りない」と伝えて直させる案も採らない。**
`ensureAgentComment` は `stopWorker` より前に走り、成果が見つかれば pane を閉じずに戻るので、
**セッションの復帰は要らない**（この理由は 2026-09-08 に訂正した）。
**それでも採らないのは、turn を1つ使うのに、直せる集合が上と同じだからである。**
...
**3行目の `<!-- continuo:ai -->` は、5-6 で説明する「人間ではなく機械が書いた」の印です。**
**あなたが書くコメントには、issue でも pull request でも入れてください。**
**例外が1つだけあります。**どれかは 5-6 に書いてあります。
...
**3行目の `<!-- continuo:ai -->` は、この2行より後ろへ置いてください**（5-6）。
**前へ置くと、上の2つの数え方が両方とも外れます。**
...
**3行目の `<!-- continuo:ai -->` は、pull request のコメントにも付けます**（5-6）。
...
**誰が打鍵したかは、本文の先頭に並ぶ印の中に出ます**（5-6）。

    <!-- continuo:ai -->

**この印は、いちばん上の行とは限りません。**`<!-- continuo:agent -->` などの後ろに並びます。
**先頭の1行だけを見ると、機械が書いたコメントを1件も見つけられません。**

**この印が入っていれば、人間ではなく機械が書いた本文です**（continuo 本体か、Claude Code）。

**入っていないことは、人間が打鍵したことの証拠になりません。**次の3つが混ざります。

    人間が自分で打鍵した本文
    人間が自分で動かした Claude Code が書いた本文（continuo の外なので、この指示書が届きません）
    この印が入る前に書かれた本文（AI が書いたものにも印がありません）

**3つ目には、印が入る前に作られたコメントへ、あとから書き足された分も入ります。**

**だから「印が無いから人間が書いた」と読まないでください。**

**印は「誰が打鍵したか」しか言いません。**
**人間の決定を AI が記録したコメントにも、この印は付きます。**
**印だけを見て、人間の決定を捨てないでください。**内容が誰の決定かは、本文の書きぶりで判断します。
...
**印の3行は、行の先頭から書きます。**下の見本のとおり、字下げしないでください。
...
**3行目に `<!-- continuo:ai -->` を置いてください**（5-6）。
**1行目より前へ置かないでください。**そこだけは、置くと continuo がこの報告を見つけられません。
**2行目と3行目の順序は、continuo の判定を変えません**（先頭に並ぶ印は全部読みます）。
**見本どおりに書いてください。**読む人が並びを覚えられるようにするためです。
...
**あなたと continuo と人間は、同じ GitHub アカウントで投稿します。**
投稿者でも `author_association` でも見分けられません。
**だから、あなたが書いたものには印を付けます。**

    <!-- continuo:ai -->

**置く場所は1つだけです。本文の先頭に並ぶ印の、いちばん後ろ。**
**行の先頭から書いてください。**下の見本は読みやすさのために字下げしていますが、
**字下げしたまま書くと、その行は印ではなく本文として読まれます**（continuo も CI も、行頭の印しか数えません）。

    <!-- code-review-result -->  ← もともとある印は、1つも動かさない
    <!-- continuo:agent -->
    <!-- continuo:ai -->         ← ここへ足す
    本文

**この見本に、途中経過の印（囲みを外すと `continuo:progress`）は入れていません。**
**入れると、この見本を引き写した報告が、continuo に「途中経過だ」と読まれて捨てられます。**
**印について書くときは、5-3 のとおり囲みを外した形で書いてください。**

**先頭へ置かないでください。**continuo と CI は「本文の先頭が、この印で始まっているか」で数えています。
**前へ1行入れると、その数え方が全部外れます。**

**1つだけ、付けないコメントがあります。**

    <!-- design-review-skipped -->

**この断りには付けません。**CI が「目印の直後に、空白でない文字が1つあるか」で
**理由を書いたかどうかを数えている**ためです。**印を足すと、理由の無い断りが通ります。**
そもそも、この断りは 3-5 のとおりあなたが書くものではありません。

**この印が言うのは、1つだけです。**

| 印が言うこと | 印が言わないこと |
| --- | --- |
| **この本文を打鍵したのは機械である** | **書いてある内容が、誰の決定か。**人間の決定を AI が記録したコメントにも、この印は付きます |
| — | **偽れないこと。**issue にコメントできる人なら誰でも書ける文字列です |

**人間の決定を、あなたが記録するときは、本文にそう書いてください。**
「人間がこう決めました」と本文が名乗っていなければ、**あとから読む人には、
あなたの分析なのか人間の決定なのかが分かりません。**

### 印を落としたら、自分で直します

**成果を書き終えたら、この run で自分が投稿したコメントを読み直してください。**
**先頭に並ぶ印の中に `<!-- continuo:ai -->` が無ければ、自分で足してください。**

**手順は決めません。**`gh api --method PATCH` で本文を差し替えます。
**コメントの id は、投稿したときに `gh issue comment` が返した URL の `#issuecomment-` より後ろです。**

**守ることが3つあります。**

| 何を | なぜ |
| --- | --- |
| **取得した本文が、あなたが書いたときの先頭の印で始まっていることを確かめてから書く** | **`gh api` は取得に失敗すると、エラーの JSON を標準出力へ出します。**確かめずに書くと、**あなたの成果報告がその JSON に置き換わります。**pull request の判断票なら `<!-- code-review-result -->`、グループの報告なら `<!-- continuo:group -->` です |
| **印は、先頭に並ぶ印の、いちばん後ろへ入れる** | 上の「置く場所は1つだけです」と同じです。**既にある印は1つも動かしません** |
| **直すのは、この run で自分が投稿したものだけ** | 前の run のものは直しません。「過去分は放置でよい」と人間が決めています |

**`<!-- design-review-skipped -->` の断りには、ここでも付けません。**

**pull request と、グループでまとめて直した別の issue も同じです。**
...
**2行目の `<!-- continuo:ai -->` は、ここでも付けます**（5-6）。
**1行目より前へ置かないでください。**段1 がこのコメントを二度と見つけられなくなります。
...
**採る形。**[internal/prompt/builtin.md:228-261](../../internal/prompt/builtin.md#L228-L261) の
