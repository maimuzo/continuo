...
7. agent.prompt で「作業の内容を issue のコメントに書いてください」とだけ送る
   ...
   → **「新しく1件投稿してください。囲み付きの印を先頭に置かないでください」を必ず添える**
     **文面には印そのものを埋めない**（囲みを外した形で書く）。埋めると、書き写しただけで同じ穴に落ちる
     添えないと、エージェントは 5-3j の指示どおり進捗報告へ書き足し、**書いたのに段9 で人間へ渡される**。**1回目に送る 3-7 にも「新しく1件投稿してください」を置く**（囲み付きの印の話は 5-3 の側にある）
...
**4-4 の見本は「この worktree の中では commit しないでください」と命じるので、3-5 が求める push は原理的に済ませられない。**
...
**落とすと、次の turn でエージェントが 4-2 の一覧から自分の pull request を見つけられず、
レビューの指摘を読む先が消える**（組み込みの 3-5 がそう書いている）。
...
**字下げすると、continuo はその印を「本文の中の引用」とみなし、進捗報告として数えません。**
**数えられないと、18時間で担当が外れます。**
**落とすと、次の1時間後に段1が見つけられずコメントが1件増えるうえ、
18時間で担当が外れて、別の機械がこの issue を最初からやり直します。**

**この印を、最後の成果報告には付けないでください。**
**付けると、continuo はその成果報告を数えず、セッションを復元してもう一度書かせます。**
**さらに、次の進捗報告が成果報告に書き足して、読む人には別の話が1件に混ざって見えます。**
...
**組み込みを言語ごとに持つかは、人間が決めた**（2026-09-05。日本語と英語の両方を持つ）。
**いまは日本語だけだが、それは完成していないだけであって、制約ではない。**
作るのは #226（日本語を読まない利用者が、エージェントへ送られる指示書の大半を日本語のまま受け取る）である。
...
**雛形そのものは、どの言語の指示も持たない。**目印の行（`<!-- continuo:write-language -->`）だけを置き、
...
**組み込みの指示書は、いまのところ日本語だけである。**`language` が `en` でも大半は日本語のまま届く。
...
| **`--url` の形が issue の URL でない** | **終了コード 2。**GitHub を叩く前に断る |
...
**費用は「2リクエスト・計 8 point」である**（104件のボード。
[internal/tracker/by_identifier.go](../../internal/tracker/by_identifier.go) の実測）。
