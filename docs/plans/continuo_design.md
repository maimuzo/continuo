...
**3-82b が `--id` で複数動かす形を明示的に許している。**
**資格情報は人間1人につき1つなので、`~/.continuo/` のロックは全部で1本を取り合う。**
**「往復の実測 × 主体の数」を上限にし、主体の数は `agent.max_concurrent_agents` から計算する。**
...
**持たせると壊れる。**8931 が塞がっていて `--port 9000` で最初の認可を通した人は、
**約181日後に `continuo github-app setup` を叩き直すと、continuo は既定の 8931 で待ち、GitHub は 9000 へ返す。**
**逆はもっと悪い。**8931 で作った人があとで `--port 9000` を使うと、
**GitHub は 8931 へ `code` を返し、そのとき 8931 を掴んでいる別のプロセスが受け取る。**
...
**GitHub App の表示名は GitHub 全体で一意である。**
**manifest に載せた名前が使われていれば、GitHub の作成画面はその場でエラーを出し、
人間が別の名前を打つまで先へ進めない。**

**だから、送る名前に持ち主のアカウント名を混ぜる。**`continuo-<アカウント名>` とする。
**アカウント名は `gh api user --jq .login` で引ける**（[internal/tracker/ghuser.go:38](../../internal/tracker/ghuser.go#L38)）。
...
**起動時の検査で取ったトークンを、そのまま2本目のクライアントの初期値にする**（3-82i）。
...
**この1行を書かずに実装へ渡すと、実装者が既存の作法（WARN）に合わせて弱める。**

**起動時の検査で、GitHub との往復は1回増える**（`viewer` を引くぶん）。
**増えないのはトークンの取得のほうで、3-82g が「起動時に1回だけ取る」と決めた1回をそのまま使う。**
