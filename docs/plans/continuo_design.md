...
**当たるのは、人間に読ませるコメントです。**次の4つです。
    ...
    削除の記録         5-6（issue へ）。issue に無いものを削ったときに残します
...
### 毎周やること
    ...
    3. 判断票をコメントへ貼る
    4. 直す前に、使い捨ての計画を書く（下の「直す前に書くこと」）
    5. その計画を自分で読み直してから、直す
...
**直す1件ごとに、次の5つを書きます。**
    ...
    1. 指摘の箇所をどう直すか
    2. 同じ処理を使っている箇所が他に無いか（数えた件数と、叩いた検索パターンと範囲）
    3. 対応する文書はどこか
...
**止まるときは、応答の最後に `CONTINUO-STATUS: <issue番号> blocked` を書いてください**（5-4）。
...
| **issue に無いものを削る** | **毎周判定する。**削除が起きた周だけ設計から見直す |
...
周を重ねても周辺のコードには届かない。**人間はこれを確かめたうえで、選択肢が無いと判断した。**
