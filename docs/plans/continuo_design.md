...
| **閉じ（`-->`）が以降に1つも無い `<!--`** | **コメントの始まりとみなさない** | 打ち忘れると、そこから断片の終わりまで全部が消える。**プレビューでは何も壊れて見えないので、利用者が自力で気づけない** |
| **閉じたあとの、同じ行の残り** | **残す** | 落とすのは HTML のコメントであって、行ではない |
| **コードブロック（囲み）の中** | **コメントも含めてそのまま残す。**囲みの長さも数える | **これが、落とされたくない人への唯一の逃げ道である** |
...
**「本文があるか」は、取り除いたあとで決める**（[internal/prompt/prompt.go](../../internal/prompt/prompt.go) の `Build`）。
