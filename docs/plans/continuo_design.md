...
**入る場所は、組み込みの `## 4-4. このプロジェクトの決まり` の中である。**
`continuo init` が雛形ごと置くので、**要らない節は消す。全部消してもよい。**

**見出しは `###` にする。**`4-4` が `##` なので、その下に入る深さで書く。
...
**組み込みの 3-2 と 3-6 が持つ。**どの project でも同じ段取りなので、project ごとに書き直させる理由が無い。
**本文に残すのは「どの subagent を使うか」だけである。**
