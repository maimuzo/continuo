...
**判断票と同じコメントに、これから実装する内容の概要を図で入れてください。**
文章だけが並ぶと、読む人が構造を頭の中で組み立て直すことになります。
...
**変えるファイルの一覧は図ではありません。**表か箇条書きで別に書いてください。

**このコメントの節の書き方は 5-5 にあります。**
...
**当たるのは、人間に読ませるコメントです。**次の3つです。
    ...
    判断票                     3-2（計画。issue へ）と 3-6（実装。pull request へ）
    何をしたかの報告           3-7 と 7-2
    blocked を出すときの理由   5-4 と 7-4

**途中経過の報告（5-3）は当たりません。**あちらは1行足すだけの決まりで、5つを置く場所がありません。
...
**印が先頭から外れると、continuo はそのコメントを数えず**（3-2 と 3-7）、
**CI の検査も数えません**（3-2 と 3-6）。
...
### 5-3q. エージェントが issue へ書くコメントに、図と前提を必ず入れさせる

**言いたいこと。**エージェントのコメントに、実装の概要の図と、読むために要る前提が入っていない。
**組み込みの指示書へ、図を書かせる段（3-2 の末尾）と、人間へ質問・報告するときの形（5-5）を足す。**
...
| 何を決めたか | 採った案 | なぜ |
| --- | --- | --- |
| **図の書式** | **mermaid。種類は `flowchart` か `sequenceDiagram`** | **GitHub が issue のコメントの mermaid をそのまま描画する。**読む人に別の道具が要らない。組み込みの `# 1. 概要` が既に mermaid を使っており、**同じ文書で2つの書式を使わない** |
...
判断票（3-2 と 3-6）と成果の報告（3-7）は、**本文の先頭が HTML のコメントの印でなければ数えられない**
...
**引用を冒頭に置くと印が先頭から外れ、continuo は成果が書かれていないと判断して run を人間へ渡す。**
