...
**`--id` は5つのうち4つを名前から機械的に導き、残る1つ（ボード）は起動時に検査する。**
**分け忘れが構造的に起きない形にする。**
...
| **branch 名** | `herdr.worktree.branch_template` の既定に `<名前>` を挟む |
| **ボード** | **導けない。**下の検査で見る |
...
**ボードは、起動時に「同じボードを見ている生きた continuo がいないか」を検査する。**

| 順 | 何をするか |
| --- | --- |
| 1 | `~/.continuo/id/*/continuo.lock` と `~/.continuo/continuo.lock` を全部見つける |
| 2 | **それぞれに `flock` を非待機で試す。取れなければ、そのロックは生きている** |
| 3 | 生きているものの隣にある `board.json` を読む |
| 4 | **`owner` と `project_number` が自分と同じなら、起動を止める** |

**`board.json` のサンプル**（`~/.continuo/id/e2e/board.json`。**continuo がロックを取った直後に書き、終了時に消す**）。
...
| 何 | どうする |
| --- | --- |
| ロックの場所の決め方 | **常駐側と同じ関数を使う。**abandon が独自に設定から導かない |
...
**言いたいこと。**この1件が片付くまで、**continuo を maimuzo/continuo のボードで動かさない**（2026-08-28、人間の判断）。
...
| 案 | 損 |
| --- | --- |
| **continuo が本文を取ってプロンプトに埋める** | **設計 3-9 の「本文を埋め込まない」方針を変えることになる。塞がるのはこちらだけである** |
