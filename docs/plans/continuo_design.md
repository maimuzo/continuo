...
| 見出し | 中身 |
| --- | --- |
| サブエージェントの記録（新しい順） | `agent-*.jsonl` を更新時刻の新しい順に3件まで |
| サブエージェントの記録の置き場所 | その `subagents` ディレクトリそのもの |

**パスの導き方。**hook が渡す親の `transcript_path` から字句で組み立てる。
`<親の記録から .jsonl を落としたパス>/subagents/agent-*.jsonl` を Glob するだけである。
...
**この2つは同じ行に入っている**（[docs/evidence/hooks_probe_20260817.jsonl](../evidence/hooks_probe_20260817.jsonl) の `SubagentStop`）。
**ファイルは1バイトも開かない**（`os.Lstat` だけ）。**Glob の結果を見る件数にも上限を置く**
（`transcriptMaxRequestIDs` と同じ考え方）。検査は `acceptTranscriptPath` と同じ順である
（まず解決し、実在とディレクトリであることを見て、そのあと許可された根の内側かを比べる）。
