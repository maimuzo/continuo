...
**設定の検査は5つを見る**（`internal/config/validate.go` の `validateAutomatedStateRewrite`）。
...
**知っている Status を集める処理は `config.KnownStates` の1箇所である。**
...
**書き込みのあいだは `beginTerminal` で印を取る**（`rewriteAutomatedStateAsync` と `rewriteAndDecide`）。
...
**書き終えたら必ず印を返す**（`endTerminal`）。返さないと、そのあと run を終わらせる経路が1つも通らない。
