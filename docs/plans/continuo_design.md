...
**新しい版は、古い版が書いた入札も数える**（知らないキーは捨てるだけで読める）。
**だから、新しい版の判定スコアが下なら「入札に負けた」と読んで、担当者を書かない。**そのときは担当者が1人のままである。

**新しい版の判定スコアが上で、かつ2台が同じ巡回で締め切りを跨ぐと、両方が自分を担当者に加える。**
アカウントが別なので**担当者が2人になる。**
**そのまま両方が hold を書いて着手する。**branch の名前は issue から決まるので同じになり、**片方の push が通らない。**

**新しい版の run は、そのあとも止まらない。**`verifyHandoff` は担当者の一覧に自分が入っていれば
「担当は自分のまま」と読むので、**2人のうち片方が自分である限り真にならない。**
**古い版の run が止まるかどうかは、古い版の判定に依る。**
**止まるのは、その run が終わったあとの着手だけである**（`GateReasonManyAssigneesWithSelf` の枝へ入り、issue へは1バイトも書かない）。
**走っているあいだは、`dispatchCandidates` が関門より前で飛ばす**（印を持っている issue は `handoffGate` を通らない）
**ので、ダッシュボードの「関門で止めた一覧」には出ない。****実行中の一覧には、それぞれの機械でふつうに出る**
（同じ issue が2台の実行中の一覧に並ぶ）。
**人間が担当者を外すまで戻らない。**
