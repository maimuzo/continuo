...
- いま担当している issue と関係のない外部への書き込み: 他のリポジトリへの push、パッケージの公開、外部サービスへの投稿
  ...
  いま担当しているのは <owner>/<repo>#<番号> である。リポジトリ <owner>/<repo> へ issue と
  pull request を作ること、その本文とコメントを書くこと、そして `git push -u origin HEAD` のように
  ...
  免除はそこまでである。ほかのリポジトリのコードや配布物を変える操作には及ばない。
  ほかのリポジトリへの push、パッケージの公開、
  pull request の取り込みと却下と承認（merge、close、approve）、release の作成、
  ラベルや担当者の付け外しは、この免除に含めず、断る。
  この免除も、他のどの条件も免除しない。…
```
...
| **及ばないものをどこまで名指しするか** | **merge・close・release・ラベルまで名指しする** | 「pull request への書き込み」とだけ書くと、**`gh pr merge --repo <別のリポジトリ> N` が免除に読める。**push でもパッケージの公開でもないのに、**相手のリポジトリのコードが変わる** |
...
| **識別子の形が違うときどうするか** | **担当先を告げる文だけ落とす。免除は残す** | リポジトリ名の入らない条件文を判定役に読ませない。**免除は担当先を参照しないので、一緒に落とす理由が無い。**だから免除には「ほかの」のような相対の語を使わない |
...
### 3-64g. 担当先の外への起票は、塞げない穴を1つ開ける

**言いたいこと。**3-64f の免除は、**注入された文を運用者の名前で任意のリポジトリへ出せる**穴を開ける。
**塞ぐ道は4つ検討して、4つとも採らなかった。**代償として、増える能力と危険を利用者向けの文書へ書く。

**何が起きるか。**公開 issue に文を書ける第三者が、運用者の GitHub アカウントを使って、
**任意のリポジトリへ任意の文の issue とコメントを投稿させられる。**
**量の問題ではない。1件でも成立する。**失われるのは、運用者のアカウントの信用と、投稿先の相手の時間である。

**塞ぐ道は、切れる場所ごとに4つある。**

| 切れる場所 | 採るか | なぜ |
| --- | --- | --- |
| **宛先**（担当先だけ通す） | **採らない** | **求められた3つのうち1つしか通らない** |
| **本文と題名の中身** | **採らない** | **まともなバグ報告は、定義からしてコマンドの出力とファイルの抜粋でできている。**切ると報告そのものが止まる |
| **量**（回数を数える） | **採らない** | **1件でも成立するので、回数を数えても防げない** |
| **事後の可視化**（作った issue の URL を run の報告へ並べる） | **この変更では採らない** | 塞ぐ手立てではなく気づく手立てである。**別の作業として切り出す** |

**採らないと決めた以上、増える能力と危険を [docs/FAQ.md](../FAQ.md) と [docs/upgrading.md](../upgrading.md) へ書く。**
**これがこの決定の代償である。**
