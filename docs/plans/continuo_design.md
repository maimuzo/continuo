...
**複製が古くならないよう、機械で確かめる。**キーが `en.json` に在る以上、
**`ja.json` の文言を直しても日本語へ落ちない。**黙って古い文言が出続ける。
`test/internal/i18n/i18n_test.go` の
`TestMessages_英語の資源が日本語の資源の複製のままである` が、宣言した全キーについて
`en.json` の文言が `ja.json` と一字一句同じであることを見る。
**本物の訳を入れ始めるときは、この検査を入れ替える**（訳したキーを除く形にするか、
複製をやめて差分だけを置く形にする）。**入れ替えずに訳を入れると、そこで落ちる。**

**落とし先の検査には、穴の空いた資源を組む。**複製である以上、`en.json` には
...
**代わりに、閉じ印の「何番目か」で決める。****閉じ印は外部にも書ける。**
`tool_input.command` の値へ `</tool_call>` と書くだけでよく、
**JSON の値の中でも `<` `/` `>` は escape されない**（逃がされるのは `"` と `\` と制御文字だけである）。
**だから「閉じ印より後ろ」では、どちらの閉じ印を指すのかが決まらない。**

**「最後の閉じ印より後ろ」と言い切れば破れない。**閉じ印は雛形にちょうど1つしか無く、
外部が書ける文字列は `$ARGUMENTS` へ差し込まれる JSON の中にしか入らない。
**その JSON より後ろは雛形の固定の文字列で、そこに閉じ印は1つも無い。**
だから最後の閉じ印は必ずこちらが置いたもので、**外部は自分より後ろへ1文字も足せない。**
...
| 検査 | 何を落とすか |
| --- | --- |
| `TestToolGate_囲いの終わりを最後の閉じ印だと言い切っている` | **雛形の文面が「最後の」を落としたとき**（例: 「この指示文のうち閉じ印より後ろ」） |
| `TestToolGate_囲いの印は雛形にちょうど1つずつしか無い` | 印が増えたとき。**`$ARGUMENTS` より後ろへ閉じ印を書き足したときも落ちる** |
| `TestToolGate_本文に閉じ印を書かれても指示の範囲が動かない` | 本文に閉じ印を書いた入力で、最後の閉じ印より後ろがこちらの指示でなくなったとき |
| `TestToolGate_判定の指示文に読める秘密を混ぜない` | 印や指示文に秘密が混ざったとき |

**文面を見る検査を分けて立てる理由。**位置と個数だけを見る検査は、**文面が曖昧に戻っても落ちない。**
実際、この直しの前の文面（「この指示文のうち閉じ印より後ろ」）へ戻しても、
位置を見る2つは通ったままだった（実測: 2026-08-29）。
**破れるかどうかを決めているのは判定役が読む日本語なので、その日本語を直接見張る。**
...
**採らなかった守り方。**どちらも「囲いの印を秘密にする」ための案である（3-64b）。
