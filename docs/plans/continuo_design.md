...
| 順 | 何をするか |
| --- | --- |
| 1 | `~/.continuo/github-app-credentials.json` を読む。**無ければ終了コード 1**（下） |
...
**投稿を止めるのは、run を失うことより重い判断である。**
止めると、成果報告が投稿できず、continuo が「エージェントが書いていない」と判定して run を人間へ渡す
（[internal/orchestrator/comment.go:348](../../internal/orchestrator/comment.go#L348) の `hasRunComment`）。
**それでも止める。****印の無いコメントが混ざると、この設計が守ろうとしているものが無くなるためである。**
**`false` にすれば、その重さを引き受けずに済む。**選ぶのは利用者である。
...
**`Issues` だけなら、漏れたときに起きるのは「issue へ勝手にコメントが付く」ことである。**
...
| 順 | 何をするか | 何が止まるか |
| --- | --- | --- |
| **1** | **GitHub の画面で App の client secret を作り直す** | **その瞬間、古い資格情報では更新できなくなる** |
