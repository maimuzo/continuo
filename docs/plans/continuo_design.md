...
**渡さないと、根拠も件数も分類も返ってきません。**そのうえで 5-6 は、それらが無い指摘を落とすよう指示しています。
...
**ファイルを書き換えるときは、書き込む先をその場で空にしてから書かないでください。**
**途中で落ちると、元の内容が失われます。**
...
**どれがそれかは、`.github/workflows/` の中を `code-review-result` で検索し、当たったファイルの `name:` の値で見分けます。**
...
**資格情報（API キー・アクセストークン）と、社内だけで引けるホスト名も書かないでください。**
**書いてしまったときは、消す前にその資格情報を無効化してください。**消せたかどうかに関わらず。
...
**採る形。**[internal/prompt/builtin.md:284-318](../../internal/prompt/builtin.md#L284-L318) の
