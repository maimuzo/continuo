...
**候補の取得そのものが失敗したときは、候補が1件も無いので救わない。**
**`gh` の認証の検査が落ちたときにどうなるかは測っていない。**
認証の検査と候補の取得は別の経路なので、候補が取れることもある。
...
**門1 は「印を持っている」ではなく「印を持っていて、pane も持っている」である。**
**印だけを見ると、pane を持たない run が永久に飛ばされる。**

**その run は実在する。**着手に失敗してバックオフに入った run は、pane を閉じられたうえで印を持ち続ける。
**そこへ人間がカードを direct chat へ動かすと、こうなる。**

| 何が | どうなるか |
| --- | --- |
| バックオフが明ける | 巡回が毎回 `redispatch` を呼ぶ |
| `redispatch` は入口で戻る | **バックオフを消す行まで届かないので、毎巡回これを繰り返す** |
| 巡回が印を立てる | `enterDirectChatMode` は真を返すが、**残す pane が無い** |
| このパスの門1 | **印を持っているので飛ばす** |

**pane は永久にできない。**WARN も出ない（門3 の WARN は門1 で飛ばされるので届かない）。
**そのあいだ `agent.max_concurrent_agents` の枠を1つ持ち続ける。**

**そしてこれは、人間がいちばん使いたい場面である。**この設計自身が
「**人間はまさに、その失敗を自分で解きほぐすためにカードを動かしている**」と書いている。
**着手に失敗してバックオフに入った issue は、まさにその issue である。**

| 順 | どういうときに飛ばすか | 飛ばすときに何を出すか |
| --- | --- | --- |
| 1 | **既に印を持っていて、pane も持っている** | 何も出さない（巡回の側が direct chat へ入れる） |
...
| 3 | **空きスロットが無い**（`agent.max_concurrent_agents`） | **WARN で名指しする。ただし1つの issue につき1回だけ。**direct chat の run は自分では終わらないので、上限を上げても次に同じことが起きる。効く直し方は「どれかの issue を direct chat から戻す」である。**毎巡回で出すと、枠が詰まっているあいだ他の WARN が全部これに埋もれる。****この記録は2箇所で消す**（消さないと、次にまた枠が尽きたときに1行も出ない）。**1つは、この issue の pane を用意できたとき**（このパスの段7）。**もう1つは、巡回が取り直した候補のうち、Status が `direct_chat_state` でないものの記録を消す。****「カンバンに載っていないものを消す」形にしてはならない。**候補の一覧は `active_states` も含むので、**direct chat から `Ready` へ戻した issue は載ったままになり、記録が消えない。****消えないと、その issue を次に direct chat へ動かしたときに1行も出ない。****1回目とは違って「1回は出た」という記憶があるので、人間は「今回は別の理由だ」と読み違える** |
...
| 順 | 何をするか |
| --- | --- |
| 5 | **印を付ける**（着手の段1）。**ただし写しの Status は書き換えない。**書き換えると、状態ごとの上限（`agent.max_concurrent_agents_by_state`）の勘定に入り、**direct chat のカードを1枚置いただけで通常の着手を1件ぶん失う** |
...
| **復元の引き渡しの通知**（`moveToFailure`） | **Status を書かないだけでは足りない。通知そのものを投稿しない。**pane を閉じない2つの道（herdr が `blocked` を返した／引き継いだ回数が上限）は、**`closePane` より先に `moveToFailure` を呼ぶ。**あれは Status が書けなくても通知を投稿し、その本文には**「continuo が pane を閉じたので画面は残っていません」**が入っている。**画面は目の前に残っているのに、そう書かれたコメントが再起動のたびに1件積まれる。****issue のコメントは消せない** |
...
**だから、時計ではなく判定そのものを直す。**
**「この run で continuo が担当者を書いたか」を見て、書いていないなら止めない。**
run はその印を既に持っている（着手のときに `setHandoffAcquired` で入れている）。

**なぜそれで正しいか。**確かめ直しが守ろうとしているのは「担当を**外された**」場合である。
**一度も担当者になっていない run には、外される担当が無い。**
既存の判定が「担当者が1人もいないだけでは止めない」としているのと同じ理由である
（**「まだ誰も担当していない」と「担当を外された」を見分けられない**）。

**direct chat の用意は、担当者を1バイトも書かない。**だからこの印は必ず偽である。
**人間が自分を担当者に付けても、機械はそれを「自分が失った担当」とは読まない。**

**入札で担当者になった run では、いままでどおり効く。**そちらの印は真である。
