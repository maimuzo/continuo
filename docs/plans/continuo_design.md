...
**渡さないと、根拠も件数も分類も返ってきません。**5-6 は、それらが無い指摘を落とさずに、
**あなたが受け取る側で補う**よう指示しています。**補う手間を減らすために、先に渡してください。**
...
**あなたが書くコードが、既にあるファイルを書き換えるときの話です。**
**あなた自身の編集は、Claude Code の編集の道具を使ってください。**

**書き込む先をその場で空にしてから書くコードを、書かないでください。**
**途中で落ちると、元の内容が失われます。**
**同じディレクトリへ一時ファイルを作り、書き切ってから差し替えてください。**
...
当たったファイルの先頭にある `name:` の値で見分けます**（字下げの無い行。job や step の `- name:` ではありません）。
**その名前が `workflowName` に出ます。**
**先頭に `name:` が無い workflow では、`workflowName` にファイルのパスが入ります。**そのときはパスで見分けてください。
...
**指摘1件ごとに、次の4つを添えさせてください。**
    ...
    3. 同じ誤りが他に無いかを数えた結果。叩いた検索パターンと、数えた範囲と、当たった件数
...
**ただし、差分を読む役には数える範囲を渡していません**（この節のすぐ上の表）。
...
**使い捨ての計画を、消えてよい場所へ書いてください。**リポジトリには置きません。
**消せない設定のときは、消さずに残してかまいません。**`rm` が拒否される設定があり、拒否が続くと実行そのものが止まります。
...
| **関連するファイルの絶対パスと行番号** | 「この文書の中にあります」は、渡していないのと同じです |
| **その材料を確かめてあるか / 確かめていないか** | 別の subagent の報告を確定した事実として扱い、その上に結論を積みます。間違いは積んだ結論の全部に伝わります |
| **数える範囲**（絶対パス） | subagent が別の checkout を数え、同じ文字列が0件と1件に割れます |
| **前の周の判断票**（2周目以降のレビュー） | 否定した指摘が毎周また挙がります（5-6） |
...
**採る形。**[internal/prompt/builtin.md:288-322](../../internal/prompt/builtin.md#L288-L322) の
