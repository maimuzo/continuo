...
外したら `LastSeenAt` を現在時刻にしてから継続の指示を1回送る（下記）。
...
| **0b** | **画面の版が窓のあいだ増えていないか**（`agent.get`） | **手放さない。**枠待ちの標識はそのままにする |
| **0c** | **外す相手が決まるか**（issue のノード ID と `gh` の持ち主） | **手放さない。**次の巡回でやり直す |
| **1** | **`workspace_hooks.after_run` を走らせる** | — |
| **2** | **自分の担当者を外し、`<!-- continuo:released -->` を1件書く** | **pane を閉じずに戻る** |
| **3** | **worker を止める** | — |
| **4** | **run の登録から外す** | — |

**段0a を段0b より前に置く。順番を入れ替えてはならない。**
**画面は herdr、担当は GitHub と、別々の外部に依存する。**
**画面を前に置くと、herdr が落ちているあいだ担当の確認が1回も走らない。**
**枠待ちの run に turn の終わりは来ない**ので、担当を確かめる経路は他に無く、
**担当が移ったまま同じ branch で作業を続けることになる**（3-77c が禁じている）。

**段0a〜0c の答えは3つある。「はい」「いいえ」「分からない」である。**
...
| **0b** | 画面が動いていれば手放さない。**枠待ちの標識も外す** | **枠待ちの条件その2（hook の無音）は、「枠を待っている」と「長い1つの仕事をしている」を区別できない。**hook はツールが終わってから飛ぶので、**1時間を超える1回のツール呼び出しの最中は1件も来ない。**そこへ `weekly_scoped` が100%だと条件が両方そろい、**正常に走っている run を殺す。**`weekly_scoped` は使っていないモデルでも返る（3-77 の「最初から返ってくる」） |
| **0c** | 外す相手が決まらなければ、`after_run` を走らせずに戻る | `after_run` の標識は**実行の前に**立ち、**外れるのは着手と片付けのときだけである。**先に走らせて外す相手が分からなかった場合、**この run が枠明けに完走しても、終了時の `after_run` が標識に弾かれて1回も走らない** |

**段0a は「担当者が1人もいない」も「自分ではない」として扱う。**
**「別の機械が担当になった」だけを見てはならない。**期限切れの担当を外してから次の入札が
決まるまでの窓では、**担当者が0人になる。**その窓で `after_run` を走らせると、
**もう自分のものではない branch へ利用者の `git push` が飛ぶ。**

**段0b の窓は `claude.turn_timeout_ms`（既定1時間）である。巡回の間隔（既定30秒）ではない。**
**「前の巡回から版が増えたか」だけで決めてはならない。**この段の根拠は
「1時間を超える1回のツール呼び出し」なので、**30秒動かない窓を1つ拾っただけで手放すと、
90分のビルドの最中に殺すことになる。**
**版が変わった時刻を控え、いまとの差を「静止が続いた長さ」とする。**
**stall の時計（最後に動いていた時刻）を進めてはならない。**枠待ちの run はそれを止めると決めており、
**進めると窓が毎回いまの時刻に揃って永久に閉じない。**
**`claude.turn_timeout_ms` が0以下のときは `rate_limit.weekly_wait_limit_minutes` を窓に使う**
（**窓を外してはならない。**外すと、打ち切りの判定を切った機械だけが
「30秒動かない窓を1つ拾って手放す」形になる）。

**段0b で画面が動いていたときは、枠待ちの標識を外す。**
**枠を待っていない run に「枠待ち」と名乗らせたまま stall の時計を止めると、
そのあと本当に固まっても誰も止めない。**
**段0b で版を読めなかったときは、標識をそのままにする。**
**外すと、その run は枠待ちでも stall の対象でもなくなり、どちらの経路からも見えなくなる。**

**3つとも、答えが出なかったときは手放さない側へ倒す。**
...
**1周の下限は `claude.turn_timeout_ms`（既定1時間）である。**
**ただし 0 以下にしている場合は、その下限が無い**（無音の長さを見ずに枠待ちと判定されるため。上の段落）。
...
**読み取りも数える。**段0a が herdr へ1回、段0b が GitHub へ2回（issue の取り直しとコメントの全件）である。
**段2 が失敗し続ける間は、この3回が巡回のたびに繰り返される**（既定30秒に1回、run ごと）。
**上限は掛けない。**間引くと、間引いた窓のあいだは段0b が「移っていない」と答えることになり、
...
**いまどうなっているか。**[internal/orchestrator/dispatch.go:700-706](../../internal/orchestrator/dispatch.go#L700-L706) は
...
**巡回に後片付けを寄せることはできない。**
[internal/orchestrator/runstate.go:1599-1601](../../internal/orchestrator/runstate.go#L1599-L1601) が
...
| **使い切っている枠の `kind`** | **1週間の枠は2つある**（`weekly_all` と `weekly_scoped`）。**使用率は最大を採って1つに畳むので、それだけでは `weekly_scoped` が原因のときに読めない。**claude.ai の画面に出る週次の全体が30%でも、使っていないモデルの枠が100%ならこちらで止まる |
...
### 5-3g. `continuo init` が置く1枚

**言いたいこと。**`continuo init` が置くのは `WORKFLOW.md` だけである。
**front matter（設定）と本文（固有の指示）が、1つのファイルに入っている。**

| 状態 | 何をするか | 終了コード |
| --- | --- | --- |
| 無い | 書く | 0 |
| 在る | 何も書かない。`--force` を勧める | 1 |
| ディレクトリが無い・ディレクトリでない・symlink・書けない | 何も書かない | 1 |

**なぜ2枚に分けないか。**設定も固有の指示も「この project の人が書くもの」である。
...
| `trust.repositories` の形が違うときのエラー | [internal/config/validate.go:639-642](../../internal/config/validate.go#L639-L642) |
