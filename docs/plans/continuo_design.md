...
**`prepareDirectChatPanes` が通す門は、上から順にこれだけである。**否定形を1つも書かない。

| 順 | 門 | 通らなかったらどうするか |
| --- | --- | --- |
| 1 | **既に印を持っているか** | 何もしない（巡回の側が direct chat へ入れる） |
| 2 | **この worktree に pane が1枚でもあるか** | 何もしない。**「Claude Code が居るか」は判定しない**（下） |
| 3 | **空きスロットがあるか**（`agent.max_concurrent_agents`） | **WARN で名指しする。ただし1つの issue につき1回だけ。**direct chat の run は自分では終わらないので、上限を上げても次に同じことが起きる。効く直し方は「どれかの issue を direct chat から戻す」である。**毎巡回で出すと、枠が詰まっているあいだ他の WARN が全部これに埋もれる**（`noteLabelSkip` と同じ形にする） |
| 4 | **着手の直前の検査**（`preflight`） | 何もしない。次の巡回で見る。**信頼登録の判定をこの門より前に置いてはならない。**`Dispatchable` で先に落とすと `preflight` へ届かず、**未信頼のリポジトリで direct chat を頼んだ人へ、直し方のコメントが1件も出ない**（3-33） |
| 6 | 印を付ける（着手の段1） | — |
| 7 | 着手の段3〜段10 を踏む（**段2 と段11 は踏まない**） | 落ちたら、自分が開いた pane を pane ID で閉じ、印を外す。**カンバンへは1バイトも書かない** |
| 8 | direct chat へ入れ、issue へ1件書く | — |
...
**`protectedStates()` の呼び出しは9箇所である**（`git grep -n 'o\.protectedStates()' -- internal/` で実測）。
**1本でも渡し忘れると、人間が話している最中のカードが書き換えられ、次の巡回で pane が閉じる。**
...
**印を見るどんな門を置いても守れない。`protectedStates()` が唯一の守りである。**
...
**`enterDirectChatMode` の中で、確かめ直しの時計を進める。**
**呼び出し側に書かせてはならない。**入れる場所は4箇所ある
（用意し終えたとき／巡回が気づいたとき／turn の終わりに気づいたとき／復元が引き取ったとき）。
**1つでも落とすと、その経路で入った run は戻した瞬間に止められ、pane が閉じる。**
...
**指示を1回も送る前に、である**（巡回は `reconcileRunning` → `checkStalls` → dispatch → `wakeRuns` の順に走る）。
...
**`continuo doctor` は、綴りを取り違えたときだけ `!` を出す。**

**出す条件は「カンバンに紛らわしい列があるのに、設定に書いた名前と一致しない」ときだけである。**
**カンバンに列が1つも無いときは黙る。**

**なぜこの条件か。**害が出る向きは1つだけである。
**カンバンに `Direct Chat` の列があるのに、設定に `DirectChat` と書いてある**——
このとき continuo はその名前をカンバンに見つけられないので、
人間が置いたカードを「知らない Status」として扱い、**猶予（既定10分）のあとで pane を閉じる。**
**issue が消したかった症状そのものである。**
**そして起動時の照合から外してある以上、気づける場所は `continuo doctor` しか無い。**

**逆向き（カンバンに列が1つも無い）には害が無い。**そこへ遷移できる issue が存在しないためである。
**そちらで出すと、既定が非空である以上、版を上げた全員に恒久的に出る。**
**そして「黙らせたいから空文字にする」という需要を人工的に作る。**
