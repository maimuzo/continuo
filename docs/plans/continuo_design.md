...
| 何 | 識別子の出どころ | なぜそこか |
| --- | --- | --- |
| **入札** | **コメントの投稿者**（GitHub が付ける `author.login`） | **本文には書かない。**本文は第三者にも書けるので、他の continuo の名前を騙られると、**騙られたほうは `HasBidBy` が真になって以後1度も入札しなくなる** |
| **hold** | **本文の `assignee`。あわせてコメントの投稿者も見る** | `LatestHoldFor` が issue の担当者と突き合わせるので、**その相手は投稿者では代われない。**そのうえで**投稿者と `assignee` が食い違う hold は数えない**（下の「第三者が書いた hold」） |
| **released の `from`** | **本文の `from`** | **投稿者とは別のアカウントを指す。**書くのは外した側で、`from` に入るのは外された側である |

**書く側でも識別子を埋める。**`bidForIssue` はその巡回で書いた入札の写しをそのまま勝敗の判定へ混ぜるので、
**空のままだと自分が勝っても「負けた」と読む。**`bid_window_ms: 0` では書いた巡回でそのまま勝敗を決めるため、
**担当者が永久に決まらない。**

**投稿者の分からない入札は数えない。**GitHub は削除済みアカウントのコメントに投稿者を付けない。
**数えると、同点の3段目（3-77d）で空文字がどのログイン名にも勝ち、その回はどの continuo も着手しない。**

**第三者が書いた hold は数えない。**hold の本文は issue にコメントできる誰でも書けるので、
**本文だけを信じると、`{"assignee":"<人間のログイン名>"}` と書いた1件で、その人間の担当を外させられる。**
3-77b は「**hold のコメントがあることが『その担当者は機械である』の唯一の証拠である**」と決めており、
**その証拠を第三者が作れてはならない。**
**continuo が書く hold は、担当を取った当人が自分のログイン名を書く**ので、正しい hold は必ずこの検査を通る。

**掛けるのは `LatestHoldFor`（担当を外してよいかの判定）だけである。**
**`endsRound`（入札の回を閉じるかの判定）には掛けない。**
アカウントを改名すると、**GitHub は古いコメントの投稿者名だけを新しい名前へ書き換え、
本文の `assignee` は古い名前のまま残る。**掛けると、その issue で回が二度と閉じなくなる。
**`LatestHoldFor` のほうは、本文の `assignee` と issue の担当者を突き合わせる段で、
この検査を足す前から同じように落ちる。**新しい壊れ方は増えない。
...
**「担当者は自分のアカウントだが、担当しているのは別の機械」という状態を作れなくなった。**
3-77b の表から、その2行が消えている。**同じアカウントで2台動かしている人は、いまはホスト名で
区別できていたが、変えると区別できなくなる。**人間はそれを了解したうえで決めた。
...
**入れたとしても、この検知は「出なければ大丈夫」とは読めない。**
**片方が hold を書き終えたあとに、もう片方が初めてその issue を見る経路では、
「自分のアカウント1人」で着手へ進み、コメントを1件も書かない。**痕跡が1バイトも残らないので、
**取得の競争でだけ警告が出る。**入れるなら、この限界を利用者向けの文書に併記することになる。
...
| [docs/upgrading.md](../upgrading.md) | **版が混ざったときに何が起きるか。**全機械を同時に上げること |
| この節 | 決定と、その理由 |
...
**散文にはアカウントの名前を出すが、continuo はその散文を1バイトも読まない。**
...
**見ないのは判定であって、読み取りではない。**`handoffGate` はコメントを取ってから `Assess` を呼ぶので、
**担当者が自分1人の issue でも、1回の巡回でコメントを読める枠を1つ使う**（3-77f）。
**取ったコメントは、この行の判定には1件も使わない。**
...
**あわせて、コメントの投稿者が `assignee` と一致することも見る**（3-77-0 の「第三者が書いた hold」）。
...
| 判定スコアも投稿の時刻も同じ | **入札を書いたアカウントの名前の小さい順で勝者を決める**（3-77-0） |
...
**時計を戻せば必ず勝ててしまう。**
**3段目に空文字が来てはならない。**空文字はどのログイン名よりも小さいので、必ず勝ってしまう。
**投稿者の分からない入札を1件も数えないことで、そうならないようにしている**（3-77-0）。
