...
#### 訂正（2026-09-08）: `revision` は「本当に動いているか」を示さない

**この節はもともと「画面の版は『本当に動いているか』を直接示す」と書いていた。誤りである。**

**`revision` は画面を1バイトも見ていない。**herdr が増やすのは
...
状態が `working` → `done` へ移った1つだけ、`state_change_seq` が 1384 → 1385 と動いた。

**この節の段1（版が増えていれば時計を起こし直す）は、1度も発火していない。**
**つまり「1つの turn に何時間かかっていても殺さない」という約束は、いまも効いていない。**
**塞ぐには画面の本文そのものを読む必要がある**（`agent.read`）。
**「止まっているのに変わる」を先に測る必要があるので、別の issue とする。**

**打ち切りの側は、この訂正では変えない。**
**入口が「hook が `claude.turn_timeout_ms` 来ていない run」であり、
その窓でいちばん多い「長い1回のツール呼び出し」では状態も `working` のままなので、
`state_change_seq` へ替えても良くならない。**
**そのうえ、状態が往復する run を永久に打ち切れなくする穴が開く。**

**手放しの側（3-27）は替えた。**あちらは `agent_status` が `idle` か `done` のときしか見ないので、
...
| **新規の着手を止める線** | **入札の余裕値が0以下**（`100 − 使用率 − 種別ごとのマージン`。マージンは既定10なので使用率90%から）。**走行中の turn は止めない。担当が既に自分にある issue も止めない** |
...
**知らない種別には5時間のマージンを当てる**（マージン0へ倒れるより、取り置きを守るほうが安全である）。

**線を2本持ってはならない。**以前は「使用率100」と「余裕値がマイナス」の2本があり、
...
claude.turn_timeout_ms のあいだ何も観測できなかった run について、上から順に見る
  1. agent.get の revision（画面の版）が増えているか
...
**画面の版のためだけに、別の時計を作ってはならない。**
`noteRevision` は**版が変わったときだけ**「最後に動いていた時刻」を進める。
**版が変わった run は、そもそも枠待ちではない。**だから「枠待ちの run はその時刻を進めない」という
約束は、この順番でも破れない。
...
flowchart TB
    ...
    full --> waiting["枠待ちとして記録する<br/>stall の時計と turn の時計を止める"]
...
| **1つでも `resets_at` を持たない** | **「分からない」として、満杯になってからの経過で測る** |
| **満杯の枠が5時間の枠だけ** | **上限を掛けない。**いつまでも待つ |
...
**測るのは「満杯の1週間の枠を最初に見てから、どれだけ経ったか」である。**
**run ごとに持つ。**機械に1つだけ持つと、**枠が満杯になったあとに着手した run を、
1分も待たずに手放すことになる。**

**時計を進めるのは、満杯かどうかを見るたびである。**
**起点は「この run について、満杯の1週間の枠を最初に見た巡回の時刻」になる。**
**「実際に枠が満杯になった時刻」とは、最大で巡回の間隔（既定30秒）ずれる。**
...
**満杯かどうかは、標識の有無によらず巡回のたびに見る。**
**判定の中だけで見てはならない。**判定は標識が立っている run しか通らないので、
**標識が別の経路で外れると、以後どこからも消されない。**
...
| 順 | 何をするか | 答えが出なかったら |
| --- | --- | --- |
| **0a** | **まだ自分が担当か**（issue の担当者と hold のコメントを読み直す） | **手放さない。**次の巡回でやり直す |
| **0b** | **外す相手が決まるか**（issue のノード ID と `gh` の持ち主） | **手放さない。**次の巡回でやり直す |
| **1** | **`workspace_hooks.after_run` を走らせる** | — |
| **2** | **自分の担当者を外し、`<!-- continuo:released -->` を1件書く** | **pane を閉じずに戻る** |
| **3** | **worker を止める** | — |
| **4** | **run の登録から外す** | — |

**画面が動いていないことは、ここでは確かめない。**
**上の評価順の段1 が、枠待ちの標識を立てる前に確かめている。**
**同じ判定を2箇所へ置いてはならない。**片方だけが直る。

**段0a と段0b の答えは3つある。「はい」「いいえ」「分からない」である。**
**「分からない」を「はい」へ畳んではならない。**畳むと、コメントを1回読めなかっただけで
...
**段0a は「担当者が1人もいない」も、そのままでは「自分ではない」として扱う。**
**「別の機械が担当になった」だけを見てはならない。**期限切れの担当を外してから次の入札が
決まるまでの窓では、**担当者が0人になる。**その窓で `after_run` を走らせると、
**もう自分のものではない branch へ利用者の `git push` が飛ぶ。**

**ただし、担当者が0人になる場面は2つあり、行き先が正反対である。**

| どちらか | 何が起きたか | どうするか |
| --- | --- | --- |
| **自分のものではない** | 別の機械が期限切れの担当を外した直後 | **手放さない。**`after_run` を走らせずに worker を止める |
| **誰のものでもない** | 復元した run・この機能より前に着手した run・hold を書けなかった run | **手放す。**`after_run` を走らせ、`released` を書く |

**見分けるのは `released` のコメントである。**この機械の名前で「担当を外した」記録が残っていれば、
外されたほうである（3-77c）。**記録が無いほうを「誰のものでもない」に倒す。**
**倒さないと、復元した run が `after_run` も `released` も無しに pane を閉じられ、
成果が worktree に残るだけで issue には1行も残らない。**

**どちらの段も、答えが出なかったときは手放さない側へ倒す。**
**stall の段3（「画面の版を読めませんでした。止まったものとして扱います」）と向きを変える。**
...
**どの段も、既存の確かめでは間に合わない。**画面の版を見る段は、stall の評価順で**枠待ちの枝より前**にある
（2026-09-08 に入れ替えた）。**そのうえ、その段は1度も発火していない**（3-21 の訂正）。
...
**段0a〜0c と段2 で戻るときは、確保した「終わらせる最中」の標識を必ず戻す。**
...
> **mac-studio が1週間の枠を待つ上限を超えたので、自分で担当を手放しました。**
> `workspace_hooks.after_run` は実行済みです。この branch を次に取る機械は、
> remote の続きから始めてください。

**1行目は、外された側と共通である。**差し込むのは機械の名前だけで、上限の分数は入らない。

**`after_run` が走らなかったときは、理由を分ける。**`reason` は `weekly_wait_limit_no_push` になり、
本文は「**`workspace_hooks.after_run` は走りませんでした。**この branch の remote には、
続きが入っていないことがあります」に変わる。
**走っていないのに「実行済みです。remote の続きから始めてください」と断言すると、
...
3-77i は「枠待ちと新規 dispatch を止める閾値は、最後に読めた値を使い続ける」と決めているが、
...
**判定は、枠待ちの標識が立っている run について巡回のたびに行う。**
...
**後ろに置くと、既定値では一度も走らない。**枠待ちの条件その1 は「`percent` が 100 に達している」で、
**新規の dispatch を止める門は既定95%を超えたら効く。**
**つまり既定値では、枠待ちが存在する場面でその門は先に効いている。**
**「必ず」である**（人間の決定。2026-09-06。issue #173 / #197）。
**新しい着手を止める線と、枠待ちの条件その1 は、同じ `handoff.Short` を使う。**
**片方だけが成立することは無い。**
...
**ブレーキを全部外した設定では、手放した issue をすぐ拾い直す。**
**2つのマージンを両方0にすると、使用率100に達するまで入札が通る**ので、
**「着手 → 枠待ち → 上限で手放す」を繰り返す。**
**1周の下限は `claude.turn_timeout_ms`（既定1時間）である**（枠待ちに入るにはその長さの無音が要る）。
**0 以下にしている場合の下限は `rate_limit.weekly_wait_limit_minutes` である。**
その値が0以下だと上限の判定そのものが起きないので、**手放しが起きる設定では下限は必ず正である。**
**1周あたりの GitHub への書き込みは、手放す側で2回**（担当者を外す・`released`）、
**取り直す側で3回**（入札・担当者を足す・`hold`）**の計5回である。**
**加えて、手放す側で `workspace_hooks.after_run` が1回走る。**
**そこに `git push` を書いている人は、1周ごとに remote への push が1回増える。**

**読み取りも数える。**段0a が GitHub へ2回（issue の取り直しとコメントの全件）である。
**段2 が失敗し続ける間は、この2回が巡回のたびに繰り返される**（既定30秒に1回、run ごと）。
...
| --- | --- |
| `Ready` へ戻す | **止まらない。**`Ready` は `tracker.active_states` の1つであり（[internal/scaffold/template.go:44](../../internal/scaffold/template.go#L44)）、巡回は「まだ作業中で routable」としてスナップショットを更新するだけである（[internal/orchestrator/reconcile.go:101-102](../../internal/orchestrator/reconcile.go#L101-L102)）。しかも `Ready` は `dispatch_state` なので、印から外れていれば**もう一度着手される** |
...
**いまどうなっているか。**[internal/orchestrator/dispatch.go:1146-1152](../../internal/orchestrator/dispatch.go#L1146-L1152) は
...
**巡回に後片付けを寄せることはできない。**
[internal/orchestrator/runstate.go:1660-1661](../../internal/orchestrator/runstate.go#L1660-L1661) が
...
**モデル別の枠（`weekly_scoped`）は最初から返ってくる。**「一定量を使うまで現れる」ではない。
...
使用率0で返る以上、**最大を採れば、使っていない枠は自動的に判定へ効かない。**

**`resets_at` の扱い。この2つを混ぜてはならない。**
...
**測っていない以上、「`resets_at` は必ず読める」を前提にした判定を書いてはならない。**
**読めないことがある前提で書く**（3-27 の「1週間の枠が明けるのを待つ上限」）。
...
既定（マージン10）では余裕値が使用率90%で先に効くので、**あちら（96%から）は一度も発火しない。**
...
既定のログの水準では何も出なかった。****止める条件は変えず、喋る口を1つにまとめる。**
...
| --- | --- | --- | --- |
| 入札の判定 | 余裕値 `100 − 使用率 − マージン` | **91%から**（マージンはどちらも既定10） | **`Debug` の1行** |
...
**入札の判定のほうが低い使用率で効き始めるので、91〜95% の帯には `Info` を出す口が1つも無い。**
**枠そのものを読めないときも同じである。**`rate_limit.pause_above_percent` の判定は、読めないと「止めない」と答えるので、
**使用率に関わらず `Debug` だけになる。**

**「低い使用率で効き始める」と「コードで先に走る」を混ぜてはならない。**
**コードの上では `rate_limit.pause_above_percent` の判定が先である**（下の段1）。

**採る形。判定を2段にする。**

| 段 | 何を見るか | どの写しを使うか | 新規の dispatch を全部やめるか |
| --- | --- | --- | --- |
| **2** | 枠を読めない / 余裕値がマイナス | **入札の写し**（読み取りに失敗したら無いものとして扱う） | **やめない**（issue ごとに落とす） |

**段1 を入札の写しへ寄せてはならない。**あちらは読み取りに失敗すると無いものになるので、
**使用率99%で資格情報が切れた機械が、新規 dispatch を止めなくなる**（3-77i を破る）。

**理由と「新規の dispatch を全部やめるか」は、別々に返す。**同じ「枠の使い過ぎ」でも、
段1 から来たものは全部やめ、段2 から来たものは担当が自分の issue だけ続ける。
**1つの値へ畳んで別の理由を名乗らせると、理由を出す仕掛けの中で嘘をつくことになる。**
**段2 だけが閾値超えと答えるのは、写しを取り直す2回の間に新しい値へ差し替わったときであり、
そのとき枠は読めている。**

**新規の dispatch をこの巡回で全部やめるのは段1 だけである。**
**「巡回を止める」と呼んではならない。**止まるのは新しい issue を取る側だけで、
**巡回そのもの（走っている run の面倒・枠の読み直し・期限切れの担当を外す経路）は続く。**
**どの理由でも全部やめる形にしてはならない。**枠を読めないだけで打ち切ると、
...
| 何を | なぜ |
| --- | --- |
| **理由** | 「枠を読めない」「枠の使い過ぎ」「余裕値がマイナス」のどれか |
| **観測した使用率**（5時間と1週間で別々に） | **閾値だけでは、どちらの枠が原因かを読めない** |
| **使い切っている枠の `kind`** | **1週間の枠は2つある**（`weekly_all` と `weekly_scoped`）。**使用率は最大を採って1つに畳むので、それだけでは `weekly_scoped` が原因のときに読めない。**claude.ai の画面に出る週次の全体が30%でも、**よく使っているモデルの枠が100%なら**こちらで止まる |
| **閾値**（枠ごとに2つ） | **マージンが左右で違うと、1つにまとめた値はどちらの枠にも当たらない** |
| **新規の dispatch をこの巡回で全部やめるか** | 止まる範囲が違う |

**読めていない枠は行ごと出さない。**`0` と書くと「1バイトも使っていない」に見え、
**枠を読めない機械が「いちばん暇」に見える。**
**直前の読み取りに失敗しているときは「使用率は最後に読めた値」を添える。**
**閾値が100 のときは「この設定では止まりません」と出す。**
`100` とだけ出すと「100%で止まる」と読まれるためである。
**これは設定の側の話であって、観測した使用率の話ではない**（使用率は100を超えて返りうる。
だから「使い切っている」の判定は `>=` で行う）。

**止めるかどうかを決めているのは、枠の使用率の最大値であって種別ではない。**
**usage API が知らない種別を返すようになると、画面とログに出る2行（5時間と1週間）の
どちらも閾値以下なのに「閾値を超えた」と出る。**
**いまは起きない**（種別は `session` / `weekly_all` / `weekly_scoped` の3つだけである）。

**出す条件は付けない。この判定を通るたびに出す。**巡回は既定30秒なので1時間で120行になる。
...
| `trust.repositories` の形が違うときのエラー | [internal/config/validate.go:735-739](../../internal/config/validate.go#L735-L739) |
