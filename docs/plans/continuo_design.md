...
止まったことと詰まったことは同じである）。**そのために、pane の一覧を引けないときの文言は
pane 待ちと鍵を分ける**（`abandon.err_pane_list_check` と `abandon.err_pane_list`）。
pane 待ちの `--force` は herdr の失敗を越えないので、1つの鍵に越え方を書くと、
**越えられない側にもそう書いてしまう**（3-37-12）。
...
**越えられるのは時間切れだけである。**待っている最中に herdr が pane の一覧に答えなくなったら、
`--force` を見ずに止まる（`abandon.err_pane_list`）。**この `--force` は「上限まで待っても
閉じなかったら越えろ」であり、herdr が答えなければその待ち自体を1度も行えていない。**
`stopIfPaneAlive` の `--force`（「確かめずに消せ」）とは別物である。
...
例: `システムは VALIDATES THAT 手を離させる書き込みを行っていないか、herdr が pane の一覧に答えたうえで worktree を作業ディレクトリに持つ pane が期限内に無くなるか利用者が force を指定している。`
...
1行だけ並び、**どちらの側を通ったかは書かれない。**現に abandon のテスト55本は26本の経路に
...
ノードは99から91へ、エッジは116から98へ減った）。**数えるべきは、段の文に条件が残っているかである。**
