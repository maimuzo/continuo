...
- 3-2b hook の送り主は確かめられない。どこまでを守れるかを、ここに書いておく
...
- 3-17h 線は「消すかどうか」で引く
- 3-17i 生きているかは、ロックの隣の覚え書きで見る。flock は掴まない
- 3-17k 同じ事実を2箇所に持たない
...
**言いたいこと。**socket に届いた hook が「本当にその run の Claude Code から来たか」を、
**いまの作り方では確かめる手立てが無い。**塞げないので、**塞げない範囲と、
塞ぐなら何を変えることになるかを、設計に残す。**

**何ができるか。**エージェントは `--permission-mode dontAsk` で走り、`Bash` は
**引数を限定せずツール名だけで許可している**（`Deny` は空。3-11）。
偽の hook を1本送るのに要るものは3つで、**どれも取れる。**

| 要るもの | どこから取れるか |
| --- | --- |
| **相手の `session_id`** | **隣の worktree の身元ファイル**（`<worktree>/.continuo.json` の `session_uuid`）。`workspace.root` の下は同じ利用者の読める場所である |
| **socket のパス** | 同じ身元ファイルの `socket_path`。**自分の設定ファイルにも書いてある** |
| **相手の worktree のパス** | ディレクトリを1つ上へ辿れば分かる |

```bash
echo '{"hook_event_name":"Stop","session_id":"<相手の UUID>","cwd":"<相手の worktree>"}' \
  | continuo hook --socket <socket> --pending-dir <逃がし先>
```

**これが通ると、相手の turn は終わったことになる。**`transcript_path` を差し替えれば
**表明（`CONTINUO-STATUS: …`）も偽造できる。**

**いまの守りは2つだけで、どちらも送り主を見ていない**（`internal/orchestrator/hookinput.go`）。

| 守り | 何を止めるか | 止まらないもの |
| --- | --- | --- |
| `cwd` がその run の worktree の内側か | **でたらめな `cwd`** | **相手の worktree のパスを書けば通る** |
| `transcript_path` が許可された根の内側の通常のファイルか | FIFO・worktree の外のファイル | **`~/.claude/projects/` の下に自分で書いたファイルは通る** |

**なぜ塞げないか。****同じ利用者・同じ機械で走る2つのエージェントを、OS が区別しないからである。**

| 塞ぎ方の案 | なぜ足りないか |
| --- | --- |
| **run ごとの合言葉を hook のコマンド行に埋める** | 合言葉は Claude Code の設定ファイル（worktree の外。3-12）に書くことになる。**引数無制限の `Bash` を持つ相手は、その設定ファイルを読める。**身元ファイルから `settings_path` を消しても、**探せば見つかる**（手数が増えるだけである） |
| **socket を run ごとに分ける** | **パスを知っていれば誰でも繋げる。**分けても、隣のパスは身元ファイルにある |
| **接続元の資格情報を見る**（`SO_PEERCRED` / `getpeereid`） | **全部同じ利用者である。**uid では区別できない。PID から親を辿る方法は、`continuo hook` が Claude Code の子として起動されるとは限らない以上、当てにできない |

**塞ぐなら、変えるのは continuo ではなく実行環境である。**

| どう変えるか | 何が変わるか | 代償 |
| --- | --- | --- |
| **エージェントを run ごとに別の OS 利用者で走らせる** | socket のファイル権限と接続元の uid で分けられる。**身元ファイルも読めなくなる** | **herdr の pane も利用者ごとに分ける必要がある。**いまの作り方から大きく外れる |
| **エージェントを worktree ごとのコンテナで走らせる** | 同上。**そもそも隣の worktree が見えない** | clone・`gh`・資格情報の受け渡しを全部作り直す |
| **`Bash` の引数を絞る**（`Deny` に `continuo hook` などを足す） | **手数は増える。**塞ぎきれはしない（実行ファイルを写せば同じことができる） | **3-11 が「無人運用で人間の入力を待つ箇所を全部潰す」ために引数を限定しないと決めている。**絞ると、許可リストに載らない書き込みで作業が止まる |

**いまの立場。**

- **1台・1利用者で、自分の issue だけを回す使い方では、脅威にならない。**偽の hook を送れるのは、既に自分が起動したエージェントである
- **公開の issue を無人で回す使い方では、脅威になる。**外部の人が書いた本文が、そのエージェントの入力になる（6-23）
- **したがって、これは「対応しない」ではなく「実行環境で分けるまで残る」ものである。**
  **[SECURITY.md](../../SECURITY.md) と [docs/FAQ.md](../FAQ.md) に、この前提を書く**
...
**`Normalize` の後に加工を足すなら、その加工の情報落ちも自分で言うこと。**
`strings.ToLower` や `strings.ReplaceAll` を黙って足すと、**その分の情報落ちには警告が付かない。**

**`Normalize` の中へ入れてはならない。**戻り値は **git の branch 名にも使う**ので、
**ドットとスラッシュがそのまま要る**（`herdr.worktree.branch_template` の既定は
`continuo/{{.issue.owner}}/{{.issue.repo}}/{{.issue.number}}` である）。
**小文字化とスラッシュ潰しを中へ入れると、branch 名が壊れる。**

**用途ごとの派生を、使う側に置く。**

| 使う側 | 何を足すか | 情報落ちをどう言うか |
| --- | --- | --- |
| **ボードのロックの名前**（`instance.boardKey`） | 小文字化・スラッシュをハイフンへ | **小文字化は情報落ちではない。**GitHub のログイン名は大文字小文字を区別しないので、`Octocat` と `octocat` は**同じボードである。**揃えないとロックが2本に分かれ、**同じボードを2つの continuo が見る**（3-17e）。**この理由を GoDoc に書く** |
| **branch 名**（`workspace` の layout） | **何も足さない** | — |

**`Normalize` 自身の警告は、どちらの使い側も必ず1行ログに残す**（3-7 の既定の規則）。
...
| 3 | `worktree.remove` を herdr の socket API 経由で呼ぶ。**引数は path でも branch でもなく herdr workspace の ID である**（実測）。**この ID は herdr に答えさせる**（`worktree.open`。断られたら `workspace.list`）。**身元ファイルの値へは決して落とさない**（下記） |
| — なぜ herdr に訊くか | **身元ファイル（`<worktree>/.continuo.json`）は worktree の直下にあり、そこでエージェントが `--permission-mode dontAsk` で動く。**そこに書かれた ID をそのまま `worktree.remove`（force）へ渡すと、**同じ機械で走っている別の run の worktree が、未 push の成果ごと消える。**`herdr` が返した path が消す相手と違えば、**そこで止める**（`internal/workspace/cleanup.go` の `resolveWorkspaceID`）。**身元ファイルの `herdr_workspace_id` は照合にしか使わず、食い違ったら警告を1行出して現物のほうを消す** |
...
| [docs/upgrading.md](../upgrading.md) と [docs/FAQ.md](../FAQ.md) | **設定は壊れない**（`runtime.lock_file` を書いてある `WORKFLOW.md` も起動する）。**だが入れ替えの手順は変わる。**下を両方に置く |

**入れ替える前に、動いている continuo を止める。**
**古い版と新しい版は、互いのロックを見つけられない。**古い版は
`<実行時ディレクトリ>/continuo.lock` を握り、新しい版は `~/.continuo/continuo.lock` を握る。
**ボードのロックも古い版は取らない**（3-17e はこの版で入る）。
**だから2つとも起動し、同じ issue に pane を2つ立てる。**

**設定が壊れないことと、入れ替えが安全であることは、別である。**
**排他の置き場所を動かす版は、必ず「古いほうを止めてから入れ替える」を案内に書く。**
「破壊的変更はありません」とだけ書くと、**利用者は動かしたまま入れ替える。**
...
**`--id` は6つのうち5つを名前から機械的に導く。**残る1つ（ボード）はボードごとのロックで断る（3-17e）。
**分け忘れが構造的に起きない形にする。**
...
| **socket と実行時ディレクトリ** | `~/.continuo/id/<名前>/run/`（3-23 の探索順より優先する。**`CONTINUO_RUNTIME_DIR` と `claude.hook_bridge.listen` は使わず、使わなかったことを1行ログに残す**） |
| **worktree の置き場所** | `workspace.root` の末尾に `/<名前>` を足す（**既定側の走査から外す。3-17f**） |
| **branch 名** | `herdr.worktree.branch_template` の**先頭に `<名前>/` を足す**。**既定かどうかを問わない**（**孤児 branch の掃除では接頭辞に数えない。3-17f**） |
| **herdr の agent 名** | `continuo-<名前>-<repo>-<番号>`（**`--id` が無ければ `continuo-<repo>-<番号>`**） |
| **ボード** | **導けない。**ボードごとのロックで断る（3-17e） |

**agent 名に名前を混ぜるのは、人間が端末で見分けるためである**（3-3）。
混ぜないと**別のボードの同じ番号の issue が同じ名前になる。**
**32文字に収まらなければ repo を削り、それでも収まらなければ `--id` の名前も削る。**
**一意性はここでは保証しない**（重複は `agent.list` を見て `-2`, `-3` を足す段が解消する。3-3）。
...
| 何 | どうする |
| --- | --- |
| **`--id` から導く5つ** | **常駐側と同じ関数を使う。**ロックだけでなく、実行時ディレクトリ・`workspace.root`・branch 名も同じ関数から導く |
| `--id` | **常駐側と同じ名前を渡す。**渡さなければ既定の1本を見る |
| **ボードのロック** | **自分のロックが空いていたら、ボードのロック（3-17e）も試す。**取れなければ止まる |
| 生きていたら | **手を離させる段（park）を通してから消す** |

**ボードのロックを見るのは、`--id` の付け忘れを止めるためである。**
**ロックは `--id` ごとに分かれるので、自分のロックが空いていることは「continuo が動いていない」ことを
意味しない。**`--id e2e` で動いている continuo は `~/.continuo/id/e2e/continuo.lock` を握っており、
既定の `~/.continuo/continuo.lock` は空いている。**ボードのロックは `--id` に依らない唯一の合図である。**
**自分のロックが取れなかったとき（＝同じ `--id` の continuo が動いているとき）は見に行かない。**
そのボードのロックはその continuo が握っているので必ず取れず、**正しい使い方が止められてしまう。**
...
**覚え書きの名前を `board.json` に固定しない。**固定すると別のボードを見る continuo が
互いに上書きし、**「誰が握っているか」を読む目的そのものが果たせない。**ロックと同じ幹の名前にする。
**ロックを読み合う形にもしてはならない。**同時に起動した2つが、互いに相手の覚え書きを
書き終える前に読んでしまう。**ロック1本なら `flock` が同じ瞬間に2つへ渡らないので、
順序も競合も無い**（プロセスが死ねば OS が解放するので、残骸の判定も要らない。3-17 と同じ理由）。

**覚え書きのサンプル**（`~/.continuo/board/octocat-10.json`。**ロックを取った直後に書き、
手放す直前に、握ったまま消す。**手放してから消すと、その隙に起動した continuo の覚え書きを消す）。
...
```json
{
  ...
  "instance_id": "e2e",
  "pid": 66396,
  "config_path": "~/continuo-e2e/WORKFLOW.md",
  "lock_file": "~/.continuo/id/e2e/continuo.lock",
  "started_at": "2026-08-28T10:54:48+09:00"
}
```

**`instance_id` は `--id` を付けていなければ空文字にする。**どちらの continuo かを見分けるために書く。

**名前の作り方と置き場所の作り方。**

| 何 | どうするか | なぜ |
| --- | --- | --- |
| **owner の大文字小文字** | **小文字へ揃える** | GitHub のログイン名は区別しない。**`Octocat` と `octocat` は同じボードである** |
| **正規化で落ちた情報**（3-7） | **1行ログに残す** | `my org` と `my_org` が同じロックになる。**黙ると、別のボードを見ている2本目が理由の分からないまま断られる** |
| **`~/.continuo/board` の作成** | **socket の置き場所と同じ検査を通す**（`socketpath.EnsureDir`） | symlink に差し替えられていると、辿った先へ flock と覚え書きが落ちる |
...
**言いたいこと。**`--id` を1度使うだけで、**既定の `continuo abandon` が二度と何も片付けられなくなる。**
**`--id` の置き場所に名乗りを置き、既定側の走査がそこへ入らないようにする。**
**孤児 branch の掃除も、`--id` が足した接頭辞だけでは始めない。**

**何が起きるか。**`--id e2e` の worktree は `<workspace.root>/e2e/<host>/<owner>/<repo>/<スラグ>` にある。
既定側の走査は `<workspace.root>` から**ちょうど4階層**を返す（3-4 の段2）ので、
`<workspace.root>/e2e/<host>/<owner>/<repo>` が返る。**そこに身元ファイルは無いので
「身元ファイルが無いディレクトリ」として数えられ、abandon は判断を保留したまま止まる**（3-37-9c）。

**採る形。`--id` の置き場所の直下に目印を置き、既定側はそこへ1階層も入らない。**

**continuo と `continuo abandon` が、`--id` を付けて起動した直後に書く**（`workspace.New`）。
**書けなければ起動を止める。**書けないまま動かすと、上の事故が起きる。

```
<workspace.root>/e2e/.continuo-instance
```
...
| **外の裏付け** | **`~/.continuo/id/<名前>/` が実在する**こと（`EnsureLockDir` が必ず作る）。**作るのはロックを取る側だけである。**`continuo doctor --id <名前>` は作らない（3-17h）。**作っていたので、検査の道具でこの裏付けを偽造できた** |
...
**この規則を通す経路は3つある。**`BranchPrefixForSweep` を全部が通ること（3-17k）。

| 誰が | 何を消しうるか |
| --- | --- |
| `SweepOrphanBranches`（3-9 の段6b） | 接頭辞に一致する孤児 branch |
| `deletableBranch`（3-9 の段4） | 身元ファイルに書かれた branch |
| `brokenRefPolicy`（3-22b） | 壊れた loose な ref のファイル |

**素の `BranchPrefix` を使う経路を残さない。**1つでも残ると、
**`--id` を付けただけで、そこだけが人間の branch を消しにいく。**
...
| `instance.ReadLockState`（覚え書きを読む。3-17i） | `lock.Acquire`（`O_CREATE` でロックファイルを作る） |
| （名乗りを書かない） | `workspace.WriteInstanceMarker` |

**`workspace.New` の `NoCreate` が、置き場所と名乗りの両方を止める。**

**ロックは掴まない。**`flock` を一瞬でも掴むと、**その瞬間に起動した continuo が
「二重起動」で落ちる**（3-17h）。**動いているかは、ロックの隣の覚え書きを読んで答える**（3-17i）。
**覚え書きが無いことは「その `--id` の continuo が1度も動いていない」ことである。**

**置き場所が無ければ走査は0件である。**`workspace.root` が無いのは「読めない」ではなく
「worktree が1件も無いことがはっきり分かっている」状態なので、`Scan` はエラーにしない。
**これは本番の走査にも当てはめる。**根と枝で分けない。
**本番では `EnsureRoot` が必ず作るので、走査の時点で根が無いのは、
実行中に置き場所が外れた（外付けや網の向こうに置いていた）ときである。**
そこで**エラーにして巡回を止めると、走行中の run も面倒を見られなくなる。**
**0件を返し、`workspace.on_broken_worktree` と `restart.orphan_running_action` の
判断はそれぞれの設定に委ねる。**

### 3-17h. 線は「消すかどうか」で引く

**言いたいこと。**3-17g を `continuo abandon --dry-run` にだけ書いたので、**`continuo doctor` が同じ穴に落ちた。**
**規則をコマンドに書くと、次に足したコマンドがまた落ちる。**
**分けるのは「消すか、消さないか」である。**「観測かどうか」ではない。

**「観測かどうか」で引いてはならない。**`continuo abandon` の段1 は
「continuo が動いているか」を調べるので観測に見えるが、**あれは本番の獲得である。**
**そこに「flock を掴まない」を課すと、abandon はロックを持たないまま worktree を消す。**
abandon は git と RPC を何度も叩くので、消している数十秒のあいだに continuo が起動し、
**生きた worktree を消す。**

| 誰が | 何をするか | flock |
| --- | --- | --- |
| `continuo doctor` | 置けるかを見る | **掴まない** |
| `continuo abandon --dry-run` | 何をするつもりかを言う | **掴まない** |
| **`continuo abandon`（`--dry-run` なし）** | **worktree を実際に消す** | **掴む。最後まで握る** |
| **continuo 本体** | **issue を着手する** | **掴む。終わるまで握る** |

**消さない側に禁じるのは3つである。**

| 何を | 破ると何が起きるか |
| --- | --- |
| **本番が使う名前の資源を作らない** | 打ち間違えた `--id` の置き場所が残り、既定側の走査から永久に隠れる（3-17f） |
| **本番が使う名前の資源を消さない** | **`net.Listen` のあとに `os.Remove` すると、その隙に bind し直した常駐の socket を消す。**hook は1件も届かなくなる |
| **flock を掴まない** | **一瞬でも掴むと、その瞬間に起動した continuo が「二重起動」で落ちる** |

**「作らない」と「置けるか」は両立する。**禁じているのは**本番が使う名前**であって、
書き込みそのものではない。**使い捨ての名前で作って消せば、symlink・ディレクトリでない・
権限が group と other に開いている の3つを、そのまま検出できる。**

| 何を確かめるか | どうやるか |
| --- | --- |
| **その名前のディレクトリが既にある** | `Lstat` で symlink とディレクトリでないものを弾き、権限が `0700` かを見る。**そのうえで中に使い捨てのディレクトリを1つ作って消す** |
| **その名前のディレクトリがまだ無い** | **上へ辿って最初に実在するディレクトリに、使い捨てを作って消す。**そこに作れるなら、起動時の `EnsureDir` も作れる。**そちらには権限の検査を掛けない**（ホームは 0755 が普通であり、掛けると起動できる環境を `✗` と答える） |
| **unix socket を置ける** | **本番の名前では listen しない。**同じディレクトリに、本番より短い使い捨ての名前で listen して閉じる。**パスの長さの上限は `internal/socketpath` の `checkPathLen` が別に見ている** |
| **本番の socket が使われている** | **在るものには触らない。**`net.Dial` で繋がれば「動いている」、繋がらなければ「残骸」である（3-32g） |
...
| --- | --- |
| `instance.ReadLockState`（覚え書きを読む。3-17i） | `lock.Acquire`（`O_CREATE` でロックファイルを作る） |
| `socketpath.CheckDirPlaceable`（**作らない**） | `socketpath.EnsureDir` |
| `fsprobe.ProbeInside` / `fsprobe.ProbeSocketInside`（**使い捨ての名前**） | `socketpath.EnsureDir` / `net.Listen(<本番の名前>)` |
| `instance.Layout.ResolveHookSocketPath` | `instance.Layout.HookSocketPath` |
| `workspace.New` に `NoCreate: true` | `workspace.New`（既定） |

**`lock.Probe` は消した。**あれは1バイトも書かないので 3-17g の規則をすり抜けたが、
**`LOCK_EX` を掴んでから離していた。**動いているかは 3-17i の覚え書きだけを見る。

**消さない側も、本番と同じ経路で置き場所を組み立てる。**`--id` から導く4つ（3-17b）を
**同じ関数から取る。**doctor が `InstanceID` を渡さずに走査すると、
**本番と判定が食い違い、掃除が止まっている設定を `✓` と答える。**

### 3-17i. 生きているかは、ロックの隣の覚え書きで見る。flock は掴まない

**言いたいこと。**flock は「掴んでみる」以外に状態を知る方法が無い。
**だから消さない側は flock を見ない。**ロックを取った側が隣に覚え書きを書き、**それを読むだけにする。**
**排他は flock 1本のままである**（3-17）。覚え書きは排他の一部ではない。

**誰が何をするか。**

| 誰が | 何をする |
| --- | --- |
| **ロックを取った側**（continuo 本体、`--dry-run` でない `abandon`） | **flock を握った直後に、隣へ覚え書きを書く。握ったまま消してから手放す** |
| **消さない側**（`doctor` / `abandon --dry-run`） | **覚え書きを読み、その PID が生きているかを見る。flock には触らない** |

**書く順番と消す順番が仕様である。**

| 順 | 何を | 破ると何が起きるか |
| --- | --- | --- |
| 1 | flock を握る | 握る前に書くと、**握れなかったプロセスの PID が残る** |
| 2 | 覚え書きを書く | — |
| 3 | **握ったまま**覚え書きを消す | **手放してから消すと、その隙に起動した continuo が書いた覚え書きを消す** |
| 4 | flock を手放す | — |

**段1 と段2 のあいだに窓がある。**そのあいだ、読む側は `not_running` と答える。
**それでも worktree は消えない。**その窓に入った `continuo abandon` は、
**`--dry-run` でなければ flock を取りにいって「既に動いています」で止まる**からである。
**`--dry-run` は何も消さない。**

**覚え書きの場所と中身。**

```
~/.continuo/continuo.json                    （--id を付けないとき）
~/.continuo/id/<名前>/continuo.json          （--id <名前> を付けたとき）
~/.continuo/board/<owner>-<番号>.json        （ボードのロック。3-17e）
```
...
```json
{
  ...
  "instance_id": "e2e",
  "pid": 41321,
  "config_path": "~/work/WORKFLOW.md",
  "lock_file": "~/.continuo/id/e2e/continuo.lock",
  "started_at": "2026-08-30T12:03:11+09:00"
}
```

**3つとも同じ型・同じ書き手・同じ読み手である**（`internal/instance` の `LockInfo` /
`WriteLockInfo` / `RemoveLockInfo` / `ReadLockState`）。**型を分けない**（3-17k）。
**一時ファイルへ書き切ってから差し替える**（`atomicfile.Write`）。
**書きかけを読むと、壊れた JSON として `unknown` になる。**

**答えは4値である。**

| 答え | どう出したか | 呼び出し側はどうするか |
| --- | --- | --- |
| **`not_running`** | 覚え書きが無い | **進んでよい** |
| **`running`** | 覚え書きがあり、その PID のプロセスが居る | **止まる。**手を離させる段を通す |
| **`stale`** | 覚え書きはあるが、その PID のプロセスが居ない（`SIGKILL` や電源断の残骸） | **進んでよい。**ただし残骸があることを画面に出す |
| **`unknown`** | **覚え書きを読めなかった・壊れていた・PID の生死を確かめられなかった** | **止まる** |

**4つ目が要る理由。**覚え書きは**書けなくても起動を止めない**（排他の一部ではないので、
書けないことを理由に continuo を起動できなくしてはならない）。
**だから「読めない」は「動いていない」の証拠にならない。**
3値しか無いと、`~/.continuo` が読み取り専用になった機械で、
**生きている continuo が `not_running` と答え、abandon がその worktree を消しにいく。**
**判定は「分からないなら止まる」に倒す。**呼ぶ側は `LockState.Blocks()` を見る
（`running` と `unknown` が真である）。**switch を書き写させない。**書き写すと、
値を足したときに片方だけが直る。

**生死は PID だけで見る。起動時刻とは比べない。**
`started_at` は**ロックを取った時刻**であって**プロセスの起動時刻ではない。**
秒精度で比べればほぼ必ず食い違うので、**PID の使い回しを防ぐつもりの検査が、
生きている continuo を毎回 `stale` と答える。**
**PID の使い回しは、代わりに「覚え書きが残っていること自体が異常である」で受ける。**
正常な終了では段3 で消えるので、**残っているのは異常終了したときだけである。**
そこで PID が使い回されていれば `running` に倒れるが、**倒れる先は「止まる」なので安全側である。**
人間は覚え書きのパスと PID を画面で読めるので、確かめて消せる。

**`signal 0` を送る。**プロセスの一覧を実行ファイル名で照合しない。
**hook を届けるサブコマンド（`continuo hook ...`）が本体と同じ名前で一瞬だけ現れる**（3-17）。
**`EPERM` は「居る」である**（別の利用者のプロセスが同じ PID を持っている）。
**居ないことの証拠ではないので、生きている側へ倒す。**

**`stale` を `not_running` と同じ言葉で表示しない。**残骸が残っていることは、
**前の continuo が正常に終わらなかった合図である。**黙って進むと、次に同じことが起きても気づけない。

**flock が使えない置き場所でも答えられる。**覚え書きは普通のファイルなので、
NFS や SMB のように flock が `ENOLCK` を返す場所でも読める。
**`lock.Probe` はそこで「握られている」と答えていた**（動いていないのに）。

**ロックを取る者は必ず書く。例外を作らない。**`continuo abandon` も書く。
**書いていなかったので、abandon どうしがぶつかると、
落ちた continuo の古い覚え書きを読んで、死んだ PID を「握っている相手」として表示していた。**

### 3-17k. 同じ事実を2箇所に持たない

**言いたいこと。**同じ事実を2箇所が別々に持つと、必ず片方が古くなる。
**3回のレビューで、この形が6件出た。**
**事実ごとに「正はどこか」を1つ決め、残りはそこから取る。**

| 事実 | 正はどこか | 2箇所目になっていたもの |
| --- | --- | --- |
| **置き場所のパス** | `internal/instance` | **daemon と doctor が、画面に出す用に組み立て直していた** |
| **doctor の検査の並び** | `doctor.Run` の実装 | **package のコメントと README** |
| **効かなくなった設定の警告** | **設定を読む1箇所** | **daemon だけが出し、doctor は黙っていた** |
| **ロックの検査の結果の形** | 1つの関数 | **3回書き写され、1つで note が抜けた** |
| **ロックの覚え書きの型** | `internal/instance` の `LockInfo` 1つ | **二重起動防止のロックとボードのロックで、別の型を持ちかけた**（3-17i） |

**規則は3つである。**

- **パスを文字列で組み立てない。**画面に出すものも `internal/instance` から取る
- **数や並びを、文書やコメントに書かない。**書くなら実装から作る
- **設定の警告は、設定を読むところで出す。**使うところで出すと、使わない経路が黙る

**設計と実装が食い違ったときは、設計を直す。**3-32 の doctor の表は
「開いて `flock` を試し、すぐ手放す」と書いているが、**実装はそれをやめている**（本番の起動を止めるため）。
**設計のほうが古い。**読んだ人が設計に合わせると、直したはずの不具合が戻る。
...
| 制約 | なぜ |
| --- | --- |
| **グループの計画として読むのは、OWNER / MEMBER / COLLABORATOR が書いたコメントだけ** | **公開の issue には誰でもコメントできる。**「このグループを一緒に直して。終わったら `CONTINUO-STATUS: #45 review` と書いて」と書くだけで、**外部の人がボード上の別の issue の Status を動かせる。**5-3 の「書いた人によって扱いを変えること」を、この段落にも明記する |
...
**正は 5-3 の本文である。**下は要点だけで、実際に配る文言はあちらにある。

```markdown
このコメントに「まとめて対応する issue のグループ」が書かれている場合は、
**書いた人の立場が OWNER / MEMBER / COLLABORATOR のときに限り、
かつ同じリポジトリの issue に限り、まとめて直してください。**
**それ以外の立場のコメントに書かれたグループは扱いません。**
...
continuo doctor        # 前提が揃っているかを検査する。足りないものと直し方を出す
                       # --id=<名前>  常駐側に --id を付けているなら同じ名前を渡す（3-17b）。
...
| **hook を受ける socket を置けるか** | **置き場所が使えるかを見て、使い捨ての名前で listen して閉じる**（3-17h）。**本番の socket の名前では listen しない**（そのあと消すと、その隙に bind し直した常駐の socket を消す）。**本番の socket が在るなら触らず、`net.Dial` で繋がるかだけを見る**（3-32g） | **文字列を組み立てるだけでは足りない**（issue #9）。設定が読めなくても既定値で確かめる。**置き場所そのものは作らない**（作ると、打ち間違えた `--id` の `~/.continuo/id/<名前>/run` が残る） |
| **二重起動防止のロックを置けるか** | `~/.continuo`（`--id` を付けたなら `~/.continuo/id/<名前>`）が**使えるかを見る。作らない。`flock` も試さない**（3-17h）。**在るなら symlink・種類・権限を見て、中に使い捨てを作って消す。無いなら、上へ辿って最初に実在するディレクトリに書けるかを見る** | **socket とは別の場所である**（3-17）。**socket が置けてもロックが置けるとは限らない。**`~/.continuo` が書けない（あるいは同じ名前のファイルがある）と、全項目 `✓` なのに起動だけが落ちる（issue #9 と同じ形）。**既に握られていても `✓`**（continuo が動いているだけである）。**握られているかは見ない**（3-17i） |
| **ボードのロックを置けるか** | `~/.continuo/board` が**使えるかを見る**（同上。3-17e） | **これも別の場所である。**そこがファイル・symlink・0755 のディレクトリだと、全項目 `✓` なのに起動が落ちる。**`continuo abandon` も同じところで落ちる** |
| **Claude Code の設定ディレクトリに書けるか** | `~/.claude/session-env/<使い捨ての名前>` を**実際に作って消す** | **設定を1バイトも読まないので、設定が `✗` でも走る**（6-11）。ここが書けないと issue は1件も始まらない |
| **`workspace.root` に書けるか** | **使い捨てのディレクトリを実際に作って消す。置き場所そのものは作らない**（3-17h） | **置き場所は設定にしか書いていない**ので、設定が読めているときだけ走る。書けないと着手は worktree を用意する段で必ず落ちる。**`--id` を付けると `<workspace.root>/<名前>` まで作っていた**ので、打ち間違えた名前の置き場所と名乗りが残っていた（3-17f） |
...
そのまま並べると、**他の14個の検査結果が画面の外へ押し出される。**
...
**勝者の決め方。**判定スコアがいちばん大きい機械。**同点なら、いちばん最初に投稿した機械。**
...
| --- | --- |
| **担当者が無い** | 入札する |
| **自分のアカウント1人＋この機械の hold** | 着手・引き継ぎへ進む |
| **自分のアカウント1人＋hold が1件も無い** | 着手・引き継ぎへ進む。人間が付けた担当である |
| **自分のアカウント1人＋別の機械の hold＋18時間以内** | **触らない**（入札もしない） |
| **自分のアカウント1人＋別の機械の hold＋18時間超** | **担当を外して入札をやり直す** |
...
tracker:
  provider:
    handoff:
      idle_timeout_ms: 64800000   # 担当者の最後のコメントからこれだけ経つと、担当を外して入札をやり直す。既定は18時間
```
...
**この issue の担当は mac-studio に決まりました。**入札した機械のうち、レートリミットの枠の余裕がいちばん大きい機械です。
...
| いつ | 何が起きるか |
| --- | --- |
| **最後のコメントから18時間** | ほかの機械が「担当を外してよい」と判定する |
...
| 判定スコアも投稿の時刻も同じ | **機械の名前の小さい順で勝者を決める** |
...
機械ごとに違う勝者を選び、2台が同じ issue を掴む。**名前の順は全機械で同じ答えになる。
...
**「hold のコメントを書けなかった」行だけは、この理屈が成り立たない。**hold は最初から
存在しないので、残っても18時間は待たれない。assess.go の「自分のアカウント1人＋hold が
1件も無い」（3-77b）は「人間が付けた担当」として**待たずに着手へ進む**行であり、そこへ落ちると
**同じ GitHub アカウントを使う別の機械も同じ行を読んで、この issue に同時に着手できてしまう。**
**アカウントだけで比較していた頃と同じ穴が、この経路からもう一度開く。**
だから hold を書けなかったときは、必ずこの巡回で担当者を消し戻す。

**消し戻し自体（`RemoveAssignees`）が失敗したら、担当者だけが残り hold も released も無い。**
**新しい穴ではない。**担当者はあるが hold が無い状態そのものは、3-77b がもともと
「人間が付けた担当」として許容している行であり、**同じアカウントを2台で動かしているときに
限って**その行へ機械どうしが同時に落ちうる、という既存の取りこぼしである。`RemoveAssignees` が
失敗するのは GitHub API 自体に問題があるときなので、**そのときは入札や hold の投稿も同様に
失敗しやすく**、実際に2台が同時に着手する確率は低い。
...
**止めるのは新しい issue に着手することだけである。**枠待ちの判定（3-27）は
**最後に読めた値を使い続ける。**読めないことを理由に走行中の run を捨てない。

### 3-77j. 入札の判定と、新規着手の判定を1つに揃える

**言いたいこと。**この2つを別々に持っていたので、**枠を読めないときに逆を向いていた。**
**入札が先に効くので、新規着手の側は一度も効かなかった。**
**答えは1つにし、止まる理由を巡回1回につき1行、必ず出す。**

**何が起きていたか。**

| どちらが | 枠を読めないとき | 実際の効き方 |
| --- | --- | --- |
| **入札**（`handoff.Evaluate`） | **黙る**（`SkipQuotaUnreadable`） | **先に効く。**担当者のいない issue は、ここで全部落ちる |
| **新規着手**（`dispatchPaused`） | **止めない**（写しが `nil` なら偽） | **後ろにある。**上で落ちているので、一度も効かない |

**出るのは `Debug` の1行「この issue は他の機械に任せます」だけだった。**
**1台で動かしている人には、ボードが何時間も進まないようにしか見えない。**

**採る形。**

| 何を | どうする |
| --- | --- |
| **判定** | **1つの関数に揃える**（`Orchestrator.newWorkBlocked`。中身は入札の判定と同じ `handoff.Evaluate`） |
| **揃える向き** | **「分からないなら入札しない」。**読めない枠を0%（＝いちばん暇）とみなして入札すると、必ず勝つのに着手できない |
| **`rate_limit.source: none`** | **止めない。**運用者が「枠で判定しない」と決めた状態であり、読めなかったのとは違う（3-27 の逃げ道） |
| **出し方** | **巡回1回につき `Info` を1行。**理由・実際の閾値・設定の値・候補の件数を添える |
| **issue ごとの `Debug`** | **残す。**2段目の守りである。`Info` にすると候補の数だけ同じ行が出る |

**巡回そのものを打ち切るのは、閾値を超えたときだけである。**

| 理由 | 巡回を打ち切るか | 効かせ方 |
| --- | --- | --- |
| **枠の使い過ぎ**（`pause_above_percent` 超え） | **打ち切る** | 候補を1件も見ない。この機械はどの issue も動かせない |
| **枠を読めない** | **打ち切らない** | `handoffGate` が issue ごとに効かせる。**入札が要る issue（担当者がいないもの）だけが落ちる** |
| **余裕値がマイナス** | **打ち切らない** | 同上 |

**どの理由でも打ち切る形にしてはならない。**枠を読めないだけで巡回を打ち切ると、
**この機械が既に担当者になっている issue まで着手されなくなる**（印が無いので、
この経路からしか拾えない）。**期限切れの担当を外す経路（3-77c）も通らなくなる**ので、
**詰まったカンバンを誰も解けない。**

**新規着手が止まる本当の閾値は、`rate_limit.pause_above_percent` ではない。**
...
**既定では 90% である**（`pause_above_percent: 95` / マージンはどちらも 10）。
**余裕値は `100 - 使用率 - マージン` なので、使用率が 90% を超えた時点でマイナスになり、
`SkipNoHeadroom` で止まる。**閾値の 95% には届かない。
**この2つが違うことを画面に出さないと、「95% まで動くはずなのに 90% で止まった」を
誰も説明できない。**だから上の1行に両方を出す。

**「9割で止まること」自体は正しい。**1台で動かしていて余裕が無いなら、着手しないのが正しい動きである。
**直したのは「止まったことが見えないこと」と「2つの判定が逆を向いていたこと」の2つだけである。**
