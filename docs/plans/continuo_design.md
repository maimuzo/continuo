...
**書かないと、次の周が数えられません**（5-6 の「何周回すか」）。
    ...
    | --- | --- | --- | --- | --- | --- | --- |
    | 片付けの順序 | high | worktree を消す前に branch を消している | 直す | — | `git worktree remove` で2件 | 前の周に既に在った |
    | 変数名の揺れ | low | repoDir と repoPath が混在 | 直さない | この issue の範囲外 | `repoDir` で5件 | 直しが持ち込んだ |
