...
**取り戻しで記録が無かったときは、`復元の断念` と同じところへ抜ける。**issue にコメントは残らず、
Status は表明の値の遷移先のままで、片付けは続く。**`failure_state` へは落とさない。**
**書かせるものが最初から無いので、herdr の待ちを使い切ってから失敗する意味が無い。**

**RUCM の経路は、着手の側では増えない。**起動フラグを決める段（B20）は条件ステップではないので、
記録の有無で枝が分かれることはない。**取り戻しの側は、`復元の断念` の条件が1つ広がる**
（身元ファイルを読めない・UUID か設定ファイルのパスが空・worktree の実体が無い、に「記録が無い」が加わる）。**`RUCM-CFG-SHA256` を手で書き換えてはならない**
（[CONTRIBUTING.md:108-121](../../CONTRIBUTING.md#L108-L121) の「触らないもの」）。

**基本フローの段20 の文は、2つの枝しか書いていない**（UUID があれば復帰、無ければ採番）。
**実装は3つである**（記録があれば復帰、UUID はあるが記録が無ければ採番、UUID が無ければ採番）。
**経路は増えないが、文は実装より狭い。**直すかどうかは、仕様を再生成する側が決める。
