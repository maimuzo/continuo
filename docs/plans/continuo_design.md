...
同じ印を別の行の場所が見ていれば、それぞれ数える（`code-review-result` は CI・hook・release の検査の3行に出るので3）。
...
**やること。**成果を書き終えたエージェントが、この issue に自分が書いたコメントを読み直し、
**先頭に並ぶ印の中に `<!-- continuo:ai -->` が無いものを、自分で直す。**
**pull request と、グループでまとめて直した別の issue にも同じ点検をさせる。**
手順と、そのまま叩けるコマンドは組み込みの 5-6 にある。
