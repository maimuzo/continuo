...
- 3-17h 「観測するだけ」の経路は、コマンドではなく道具で縛る
- 3-17i 生きているかは覚え書きと PID で見る。flock は掴まない
- 3-17j 「無い」を、根と枝で分ける
- 3-17k 同じ事実を2箇所に持たない
...
**この型の外で識別子を加工してはならない。**`Normalize` を通したあとに `strings.ToLower` や
`strings.ReplaceAll` を足すと、**その加工の情報落ちには警告が付かない。**

**実例。**ボードのロックのファイル名を作るところが、`Normalize` の後で小文字へ揃えてスラッシュを潰していた。
**`owner: Octocat` と `owner: octocat` が同じロックへ落ちることを、誰にも伝えないまま行っていた。**
**加工したくなったら、その加工を `Normalize` の中へ入れる。**警告を返す責任も一緒に移る。
...
| `observe.LockState`（覚え書きを読む。3-17i） | `lock.Acquire`（`O_CREATE` でロックファイルを作る） |
...
**観測する側が使う道具を1組に決め、その組でしか観測できないようにする。**

**観測する側は3つある。**

| 誰が | 何を答えるか |
| --- | --- |
| `continuo doctor` | **置けるか。**押さえるのではない |
| `continuo abandon --dry-run` | **何をするつもりか。**実行はしない |
| `continuo abandon` の段1 | **continuo が動いているか。**自分では握らない |

**観測する側に禁じるのは3つである。**

| 何を | 破ると何が起きるか |
| --- | --- |
| **作らない** | 打ち間違えた `--id` の置き場所が残り、既定側の走査から永久に隠れる（3-17f） |
| **書かない** | 「何も消していません」と言いながら書くことになる |
| **掴まない** | **flock を一瞬でも掴むと、その瞬間に起動した continuo が「二重起動」で落ちる** |

**「掴まない」が抜けていた。**3-17g の表は `lock.Probe` を「決める」側に置いたが、
**`Probe` 自身が `LOCK_EX` を掴んでから離していた。**1バイトも書いていないので、規則をすり抜けた。

**採る形。観測用の道具を1つの package（`internal/observe`）にまとめる。**

| 観測用（この package の外へ出さない） | 本番用 |
| --- | --- |
| `observe.LockState`（3-17i） | `lock.Acquire` |
| `observe.RootExists` | `workspace.EnsureRoot` |
| `observe.DirWritable`（**親を見る。作って消さない**） | `socketpath.EnsureDir` |

**`lock.Probe` は消す。**観測は 3-17i の覚え書きだけを見る。

**観測する側は、本番と同じ経路で置き場所を組み立てる。**`--id` から導く4つ（3-17b）を
**同じ関数から取る。**doctor が `InstanceID` を渡さずに走査すると、
**本番と判定が食い違い、掃除が止まっている設定を `✓` と答える。**

**差し替え口も1つにする。**観測用の道具は `internal/observe` の1つの構造体でまとめて差し替える。
**片方だけ差し替えたテストが、本物の `~/.continuo` を触る形にしない。**

### 3-17i. 生きているかは覚え書きと PID で見る。flock は掴まない

**言いたいこと。**flock は「掴んでみる」以外に状態を知る方法が無い。
**だから観測は flock を見ない。**ロックを取った側が隣に覚え書きを書き、**観測はそれを読むだけにする。**
**排他は flock 1本のままである**（3-17）。覚え書きは判定に使わない。

**誰が何をするか。**

| 誰が | 何をする |
| --- | --- |
| **ロックを取った側**（continuo、`--dry-run` でない `abandon`） | flock を握った直後に、**隣へ覚え書きを書く。**手を離すときに消す |
| **観測する側** | **覚え書きを読み、その PID が生きているかを見る。**flock には触らない |
...
**観測の答えは3値である。**2値だと「分からない」をどちらかへ丸めることになる。

| 答え | どう出したか | 呼び出し側はどうするか |
| --- | --- | --- |
| **`not_running`** | 覚え書きが無い | 進んでよい |
| **`running`** | 覚え書きがあり、その PID が生きていて、起動時刻も一致する | **止まる。**手を離させる段を通す |
| **`stale`** | **覚え書きはあるが PID が死んでいる**（`SIGKILL` や電源断の残骸） | **進んでよい。**ただし残骸があることを画面に出す |

**`stale` を `not_running` と同じ言葉で表示しない。**残骸が残っていることは、
**前の continuo が正常に終わらなかった合図である。**黙って進むと、次に同じことが起きても気づけない。

**起動時刻も比べる。**PID は使い回される。**その PID の起動時刻と覚え書きの `started_at` が
食い違えば `stale` とする。**PID だけで一致と判定すると、無関係のプロセスを continuo と見なす。

**flock が使えない置き場所でも答えられる。**覚え書きは普通のファイルなので、
NFS や SMB のように flock が `ENOLCK` を返す場所でも読める。
**`lock.Probe` はそこで「握られている」と答えていた**（動いていないのに）。

**ボードの覚え書きも同じ形にする。**`~/.continuo/board/<owner>-<番号>.json` は既にあるが（3-17e）、
**`continuo abandon` が書いていなかった。**だから abandon どうしがぶつかると、
**落ちた continuo の古い覚え書きを読んで、死んだ PID を「握っている相手」として表示していた。**
**ロックを取る者は必ず覚え書きを書き、離すときに消す。**例外を作らない。

### 3-17j. 「無い」を、根と枝で分ける

**言いたいこと。**「置き場所が無い＝worktree が0件」という握り潰しを、走査の全階層に掛けた。
**`workspace.root` が実行中に消えると0件と答え、復元が生きている worktree を作り直す。**
**根が無いのと、枝が無いのは、別の意味である。**

| どこ | 無いとき | なぜ |
| --- | --- | --- |
| **根**（`workspace.root` そのもの） | **観測なら0件。本番ならエラー** | 本番では `EnsureRoot` が必ず作るので、**無いのは異常である** |
| **枝**（`<host>` / `<owner>` / `<repo>`） | **常に0件** | その host のものがまだ1つも無い、は正常である |

**観測かどうかは、既にある `workspace.Options.NoCreate` で分かる。**新しい旗を足さない。

**根が消えると何が起きるか。**`workspace.root` を外付けや網の向こうに置いていると、
外れた瞬間に走査が0件を返す。**`workspace.on_broken_worktree: stop` は止まらなくなり、
`restart.orphan_running_action: redispatch` は active_states の issue を全部
新しい worktree へ配り直す。**本物の worktree は外れた置き場所の下に隠れたままである。
...
| **二重起動防止のロックを置けるか** | `~/.continuo`（`--id` を付けたなら `~/.continuo/id/<名前>`）の**親が書けるかを見る。作らない。`flock` も試さない**（3-17h） | **socket とは別の場所である**（3-17）。**socket が置けてもロックが置けるとは限らない。**`~/.continuo` が書けない（あるいは同じ名前のファイルがある）と、全項目 `✓` なのに起動だけが落ちる（issue #9 と同じ形）。**既に握られていても `✓`**（continuo が動いているだけである）。**握られているかは見ない**（3-17i） |
