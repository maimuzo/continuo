...
CLAUDE.md は個人の絶対パスを「絶対にコミットしないもの」に挙げているが、
**コミットしないだけでは足りない。**continuo が実行時に書くものにも同じ配慮が要る。
...
```
前:  /home/alice/.claude/projects/-home-alice-wt-issue-1/1f2e….jsonl
後:  ~/.claude/projects/-home-alice-wt-issue-1/1f2e….jsonl
```

**前後を見ずに置き換えてはならない。**`/home/alice` を縮める場面で、`/home/alice2/x` を
`~2/x` に、`/mnt/home/alice` を `/mnt~` にしてしまう。**home の直前が名前の続きでなく、
直後が `/` か名前の終わりのときだけ縮める。**

**home の外にあるパスはそのまま出す。**縮めようが無いうえ、伏せると引き渡しの通知の
【調べるところ】が「どこを見ればよいか分からない」ものになる。
**利用者名が入るのは home の下であり、そこは縮まる。**

**`~` は「continuo を動かしている機械の home」を指す。**issue を読む人間の home とは限らない
（別の機械で動かしている場合）。**それでも `~` にする。**読む人が別の機械にいるなら、
絶対パスを出しても同じくその人の手元には無い。**縮めても失われる案内は無く、
公開される情報だけが減る。**
