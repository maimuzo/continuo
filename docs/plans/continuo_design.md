...
**先頭に並ぶ印の中に `<!-- continuo:ai -->` が無ければ、
`gh api --method PATCH "repos/{{.issue.owner}}/{{.issue.repo}}/issues/comments/<コメントの id>"` で足してください。**
**直すのは、この run で自分が投稿したものだけです。**前の run のものは直しません。
