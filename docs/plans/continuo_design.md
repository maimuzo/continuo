...
**まず、既にある PR を使えないかを確かめてください。**
上の「この issue に紐づく PR も読むこと」で出した一覧に `"state": "OPEN"` のものがあれば、
**それが行き先です。新しく作らないでください。**その branch へ push すれば中身は入れ替わります。
**OWNER / MEMBER / COLLABORATOR が「2本目を出せ」と書いているときだけ、もう1本作ります。**
その push 先の分け方は、下の「終わったらやること」にあります。

**1本も無いときだけ、新しく作ります。**この worktree の中で、commit して push したあとに叩いてください。
**push のしかたは、下の「終わったらやること」にあります。**
...
**`Closes #{{.issue.number}}` を落とさないでください。**この1行が PR と issue を結びつけます。
...
**次の3つは、この指示書では決めていません。**WORKFLOW.md の本文に書いてあれば、そちらに従ってください。
    ...
    draft で作るかどうか
    base にする branch（--base を書かなければ、このリポジトリの既定の branch です）
    成果がこの worktree の外（別のリポジトリの clone）にあるときの出し方

**作れなかったときは、その理由を書いて `CONTINUO-STATUS: blocked` を出してください。**
...
| **PR の決まり** | draft にするか・base にする branch・付けるラベル | **組み込みの指示のとおりに作られる**（5-3i） |
| **レビューの手順** | 計画と PR を subagent にレビューさせる段取り | レビューさせない |

**節の並びは、作業の順番に合わせる。**上の表の順がそのまま雛形の順である。
...
| 何を書くか | なぜ |
| --- | --- |
| **既に `OPEN` の PR があれば作らない** | 2回目以降の turn と、`attempt` が2以上の起動で2本目ができる |
...
| **本文は全部消せる** | 「全部消しても continuo は動きます」と雛形が言っている。**本文に置くと、消した人の流れだけが途中で切れる** |
| **本文は前半と後半のあいだに挟まる**（5-3c） | 後半に置けば、本文に何を書いても最後に読まれる |
...
| **設定のキーで作る／作らないを切り替える** | **採らない** | **切り替える先が「人間が手で作る」しかない。**選べるようにする意味が無い |
