...
| 遷移 | 誰が | いつ | 手段 |
| --- | --- | --- | --- |
| **（ボードに載る）** → `Ice Box` | **人間か AI** | **issue を作ったあと、ボードへ載せて `Ice Box` を付けるとき。**continuo はボードに載っていない issue を見ない。**グループにまとめたときは、代表にだけ付ける** | GitHub の画面／GraphQL |
| `Ice Box` → `Ready` | 人間 | 着手を決めたとき | GitHub の画面 |
| `Ready` / `In Progress` → **Status 無し** | **人間か AI** | **グループにまとめた issue のうち、代表以外を退避させるとき**（3-26）。**外さないと continuo が別々に dispatch する。**`Ice Box` へ落としてもよいが、**代表と見分けが付かなくなるので Status を外す** | GitHub の画面／GraphQL |
