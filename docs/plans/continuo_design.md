...
段2b なら `gh issue comment` が1回。**書き足すときは `gh api` の GET を2回叩く**（読んで表示する塊と、書き換える塊で1回ずつ。**塊ごとに別のシェルで走るので、変数を持ち越せない**）。
**対象1件につき最大4回である。**
...
| 何が起きうるか | なぜ起きないか |
| --- | --- |
| **continuo が Status を書けなかったときに、記録も書かれない** | `postStatusMove` の `if !move.Wrote` を1度も通らない。**代表が `In Review` と `Blocked` を往復しても、表明のたびに段1〜段3 が走る** |
...
| **番号を書き間違えると、無関係な issue に嘘が残る** | continuo 側には「ボードに載っていなければ捨てる」門があるが、**エージェントの `gh issue comment` には門が無い。**「まとめて直しました」が公開の形で残りうる。**issue のコメントは編集履歴が残るので取り消せない。承知のうえで門を置かない**（門を置くにはエージェントにボードを引かせることになり、API の呼び出しがさらに増える） |
