...
| **hold** | **本文の `assignee` と、コメントの投稿者の両方** | `LatestHoldFor` が issue の担当者と突き合わせるので `assignee` は要る。**そのうえで、投稿者が `assignee` と一致するものだけを数える**（下） |
...
**hold は、投稿者と `assignee` が食い違うものを数えない。**
**hold は「その担当者は機械である」の唯一の証拠であり、それがあると continuo は担当者を外してよいと判断する。**
**本文だけを信じると、リポジトリにコメントできる誰かが `{"assignee": "<人間のログイン名>", …}` と書くだけで、
その人間の担当を外させられる。**continuo が書く hold は、担当を取った当人が自分のログイン名を書くので、
**正しい hold はこの検査を必ず通る。**入札の回を閉じる判定（3-77e）でも同じ検査を掛ける。
...
**それでも入れない。**経路がもう1つあり、そちらは痕跡を1バイトも残さないためである。
**片方が hold を書き終えたあとに、もう片方が初めてその issue を見る経路では、
「自分のアカウント1人」で着手へ進み、コメントを1件も書かない。**
**警告が出ないことが「大丈夫」の証拠にならない検知は、無いほうが誤解が少ない。**
