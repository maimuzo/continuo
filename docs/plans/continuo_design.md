...
| **PR の決まり** | draft にするか・base にする branch・成果がこの worktree の外にあるときの出し方。**「PR を作らない」は書けない** | **組み込みの指示のとおりに作られる**（5-3i） |
...
| 何を書くか | なぜ |
| --- | --- |
| **打ち消しの口は開けない** | **2026-09-03 の人間の判断である。**「まったく根拠がないだけでなく、害がある。変更するな」。**調査だけ・レビューだけの issue は、continuo に渡さない側で分ける。**この決定は [test/internal/prompt/pull_request_test.go:62-66](../../test/internal/prompt/pull_request_test.go#L62-L66) が固定している |
