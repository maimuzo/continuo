...
- いま担当している issue と関係のない外部への書き込み: 他のリポジトリへの push、パッケージの公開、外部サービスへの投稿
  ...
  この肯定に含めず、断る。
  リポジトリ <owner>/<repo> の GitHub 上のもの（issue、pull request、ref、release、設定）と、
...
| **肯定した形から、別に外すもの** | **pull request を通さずにファイルを書き込む形**（`gh api` の contents） | pull request を通さずに、直 push と同じ結果へ別の道具で届く |
