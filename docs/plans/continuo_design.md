...
| **計画のレビュー** | 実装の前に subagent へ計画をレビューさせる段取り | general-purpose へ頼む |
| **PR の決まり** | draft にするか・base にする branch・付けるラベルのような project 固有の決まり。**「PR を作らない」は書けない** | **組み込みの指示のとおりに作られる**（5-3i） |
| **PR のレビュー** | PR を出したあとに subagent へレビューさせる段取り | general-purpose へ頼む |
...
[internal/orchestrator/comment.go:303](../../internal/orchestrator/comment.go#L303) の `hasRunComment` は、
...
**既定のレビュワーは general-purpose である**（5-3d）。
