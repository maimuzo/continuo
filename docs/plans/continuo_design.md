...
**この守りは「その担当者名の hold が1件も無い」ときにだけ働く。**
**hold のコメントは、担当が移っても入札の回が変わっても消えない**（`LatestHoldFor` は
[internal/handoff/assess.go:400-423](../../internal/handoff/assess.go#L400-L423) で
hold の `assignee` だけを照合し、人間が付けたか機械が付けたかを区別しない）。
**したがって、過去にその issue を入札で取ったことのある人を人間が担当者に付け直すと、
上の「他人1人＋その担当者の hold あり」の行に落ち、担当を外される。**
**18時間を数える起点は、担当者を付け直した時刻ではない。**`lastProgressOf` が
`latest := holdAt`（残っている古い hold のコメントの時刻）から始めるためである
（[internal/handoff/assess.go:338](../../internal/handoff/assess.go#L338)）。
**`Situation` は担当者を付けた時刻を運ぶ欄を持たないので、付け直した時刻は原理的に使えない。**
**その時刻はたいてい既に18時間より前なので、次の巡回で外れる。**
**この経路は塞がない。**塞ぐには「人間が付けた担当」と「機械が取った担当」を
区別する印が要るが、それは担当者の欄からは読めない。
**利用者向けの案内は [docs/FAQ.md](../FAQ.md) の「チームで、issue ごとに担当の PC を決めたい」にある。**
