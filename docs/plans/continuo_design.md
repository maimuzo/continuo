...
- いま担当している issue と関係のない外部への書き込み: 他のリポジトリへの push、パッケージの公開、外部サービスへの投稿
  ...
  pull request の作成、その本文とコメントの書き込み（issue と pull request のコメントを
  `gh api` で書く形も含む。ただし、pull request の取り込みと却下と承認と、
  issue と pull request を閉じることは、コメントの書き込みに数えない）、
  そしてリポジトリ <owner>/<repo> の、既定の branch 以外の branch への push は、
  担当している作業そのものなので「関係のない」に当たらない。
  リポジトリ <owner>/<repo> への書き込みのうち、いま挙げた形に当たらないものは、
  リポジトリ <owner>/<repo> が担当先であることを理由に「関係がある」として通してはならない。
  branch や tag を消す push と、
  pull request を通さずにファイルを書き込む形は、いま挙げた形に含めない。
...
| **担当先を告げる文で、免除をどこで閉じるか** | **肯定した形そのもので閉じる。**「リポジトリ X への書き込みのうち、いま挙げた形に当たらないものは、リポジトリ X が担当先であることを理由に『関係がある』として通してはならない」と書く | **「担当している作業そのもの」という宣言は、リポジトリ X に掛かっていて、操作の種類を1つも区別しない。**閉じる文が無いと、**リポジトリ X が相手なら merge も release も「関係がある→通す」へ倒れる**（すぐ上の「担当先の文では『断る』と言い切る」と同じ理由である）。**種類で閉じる形は採らない**（「コードや配布物を変える操作」など）。**承認・却下・close は、その瞬間にコードも配布物も設定も1バイトも変えないので、どの種類にも当たらない。****綴りで数え上げてもならない。**列挙に無いものが「断られる側へ落ちなかった」と読まれる。**肯定した形を基準にすれば、並べていない操作も全部同じ側に入る。**ラベルと担当者の付け外しも断られる側へ落ちるが、**組み込みの指示書はこれを1度も叩かせない**（`gh label` で検索して0件） |
| **push の肯定を、どこまで広げるか** | **既定の branch 以外の branch へ限る。**既定の branch の名前は実物から取る | **「リポジトリ X への push」と書くと、送り先も ref の種類も限定しないので、`git push origin HEAD:<既定の branch>` も `git push origin v1.2.3` も通る。****タグを作る push は、`v*` で release を公開する workflow を持つリポジトリでは `gh release create` と同じ結果へ届く。****`HEAD:<別の branch 名>` は肯定の側に残す**（組み込みの指示書が2本目の pull request で叩かせる形である）。**`main / master` と綴りを数え上げると、`develop` を既定にしているリポジトリで素通りする** |
| **肯定した形から、別に外すもの** | **branch や tag を消す push と、pull request を通さずにファイルを書き込む形**（`gh api` の contents） | 前者は「への push」に当たると読め、**断る条件の1つ目が挙げているのは force push だけで、ref を消す push は入っていない。**continuo は既定で2本の run を同時に走らせるので、**並行して走る別の run の branch を remote から消せる。**後者は pull request を通さずに同じ結果へ別の道具で届く |
| **肯定の一覧から何を外すか** | **pull request の取り込みと却下と承認、issue と pull request を閉じること。**この4つを「コメントの書き込みに数えない」と1句で外す | **`gh pr review --approve` は、道具の形としては pull request へのコメントの投稿である。**`gh api --method POST .../pulls/<番号>/reviews -f event=APPROVE` は、括弧の中の綴りと一字一句で重なる。**外さないと肯定の一覧に入ってしまい、閉じる文が届かない。****必須のレビューや自動マージを設定した担当先で、人間が1度も見ていない pull request がそのまま既定の branch へ入る** |
