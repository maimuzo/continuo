...
| pane | **閉じない。無ければ1つ用意する**（**用意する条件の正は 3-82c の門1 である**） |
...
| 節 | 何の正か |
| --- | --- |
| **3-82**（この節） | 何をする機能か。既定値。起動を止めない理由。他の案を採らない理由。権限モード |
| **3-82b** | 巡回のどこで、どの順に走らせるか |
...
### 3-82b. direct chat の候補は、専用の1パスへ分ける
...
candidates = FetchIssuesByStates(active_states ＋（実在すれば）direct_chat_state)
  ...
  Status が direct_chat_state のもの  → prepareDirectChatPanes（3-82c）
...
**残る2つの理由**（`gh` の認証の検査が落ちた／候補の取得そのものが失敗した）**では、
渡す候補が1件も無いので、例外を作っても何も救わない。**
**救えるのは最大30秒の遅れで、しかも20巡回に1回である。**
...
**そのぶんは `redispatch` から着手の段2 へ進み、そこで印が外れる**（3-82c の門1 の表）。
...
ところが 3-82c の門1 の表は、**まさにその着手の段2 で印が外れることを、
...
### 3-82c. pane を用意するかどうかを決める門

**言いたいこと。****「いつ pane を用意するか」を決めるのは、この節だけである。**
一覧表（3-82）・4-1 の遷移表・[docs/FAQ.md](../FAQ.md)・`WORKFLOW.md` の雛形・
...
**「担当」と書いてはならない。**[docs/FAQ.md](../FAQ.md) では「担当」が GitHub の担当者を指しており、
**この機能は担当者を1バイトも見ない**（3-82d の「通さない門」）。
...
| --- | --- | --- |
| `Ice Box` | **持っていない**（候補に入らない Status なので、着手されたことが無い） | 門1〜門5 を通り、**worktree ごと用意する** |
| `Ready` | **たいてい持っていない** | 同上。**着手の隙間に動かすと持っていることがある**（そのときは下の門1 のとおり） |
| `In Progress` | **持っている** | **何もしない。**走っている pane をそのまま渡す |
...
**どの入口でも、判定の材料は下の3つ（Status・印・pane）だけである。**入口ごとの分岐は持たない。
...
| **3** | **この worktree に pane が1枚でもある** | Debug 1行。**「Claude Code が居るか」は判定しない**（上） |
| **4** | **空きスロットが無い**（`agent.max_concurrent_agents`） | **WARN で名指しする**（下の「WARN は1回だけ」） |
| **5** | **着手の直前の検査（`preflight`）に落ちた** | `preflight` が自分で出す。**信頼登録の判定をこの門より前に置いてはならない。**`Dispatchable` で先に落とすと `preflight` へ届かず、**未信頼のリポジトリで direct chat を頼んだ人へ、直し方のコメントが1件も出ない**（3-33） |
...
| 2 | `resumeBackoff` が `redispatch` を呼ぶ | **入口に direct chat の検査を置かないので、そのまま着手の段2 へ進む**（3-82b） |
...
**門4（空きスロットが無い）で毎巡回 WARN を出すと、枠が詰まっているあいだ他の WARN が全部これに埋もれる。**
direct chat の run は自分では終わらないので、上限を上げても次に同じことが起きる。
**効く直し方は「どれかの issue を direct chat から戻す」である。**

**専用の台帳を作ってはならない。**「1回だけ言う」も「いつ消すか」も、
**既存の関門の記録（`gated`）が既に解いてある。**

| 何が要るか | 既にある実装 |
| --- | --- |
| 理由ごとに1回だけ言う | `notices map[GateReason]gateNotice` |
| 門を通ったら記録を消す | `clearGate` |
| カンバンから消えた issue の記録を落とす | `forgetGatedNotOnBoard` |
| 候補の取得に失敗した巡回では消さない | 同上 |

**やることは1つである。****`GateReason` を1つ足し、門4 で `noteGate` を呼ぶ。**
記録するのは門4 だけ、消すのは門1 と門3 を通ったときである
（3-82d の「飛ばすたびに `clearGate` を呼ぶ」と衝突しない）。

**専用の台帳を作ると、消す規則を1から解き直すことになる。**
実際に、5周にわたって「いつ消すか」だけで指摘が出ている。

### 3-82d. pane と worktree を用意する — 用意の段1〜段3
...
| 用意の段 | 何をするか |
| --- | --- |
| **1** | **印を付ける**（着手の段1）。**`claimForDispatch` ではなく `o.claim` を直に呼ぶ。**あれは印を付けたあとに写しの Status を `running_state` へ書き換えるので、**引数を外すとその1行が無条件になる。****写しの Status は書き換えない**（下） |
| **2** | **着手の段3〜段10 を踏む**（**着手の段2 と段11 は踏まない**）。落ちたら、自分が開いた pane を pane ID で閉じ、印を外す。**カンバンへは1バイトも書かない** |
| **3** | **カードがまだ `direct_chat_state` かを確かめてから**、direct chat へ入れ、`SendFirstPrompt` を下ろし、issue へ1件書く（下） |
...
**外れていたら、`SendFirstPrompt` を下ろさず、コメントも書かず、direct chat へも入れない。**
**印は残す。**次の巡回が、通常の着手として1回目の本文を送る。
...
| direct chat から戻ったときの書き込み | 3-82g |
...
**免除は、通常の着手から入った run にも効く。**「巡回が気づいたとき」がその経路そのものだからである。
**そちらは continuo が自分を担当者に書いてあるので、外される担当が実在する。**
**それでも免除する。**この設計は「入札も担当の持ち回りも当てない」と決めており、
**下ろすと、この issue が求めた「同じ pane・同じ会話のまま続ける」が1時間で終わる。**
戻したあと、人間が自分を担当者に付けたままなら、
**`recheck_interval_ms`（既定1時間）の次の turn の終わりに `stopWorker` が呼ばれ、pane が閉じる。**
**push もせず、カンバンにも issue にも1バイトも残らない。**
**代償は 3-82j に書く。**
...
**見るのは2箇所とも `verifyHandoff` の中である。**呼ぶ側（再開する前と turn の終わりの2箇所）に書かせない。
**片方だけに置くと、`recheck_interval_ms` を超えた run で pane が閉じる。**

| 何を見るか | どこで |
| --- | --- |
| **新しい印**（direct chat を通ったか） | **`verifyHandoff` の先頭** |
| **取り直したカードの Status** | **`verifyHandoff` の中、`refreshIssue` が成功した直後** |

**2つ目を、呼ぶ側の手前に置いてはならない。**そこには取り直した Status が無い。
`handleTurnEnd` が issue を取り直すのは `applySignals` の**あと**であり、前へは動かせない
（`applySignals` が Status を動かしうるので、動かす前の値で `decideAfterTurn` を回すと別の欠陥になる）。

**手前に置くと2つ壊れる。**

| 何が | どうなるか |
| --- | --- |
| **リクエストが増える** | `handoffLostOnTurnEnd` は `recheck_interval_ms`（既定1時間）が来ていなければ**1本も投げずに返る。**その手前に取り直しを置くと、**direct chat と関係のない run も含めて、turn の終わりごとに GraphQL が1本増える** |
| **落ちたときに門が開く** | `refreshIssue` は取り直しに失敗すると**着手したときの古い写しを返す。**その写しの Status は `In Progress` なので、**枝は当たらず、確かめ直しが走り、pane が閉じる** |

**`verifyHandoff` の中なら、どちらも起きない。**そこは既に取り直しており、
**取り直せなかった場合を「判定しない」に倒してある。**リクエストは1本も増えない。
...
| **direct chat を通った run は、担当の確かめ直しで止まらない**（3-82h） | **通常の着手から入った run も免除される。****代償として、その run は「担当が本当に別の機械へ移った」ときも止まらない。****1台で動かしているなら起きない。**2台以上で見張っていて、かつ人間が direct chat を使った issue でだけ起きる |
| **18時間を超えると担当を奪われうる** | **これが当たるのは、通常の着手から direct chat へ入った run だけである。**そちらは continuo が自分を担当者に書いてある。direct chat 中は進捗のコメントが書かれないので、18時間（3-77b の `idle_timeout_ms`）を超えると別の機械が担当を引き取りにくる。**1台で動かしているなら起きない。****3-82d が用意した run には当たらない。**担当者を1バイトも書いていないので、奪われる担当が無い |
...
| **`agent.max_concurrent_agents` を1つ使い、自分では返さない** | pane で Claude Code が動くためである。**direct chat の run は自分では終わらないので、枠は人間がカードを戻すまで空かない。**既定は2なので、**1件置きっぱなしにすると通常の着手が半分になる。**枠が尽きて用意しなかったことは WARN で名指しする（3-82c） |
| **`Direct Chat` を他の役割に書いている人は、この機能だけが無効になる** | 設定の検査が重なりを見つけると、**起動は通し、direct chat だけを無効にして WARN を1回出す**（3-82k）。**起動を断ってはならない。**この機能を1度も頼んでいない人の continuo が、版を上げた瞬間に落ちる |
| **既にその名前の列を持っている人は、上げただけで挙動が変わる** | 既定が `"Direct Chat"` なので、**その列を自分用の置き場に使っていた人の issue が、いままでの「知らない Status」の扱いから外れる。**猶予のあとで worker が止まっていたものが、止まらなくなる。**`tracker.direct_chat_state` を空か別名にすれば元に戻る**（[docs/upgrading.md](../upgrading.md) が手順を持っている） |
...
**3-82c の門1 は印を持たない issue にしか効かないので、用意し直しもされない。**
...
（3-82k の設定の検査が重なりを見つけると、direct chat を無効にする。**無効なら、direct chat の run そのものが存在しない**）。
...
`failure_state` / `status_signal_map` の遷移先 / `cleanup.on_states` と重ならないことを検査する。**

**重なっていても、起動は断らない。****direct chat を無効（`direct_chat_state` が空のときと同じ）にして、
WARN を1回出す。**`continuo doctor` も `!` を出す。

**断ってはならない理由。**この機能は既定値を持つ（人間の決定「標準機能として扱え。オプションではない」）。
**だからこの検査は、この機能を1度も頼んでいない利用者にも当たる。**
**断ると、いままで動いていた continuo が、版を上げた瞬間に立ち上がらなくなる。**
**しかもエラーは、その人の WORKFLOW.md に1行も書いていないキーの名前を出す。**

**無効にすれば、害は同じだけ消える。**direct chat が無効なら、そこへ遷移できる issue が存在しない。
**3-82 が「選択肢がカンバンに無くても起動を止めない」と決めたのと、同じ判断である。**
**選択肢が無いときは止めないのに、役割が重なったときだけ止めるのは、揃っていない。**

**「利用者が明示的に書いた値のときだけ断る」形は採らない。**
既定値を置いて YAML で上書きする作りなので、
**書いていないことと、既定値と同じ値を書いたことを、設定の読み込みでは区別できない。**
区別するには型を変えるか front matter をもう一度読むことになり、**減らすつもりが増える。**

**`automated_state_rewrite` のキーとの重なりも、同じ扱いにする。**
既存の検査は `KnownStates` 経由で弾くので、**既定値が入った瞬間に、
いままで正しかった `automated_state_rewrite: {"Direct Chat": "In Progress"}` の1行が起動を止める。**
**そこも、direct chat を無効にして WARN を1回出す形にする。**

**無効にした run は、3-82j の「再起動をまたいだときの挙動」の2つ目に影響しない。**
無効なら `IsDirectChatState` が常に偽を返すので、direct chat の run そのものが存在しない。
...
stateDiagram-v2
    ...
    note right of DirectChat
        ...
        pane を用意する条件は 3-82c が正である。
        出たあとに何が起きるかは 3-82g が正である。
        入りも出も、動かすのは人間だけである。
        カンバンにこの選択肢を作るまでは、
        どの issue もここへは来られない。
    end note
...
| **どの Status** → `direct_chat_state` | 人間 | **pane に入って自分でチャットしたいとき**（3-82）。**6つのどこからでも動かせる。**動かした先で continuo は turn を送らず、表明も読まず、Status も動かさず、**pane も worktree も残す。****カンバンにこの選択肢を作ってあるときだけ使える。****pane を用意する条件は 3-82c が正である**（ここには書かない） | GitHub の画面 |
