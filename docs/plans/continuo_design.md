...
| **雛形にある設定項目が全部書かれているか** | **雛形（`internal/scaffold/template.go`）の front matter のキーが `WORKFLOW.md` に全部あるかを見る**（3-73） | **記号は `!`。**ボードを1バイトも読まない。**版を上げて増えた項目は、リリースノートを読まないかぎり存在に気づけない。**足す差分と、それを当てるコマンドの2つを出す |
...
claude:
  tool_gate:
    ...
    model: haiku                            # 判定させるモデル。既定は haiku
    tools: ["Bash"]                         # 判定に回す道具。空なら全部
```

**3択にする理由は 3-63b と同じである。**手元の非公開なリポジトリの作業まで遅くしない。
...
**「そのコマンドが外部のコメントに指示されたものだ」ということは見えない。**
**6-23 の立場の札と組み合わせる。**
...
| **足す位置** | **利用者のファイルから決める。**雛形の並び順で直前にあるキーを利用者のファイルの中から探し、その次に置く。**並び順を変えていても当たる** |
| **コメント** | **キーの右のコメントも、その下に続く説明の行も、まとめて足す。**キーと値だけでは何を書ける項目なのかが分からない |

**出すものは2つである。**

| 何 | どこに出るか |
| --- | --- |
| **差分** | 検査結果の内訳（`Notes`）に unified diff をそのまま並べる |
| **当てるコマンド** | 直し方（`Remedies`）に `continuo doctor --missing-keys-patch <パス> \| patch -p0` を出す |

**検査結果の中の差分は、そのままでは `patch` に渡せない。**内訳は見出し語の桁に揃えて
字下げされるためである。**人間が読む差分と、機械へ渡す差分の両方が要る**ので、
差分だけを字下げなしで出す口（`--missing-keys-patch`）を別に持つ。その口は検査を1つも行わない。
