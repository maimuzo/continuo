...
| pane | **閉じない。無ければ1つ用意する**（ただし**この issue の印をまだ持っていないときだけ**。下の門1） |
| worktree | **消さない。無ければ用意する**（同上） |
| 「自分が取った」印（`o.runs`） | **持ち続ける** |
| `rate_limit.pause_above_percent` | **当てない**（完全マニュアルのため） |
| 入札と担当の持ち回り（3-77） | **どちらも当てない。**入札もしないし、担当者も見ない（下） |
...
**判定の材料は3つである。カンバンの Status と、印（`o.runs`）の有無と、pane の有無。**
...
flowchart TD
    ...
    M -- "持っている" --> N["何もしない<br/>（pane が無くても用意しない）"]
    M -- "持っていない" --> D{"この worktree に<br/>pane が1枚でもあるか"}
    D -- "ある" --> E["何もしない<br/>（渡したまま）"]
    D -- "無い" --> H["着手の段を踏む<br/>段2 と段11 は飛ばす"]
    H --> I["issue へ1件書く<br/>「話しかけられます」"]
```

**印を持っている run には、pane が無くても用意しない。**理由は下の門1 にある。
...
candidates = FetchIssuesByStates(active_states ＋（実在すれば）direct_chat_state)
  ...
  Status が direct_chat_state のもの  → prepareDirectChatPanes（この節）
  それ以外                            → dispatchCandidates（3版目で入れた direct chat の分岐を全部取り消す）
```

**direct chat のパスは、`verifyPeriodically` の結果（`dispatchAllowed`）に関わらず走らせる。**

**救える場面は狭い。**`dispatchAllowed` が偽になる理由は3つあるが、
**この例外が救うのは「Status の選択肢名の照合に失敗し、なおかつ候補を取れている」ときだけである。**
**照合が落ちる典型は「カンバンの選択肢名を人間が変えた」ときで、
変えたのが `active_states` の1つなら候補の取得そのものがエラーになる**
（カンバンに無い Status 名を渡すと0件ではなくエラーが返る）。**そのときは渡すものが1件も無い。**
**救えるのは、変えたのが候補として要求していない Status（`failure_state` など）だった場合である。**
**残る2つでは救わない。**`gh` の認証の検査が落ちる巡回では、
候補の取得も同じ `gh` を通るので候補が0件になる。候補の取得そのものが失敗したときも同じである。
**どちらも、direct chat のパスに渡すものが1件も無い。**

**それでも例外にする理由。**照合の失敗は「カンバンの選択肢名を人間が変えた」ときに起きる。
**そのとき人間はカンバンを触っている最中であり、direct chat をいちばん使いたい場面である。**
...
**外すのは4本、残すのは1本である。**`startRunFromWorktree` だけは、段11 を踏むかどうかを
知る必要があるので引数で受け取る（このパスが `true` を渡す）。
**5本目まで外して印で判定し直してはならない。**印は着手の goroutine が非同期に立てるので、
そこで見ると、この設計が消したかった隙間が復活する。

**`prepareDirectChatPanes` は、候補1件ごとに次の順で見る。**

**まず、何もしないで飛ばす条件を5つ。**上から順に見て、1つでも当たったら次の候補へ移る。

**門1 は「印を持っている」である。**pane を持っているかどうかは見ない。

**「印を持つが pane が無い run には、pane を用意しない」と決める。**
**そういう run は実在する**（着手に失敗してバックオフに入った run は、pane を閉じられたうえで印を持ち続ける）。
**そこへ人間がカードを動かしても、このパスは何もしない。**

**用意しない理由。**用意するには、**既に印を持っている run に着手の段1 をもう一度踏ませる**ことになる。
着手の段1（`o.claim`）は「**既に印を持っていた場合は dispatch しない**」と決めており、
**そこを曲げると、印を持つ run に段3〜段10 を踏ませる経路が新しく1本できる。**
...
**ただし、印が外れるのはバックオフが明けたあとである。**
**バックオフ待ちの run は、巡回が入口で丸ごと飛ばすので、Status を取り直さない。**
**だからカードを動かしても、その巡回では何も起きない。**
**待ちは `agent.max_retry_backoff_ms`（既定300000ミリ秒＝5分）が上限である。**
**明けたあと、着手の段2 の取り直しが `In Review` を見て `ErrStatusNotWritten` を返し、そこで印が外れる。**
**FAQ には「最大5分かかる」まで書く。**

**`Ready` や `In Progress` へ動かしてはならない。**そちらへ動かすと、
巡回は指示を送ろうとし、pane が無いので失敗し、**またバックオフを積んで印を残す。**

| 順 | どういうときに飛ばすか | 飛ばすときに何を出すか |
| --- | --- | --- |
| 1 | **既に印を持っている** | 何も出さない（巡回の側が direct chat へ入れる）。**pane を持っているかは見ない**（上） |
| 1b | **draft issue である**（owner も repo も持たない） | Debug 1行。**worktree を作れないので、用意は必ず段6 で落ちる。****`preflight` は落とさない**（信頼登録を要求しない設定では素通りする）ので、**ここで落とさないと30秒ごとに枠を取っては落ちるのを永久に繰り返す** |
| 2 | **この worktree に pane が1枚でもある** | Debug 1行。**「Claude Code が居るか」は判定しない**（下） |
| 3 | **空きスロットが無い**（`agent.max_concurrent_agents`） | **WARN で名指しする。ただし1つの issue につき1回だけ。**direct chat の run は自分では終わらないので、上限を上げても次に同じことが起きる。効く直し方は「どれかの issue を direct chat から戻す」である。**毎巡回で出すと、枠が詰まっているあいだ他の WARN が全部これに埋もれる。****この記録は2箇所で消す**（消さないと、次にまた枠が尽きたときに1行も出ない）。**1つは、この issue の pane を用意できたとき**（このパスの段7）。**もう1つは、巡回のたびに「候補にいて Status が `direct_chat_state` でない issue」と「候補に1件もいない issue」の記録を消す。****片方だけでは足りない。**候補の一覧は `active_states` と `direct_chat_state` しか含まないので、**`Ready` へ戻した issue は前者で、`In Review` へ抜けた issue は後者でしか捕まらない。****そして `In Review` は、この設計が勧める回復手順の通り道である。****候補の取得に失敗した巡回では、後者を走らせない**（走らせると記録が全件消える。`forgetGatedNotOnBoard` が同じ守りを持っている） |
| 4 | **着手の直前の検査（`preflight`）に落ちた** | `preflight` が自分で出す。**信頼登録の判定をこの門より前に置いてはならない。**`Dispatchable` で先に落とすと `preflight` へ届かず、**未信頼のリポジトリで direct chat を頼んだ人へ、直し方のコメントが1件も出ない**（3-33） |

**5つとも当たらなければ、次の3つを順に行う。**

**段5 までは巡回のループの中で同期に行い、段6 と段7 は別の goroutine で回す**（3-8）。
**段6 は git の worktree 作成・利用者が書いた `workspace_hooks`（既定60秒）・
...
| 順 | 何をするか |
| --- | --- |
| 5 | **印を付ける**（着手の段1）。**`claimForDispatch` ではなく `o.claim` を直に呼ぶ。**あれは印を付けたあとに写しの Status を `running_state` へ書き換えるので、**引数を外すとその1行が無条件になる。****写しの Status は書き換えない。**書き換えると、状態ごとの上限（`agent.max_concurrent_agents_by_state`）の勘定に入り、**direct chat のカードを1枚置いただけで通常の着手を1件ぶん失う** |
| 6 | **着手の段3〜段10 を踏む**（**段2 と段11 は踏まない**）。落ちたら、自分が開いた pane を pane ID で閉じ、印を外す。**カンバンへは1バイトも書かない** |
| 7 | **direct chat へ入れ、issue へ1件書く** |

**この一覧に無い門は、1つも通らない。**とくに次の5つは、**意識して通さない。**
...
| **`Dispatchable`** | 信頼登録の判定を畳み込んだ値なので、先に落とすと `preflight` へ届かず、**直し方のコメントが人間へ届かない**（上の門4） |

**飛ばすたびに関門の記録を消す**（`clearGate`。設計 6-1）。
**消さないと、担当者の関門で止まっていた issue を direct chat へ動かした人が、
ダッシュボードで「担当者の関門で止まっています」といういまは効かない理由を見続ける。**

**段3〜段10 は `startRunFromWorktree(ctx, rs, issue, false, true)` を呼ぶ。**
**`startRun` から入ってはならない。**あれは段2 を踏むので、
**用意の段が `ErrStatusNotWritten` で落ち、pane が1枚もできない。**
**カードは1バイトも動かない**（段2 には守りが2枚ある。許可リストが `active_states` にあるときだけ書き、
...
**pane の写像は、この1パスで1回だけ作る。**`pane.list` は機械中の pane を全部返すので、
...
| **`direct_chat_state` のカードへ `UpdateStatus` を呼ばない** | `protectedStates()` が返す拒否リスト（`UpdateStatus` の第4引数）を、**Status を書く経路すべてに渡す** | 人間が置いたカード |

##### `UpdateStatus` は13箇所から呼ばれる。9箇所へ `protectedStates()` を渡す

**まず全部を数える**（`git grep -n '\.UpdateStatus(' -- internal/` で実測）。**13箇所である。**
**「9本」だけを数えてはならない。**残る4本について、実装者が自分で判断することになる。
...
| `nil` | 2 | `internal/abandon`。**人間が名指しで叩く操作なので、direct chat のカードを上書きしてよい** |
...
| direct chat から戻ったときの書き込み | 下の「出るとき」 |
...
**だから「終わらせる処理を、印を外す前にやめる」段を置く**（`abortTerminalForHuman`）。**呼ぶのは3箇所である。**
...
| **`handleTurnEnd` の担当の確かめ直し**（入口の門の**直後**） | **入口の門は内部の印を見るので、人間がカードを動かしてから巡回が回る前に turn が終わると通り抜ける。**そのすぐ下で担当の確かめ直しが走り、**`stopWorker` を呼ぶ。**そのときはまだ印も新しい印も立っていないので、**どの門も効かない。****pane が閉じ、印まで外れ、カンバンにも issue にも1バイトも残らない。****取り直したカードの Status で判定する枝を、確かめ直しの手前に置く** |
| **`decideAfterTurn` の `switch` の先頭** | **取り直したカードの Status で判定する枝を置く。**入口の門は内部の印を見るので、人間がカードを動かしてから巡回が回る前に turn が終わると通り抜ける。**`default` へ落ちると引き渡しの通知を投稿し、人間の pane へ指示を送り、pane を閉じる。****「カードを動かしてから話しかける」という FAQ が勧める手順を踏んだ人が、いちばん高い確率で踏む** |
...
| **復元で取り直しに失敗したとき** | **pane を閉じない。**そこでは Status がまだ読めていないので、direct chat だったかどうかを知る手立てが無い |
...
**ただし、受け口へ流さない門より手前で記録されるものが2つある**（「最後に Stop を見た時刻」と
「この turn で hook を見たか」）。**受け口そのものにも、抜けてから turn ループが立つまでの
30秒に届いた hook が溜まる。**
**`beginTurn` はこの3つを洗い直すが、`awaitTurnEnd` の経路は `beginTurn` を通らない。**
**だから、抜けるときに3つとも捨てる**（下の「出るとき」）。
...
**「pane が無ければ用意する」は、まだ印を持っていない issue にしか効かない。**
...
**バックオフ中の run は巡回が Status を取り直さないので、カードを動かしても、その巡回では何も起きない。**
**`In Review` か `Blocked` なら、バックオフが明けたあとの着手の段2 の取り直しが印から外す**（最大5分）。
**FAQ にそう書く。**
...
| `dispatch_state`（既定 `Ready`） | 上に加えて、**`running_state` を書く。****書かないと2つ壊れる。**状態ごとの上限（`agent.max_concurrent_agents_by_state`）は `running_state` の run だけを数えるので**1つ超えて走る**。そして**段2 はこの run では二度と通らないので、カードは永久に着手待ちに見える。****判定は `dispatch_state` そのものとの一致で行う。**「`active_states` にあって `running_state` でない」で判定すると、3つ目の作業中 Status を書いている利用者のカードを勝手に書き換える。**この書き込みは、`TurnCount` の数え直しとは無関係である**（印が外れていないので、書いても書かなくても同じ run が続く） |
...
そこで担当が自分でなければ、**push もせずに worker を止め、pane を閉じる。**
**人間が自分を担当者に付けたまま direct chat を使うのは普通の操作なので、これでは必ず当たる。**
**時計を進めるだけでは足りない。**確かめ直しは2本あり、時計は片方しか塞げない。

| 何 | いつ効くか | 時計を進めると塞がるか |
| --- | --- | --- |
| 再開する前の確かめ直し | **1度も確かめていない run だけ** | 塞がる |
| **turn の終わりの確かめ直し** | **`recheck_interval_ms`（既定1時間）を超えたとき** | **塞がらない。1時間先送りするだけである** |

**2本目が本命である。**direct chat は「いつまでもチャットできる」ためのものなので、
**1時間を超えるのが普通である。**
**戻して1時間働かせたところで、予告も記録も無く pane が閉じ、push もされない。**
**カンバンにも issue にも1バイトも残らないので、人間には何が起きたのか分からない。**

**だから、時計ではなく判定そのものを直す。**
**「この run は direct chat を通ったか」を表す印を1つ新設し、真なら担当の確かめ直しで止めない。**

**印を立てるのは1箇所である。**`enterDirectChatMode` の中で立てる。
**呼び出し側に書かせない。**呼ぶ場所は4つある（用意し終えたとき／巡回が気づいたとき／
turn の終わりに気づいたとき／復元が引き取ったとき）。**1つでも落とすと、その経路だけ守りが消える。**
**復元も同じ関数を通るので、別の引き継ぎの仕組みは要らない。**
**下ろさない。**一度でも direct chat を通った run は、以後ずっと免除する。

**見るのは1箇所である。**担当が移ったかを判定する `verifyHandoff` の先頭で見る。
**呼ぶ側（再開する前と turn の終わりの2箇所）に書かせない。**
**片方だけに置くと、`recheck_interval_ms` を超えた run で pane が閉じる。**
**direct chat は1時間を超えるのが普通なので、そちらが本命である。**

**なぜそれで正しいか。**この設計は「入札も担当の持ち回りも当てない」と決めている。
**当てないと決めた run に、担当を失ったかどうかの判定だけ当てるのは筋が通らない。**
確かめ直しが守ろうとしているのは「担当を**外された**」場合だが、
**direct chat の用意は担当者を1バイトも書かないので、外される担当が無い。**

**既存の `handoffAcquired`（着手のときに担当者を書いたか）を使ってはならない。**
**あれは復元が引き取った run でも偽である**（`Adopt` は1度も書かない）。
**偽なら止めない形にすると、復元した run の守りまで消える。**
そうなると、2台以上で見張っている人の手元で、
**担当を外された機械が同じ branch へ push する。**3-77c が防いでいたものが無効になる。

**入札で担当者になった run では、いままでどおり効く。**新しい印が立たないためである。
...
| 何 | なぜ捨てるか |
| --- | --- |
| **最後に Stop を見た時刻** | 古い時刻がそのまま「turn が終わった」と読まれる |
| **この turn で hook を見たか** | 枠待ちかどうかの判定が「枠待ちではない」に倒れる |
| **受け口に溜まった hook** | 抜けてから turn ループが立つまで（既定30秒）に届いたものが残る |

**1つだけ捨てる形にしてはならない。**次に読む人が、残り2つを「意図して残した」と読む。
direct chat の間、hook は受け口へ流していないが、**その手前で「最後に Stop を見た時刻」は
記録されている。**エージェントが応答を書いている最中に戻すと `beginTurn` を通らないので、
**その古い時刻がそのまま「turn が終わった」と読まれる。**
**direct chat を抜けるときに、その記録も捨てる。**

**抜けた瞬間に stall 検知の時計を引き直す。**direct chat の間は `checkStalls` を飛ばしているので、
最後に見た時刻も画面の版も止まったままである。**引き直さないと、人間が黙って3時間考えていただけで、
...
| **再起動をまたいでも pane は残る** | **復元は direct chat の run を引き取る**（3-4 の段5）。引き取れば印に入るので、人間が戻した巡回で「取り残された worktree」として閉じられることがない。**引き取れなかったときも pane は閉じない**（socket のパスが変わった・agent 名が無い・セッション UUID を取れない、などの6つの道は、direct chat では閉じずに見送る。**コードの上では7つあるが、1つ目（`cleanup.on_states`）は設定の検査が起動前に断るので、direct chat では通らない**）。**そのときだけは、戻した最初の巡回で 3-9 の手順7b が pane を閉じ、3-16 が同じセッションへ `--resume` で立て直す。**会話は残るが、送るのは1回目の本文（5-3）になる |
| **direct chat を通った run は、担当の確かめ直しで止まらない** | **入札も担当の持ち回りも当てないと決めた以上、担当を失ったかどうかの判定だけ当てるのは筋が通らない。****代償として、その run は「担当が本当に別の機械へ移った」ときも止まらない。****1台で動かしているなら起きない。**2台以上で見張っていて、かつ人間が direct chat を使った issue でだけ起きる |
| **18時間を超えると担当を奪われうる** | **これが当たるのは、通常の着手から direct chat へ入った run だけである。**そちらは continuo が自分を担当者に書いてある。direct chat 中は進捗のコメントが書かれないので、18時間（3-77b の `idle_timeout_ms`）を超えると別の機械が担当を引き取りにくる。**1台で動かしているなら起きない。****このパスが用意した run には当たらない。**担当者を1バイトも書いていないので、奪われる担当が無い |
...
| **バックオフ待ちの run は守らない** | そこでは pane が既に閉じている。**バックオフ中は巡回が Status を取り直さないので、`redispatch` の入口の検査は当たらない**（見ているのは着手した時点の写しである）。**落ちるのは着手の段2 の取り直しで、そこで印が外れる** |
...
| **`Direct Chat` を他の役割に書いている人は、上げただけで起動しなくなる** | 設定の検査は、この Status が `active_states` などと重なると起動を断る。**既定が非空になる以上、`Direct Chat` を他の役割へ書いている人は、版を上げた瞬間に落ちる。****しかもエラーは、その人の WORKFLOW.md に1行も書いていないキーの名前を出す。****エラーの文面へ「このキーを書いていない場合は既定値です」を入れる** |
...
**`continuo abandon` の `--park` と `--to` の行き先にしてはならない。**理由は別々である。

| どちら | 何が起きるか |
| --- | --- |
| `--park` | そこへ動かしても pane が閉じないので、**pane が閉じるのを待つ段（3-37 の段1）が待ち切れず、何も消せない** |
| `--to` | 片付けは通るが、**次に continuo が起動したとき、いま消したばかりの issue の worktree と pane を作り直す。**巡回のたびに作り直されるので、抜け出すにはカードを手で動かすしかない |

**そして「いまの Status が direct chat である」ときも断る。**
**片付けの段は、`active_states` に無いカードへは `park` を書かない。**
`direct_chat_state` は `active_states` に入れられないので、**必ずそこへ落ちる。**
そのため pane が消えるのを待つ段を通らず、`--force` を求められる。
**`--force` を付けると worktree は消えるが、カードは direct chat のままなので、
巡回はその run を毎回飛ばし、印は永久に外れない。**
**消えた worktree を指したまま `agent.max_concurrent_agents` の枠を1つ持ち続ける。**
**気づく手立てが無い**（ログにも issue にも何も出ない）。
...
**文面は3つに分ける。**いまは1つの文言を `--to` と `--park` の両方で出しており、
...
**`continuo doctor` は、設定に書いた名前がカンバンに無いことを `!` で出す。条件を絞らない。**
...
stateDiagram-v2
    ...
    note right of DirectChat
        ...
        （continuo がその issue の印をまだ持っていないときだけ）。
        カンバンにこの選択肢を作るまでは、
        どの issue もここへは来られない。
    end note
...
| `Ready` / `In Progress` → `direct_chat_state` | 人間 | **pane に入って自分でチャットしたいとき**（3-82）。**カンバンにこの選択肢を作ってあるときだけ使える。**動かした先で continuo は turn を送らず、表明も読まず、Status も動かさず、**pane も worktree も残す。pane がまだ無ければ1つ用意する**（continuo がその issue の印をまだ持っていないときだけ。3-82） | GitHub の画面 |
