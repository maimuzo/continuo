...
**そして、この worktree のエージェントは `.claude/rules/issue.md` を書き換えられなかった**
（`Edit` が `Permission to use Edit has been denied because Claude Code is running in don't ask mode` で断られた。実測）。
**`.claude/settings.json` に経路別の禁止は1本も無い。**許可されていないので通らなかっただけであり、
**`.claude/` の下だけが禁じられていることの裏付けは、リポジトリの中には無い。**
...
**この節は、設計文書を指すリンクのいちばん後ろ（この変更の時点で 9992 行）より後ろにあるので、
この節を足したことでは1本もずれていない。**
...
**3-26 からこの節への案内は置かない。**1行足すだけで上の20本がずれるためである。
