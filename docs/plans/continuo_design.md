...
### 6-20. `着手を取り消す` の基本フローは、同じ入力で2度分岐しない

**言いたいこと。**継続監視が動いているか・dry-run か・park を実際に書いたかは、
**入り口で1度だけ判定し、以降の段は文の中の条件として持ち回る。**
基本フローの `IF` は3つ、CFG のテストパスは38本である（2026-08-27 の実測）。
...
**採る書き方。**入り口で名前を付けた判定を2つ置き、以降は `VALIDATES THAT` の選言で受ける。

| 名前 | どこで決まるか | 真になる条件 |
| --- | --- | --- |
| 手を離させる段を通る | 継続監視と dry-run の判定 | 継続監視が動いていて、利用者が dry-run を指定していない |
| 手を離させる書き込みを行う | ボードの Status を読んだ直後の判定 | 上が真で、ボードの Status が `tracker.active_states` に入っている |

例: `システムは VALIDATES THAT 手を離させる書き込みを行っていないか、worktree を作業ディレクトリに持つ pane が期限内に無くなるか、利用者が force を指定している。`

**`IF` のまま残すもの。**真と偽で**通る段の並びが変わる**ものは畳まない。
**基本フローに残る `IF` は3つで、片付け切れずに残ったものの有無・身元ファイルに書かれた
branch の実在・片付けたあとの Status の値の指定である。**
代替フロー `worktreeが無い` も別に3つ持つ（組み立てた branch の実在、
dry-run と `cleanup.delete_branch` の組、片付けたあとの Status の値の指定）。
**1回の実行が2つのフローを通ることは無いので、フローをまたいで数えない。**

**代わりに失われるもの。**畳んだ条件は経路の分岐ではないので、
**CFG の経路1本が、その条件の真の側と偽の側の両方を含む。**`paths[].steps` にはその段の文が
1行だけ並び、**どちらの側を通ったかは書かれない。**現に abandon のテスト53本は27本の経路に
集まり（2026-08-27 の実測）、**同じ経路 ID のテストが真の側と偽の側に分かれて並ぶ。**
**経路の一覧をそのまま検査に落とす道具は、真の側にしか成り立たない検査を偽の側にも当てる。**
**どの段にどの条件が畳まれているかは `.rucm.md` の中の表が正である**
（[docs/spec/usecases/particular_case/着手を取り消す.rucm.md](docs/spec/usecases/particular_case/着手を取り消す.rucm.md)
の「同じ入力で2度分岐しない」と「経路の一覧は、文の中に畳んだ条件を数えない」）。
...
CFG のノードとエッジは100%網羅されている（2026-08-27 の実測）。
**減るのは経路の本数と、経路の ID で区別できる粒度だけである。**
