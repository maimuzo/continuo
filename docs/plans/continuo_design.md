...
**`runtime.lock_file` は、キーを受け取って値を捨てる。**設定でロックの場所を変えられると、
**`continuo abandon` が別の場所を見て「動いていない」と判定し、走っている worktree を消しに行く**（3-17c）。
**キーごと弾かないのは、`lock_file: null` が `continuo init` の雛形に入っていたからである。**
front matter は未知のキーを弾く（8-1）ので、**キーを消すと、過去に `continuo init` した全員が次の起動で落ちる。**
**値が書いてあれば、起動時に警告を1行出す**（黙って捨てると、効いていないことに無人運用では気づけない）。
**分ける必要があるなら `--id` を使う。**
...
**`runtime.lock_file` を書き換えれば `--id` を付けずに2本立てられる、という指摘がある。**
**これは問題として扱わない**（値は読まないので、そもそも立たない）。
**正しい設定をしないときに正しく動かないのは当たり前であり、開発時のテスト用に便利さを求めない。**
...
| **PR を作るか** | push までか、`gh pr create` までか | 組み込みは PR を作らせない（5-3b） |
