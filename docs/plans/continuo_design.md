...
7. agent.prompt で「作業の内容を issue のコメントに書いてください」とだけ送る
   ...
   → **「新しく1件投稿してください。その印は入れないでください」を必ず添える**
...
**いまは `language` に連動させない。**組み込みの指示書がまだ日本語しか持っていないので、
...
| **変数展開に失敗した** | **何も出さずに終了コード 1。**本文の `{{if}}` の閉じ忘れは、`--url` で初めて表に出る |
