...
#### 引き渡しの通知には subagent の記録も載せ、止まった原因は断定しない

**言いたいこと。**親のセッションの記録だけを案内すると、その末尾には何も無くて行き止まりになる。
**subagent の記録も並べる。**そして**何が確認の画面を出したかは断定しない。**
continuo の側にその情報が残らないためである。
...
**文面から原因を断定しない。**`blocked` で人間へ渡すときの理由に
「許可されていないコマンドを実行しようとした」と書かない。**continuo は拒否を検知しない**（この節の上）。
**`dontAsk` では、許可の一覧に無いツールは確認を出さずにその場で拒否される。**
**したがって確認の画面が出て止まったのなら、それは拒否とは別の原因のことがある。**
書けるのは「記録を見て確かめてください」までであり、`claude.permissions.allow` に足すのは
**記録を見て、許してよい操作だと分かったときだけ**である。
...
| `esc` を送る前に画面を読んで残す | **`esc` は、保留中の権限要求が承認されて実行されるのを防ぐためのものである**（3/3 で再現）。その前に別の呼び出しを挟むと、塞いだ穴が開く |
...
**判断方法は新しく作らない。1-7 で決めたものを、`blocked` の道へ広げるだけである。**
1-7 は「待っていれば `background_tasks` が空の `Stop` が来る」と決め、3-2 はその
`background_tasks` で turn の終わりを判定している。**`blocked` から esc を送る道だけが、
それを1度も見ていなかった。**

**2つの申告を足し合わせる。どちらかが「動いている」と言っているなら、動いていると扱う。**
...
| 直近の `Stop` の `background_tasks` | `Stop` / `SubagentStop`（3-2 が既に読んでいる） | `SubagentStart` を取りこぼすと1本目が空のまま |
...
| 何を | どうする |
| --- | --- |
| 空に戻す契機 | turn を送るとき / **`background_tasks` が空の `Stop`** を受けたとき |
...
**空に戻す契機に `Stop` を入れる理由。**`background_tasks` が空なのは
「1つも走っていない」という Claude Code 自身の申告であり（1-7 / 3-2）、こちらの数え方より確かである。
