...
| **表明で指せるのは、別の run が印を持っていない issue だけ** | **番号を1つ書き間違えるだけで、別のエージェントを turn の途中で止められる**（下記） |
...
**猶予はボードの自動化が書いたときにしか効かない**ので、continuo 自身の書き込みは待ってもらえない。
...
| 案 | 中身 | 採否 |
| --- | --- | --- |
| **担当中なら書かない（採る）** | 印を引いて、持たれていたら捨てる。ログとコメントだけ残す | **書き間違いの被害がゼロになり、グループの表明はそのまま通る**（グループの他の issue は `Ice Box` なので印を持たない） |
