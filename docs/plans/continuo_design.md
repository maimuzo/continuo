...
| **二重起動防止のロックを置けるか** | `~/.continuo`（`--id` を付けたなら `~/.continuo/id/<名前>`）を作り、**実際に開いて `flock` を試し、すぐ手放す** | **socket とは別の場所である**（3-17）。**socket が置けてもロックが置けるとは限らない。**`~/.continuo` が書けない（あるいは同じ名前のファイルがある）と、全項目 `✓` なのに起動だけが落ちる（issue #9 と同じ形）。**既に握られていたら `✓`**（continuo が動いているだけである） |
