...
**ロックの置き場所は、次の3段で決める**（上にあるものが勝つ。`internal/daemon` の `ResolveLockFilePath`）。

| 順 | 何を書いたか | ロックの場所 |
| --- | --- | --- |
| 1 | **`--id <名前>`** | **`~/.continuo/id/<名前>/continuo.lock`** |
| 2 | **`runtime.lock_file`** | **設定に書かれた絶対パスそのもの** |
| 3 | **どちらも無い**（既定） | **`~/.continuo/continuo.lock`** |

**socket の置き場所から導いてはならない。**socket の場所は `CONTINUO_RUNTIME_DIR` →
`$XDG_RUNTIME_DIR/continuo` → `$TMPDIR/continuo` → `~/.continuo/run` の順で決まり（3-23）、
`claude.hook_bridge.listen` でも動く。**そこから導くと、socket を分けただけで
2本目が黙って起動できてしまう。**「`--id` を付けない起動は1本に止める」を、
**設定の書き換えで迂回できる状態にしない。**

**`runtime.lock_file` は読み続ける。**このキーは `continuo init` の雛形に入っており、書いている人が既に居る。
**黙って無視すると、その人のロックの場所が予告なく変わる。**`--id` をその上に置くのは、
**`--id` がその場で打った明示の指定だからである**（設定より、いま打ったフラグを優先する）。

**親ディレクトリを作るのは、continuo が場所を決めたときだけである。**
既定の `~/.continuo` と `--id` の `~/.continuo/id/<名前>` は `os.MkdirAll` で用意する。
**`runtime.lock_file` に人が書いたパスの親は作らない。**作ると、パスを打ち間違えたときに
黙って別の場所へロックを置き、**二重起動を検出できなくなる。**打ち間違いは
「ロックファイルを開けません」で止めて、人間に気づかせる。

**入れ替える前に、動いている continuo を止める。**
**古い版と新しい版は、互いのロックを見つけられない。**`runtime.lock_file` を書いていない古い版は
...
（[docs/upgrading.md](../upgrading.md) と [docs/FAQ.md](../FAQ.md) の両方へ）。
...
**言いたいこと。**`--id` の目的は1つしかない。**「`--id` を付けない起動は1本に止める」を成立させることである。**
**残りは検証用の `WORKFLOW.md` で書き換える。**`--id` から自動で導かない。

**`--id` は開発者のためのものである。**本番を止めずに検証用の continuo をもう1本立てる、
という場面にしか使わない（[docs/releasing.md](../releasing.md) の実機での確認）。
**一般の利用者向けの機能ではないので、最低限で足りる。**

| 分ける対象 | どうやって分けるか |
| --- | --- |
| **ロック** | **`--id <名前>` を付ける。**`~/.continuo/id/<名前>/continuo.lock` になる |
| **worktree の置き場所** | 検証用の `WORKFLOW.md` の `workspace.root` を書き換える |
| **hook の socket** | 同じく `claude.hook_bridge.listen` を書き換える |
| **branch 名** | 同じく `herdr.worktree.branch_template` を書き換える |
| **カンバン** | **検証用のカンバンを別に用意する**（本番と別の project 番号にする） |

**なぜ `--id` から導かないか。**導くと、**利用者が設定ファイルに書いた値と、continuo が実際に使う値が食い違う。**
`workspace.root` に `~/worktrees` と書いてあるのに `~/worktrees/e2e` を使う、という状態になり、
**設定を読んだだけでは、どこに worktree ができるのかが分からなくなる。**
**書き換えを忘れたときに何が起きるかは、文書に書いて人間に渡す**（下の表）。
**機械で止めるのは、ロックの1つだけである。**

**書き換えを忘れたときに何が起きるか。**

| 忘れたもの | 起きること |
| --- | --- |
| **`claude.hook_bridge.listen`** | **2本目は起動に失敗する。**その socket には1本目が待ち受けており、起動時の検査が「既に continuo が待ち受けています」で止まる |
| **`workspace.root`** | 2本が同じ置き場所を共有する。**別のカンバンを見ていれば pane は閉じられない**（身元ファイルで自分の run を見分ける）が、**孤児 branch の掃除と worktree のパスの衝突は残る** |
| **`herdr.worktree.branch_template`** | 同じ branch を2つの worktree が出せないので、片方が着手できない |
| **カンバン** | **機械はもう止めない。**同じ issue を2本が取り合う。**検証用のカンバンは必ず別に用意する** |

### 3-17c. `continuo abandon` は、常駐している側と同じ場所を見る

**言いたいこと。**abandon がロックの場所を読み違えると、**動いている continuo を「動いていない」と判定して worktree を消す。**
**`--id` を付けて動かしているものには、abandon にも同じ `--id` を渡す。**
**検証用の `WORKFLOW.md` のパスも一緒に渡す。**

**採る形。**

| 何 | どうする |
| --- | --- |
| **`--id <名前>`** | **常駐側と同じ名前を渡す。**渡さなければ既定の1本のロックを見る |
| **`WORKFLOW.md` のパス** | **位置引数で検証用のものを渡す。**`workspace.root` はここからしか決まらない |
| 生きていたら | **手を離させる段（park）を通してから消す** |

**`--id` だけを渡しても足りない。**worktree の置き場所は `workspace.root` から決まるので、
**本番の `WORKFLOW.md` を読ませたまま `--id e2e` を渡すと、本番の置き場所を走査する。**
**残った branch の片付け（3-37-9）も、本番の `branch_template` から名前を組み立てる。**
...
**`~/.continuo` の外へ出る。**
**32 文字にするのは、ファイル名の長さの上限（多くのファイルシステムで 255 バイト）に、
利用者の home のパスと合わせて確実に収めるためである。**32 は「人間が端末で打ち分けられる長さ」でもある。

**名前の検査は、ホームディレクトリを引くより先に通す。**順序を逆にすると、`HOME` を引けない環境で
`--id ../../etc` が「ホームディレクトリを取得できません」として報告され、
**本当の誤り（名前が使えない）が人間に届かない。**
**名前の誤りとそれ以外は、エラーの型で言い分ける**（`instance.InvalidIDError`）。
**同じ検査を CLI と `internal/instance` の2箇所に置かない。**置くと、片方だけを直したときに
通るものと通らないものが食い違う。
