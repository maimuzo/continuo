...
| **4** | **印から外す** | **落とすと、その run は印に残り続けてスロットを永久に埋める** |
...
**ただし `claude.turn_timeout_ms` が0以下だと、巡回の打ち切りの判定そのものを行わない**ので、
そのときはこの判定も通らない。**枠待ちの印も立たないので、待ちに入ること自体が無い。**
...
**1周は `claude.turn_timeout_ms`（既定1時間）より短くならない**（枠待ちに入るにはその長さの無音が要る）。
...
| 段 | 何を見るか | どの写しを使うか | 巡回を止めるか |
| --- | --- | --- | --- |
| **1** | `rate_limit.pause_above_percent` を超えているか | **最後に読めた写し**（3-77i のとおり、古さを見ない） | **止める** |
| **2** | 枠を読めない / 余裕値がマイナス | **入札の写し**（読み取りに失敗したら無いものとして扱う） | **止めない**（issue ごとに落とす） |

**段1 を入札の写しへ寄せてはならない。**あちらは読み取りに失敗すると無いものになるので、
**使用率99%で資格情報が切れた機械が、新規 dispatch を止めなくなる**（3-77i を破る）。

**理由と「巡回を止めるか」は、別々に返す。**同じ「枠の使い過ぎ」でも、
段1 から来たものは巡回を止め、段2 から来たものは止めない。
**1つの値へ畳んで別の理由を名乗らせると、理由を出す仕掛けの中で嘘をつくことになる。**
**段2 だけが閾値超えと答えるのは、写しを取り直す2回の間に新しい値へ差し替わったときであり、
そのとき枠は読めている。**

**巡回そのものを止めるのは段1 だけである。**
...
**出す条件。止められている候補が1件も無ければ、1行も出さない。**
巡回は既定30秒なので、出し続けると1時間で120行になる。
**continuo はログをファイルに書かないので、pane の scrollback から押し出される。**
**数える対象は、止まる範囲で変える。**

| 巡回を止めるか | 数える対象 |
| --- | --- |
| **止める** | **まだ run を持っていない候補。**担当者の有無は関係ない |
| **止めない** | **担当者が1人もいない候補。**他の機械が担当しているものは、この症状ではない |

**数はどちらも上限である。**必須のラベルが足りない候補・信頼していないリポジトリの候補・
バックオフ中の候補も混ざる。**この時点ではまだ判定していない。**
...
**ダッシュボードへも出す。**写しの直下に置く（run 1件ごとではなく、この機械ぜんぶに掛かる状態である）。
**文言は止まる範囲で選ぶ。**理由だけで引くと、巡回を止めていないのに
「この巡回では1件も着手しません」と出る。
