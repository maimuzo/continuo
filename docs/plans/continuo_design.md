...
**やり直しが1回入ることはある。**起動直後の1回目の `agent.get` は hook が届く前に走るので、
そこでは証拠が成立しない。**そのやり直しが `agentStartBusyBudget` を使う間も hook は届き続けるので、
次の確認で拾える。**
...
| --- | --- |
| 申告が空 | そのまま進む |
| 申告があり、**hook が `claude.settle_ms` 以内に届いている** | 申告が空になるまで `claude.poll_wait_ms` を上限に待つ |
| 申告があり、hook が来ていない | **待たない** |
| 待っても残った | **WARN に何が残ったかを出す。**引き渡しの通知が出る道では、そこにも1行入れる |

**hook が来ていないなら待たない理由。**申告が入れ替わるのは新しい `Stop` を受けたときだけであり
（`noteHook`）、**`blocked` で止まった Claude Code は `Stop` を二度と出さない。**
そこで待つと、直前の `waitForRunningSubagents`（3-11）と合わせて `claude.poll_wait_ms` を
2回ぶん、人間への引き渡しが遅れるだけになる。**新しい設定は足さない。**
...
**`type` で絞らない。**3-11 の subagent の控えは `subagent` だけを取るが、
**ここは turn の終わりと同じ判定でなければならない。**3-2 は「空でなければ未完了」であって
種類を見ていない。**道連れになった実例の1つは `type` が `shell`**（`run_in_background` の Bash）である。
そこで全種類を保つ控えを別に足す（`runState.backgroundTasks`）。**3-11 の控えはそのまま残す。**
あちらは `agent_id` から subagent の記録のパスを組み立てるのに使っており、
**`shell` を混ぜると存在しないパスを人間へ案内することになる。**
**件数は `maxTrackedSubagents`（64件）で切り、通知へ載せるのは `handoffBackgroundTaskLimit`（5件）までとする。**
申告は hook から来る外部入力である（3-23）。
