...
**そこで作るのは、コメントを渡すためのその場かぎりのファイルである。**
**途中で落ちると、元の内容が失われます。**
...
**収まっていない周は、実装した内容と issue を突き合わせてください。**
    ...
    1. issue が何を求めているかを、別の subagent に取り出させる（いま在るものを見せない）
    2. issue が求めていない内容が入っていないかを見る
    3. 入っているなら、それが必要となる合理的理由をまとめる
    4. その理由で敵対的レビュワーを説得する
    5. 説得できなかった内容は削除し、削除した内容を issue のコメントへ残す

**段1 の役には、いま在るものを見せないでください。**3-6 なら差分、3-2 なら計画の本文です。
**見せると、出来上がったものに引きずられて、
...
| **数える範囲**（絶対パス） | subagent が別の checkout を数え、同じ文字列が0件と1件に割れます。**差分を読む役へは、差分の中だけを範囲として渡します** |
...
**本番の環境とカンバンへ書き込む必要があるときは、人間へ渡してください。**
**あなたが自分で叩くのも、subagent へ渡すのも、どちらもしないでください。**
**Status を動かすのは continuo です**（3-7）。
...
**採る形。**[internal/prompt/builtin.md:294-327](../../internal/prompt/builtin.md#L294-L327) の
