...
| 2 | `resumeBackoff` が `redispatch` を呼ぶ | **`reconcileRunning` より前に走るので、run が持つ写しの Status はまだ着手した時点の値である。**`redispatch` の入口の direct chat の検査は当たらない |
...
**この記録は2箇所で消す**（消さないと、次にまた枠が尽きたときに1行も出ない）。

| どこで | 何を消すか |
| --- | --- |
| **この issue の pane を用意できたとき** | その issue の記録（用意の段3） |
| **巡回のたび** | **「候補にいて Status が `direct_chat_state` でない issue」と「候補に1件もいない issue」の両方** |

**片方だけでは足りない。**候補の一覧は `active_states` と `direct_chat_state` しか含まないので、
**`Ready` へ戻した issue は前者で、`In Review` へ抜けた issue は後者でしか捕まらない。**
**候補の取得に失敗した巡回では、後者を走らせない**（走らせると記録が全件消える。
`forgetGatedNotOnBoard` が同じ守りを持っている）。
...
| **バックオフ待ちの run は守らない** | そこでは pane が既に閉じている。**バックオフ中は巡回が Status を取り直さないので、`redispatch` の入口の検査は当たらない**（見ているのは着手した時点の写しである）。**落ちるのは着手の段2 の取り直しで、そこで印が外れる**（3-82c の門1） |
...
（3-82k の設定の検査が起動前に断る）。**この検査を緩めると、この道が壊れる。**
...
**この7つの検査は、3-82j の「再起動をまたいだときの挙動」の2つ目が成り立つ前提でもある。**緩めない。
