...
**枠待ちの印を外す時刻（上の「どの枠の時刻を見るか」）を、この判定に使ってはならない。**
あれは種別を選ばないので、**1週間の枠を待っているのに5時間の枠の時刻で判定してしまう。**
5時間の枠が明けるたびに枠待ちの印が外れるので、**上限に届くのがそのぶん遅れる。**
**判定の軸は「リセットまでの残り時間」である**（人間の表がそう書いている）。

**経過の時計は、枠待ちの印と切り離して持つ。**
印は5時間の枠が明けるたびに外れるので、**印に紐づけると経過が永久に伸びない。**
...
| 何を | 成り立たなかったら | なぜ |
| --- | --- | --- |
| **画面の版が増えていないか** | **手放さない。**枠待ちの印も立てない | **枠待ちの条件その2（hook の無音）は、「枠を待っている」と「長い1つの仕事をしている」を区別できない。**hook はツールが終わってから飛ぶので、**1時間を超える1回のツール呼び出しの最中は1件も来ない。**そこへ `weekly_scoped` が100%だと条件が両方そろい、**正常に走っている run を殺す。**`weekly_scoped` は使っていないモデルでも返る（3-77 の「最初から返ってくる」） |
| **まだ自分が担当か** | **3-77h の「担当が移った」へ倒す。**`after_run` を走らせず、担当者にも触らず、worker を止めて run の登録から外す | **枠待ちのあいだ、担当は自分の意思と無関係に外れる。**`idle_timeout_ms`（既定18時間）は「担当者の最後の進捗報告から」で数え、**枠待ち中は hook が来ないので進捗のコメントも増えない。****3-77c は「担当を外された機械は、その branch へ push してはならない」と決めている。**確かめずに `after_run` を走らせると、利用者が書いた `git push` が別の機械の branch へ飛ぶ |

**どちらも、既存の確かめでは間に合わない。**画面の版を見る段は、stall の評価順で**枠待ちの枝の後ろ**にある。
担当を確かめる段（`handleTurnEnd`）は**turn の終わりでしか走らず、枠待ちの run には turn の終わりが来ない。**

**画面の版を読めなかったときは、手放す側へ倒す**（stall の段3 が「止まったものとして扱います」と同じ向きに倒している）。
**担当を確かめられなかったとき**（issue が見えない・`gh` の持ち主を取れない・コメントを読めない）**は、手放さない側へ倒す。**
止める側へ倒すと、`gh` に一度届かなかっただけで走っている run が捨てられる。

**上限を超えたら何をするか。順番を入れ替えてはならない。**

| 順 | 何をするか | なぜ |
| --- | --- | --- |
| **0** | **外す相手を確かめる**（issue のノード ID と `gh` の持ち主） | **決まらなければ、`after_run` を走らせずに戻る。**`after_run` は「1回だけ走らせる」印を**実行の前に**立て、**印を消すのは着手と片付けだけである。**先に走らせて外す相手が分からなかった場合、**この run が枠明けに完走しても、終了時の `after_run` が印に弾かれて1回も走らない** |
| **1** | **`workspace_hooks.after_run` を走らせる** | **人間の決定「push して引き渡す」をここで果たす。**担当を失ったとき（3-77h）は走らせないが、**あちらは担当が既に別の機械へ移っていて push が衝突する。****こちらは自分から手放す側で、`released` を書く時点では次の担当が決まっていない** |
| **2** | **自分の担当者を外し、`<!-- continuo:released -->` を1件書く** | **失敗したら pane を閉じずに戻る。**逆にすると、外せなかったときに pane だけ閉じて担当が残り、`idle_timeout_ms` のあいだ誰も動かない issue になる |
| **3** | **worker を止める** | pane と空きスロットを占有させない |
| **4** | **run の登録から外す** | **落とすと、その run は登録に残り続けてスロットを永久に埋める** |

**段0 と段2 で戻るときは、確保した「終わらせる最中」の印を必ず戻す。**
**戻さないと、段4 を通らないまま run が登録に残り、`agent.max_concurrent_agents` の枠を
continuo の再起動まで返さない。**段4 の理由欄が名指ししている状態は、**戻ったときにも起きる。**

**「印」を2つの意味で使わない。**この節には別々のものが2つある。
**段4 の「run の登録」**（スロットを持つ側）と、**「枠待ち中」の標識**（stall の時計を止める側）である。
**段4 で外すのは前者だけである。**
...
**判定は、枠待ちの印が立っている run について巡回のたびに行う。**
**待ちループだけに頼ってはならない。**herdr が一時的に届かないとき、
待ちループは印を外さずに goroutine を畳むので、**走っていない窓がある。**
**`claude.turn_timeout_ms` が0以下でも、枠待ちの印は立つ。**
...
**出す条件は付けない。巡回のたびに出す。**既定30秒なので1時間で120行になる。
