...
| **担当先を告げる文で、免除をどこで閉じるか** | **種類で閉じる。**「コードや配布物を変える操作と、リポジトリの設定を変える操作には及ばない」と書く。**そのうえで2つだけ名指しする**（既定の branch へ直に送る push／pull request を通さずにファイルを書き込む形） | **「担当している作業そのもの」という宣言は、リポジトリ X に掛かっていて、操作の種類を1つも区別しない。**閉じる文が無いと、**リポジトリ X が相手なら merge も release も「関係がある→通す」へ倒れる**（すぐ上の「担当先の文では『断る』と言い切る」と同じ理由である）。**綴りで数え上げると、列挙に無いものが「断られる側へ落ちなかった」と読まれる。**2つを名指しするのは、**上で push を肯定の形で通したことの例外で、種類だけでは閉じられない**ためである。**リポジトリの設定を閉じるのは、非公開へ切り替えられると `public_only` の下で判定が1件も載らなくなり、そのリポジトリで走る全部の run の守りが消えるためである** |
