...
**締め切りは、巡回の間隔の2倍待ってから決める**（既定30秒なら60秒）。
...
| **担当者が他人1人＋期限内の hold** | **触らない**（入札もしない） |
| **担当者が他人1人＋期限切れの hold** | **入札をやり直す** |
...
**書くコメントの実物。**勝ったとき1件、延ばすたびに1件。

```
<!-- continuo:hold -->
{"host":"mac-studio","assignee":"octocat-bot-a","until":"2026-09-01T09:45:00Z","at":"2026-08-29T09:45:00Z"}
```
...
| **最初の巡回（30秒以内）** | 担当者は自分なので**入札は起きない。**期限にも余裕があるので hold も書かない |
...
**だから `WORKFLOW.md` の配列で決める。**キーの名前は `obey`（従う）。
