...
| **担当の issue と関係のない外部への書き込み** | 他のリポジトリへの push / パッケージの公開 / 外部サービスへの投稿。**この条件の中で、担当している issue を名乗り、担当先の外への起票を免除する**（3-64f）。**免除の中で、この条件そのものを定義し直してはならない**（例が消える） |
...
**症状**（2026-09-06 の実測）。担当しているリポジトリそのものへ起票しようとしたところ、
判定役は **"an external repository … unrelated to the current work context"** と返した
（**訳:** いまの作業と関係のない外部のリポジトリ。**リポジトリ名は伏せてある**）。
**守っているそのリポジトリを「外部」と呼んだ。**別のリポジトリへの起票も同じ理由で断られ、
**対照に叩いた `gh issue comment <担当の issue の URL>` だけが通った。**
コマンドの中に照合の相手が書いてあるかどうかだけで、通るか断られるかが分かれていた。

**足す文は2つ。**どちらも断る条件の3つ目の中に置く。

```text
- いま担当している issue と関係のない外部への書き込み: 他のリポジトリへの push、パッケージの公開、外部サービスへの投稿
  《担当先を告げる文。識別子が読めたときだけ出す》
  いま担当しているのは <owner>/<repo>#<番号> である。リポジトリ <owner>/<repo> への issue と
  pull request の作成、そのときに付ける本文、pull request の本文の書き込み、
  pull request の draft を外すこと、
  ...
  ただし、送り先が既定の branch だと読み取れる push と、
  branch でない ref（tag など）だと読み取れる push と、ref を消す push と、
  pull request を通さずにリポジトリ <owner>/<repo> のファイルを書き込む形（`gh api` の contents など）は、
  この肯定に含めず、断る。
  リポジトリ <owner>/<repo> の GitHub 上のもの（issue、pull request、ref、release、設定）への書き込みと削除、
  そして GitHub Projects v2 のカンバン（project）への書き込みと削除のうち、
  いま肯定した形に当たらないものは、いま担当している issue に関係していても断る。
  リポジトリ <owner>/<repo> が相手のときは、この段落の扱いが下に続く免除より優先する。…
  《担当先の外への免除。常に出す》
...
**渡す値は `Issue.Identifier` である**（3-13 の `<owner>/<repo>#<番号>`）。取り回すだけである。
**担当先だけでは足りない。**人間が求めた3つのうち、**切り出したい作業の起票と第三者の OSS への報告は、
定義からして担当先の外である。**だから免除をもう1つ置く。
...
| **免除の中で条件を定義し直すか** | **しない** | 「ここで断るのは〜である」のような閉じた定義を書くと、**断る条件の3つ目が挙げている例が、その定義に当たらないものごと消える。**「外部サービスへの投稿」は相手のコードでも配布物でもないので落ちる |
| **免除に限定を付けるか** | **付ける**（「いま担当している作業の中で気づいたことを書く形なら」） | 人間の線は「**処理と関係あるか**」である。**求められた3つは、限定を残しても全部通る。**無条件にすると、その線より広くなる |
| **fork への push を免除するか** | **しない。及ばないと言い切る** | 「fork へ push する形も同じである」と書くと、**任意のリポジトリへの push が「fork への push」と名乗るだけで素通りする。**判定役は、その fork が本当に自分のものかを確かめられない。**本家への PR の経路は「関係のない」という限定が受け持つ**（この変更の前と同じ）。**列挙に無いものが断られる側へ落ちるのを防ぐため、「及ばない」と明記する** |
| **及ばないものをどこまで名指しするか**（担当先の外への免除） | **merge・close・release・ラベルまで名指しする** | 「pull request への書き込み」とだけ書くと、**`gh pr merge --repo <別のリポジトリ> N` が免除に読める。**push でもパッケージの公開でもないのに、**相手のリポジトリのコードが変わる** |
| **担当先を告げる文で、免除をどこで閉じるか** | **肯定した形そのもので閉じ、結論を固定する。**「…**いま肯定した形**に当たらないものは、**いま担当している issue に関係していても断る**」と書く | **「担当している作業そのもの」という宣言は、リポジトリ X に掛かっていて、操作の種類を1つも区別しない。**閉じる文が無いと、**リポジトリ X が相手なら merge も release も「関係がある→通す」へ倒れる**（すぐ上の「担当先の文では『断る』と言い切る」と同じ理由である）。**「〜を理由に通してはならない」と書いてはならない。**それは推論を1本封じるだけで、**結論を固定しない。**`gh pr merge <この issue を閉じる pull request>` は、担当先だからではなく、**その pull request がこの issue を閉じるものだから「関係がある」と言える。****「いま挙げた形」とも書かない。**直前が除外の文なので、**除外したもの自身を指すとも読める** |
| **閉じる文の主語を、どこまで広げるか** | **「リポジトリ X の GitHub 上のもの（issue、pull request、ref、release、設定）への書き込みと削除、そして GitHub Projects v2 のカンバン（project）への書き込みと削除」。****「への書き込み」を2回書く**（1回で済ませると「リポジトリ X の」がカンバンにも掛かると読める） | **「リポジトリ X への書き込み」で止めると2つ落ちる。**GitHub Projects v2 の**カンバンはリポジトリではない**ので `updateProjectV2Field` に届かず（あれは選択肢を全件置き換えて Status の値を全部消す）、**手元の `git commit` や `cat > plan.md` まで含むと読める**（判定へ回るのは Bash なので、worktree の中を書き換えるコマンドは全部届く）。**カンバンを「その issue が載っているカンバン」と限ってはならない。**判定役が受け取るのはノード ID だけで、**それがどのカンバンかは読み取れない。**`GitHub Projects v2 のカンバン（project）` と書けば、**`updateProjectV2` の綴りだけで当てられる** |
| **push の肯定を、どこまで広げるか** | **丸ごと肯定し、コマンドの文字列だけで読み取れる3つを外す**（送り先が既定の branch だと読み取れる push／branch でない ref だと読み取れる push／ref を消す push）。**外したものは「断る」と言い切る** | **判定役が受け取るのは道具の名前と入力だけで、`git push -u origin HEAD` の送り先はコマンドの文字列のどこにも書かれていない。****確かめられないものがどちらへ落ちるかは、実測で「通る側」だった**（2026-09-09。この判定役が載った状態で `git push -u origin HEAD` を何度も叩き、全部通った。**そのときの文面には push の肯定が1文字も無い**ので、送り先を確かめられないまま通したことになる）。**だから、外す側を「読み取れるもの」に限り、読み取れなければ肯定に残す。****そして「この肯定に含めない」で止めない。**肯定から外れるだけで結論が固定されず、**迷いが通る側へ倒れる。****3つとも「読み取れる」で揃える。**`git push origin v1.2.3` は `v1.2.3` という名前の branch を git が許すので、**tag だと決められない。決められないものは肯定に残る** |
| **`gh pr ready`（draft を外すこと）を肯定に足すか** | **足す** | `continuo init` が置く `WORKFLOW.md` の雛形が「draft で作ってからレビューを通して外す」を例示している（[internal/scaffold/template.go](../../internal/scaffold/template.go)）。**足さないと、雛形どおりに書いた利用者の pull request が毎回 draft で止まり、人間が手で `gh pr ready` を打つことになる。****draft を外すことは、その瞬間にコードも配布物も1バイトも変えない。****根拠を組み込みの指示書だけで探してはならない**（`gh label` が0件でもラベルを断ってよい理由にはなるが、雛形の側は見ていなかった） |
| **既定の branch の名前が取れないとき** | **綴りを数え上げた形へ落とす**（`既定の branch（`main` か `master`）`） | 「既定の branch」とだけ書くと、**判定役はそのリポジトリの既定の branch が何かを知らないので、この除外が1件も発火しない。**`develop` は取り逃がすが、**いちばん多い `main` は捕まえる。**取れない経路は、カンバンが `DefaultBranchRef` を返さず（commit が1つも無いリポジトリ）、かつ `herdr.worktree.base` が書いてあるときだけである。**その頻度は測っていない** |
| **肯定した形から、別に外すもの** | **pull request を通さずに「リポジトリ X の」ファイルを書き込む形**（`gh api` の contents） | pull request を通さずに、直 push と同じ結果へ別の道具で届く。**「リポジトリ X の」を落としてはならない。**落とすと、**組み込みの指示書が必ず叩く `cat > plan.md` に文字どおり当たる**（判定へ回るのは Bash なので、worktree の中を書き換えるコマンドは全部届く）。**断られると、設計レビューの判断票が1件も貼られず、CI の検査が赤のまま残る** |
| **肯定の一覧から何を外すか** | **pull request の取り込みと承認、issue と pull request を閉じること。**この3つを「コメントの書き込みに数えない」と1句で外す | **`gh pr review --approve` は、道具の形としては pull request へのコメントの投稿である。**`gh api --method POST .../pulls/<番号>/reviews -f event=APPROVE` は、括弧の中の綴りと一字一句で重なる。**外さないと肯定の一覧に入ってしまい、閉じる文が届かない。****必須のレビューや自動マージを設定した担当先で、人間が1度も見ていない pull request がそのまま既定の branch へ入る。****「却下」の語は使わない**（担当先の外への免除が「却下（close）」と定義しており、同じ語が2段落で違うものを指す） |
| **issue の本文を、どこまで肯定するか** | **作成のときだけ。**pull request の本文の書き換えは肯定に残す | 「その本文の書き込み」と書くと `gh api --method PATCH repos/X/issues/<番号> -f body=…` が肯定に入り、**人間が書いた issue の本文を、判定役に止められないまま置き換えられる。**pull request の側を残すのは、**組み込みの指示書が「pull request の本文にも、その issue の分を1行ずつ足します」と求めている**ためである |
| **担当先の文と、担当先の外への免除が衝突したらどちらが勝つか** | **担当先の文が勝つ。**「リポジトリ X が相手のときは、この段落の扱いが下に続く免除より優先する」と書く | **担当先の文は push を免除し、下の免除は「push …は、この免除に含めない。それらは、いまの作業と関係があるかどうかで、この条件のとおりに判断する」と書く。**この1文だけが、その正面の食い違いを解いている。**落とすと、担当先への push が「関係があるかどうかで判断」へ落ちる。**「下に続く」が本当に下にあることも、検査で固定する（雛形の順序を入れ替えると、上にある免除を「下に続く」と呼ぶ） |
...
| **識別子の形が違うときどうするか** | **担当先を告げる文だけ落とす。免除は残す** | リポジトリ名の入らない条件文を判定役に読ませない。**免除は担当先を参照しないので、一緒に落とす理由が無い**（前方参照を1つも書かない）**。**だから免除には「ほかの」のような相対の語を使わない |
| **他の条件を免除するか** | **しない。どちらの免除でも同じ文の中で言い切る** | 免除の1文だけを読んだ判定役が、他の条件まで通す余地が残る。**とくに2つ目（資格情報の持ち出し）が危ない** |

**足す場所。**囲いの閉じ印より後ろで、断る条件の一覧が始まったあと、
**「判断に迷うものは通す」より前**である。**囲いの直後に置いてはならない**（3-64d と同じ理由。
外部が書いた文字列のすぐ後ろに、許す向きの文が続く形になる）。
**担当先の外への免除は、担当先を告げる文の直後、次の断る条件（`- 権限の昇格`）より前に置く。**

**要約版（[docs/plans/continuo_design_slim.md](continuo_design_slim.md)）は個別に挙げない。**
人間のレビューが要るたびに詳細版から再生成する（[.claude/rules/plan-file.md](../../.claude/rules/plan-file.md)）。
