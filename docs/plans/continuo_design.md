...
**この節を 3-26 の中へ置かなかった理由は、下の「なぜ第6節へ置いたか」にある。**
...
| **2** | あれば書き足し、無ければ `gh issue comment` で1件投稿する |
...
**書き足す行にも pull request の URL を入れさせる。**そのコメントには前の試行の分が残っていることがあり、
**入れないと、読む人はどの pull request の話かを見分けられない。**
...
段2b なら `gh issue comment` が1回。**対象1件につき最大3回である。**
...
**それでも上限を置かない。**3-26 が「グループは多くて数件」と見積もっており、
**グループの大きさを決めるのは continuo の外の人間である**（3-26 の分担表）。
**エージェントが自分で対象を増やせるわけではない。**
GitHub の REST の枠は毎時5000なので、数件×3回はそこへ届かない。

#### なぜ第6節へ置いたか

**3-26 の中へ置くと、設計文書の行番号を指すリンクが20本ずれる。**うち14本は `.claude/` の下にある。
**そして `.claude/` の下は、continuo が起動したエージェントからは書き換えられなかった**
（`Edit` が `Permission to use Edit has been denied because Claude Code is running in don't ask mode` で断られた。実測）。
**禁止の設定があるのではなく、許可されていないので通らない。**

**数え方。**3-26 は 3364行から 3495行までなので、その中へ行を足すと **3496 以上を指すリンクが全部ずれる。**

```sh
git grep -on 'continuo_design\.md#L[0-9]*' -- '.claude/' 'CLAUDE.md' 'docs/' \
  | sed 's/#L/ /' | awk -F: '{print $1" "$NF}' | sed 's/continuo_design\.md //' | awk '$2>=3496'
```

**この節は 9900 より後ろにあるので、この節を足したことでは1本もずれていない。**
**ただし、同じ変更に含まれる 5-3 のブロック**（`internal/prompt/builtin.md` の写し）**は 9160 以降に伸びる。**
**そちらは避けようがない**（一字一句そろえる検査があるため）。**伸びてずれた2本は、その場で直した。**

**第6節は、実装のたびに判断を積んでいく場所である**（6-15 以降がその形）。
**内容の置き場所としても外れていない。**

**3-26 からこの節への案内は置かない。**1行足すだけで上の14本がずれるためである。
**この節が 3-26 を名指ししているので、grep では辿れる。**
