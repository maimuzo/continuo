...
9. それでも書かれなければ failure_state へ落として人間に渡す
   ...
   → **いまの実装は「復元を始められなかった」ときに、警告を1行出して黙って戻る。**
     ...
     **`Done` の枝では、この直後に worktree が片付くので、確かめる先も消える**
```
...
組み込みの 4-2 の照会は3本とも `{{.issue.owner}}/{{.issue.repo}}` に固定されていて、
...
**採る形。**[internal/prompt/builtin.md:147-168](../../internal/prompt/builtin.md#L147-L168) の
...
| **本文は前半と後半のあいだに挟まる**（5-3c） | **打ち消しを受け付けるかどうかは、組み込みの側が節ごとに決める。**3-5 は前半にあるが、**3-4 の例外を使ったときだけ 4-4 へ譲る口を1つ開けてある**（3-78b）。**それ以外の打ち消しは受け付けない** |
