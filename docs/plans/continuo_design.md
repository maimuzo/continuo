...
あれは Status の選択肢名の照合と `gh` の認証の検査である。
**pane を開くのに要るのは herdr と git だけなので、`gh` が一瞬落ちただけで
人間が頼んだ pane を用意しないのは、レートリミットを当てないと決めたのと同じ理由で誤りである。**
...
**これは「返ってきた配列の順序をそのまま使う」（4-2）の唯一の例外である。**
...
| 3 | **空きスロットが無い**（`agent.max_concurrent_agents`） | **WARN で名指しする。ただし1つの issue につき1回だけ。**direct chat の run は自分では終わらないので、上限を上げても次に同じことが起きる。効く直し方は「どれかの issue を direct chat から戻す」である。**毎巡回で出すと、枠が詰まっているあいだ他の WARN が全部これに埋もれる。****pane を用意できたときと、その issue が `direct_chat_state` から出たときに、この記録を消す**（消さないと、次にまた枠が尽きたときに1行も出ない） |
...
| **`Dispatchable`** | 信頼登録の判定を畳み込んだ値なので、先に落とすと `preflight` へ届かず、**直し方のコメントが人間へ届かない**（上の門4）。**draft issue はここを素通りするが、`preflight` の中で worktree の置き場所を決められずに落ちる** |
...
**コメントを読む枠のリセットを、`dispatchCandidates` の中から巡回の頭へ移す。**
いまは `dispatchCandidates` の入口でリセットしている。**direct chat のパスを先に走らせると、
前の巡回で使い切ったままの枠で走ることになり、候補の多いカンバンでは毎回いきなり打ち切られる。**
**出るのは「持ち回りの判定に使うコメントの読み取りが上限に達しました」の INFO だけで、
direct chat の話だとは読めない。**
...
**`leaveDirectChatMode` の中で、確かめ直しの時計を進める。**
**入るときではなく、抜けるときである。**

**入るときに進めても目的を達しない。**確かめ直しは2本あり、入るときの1回で塞げるのは片方だけである。

| 何 | いつ効くか | 入るときに進めると塞がるか |
| --- | --- | --- |
| `handoffLostOnResume` | **1度も確かめていない run だけ** | 塞がる |
| `handoffLostOnTurnEnd` | **`recheck_interval_ms`（既定1時間）を超えたとき** | **塞がらない** |

**2本目が本命である。**direct chat は「いつまでもチャットできる」ためのものなので、
**1時間を超えるのが普通である。**
**入るときに時計を進めても、3時間話して戻せば、戻したあとの最初の turn の終わりで期限切れになる。**
**そこで担当者に continuo のアカウントが無ければ**（direct chat は担当者を1バイトも書かないので、
人間が自分を付けていれば必ずそうなる）**`stopBecauseHandoffLost` が pane を閉じ、push もしない。**
**カンバンにも issue にも1バイトも残らない。**数時間ぶんの会話と未コミットの変更が消える。

**抜けるときに進めれば、両方が塞がる。**戻った直後から `recheck_interval_ms` を数え直すためである。
**そして `leaveDirectChatMode` の呼び出しは1箇所しかない**（`reconcileRunning`）。
**入る側は4箇所ある**（用意し終えたとき／巡回が気づいたとき／turn の終わりに気づいたとき／復元が引き取ったとき）。
**1箇所で済む側に置く。**

**担当者を1人も持たないまま戻ることは、害にならない。**確かめ直しは担当者が0人のとき止めない。
