...
待っている最中に一覧を引けなくなったら、**期限内は待ち直し**（`abandon.waiting_pane_list_failed`）、
期限を過ぎてから `--force` があれば確かめずに消したことを1行言って進み、無ければ止まる
（`abandon.err_pane_list_check` / `abandon.pane_check_skipped`。`stopIfPaneAlive` と同じ扱い）。
**待ち直しの最中に中断されたら、残っている pane の ID を書けない**ので別の文言を出す
（`abandon.err_pane_wait_interrupted_unknown`。空欄にすると pane が0枚だったと読める）。

**期限を見ずに越えてはならない。**herdr が答えなければ、**その待ち自体を1度も行えていない。**
**待たずに越えるのは、手を離させたばかりの pane を、閉じる暇も与えずに消すことである。**
**継続監視がその pane を閉じにいく1周は、まだ回っていない。**
期限を見れば、`--force` の待ち時間は**pane が残っている場合と同じ1本になる。**
