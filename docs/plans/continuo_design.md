...
| **判定の hook** | `claude.tool_gate.mode` の既定は `off` で、**hook を1つも張らない**（3-64）。`public_only` を書いた場合も、掛かるかどうかは issue のリポジトリが公開かどうかだけで決まる。fork への push も本家への PR も待ち時間なしで叩ける |
...
| **`auto` モードにする** | **既定はこちらである。**採らない案として残していたのは、3回連続または累計20回の遮断で一時停止して確認を出すという公式の記述による。**その経路は実機で観測できていない**（3-11） |
