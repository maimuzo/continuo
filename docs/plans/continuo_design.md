...
| `失うものを調べられない` | 足さない（実装に分岐が無い） | 基本フローのステップ21 とステップ24。`Inspect` は git の失敗をエラーにせず直線で通す |
...
### 6-20. `着手を取り消す` の基本フローは、同じ入力を2度見ない

**言いたいこと。**継続監視が動いているか・dry-run か・park を実際に書いたかは、
**入り口で1度だけ判定し、以降のステップは文の中の条件として持ち回る。**
`IF` を9つから3つに減らし、CFG のテストパスは814本から38本になった（2026-08-27 の実測）。

**何が問題だったか。**同じ入力を `IF` で何度も見ていた。

| 何を見ていたか | 何箇所で |
| --- | --- |
| 継続監視が動いているか | 4箇所 |
| dry-run か | 2箇所 |
| 片付けたあとの Status の指定があるか | 2箇所 |

**`IF` は、そこへ至る `IF` の組み合わせを後ろの段へ全部引き継ぐ。**
基本フローを完走するだけで288通りになり、末尾の代替フロー1本が144本の経路を持っていた。
...
| 名前 | どこで決まるか | 真になる条件 |
| --- | --- | --- |
| 手を離させる段を通る | ステップ7 | 継続監視が動いていて、利用者が dry-run を指定していない |
| 手を離させる書き込みを行う | ステップ16 | 上が真で、ボードの Status が `tracker.active_states` に入っている |

例: `システムは VALIDATES THAT 手を離させる書き込みを行っていないか、worktree を作業ディレクトリに持つ pane が期限内に無くなるか、利用者が force を指定している。`

**分岐のまま残すもの。**その分岐でしか決まらない入力を見ているものは `IF` を残す。
片付けの結果と branch の実在（応答が3通りに分かれる）、片付けたあとの Status の指定
（真のときだけボードを読んで書き、代替フローが2本ぶら下がる）、
`worktreeが無い` の `cleanup.delete_branch`（消すか消さないかで事後条件が変わる）の3つである。
...
**代替フローは18本のまま、意味は1つも変えていない。**畳んだ条件はすべてステップの文に残っており、
CFG のノードとエッジは100%網羅されている。テストの印は既存の53件を貼り直し、
**対応する経路が見つからないテストは1本も無かった。**
