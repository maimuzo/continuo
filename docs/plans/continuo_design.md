...
**`gh` の認証が切れたときと、候補の取得そのものが失敗したときは、候補が1件も無いので救わない。**
...
渡すと、5つの関数の引数と1つの構造体の欄を貫き、**そのどこかで否定を間違える余地が残る。**

**`prepareDirectChatPanes` は、候補1件ごとに次の順で見る。**

**まず、何もしないで飛ばす条件を4つ。**上から順に見て、1つでも当たったら次の候補へ移る。

| 順 | どういうときに飛ばすか | 飛ばすときに何を出すか |
| --- | --- | --- |
| 1 | **既に印を持っている** | 何も出さない（巡回の側が direct chat へ入れる） |
...
| 3 | **空きスロットが無い**（`agent.max_concurrent_agents`） | **WARN で名指しする。ただし1つの issue につき1回だけ。**direct chat の run は自分では終わらないので、上限を上げても次に同じことが起きる。効く直し方は「どれかの issue を direct chat から戻す」である。**毎巡回で出すと、枠が詰まっているあいだ他の WARN が全部これに埋もれる。****この記録は2箇所で消す**（消さないと、次にまた枠が尽きたときに1行も出ない）。**1つは、この issue の pane を用意できたとき**（このパスの段7）。**もう1つは、巡回が候補の一覧を取り直したとき、その一覧に載っていない issue の記録を全部消す**（`forgetGatedNotOnBoard` と同じ形。`direct_chat_state` から出た issue は、このパスには二度と現れないので、このパスの中では検知できない） |
...
**用意の段でカードが `running_state` へ書き換えられ、direct chat から外れる。**
...
**ただし「最後に Stop を見た時刻」は、受け口へ流さない門より手前で記録されている。**
**`beginTurn` を通る経路では洗い直されるが、`awaitTurnEnd` の経路は `beginTurn` を通らない。**
**だから、抜けるときにその記録も別に捨てる**（下の「出るとき」）。
...
**`internal/abandon` が、両方を起動前に断る。**
**文面も2つに分ける。**いまは1つの文言を両方で出しており、
