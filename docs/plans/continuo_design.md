...
**代わりに、同じ issue を2台が拾わない仕組みを持つ**（3-75。issue の担当者と、余裕値による入札）。
...
**言いたいこと。**`COLLABORATOR` は「そのリポジトリに書き込み権がある人」だが、
**どこまで信用するかは運用で変わる。**だから `WORKFLOW.md` の配列で決める。
...
tracker:
  provider:
    believe:            # 命令として従ってよい authorAssociation
      - OWNER
      - MEMBER
      - COLLABORATOR
```

**書かなければ既定は `[OWNER, MEMBER]` とする。**`COLLABORATOR` は既定に入れない。
**入れるかどうかを、利用者が自分のリポジトリの運用を見て決める。**
