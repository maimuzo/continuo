...
3つが固定する。`TestToolGate_判定の指示文に読める秘密を混ぜない`（言い切りが本文にあること）、
`TestToolGate_囲いの印は雛形にちょうど1つずつしか無い`（印が1つずつであること）、
`TestToolGate_本文に閉じ印を書かれても指示の範囲が動かない`（本文に閉じ印を書いた入力で、
最後の閉じ印より後ろがこちらの指示のままであること）。
...
数える1行は `grep -c 'author_association: \.author_association' ~/continuo-work/WORKFLOW.md` で、
**当たっていれば `4` が出る**（`gh api` は4本ある）。
