...
**だから、複製が古くならないことは検査で守る。**
`TestMessages_英語の資源が日本語の資源の複製のままである` が、キーごとに文言を突き合わせる。
**日本語の資源に文言を足して複製し忘れると、テストが落ちる。**実際に2回落ちて、2回とも気づけた。

**本物の英語の訳を入れる版では、この検査を入れ替える。**そのとき「複製である」は成り立たなくなる。
