...
**線は1本である**（人間の決定。2026-09-06。issue #173 / #197）。
**「余裕値が0以下」を、入札・枠待ちの印・1週間の枠を待つ上限の3箇所で使う。**
...
**「必ず」ではない。**`rate_limit.pause_above_percent: 100` にすると、
門は「100 を超えた」でしか効かないのに、枠待ちの条件その1 は「100 に達した」なので、
**ちょうど100%のときは枠待ちだけが成立する。**
...
`rate_limit.pause_above_percent: 100` かつ2つのマージンを両方0にすると、
枠が満杯でも入札が通るので、**「着手 → 枠待ち → 上限で手放す」を繰り返す。**
...
| --- | --- | --- | --- |
| **1** | `rate_limit.pause_above_percent` を超えているか | **最後に読めた写し**（3-77i のとおり、古さを見ない） | **やめる** |
