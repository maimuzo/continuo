...
[internal/config/config.go:43-53](../../internal/config/config.go#L43-L53) の `ResolvePath` が
**いまいるディレクトリの直下しか見ないので、WORKFLOW.md へ置くと必ず落ちる。**
...
| 順 | 何をするか |
| --- | --- |
| 1 | `~/.continuo/github-app-refresh-token.json` を読む。**無ければ終了コード 1**（下） |
...
| **鍵と更新用のトークンが在るか** | `~/.continuo/` の2つのファイル。**権限が `0600` かも見る** |
