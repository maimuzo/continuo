...
**`messages/en.json` は、いま `messages/ja.json` の複製である**（3-35b）。
英語の訳文は**日本語版が固まってから**作る。それまで英語を選んでも、画面には日本語が出る。
**`en.json` は、どの時点の `ja.json` を訳したものかを `_source_sha256` に持つ**（3-35b）。
**ずれるとテストが落ちる。**
...
**`LANG=en` の人にも日本語が出る。**訳ができた分から置き換えれば、そのまま本来の形になる。

**`en.json` は「どの時点の `ja.json` を訳したものか」を持つ。**キーが `en.json` に在る以上、
**`ja.json` の文言を直しても日本語へ落ちない。**黙って古い文言が出続ける。**そこを機械で止める。**
`internal/i18n/messages/en.json` の先頭に、文言ではないキーを1つ置く。**書くのは
`ja.json` を直した人（AI）で、`en.json` の訳を見直すのと同じ commit で直す。**

```json
{
  "_source_sha256": "2f1d3c9b1a6106b41f1ceebf6c33d4b36cd906e728165369313e9ff57b5ffb22",
  "abandon.board_not_listed": "issue %s はボードに載っていません"
}
```

**値は `ja.json` のファイルそのものの SHA-256 である**（16進の小文字64文字）。
`shasum -a 256 internal/i18n/messages/ja.json` で出る。**ずれると
`test/internal/i18n/i18n_test.go` の
`TestMessages_英語の資源が訳した日本語の版を記録している` が落ち、
「訳を見直して `_source_sha256` を新しい値に直すこと」と、その計算コマンドを文面に出す。**

**`_` で始まるキーは文言として扱わない**（`i18n.MetaKeyPrefix`）。`Keys()` にも出さず
`Lookup` でも引けない。読むのは `Catalog.Meta("_source_sha256")` だけである。
**扱ってしまうと、宣言したキーとの1対1の検査が落ち、画面にハッシュが出る。**

**訳文どうしを一字一句突き合わせない理由。**その形だと**本物の英語の訳を入れた瞬間に検査を捨てる**
ことになる。**ハッシュなら、訳を入れたあとも「日本語が動いた」ことだけを見続けられる。**

**落とし先の検査には、穴の空いた資源を組む。**`en.json` が宣言したキーを全部持つ以上、
訳の無いキーが1つも無いので、**埋め込んだ資源を相手にすると落とし先を1度も通らない。**
...
### 3-35c. symlink にも hardlink にもできない
...
**止めたいのは「取り消せない操作」と「持ち出し」であり、行儀の悪いコマンドではない。**
...
**コメントは全部取る。**いまは新しい方から50件だけを取っているが（`comments.max` の既定）、
**入札で押し流されると、エージェントが書いた報告が見えなくなる。**
**そもそも、全部読まずに作業を進めるべきではない。**
...
| --- | --- |
| **担当者が無い** | 入札する |
| **担当者が自分1人** | 着手・引き継ぎへ進む。期限が近ければ延ばす |
| **担当者が他人1人＋hold あり＋最後のコメントが18時間以内** | **触らない**（入札もしない） |
| **担当者が他人1人＋hold あり＋最後のコメントから18時間超** | **担当を外して入札をやり直す** |
| **担当者が他人1人＋hold が1件も無い** | **触らない。**人間が付けた担当である |
...
### 3-77d. 持ち回りを作るときに、設計だけでは決まらなかった5つ

**言いたいこと。**3-77 から 3-77c までを実装に落とすと、**そこに書かれていない場面が5つ出る。**
...
| 勝った機械が担当者を書かないまま消えた | **締め切りからさらに `bid_window_ms` で入札をやり直す** |
...
**入札に期限を置く理由。**勝った機械が担当者を書く前に落ちると、**その入札は issue に残り続ける。**
以後どの機械が読んでも同じ機械が勝つので、**issue は誰にも着手されないまま止まる**
（担当者がいないので hold の期限も効かない）。**締め切りからさらに `bid_window_ms` を過ぎた
入札は数えない。**次に見た機械が新しい入札を書き、そこから次の回が始まる。
...
language: auto                              # 画面に出す文言の言語。auto なら環境変数 LANG から決める。
                                            ...
                                            # **この版では、何を書いても画面には日本語が出る。**
                                            # 英語の資源は日本語の資源をそのまま複製したもので、
                                            # 英語の文章はまだ 1 文字も無いためである。
                                            # **日本語で使い続けるなら ja と書いておくこと。**英語の訳が
                                            # 入った版に上げても、日本語のままになる
---
```
