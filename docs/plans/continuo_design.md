...
**`continuo` とは何か。**GitHub Projects v2 のカンバン1枚を見張り、`Ready` の issue ごとに worktree を用意し、herdr の pane で Claude Code を対話モードで起動して作業させ、完了までを面倒見る**常駐プロセス**である。Go で書く。名前は**通奏低音**（basso continuo）に由来する。
...
| **未完了なら再投入される** | worker が Status を変えずに終了しても、issue がまだ active なら再度呼び出されること | turn ループ（3-8）と、turn の終わりごとのカンバンの取り直し（3-5）。**worker の正常終了を完了と見なさない** |
...
| **完了の真実の源はトラッカー** | **完了したかどうかを判断するのはエージェント。**カンバンへ書き込むのは continuo のコード（3-25）。continuo は自分の判断で勝手に完了させない | `SPEC.md` 11.5 / 7.1 |
...
- 4-2 実行順序 — カンバンの並び順は使えるが、カンバンの設定変更が前提になる
...
**Priority は使わず、カンバンの並び順だけで順序を決める**（4-2）。
...
### 2-6. カンバンの自動化が動かした Status は、`actor` の型でしか見分けられない
...
**実測（2026-08-26）。**一時カンバン（`<ACCOUNT>` の project #11。実測後に消す前提で作った）で、
`octocat/hello-world#1` に `Closes #1` を書いた PR を1本作り、timeline を引いた。
**本番のカンバン（project #3）と実機確認用のカンバン（project #10）には書き込んでいない。**
...
**応答の原文**（`project.number` で自分のカンバンに絞れることも同時に確かめた）。
...
**新しいカンバンは自動化6件がすべて有効な状態で作られる。**`gh project create` で作った project #11 の
...
**`project.number` で自分のカンバンに絞る必要がある。**1つの issue が複数のカンバンに載っていると、
他のカンバンのイベントが同じ配列に混ざって返る（実測でも #10 と #11 の両方が返った）。

**カンバンの自動化の有効・無効は API から変えられない。**GraphQL の mutation は
...
**カンバンに書く経路は4つある。continuo・エージェント・人間・人間に代わって働く道具である。**
**4つ目が何かは 3-30 に書いてある**（人間の指示で GraphQL を叩くエージェントやスクリプト。continuo 本体ではない）。

**「continuo はカンバンを読むだけ」ではない。**そう読める要約を先に書いていたが、**仕様の読み違いだった。**
...
3. 身元ファイルの project item の ID で、カンバンを ID 指定でまとめて取り直す
...
| **タスクが完了したか** | **カンバンの Status が `terminal_states` に入ったこと** | 人間（`In Review` から `Done` へ動かす） |
...
第3段階のアダプタを作り直すことになり、コストも増える。**選択肢名が変わるのは人間がカンバンを触ったときだけなので、
...
カンバンの組み込みの自動化が PR のマージで `Done` を書く運用で、実際にこの形になった
...
| **どこにあるか** | **continuo のプロセスのメモリの中だけ。**ディスクにもカンバンにも書かない |
...
| 項目 | 決定 | 理由 |
| --- | --- | --- |
| **issue の一意な名前** | **`<owner>/<repo>#<番号>`**（例 `octocat/hello-world#188`） | 1枚のカンバンに5リポジトリが載るので `#188` だけでは一意でない。`SPEC.md` 4.1.1 は名前空間をまたぐ場合の曖昧さ解消を MUST としている |
...
| **Status の大文字小文字** | **比較は大文字小文字を無視する。表示はカンバンの綴りをそのまま保つ** | `SPEC.md` 5.3.1 / 11.3 |
...
2. カンバンの Status を tracker.running_state（既定 In Progress）へ書く   ← 外部に残る最初の段
...
| --- | --- |
| 書けた | カンバンは `failure_state` にある。それが候補（active_states）に見えたら人間が動かしたということである。**索引の遅れを踏むので60秒は信じない** |
...
`In Progress` を active_states に入れている（3-10）ため、カンバンの Status も排他の役に立たない。
...
| project item の ID | カンバンを ID 指定で取り直すため（1リクエスト 1 point） |
...
**採る形。再起動の直後に、引き継いだ run すべてについて、hook を待たずにカンバンを1回取り直す。**
...
| **判断の結果をカンバンへ反映すること**（`gh` のコマンドを正しく組み立てて実行する） | **できる。**continuo が Go のコードで実行すればよい |
...
**表明を受けて continuo がカンバンへ書き込んだら、何から何へ動かしたかを issue に残す**（3-29）。
...
FetchIssueByIdentifier(ctx, "octocat/hello-world#45") → (Issue, bool, error)
  → カンバンの item を Status で絞らずに取り、identifier が一致する1件を返す。
...
**104件のカンバンで多くて2リクエスト・計 8 point である**（5-3f と同じ値。3-31 の式から計算した値であって、実測ではない）。
...
| **コスト** | **1件につき2リクエスト・計 8 point**（104件のカンバン）。**グループは多くて数件なので、3-31 の見積りを崩さない** |
| **カンバンに載っていなかったら** | **その行を捨て、issue のコメントに「カンバンに無いので動かせなかった」と書く**（人間が気づけるようにする） |
| **別のリポジトリだったら** | **Status は動かす。**worktree が違うだけで、カンバン上の item は同じカンバンにある |

**なぜこれが確実になるのか。**

| プロンプトで `gh` を実行させる場合 | 最終応答に1行書かせる場合 |
| --- | --- |
| コマンドの綴り・引数・カンバン番号を正しく組み立てる必要がある | **決まった1行を書くだけ** |
...
| **2** | エージェントが自分で `gh` を叩いていた場合は、それを尊重する。**continuo は書く前に必ずカンバンを取り直すので、既に動いていれば書かない**（3-4） |
...
| カンバンが `terminal_states` になっていた | **走らせる。**片付ける前に確認する |
...
それを**代表となる issue のコメントに書いて**一緒に対応できるようにしてから、カンバンの並び順を入れ替える。
...
| **continuo の外** | カンバンの並び順で代表の issue を前へ動かす（3-30 / 4-4） |
...
| 制約 | なぜ |
| --- | --- |
| **表明で指せるのは、カンバンに載っている issue だけ** | 載っていない issue の Status は動かせない。**指定されたらログに残して無視する** |
...
**条件「複数カンバン監視」は凍結されている。**当面 project #3 の1枚だけを使う。
...
| 何を | どうするか |
| --- | --- |
| **設定の形** | **いまは変えない。**`tracker` は写像のままで、カンバンは1枚だけ書ける。**凍結が解けたときに `trackers`（複数形）というキーを足し、両方を受理する。**その時点で第8節に差分を足す |
| **識別子** | **いまは `<owner>/<repo>#<番号>` のままでよい**（3-13）。カンバンが1枚なので曖昧さが無い。**凍結が解けたら、どのカンバンのものかを足す必要がある** |
| **巡回とトラッカーの扱い** | **トラッカーを扱うコードを「カンバン1枚ぶん」の単位で書く。**プロセス全体で1つしかない前提のコードにしない。**こうしておけば、増やすときに呼び出し側だけを変えれば済む** |
| **同時実行の上限** | `max_concurrent_agents` は**プロセス全体の上限**である（カンバンごとではない）。原典もそう定めている。**この点は増えても変わらない** |

**やってはいけないこと。**原典は「**1枚のカンバンをリポジトリで絞り込んで使い分ける方式は不可**」と明示している。
**この方式で複数カンバン監視を満たしたことにしてはならない。**
...
**言いたいこと。**カンバンの Status を書くのは continuo であって、エージェントではない（3-25）。
...
**`gh` の持ち主とカンバンのトークンが別のアカウントのときだけは外れない。**その条件と何が起きるかは 3-65 にある。
...
**言いたいこと。continuo はカンバンの並び順を読むだけである。書き換えない。**
**並び順を決めるのは continuo の外であり、人間がカンバンの画面でドラッグするか、人間に代わって働く道具が API で並べる**（4-4）。
...
これは**カンバンを並べるときの指針**であって、continuo が実行する規則ではない。
...
| **書き戻しの対応表のキーがカンバンにあるか** | **`tracker.automated_state_rewrite` のキーが、カンバンの Status の選択肢にあるかを見る**（3-57） | **記号は `!`。**キーはカンバンに実在しなくてよい。**綴りを打ち間違えた行は一度も効かないのに、起動時の警告は doctor に出てこない**（doctor は tracker のログを捨てる）。**紛らわしさの検査では拾えない**（`In Progres` と `In Progress` は「同じ綴り」でも「含む」でもない） |
| **雛形にある設定項目が全部書かれているか** | **雛形（`internal/scaffold/template.go`）の front matter のキーが `WORKFLOW.md` に全部あるかを見る**（3-75） | **記号は `!`。**カンバンを1バイトも読まない。**版を上げて増えた項目は、リリースノートを読まないかぎり存在に気づけない。**足りない項目の名前と、差分を読むコマンド・当てるコマンドを出す |
...
#### 対象のリポジトリはカンバンを読んで決める

**`<owner>/<repo>` は設定に書かれていない。**対象のリポジトリの集合は、**カンバンに載っている issue から決まる。**

| 何を | どうするか |
| --- | --- |
| **どう集めるか** | **doctor がカンバンを1回読み**（`active_states` の候補）、返ってきた issue の `nameWithOwner` を重複なく集める |
...
> 3-6 が言っているのは**常駐プロセスの起動時**の話で、カンバンを読む前だから検査できない。
> **`doctor` は人間が明示的に叩くコマンドなので、カンバンを読んでよい。**
...
設定ファイル ─┬─ 片付けの状態（設定の2つのキーを突き合わせる。3-9e）
              ...
              └─ gh の認証 ── カンバンを読める ─┬─ Status の名前（選択肢名を照合する）
...
| **信頼登録** | **`!`** | 同上 |
| **終了コード** | **影響しない。**`!` だけなら 0 を返す | カンバンが空なのは設定の誤りではない |
...
### 3-32c. カンバンは organization にもある

**言いたいこと。**`gh api user` はログイン名しか返さない。**organization に置いたカンバンは、
...
| 順 | 何をするか |
| --- | --- |
| 1 | `gh api user` でログイン名を引き、そのカンバンを探す |
| 2 | **0件なら** `gh api user/orgs` で所属する organization を引く |
| 3 | organization ごとに `gh project list` を試す（**1つ失敗しても残りを探す**） |
| 4 | 候補が1件に決まったら、**owner をそのカンバンの持ち主に決め直す** |

**owner を決め直さないと、どこにも存在しない組み合わせが書かれる。**
`project_number` は organization のカンバンを指すのに、`owner` はログイン名のまま、という状態になる。
...
**なぜ出すか。**ログイン名のカンバンがちょうど1件だけあると、gh から引いた別のカンバンの
...
**対象は `trust.repositories` に人間が書いたものだけである。カンバンから自動で集めて登録しない。**
...
| 条件 | 何をするか |
| --- | --- |
| **列挙されたものだけを対象にする** | `WORKFLOW.md` の `trust.repositories` に書かれた `owner/repo` だけ。**カンバンから拾った一覧をそのまま登録しない** |
...
**なぜ列挙が要るか。カンバンは他人が編集できる。**issue を足せる人は、カンバンに載るリポジトリの
集合を変えられる。**そこから自動で登録すると、issue を足せる人が「continuo に信頼させる
リポジトリ」を増やせてしまう。**`continuo init` はカンバンから拾った一覧を `WORKFLOW.md` に
**並べるだけ**で、**要らない行を消すのは人間である。**

> **拾えるのは「そのときカンバンに載っていたリポジトリ」だけである。**
> **これから issue を作るリポジトリは、まだカンバンに無いので1件も入らない。**
...
**採らなかった案。**`continuo doctor --trust-repos` で、カンバンから引いた対象リポジトリを
一括で登録する。**カンバンに載っているだけのリポジトリを、人間が中身を見ずに信頼することになる。**
...
**言いたいこと。**カンバンを新しく作る手順は、主たる道ではない。
**continuo は「既にあるカンバン」に後から足して使うものである。**
...
| 絞り込みのキーの解決に失敗 | **大半**（条件ごと無視されるので、カンバンのほぼ全件が返る） | `CategoryResponse` で巡回を落とす |
...
数えている最中に人間がカンバンへ item を足すと合計が1件ずれ、差で見ると誤検知する。
**item が0件のカンバンでは判定できないので検査を飛ばす。**
...
| 運用中のカンバンに後から足す | 既存を使う | 足りない選択肢を画面で足す |
| 1つのリポジトリだけを回す | 既存を使う（無ければ新規1枚） | 同上 |
| organization のカンバン | 既存を使う | `--owner <org>` を指定する |
| **使い捨ての検証** | 新規に作る | 下記 |
...
| `gh` | カンバンを読めない | `brew install gh` / apt は GitHub の手順 |
...
**言いたいこと。**着手の取り消しは、カンバンの操作だけでは作れない。
...
| 段 | 何をするか |
| --- | --- |
| 1 | `lock.Acquire` で continuo が動いているかを調べる（3-17）。**取れたロックは実行の最後まで握る。**動いていて、**かつ `--dry-run` でなければ**、カンバンの Status が `tracker.active_states` に入っているときに `--park`（既定 `tracker.failure_state`）へ動かして手を離させる。そのうえで**その worktree を cwd に持つ pane が消えるまで待つ。**上限は `herdr.read_timeout_ms` の10倍（既定50秒）で、**超えたら何も消さずに止まる** |
| 1 の後 | **書き込みが通ったら、持ち回っている Status もその値に更新する。**カンバンは1回しか読まないので、更新しないと段3 の計画表示に park の**前**の値が出る（これから消す worktree の issue が「まだ作業中」に見える） |
...
| 2 の後 | **これから書きうる Status の値を確かめる**（3-37-5）。`--to` と park の先がカンバンの選択肢にあるか、park の先が `tracker.active_states` に入っていないか。**読むだけなので `--dry-run` でも通す** |
...
「何も消していません」だけだと、**カンバンも元のままだ**と読まれ、その issue が置き去りになる。
**continuo は元へ戻さない。**戻す先は `tracker.active_states` の値なので、**戻した瞬間に
動いている継続監視がその issue を拾い直しうる。**戻すかどうかは人間がカンバンで決める。
...
**片付けたあとにその issue をどう扱うかは、カンバンの外にある事情で決まる。**
...
**カンバンが `Blocked` になったことを誰も言わないまま終了コード 0 で終わる。**
...
**言いたいこと。**通ると、**見せるだけのはずの実行がカンバンを書き換え、エージェントに手を離させる。**
...
**予告が要る理由。**`--dry-run` はカンバンへ1文字も書かないので、
...
カンバンの Status が `tracker.active_states` の外なら手を離させる書き込みは入らず、ここへ落ちる。
...
| 確かめること | 確かめないと何が起きるか |
| --- | --- |
| `--to` と park の先がカンバンの Status の選択肢にあるか | `updateProjectV2ItemFieldValue` は選択肢に無い名前を断るが、**それを呼ぶのは worktree を消したあとである** |
...
**照合はカンバンを読むだけで、1文字も書かない**ので `--dry-run` でも通す。
...
**付けて叩き直した実行は待たない。**1回目がカンバンを park の値（`failure_state`）へ
...
**カンバンを park の値へ動かし終えている。**越えられないと、**カンバンだけ動いた状態のまま、
...
**言いたいこと。**拒否リストでは守れない。**カンバンの Status は人間が自由に増やせる**ので、
...
人間からは「カンバンに置いた印が勝手に戻される」ように見える。
...
**herdr が一言も言っていない「agent が待機状態になった」というコメントが本番のカンバンへ投稿される。**
...
どれも候補の取得（`items(...)`）が最初から返さないもので、カンバンとしては正常である
（本番のカンバンは104件中4件が Status 未設定）。
...
| カンバンの issue | 上の2つで作った `<owner>/<repo>#<番号>` で1件だけ引き直す | — |
...
**既定を `stop` にする理由。**飛ばして走り続けると、その issue はカンバンの上で
...
**起動時に1回だけ、知らない選択肢を名前で出す。**起動時の照合は「設定の名前がカンバンに在るか」の
一方向だけであり、**カンバンにあって設定に無いものは件数（`status_options=11`）にしか出ない。**
...
カンバンが動いていないので、書けば嘘の記録になる。

**設定で選べるようにはしない。**同じ値を書きに行きたい場面が1つも無い。

---

### 3-54. カンバンの自動化が動かした Status では worker を止めず、本来の Status へ戻す

**言いたいこと。**エージェントが PR を作ると、カンバンの組み込みの自動化が Status を動かす。
...
**人間はカンバンを見て状況を判断するので、列を分けた意味が消える。**
...
**`project.number` で自分のカンバンへ絞る**（複数のカンバンに載っていると両方返る。設計 2-6）。
**窓は `last: 50` である。**カンバンで絞る引数が無いので絞るのは返ってきたあとであり、
**別のカンバンで Status が何度も動くと自分のカンバンのイベントが窓から押し出される**（書き戻しが効かなくなる）。
...
| **カンバンの自動化を切る** | 利用者に `Workflows` を無効化してもらう | **カンバンは人間も使っている。**continuo を入れることが、人間の設定を変える理由になってはならない |
...
**対応表は設定に持たせる。**カンバンごとに Status の名前が違うので、コードに埋め込まない。
...
**名前をカンバンと照合する範囲は 3-57、人間へ出す案内の出し方は 3-57b にある。**
...
組み合わせがあると、continuo とカンバンが同じ issue を押し合い続ける。
**上限に達したら猶予を置いて worker を止める**（押し合いを人間へ渡す）。

**数えるのは、カンバンが実際に動いたときだけである。**通信の失敗・item が見えない・
...
取ったときに片方の失敗がもう片方の枠を返し、**カンバンが動いた回数が上限を超える。**
...
**これが起きるのは、人間がカンバンから戻す先の選択肢を消したときである**（起動時の照合は通っていた）。
...
数えると「カンバンから戻す先の選択肢が消えている」という的外れな案内が人間へ出る
...
| 書き込みの結果 | 次にどうするか |
| --- | --- |
| 書けた／既にその値だった | カンバンは戻す先である。**戻す先は `active_states` に限られる**（3-55）ので、次の turn へ |
...
### 3-57. 対応表のキーはカンバンに実在しなくてよい

**言いたいこと。**`automated_state_rewrite` のキーを起動時にカンバンと照合していたので、
**カンバンの自動化をやめて選択肢を消した人が、continuo を二度と起動できなくなっていた**（issue #67）。
...
| 採らなかった案 | 否定根拠 |
| --- | --- |
| 照合は続け、失敗の文言に「対応表のその行を消してください」を出す | 文言が正しくなっても、**設定を書き換えて再起動するまで continuo は動かない。継続を人間のカンバンの設定変更に依存させない**（3-54 の同じ判断） |
...
| 関数 | 何の一覧か | 対応表のキー |
| --- | --- | --- |
| `config.KnownStates` | 実行時に「知っている Status か」（`knownStates`）。**起動時にカンバンへ実在を要求するのもこれ** | **入れない。**入れると書き戻しの分岐が二度と通らない |
| `config.NamedStates` | カンバンの選択肢が設定に出てくるか（`unknownStatusOptions`） | **入れる。**キーは人間が WORKFLOW.md に書いた名前なので、「知らない Status」に数えない |

**`config.NamedStates` にキーを入れる理由を「キーの Status では worker が止まらないから」にしない。**
**書き戻して worker を続けるのは、カンバンの自動化がその Status を書いたときだけである**（3-54）。
**人間がキーの Status へ動かしたときは、いままでどおり worker を止めて人間へ渡す**（3-50）。

**`config.KnownStates` が1つも返さない設定は、組み立ての時点で弾く**（`orchestrator.New`）。
1つも無いと、continuo は**カンバン上のどの Status も「知らない Status」と判定し、着手した run を
...
**揃えられない2箇所には、その理由をコードのコメントに書いた**（CLAUDE.md の「ファイルの書き換えは「一時ファイルへ書いてから差し替える」」）。
...
| **担当の issue と関係のない外部への書き込み** | 他のリポジトリへの push / パッケージの公開 / 外部サービスへの投稿 |
...
#### `gh` の持ち主とカンバンのトークンが別だと、continuo 自身の投稿も第三者に見える

**言いたいこと。**照合が見るのは**投稿者と `gh api user` の名前**だけで、
**カンバンを読み書きするトークンが誰のものかは見ていない。**
...
**いまは直さず、条件をここに書いて留める。**カンバン用のトークンの持ち主も取れば直るが、
...
**着手の承認は Status が担う。**カンバンは非公開で、`Ready` へ動かせるのは維持者だけである（6-23）。
...
| `tracker.automated_state_rewrite` | カンバンの Status 名 |
| `agent.max_concurrent_agents_by_state` | カンバンの Status 名 |
...
| 何 | 識別子の出どころ | なぜそこか |
| --- | --- | --- |
| **入札** | **コメントの投稿者**（GitHub が付ける `author.login`） | **同じ事実の出どころを2つ作らない。**本文にも書くと、自分で名乗った値と投稿者の2つができる。**本文は第三者にも書けるので、他の continuo のログイン名を騙られると、騙られたほうは `HasBidBy` が真になって、その回は入札しない** |
...
**期限の内は他の機械が触らない。切れて初めて入札をやり直す。**カンバンに新しい欄は足さない。
...
**止め方。****カンバンへも issue へも1バイトも書かない。**Status を動かすと、
**新しい担当の機械が着手しようとしているカンバンを、外された機械が横から書き換える。**
...
**候補の多いカンバンでは、そのままだと巡回1回が返らなくなる。**上限を4つ置く。

| 何を | どうするか | 置かないと何が起きるか |
| --- | --- | --- |
| **入札できない機械の読み取り** | 枠の判定を**コメントを読む前**に行い、入札しないと決まったら読まない | 黙ると分かっている issue のコメントを、候補の数だけ読む |
| **1回の巡回でコメントを読む issue の数** | **10件まで。**使い切ったらその巡回を打ち切る | 入札に負ける機械は空きスロットを埋めないので、候補の最後まで止まらない。104件のカンバンで数百リクエストになる |
...
**打ち切っても取りこぼさない。**候補はカンバンの並び順で来るので、**上から順に見ることは保たれる。**
続きは次の巡回で見る。
...
| 何を | 走らせるか |
| --- | --- |
| **カンバンの Status** | **書かない。**新しい担当の機械が着手しているカンバンを横から書き換えることになる |
...
### 3-82a. 投稿者を変えない経路を採る理由と、外で走るセッション

**言いたいこと。**投稿者は continuo の中で**身元そのもの**として使われている。
**そこを動かすと、印の照合も、持ち回りの入札も、CI の門も同時に作り直すことになる。**
...
| 何が投稿者を見ているか | どこ |
| --- | --- |
| **印を信じてよいかの判定** | 3-65。[internal/tracker/adapter.go:1156](../../internal/tracker/adapter.go#L1156) |
| **持ち回りで機械を見分ける識別子** | 3-77-0。[internal/handoff/assess.go:381](../../internal/handoff/assess.go#L381) |
| **レビュー結果を数える門** | [.github/workflows/review-gate.yml:162](../../.github/workflows/review-gate.yml#L162) |

**比較した案。****本文の先頭へ `<!-- continuo:ai -->` のような印を書き込む形。**
**採らない理由は、HTML のコメントが GitHub の画面に出ないことである。**
**人間が画面を上から読み返すときに見えない。**見える文字列にすると本文が汚れる。
**GitHub App のバッジは GitHub が投稿者の隣に描くので、本文へ何も足さずに画面へ出る。**
...
**それでも、3-82j と同じ `case` の形を書けば足りる**（テンプレートは展開されないので、値を手で埋める）。
**素の `GH_TOKEN=$(continuo github-app token) gh …` を書いてはならない**（理由は 3-82c）。
...
| どの書き手か | 成り立つか |
| --- | --- |
| **continuo 本体**（Go の投稿） | **成り立つ。**取れなければ止まる（3-82g） |
| **continuo が起動したエージェント** | **指示書に従う限り成り立つ。**従わなければ成り立たない |
| **外で走るセッション** | **成り立たない。**continuo は関与しない |

**道具の門で塞ぐ案は採らない。**`PreToolUse` の口は既に在るが
（[internal/orchestrator/settings.go:363](../../internal/orchestrator/settings.go#L363)）、**hook の挙動を変えるので人間の確認が要る。**follow-up の issue にする。
**文書で案内する**（[docs/FAQ.md](../FAQ.md)）。

### 3-82b. 秘密鍵は保存しない。App の権限は `Issues` だけにする

**言いたいこと。****この設計は秘密鍵を1度も使わない。**だから保存しない。
**保存しなければ、漏れても App 自身として動くトークンを作られない。**
**ただし、これで安全になるわけではない。**置く資格情報が漏れれば、**約6か月のあいだ同じことができる**（3-82h）。
**守りの本体は権限を `Issues` の読み書きだけにすることである。**

**秘密鍵が要るのは、App 自身として動くトークン（installation token）を作るときである。**
**この設計はそちらを採らない**（3-82e）。
採るのは人間の代理として動くトークンで、**`client_id` と `client_secret` と更新用のトークンの3つで作れる。**
**秘密鍵は1度も登場しない。**
...
| 何 | 置き場所 | 権限 |
| --- | --- | --- |
| **App の資格情報** | `~/.continuo/github-app-credentials.json` | `0600` |
...
| 権限 | なぜ要るか |
| --- | --- |
| **Issues: Read and write** | issue のコメントを書く。**この設計が使う唯一の書き込み** |
| **Metadata: Read-only** | GitHub App が必ず持つ。外せない |

**`Pull requests` も `Contents` も入れない。漏れたときに何ができるかが、そのまま権限の一覧だからである。**
`Issues` だけなら **issue へコメントを書かれるだけ**で済むが、`Pull requests: write` を足すと
**pull request を作られ、書き換えられ、マージされうる。**
...
新しいアクセストークンを取り、**返ってきた新しい更新用のトークンと、その新しい期限を書き戻してから**標準出力へ出す。
**期限を書き戻さないと、最初の認可から181日目に、資格情報が生きているのに全部止まる。**
**GitHub は更新のたびに新しい6か月の期限を返す。**
**doctor は `✗`、巡回は WARN、このコマンドは終了コード 1 を返す。**
**資格情報は有効なので、人間は原因に辿り着けない。**
**逆にすると、配ったあとに次回が壊れる。書き戻しは一時ファイルからの `os.Rename` で行う。**

#### 二、毎回叩く危険と、それを減らせない理由

**毎回 GitHub を叩くことの危険を、そのまま引き受ける。**
**1つの run で叩く回数は、新しく投稿する数だけである。**
**書き足し（`PATCH`）では叩かない**（3-82j）。**進捗報告は2回目以降が書き足しなので、1時間ごとには増えない。**
**18時間の run 1本で3回前後、2本同時なら1日に6回前後である。**
**continuo 自身の投稿は、この数に入らない。**あちらは常駐しているので、
**取ったトークンを期限までメモリで使い回し、8時間に1回しか回さない**（3-82i）。
**このコマンドは1回叩かれて終わるので、持ち回す先が無い。**
**更新用のトークンは1回使うと無効になるので、叩くたびに入れ替わる。**
**書き戻しの直前で落ちると、古いものは死んでいて新しいものは保存されておらず、認可のやり直しになる。**

**この危険は、エージェント側では減らせていない。**
**続く節（3-82i）が「メモリで使い回す」と決めるのは continuo 本体の投稿だけで、
上に数えた7本と進捗報告には1つも効かない。**
**回転の回数を支配しているのはエージェント側であり、そこは無対策である。**
**減らす案は2つある。どちらも採らない。**

| 案 | なぜ採らないか |
| --- | --- |
| **アクセストークンを期限つきでファイルへ書き、8時間に1回しか回さない** | **人間の決定（2026-09-08）「このアクセストークンはファイルには出力しない」と衝突する** |
| **常駐 continuo が持っているトークンを、専用の unix socket 経由で配る** | **ファイルへ1バイトも書かず、回転は8時間に1回になる。**ただし**常駐していないと取れない。**エージェントは常駐 continuo が起動するので成り立つが、**`continuo github-app token` を人が手で叩く経路が死ぬ。**hook の socket とは別に1本立てることになり、**hook の4つの約束のどれにも触れないが、実装するならその判定を pull request の本文へ書く必要がある**（[CLAUDE.md](../../CLAUDE.md)）。**この設計では持たない。**採るなら別の issue にする |

**採らない以上、この危険はそのまま残る。**書き戻しの直前で落ちれば、認可のやり直しになる。
...
**待つ長さは、実測してから決める。**1回の更新は GitHub との往復1回で済む。

**取り合う主体の数は、`--id` で変わる。**

| どう動かしているか | 取り合う主体 |
| --- | --- |
| continuo 1本（既定） | **3つ**（本体1＋エージェント2） |
| **`--id` で2本** | **6つ**（本体2＋エージェント4） |

**資格情報は人間1人につき1つなので、`~/.continuo/` のロックは全部で1本を取り合う。**
**`--id` で2つ動かしている場合、片方のプロセスは他方の設定を知らない。**

**待つ長さは、このコマンドの中の定数で持つ。**
**`agent.max_concurrent_agents` から計算してはならない。**
**このコマンドは設定を読まないので、その値を引けない**（この小節の末尾）。
**設定を読む形にすると、エージェントの worktree から WORKFLOW.md を探すことになり、3-82b の決定と衝突する。**

**上限は「往復の実測 × 8」とする。**
**8 は `agent.max_concurrent_agents` の既定 2 の4倍で、`--id` を2つ使っても足りる。**
**測ったら値を直す。**

**上限に達したら、待つのをやめて終了コード 1 を返す。**
**`agent.max_concurrent_agents` を8に上げた利用者では、取り合う主体が9になって上限を超えうる。**
**そのとき投稿できずに run が人間へ渡るが、それは 3-82g が決めた形と同じである。**
**標準エラーへ「ロックを取れませんでした。同時に走るエージェントを減らすか、時間を置いて叩き直してください」と出す。**
**負荷が高いときだけ起きるので、この一言が無いと再現しない故障として残る。**

**実測**（2026-09-09。専用の App で web flow を1回通した）。

| 何 | 実測値 |
| --- | --- |
| アクセストークンの期限 | **28800秒（8時間）** |
| 更新用のトークンの期限 | **15638399秒（約181日）** |
| トークンの頭 | `ghu_`（人間の代理として動くトークン） |
...
**`false` の利用者は、そもそも指示書の枝がこのコマンドを呼ばない**（3-82j）。

**呼ぶ側は、返ってきた文字列の形を `case` で確かめてから使う。**

    TOKEN=$(<実行ファイルの絶対パス> github-app token)
    case "$TOKEN" in
      ghu_*) ;;
      *) echo "トークンを取れませんでした" >&2; exit 1 ;;
    esac

**空かどうかで見てはならない。**
**この作法は、この設計が発明したものではない。**
[internal/prompt/builtin.md:423-425](../prompt/builtin.md#L423-L425) が
**「`case` で印そのものを確かめてから書き込みます。中身が空でないかを見るだけでは足りません」**と、
**別の理由**（`gh api` が失敗したときにエラーの JSON を標準出力へ出す）**付きで既に出荷している。**
**通すのは `ghu_` だけにする。**`ghs_` は 3-82e が採らないと決めた側である。
**空だったときにどうするかは、設定で決まる**（3-82g）。
**印を付けると決めた設定なら、投稿せずにエラーで止める。**
...
| 何を | どのトークンで |
| --- | --- |
| **カンバンの読み書き**（Status・並び順・担当者） | **いままでのまま**（`tracker.provider.token_source`） |
| **issue へのコメントの投稿** | **App の user-to-server token** |
...
### 3-82f. App の作成と install は、押す前に画面で説明する
...
**利用者が取り違える。**どちらを打ったのかを、打った本人が見分けられない。
...
**`127.0.0.1` にしか bind しない。**`:8931` のように書いてはならない。
**`internal/server` を使わないということは、そこが持っていた `LoopbackHost` を引き継がないということである。**
**`http.ListenAndServe(":8931", …)` と書くと、そのマシンが到達可能な全ネットワークから認可の口が見える。**
**攻撃者は `state` を知らなくても、`code` が飛んでくる瞬間の口を先に掴めば済む。**
**受け取った `code` は user-to-server token に交換でき、その人の代理として issue へ書ける。**
**この設計は「同じ機械のどのプロセスからでも接続できるのは危ない」と言って常駐の口を避けたのだから、
それより広い相手へ開けては、名乗りと実物が逆向きになる。**

**待ち受けるポートは、App へ登録する callback の URL と同じ値に固定する。**
**App に登録した URL は変えられないので、ずれると2回目の認可が戻ってこなくなる。**
**ポートは 8931 に固定し、`--port` は持たせない。**

**持たせると、App に登録した URL と食い違ったときに、`code` が別のプロセスへ届く。**
**8931 が塞がっていたら、その場でエラーにして「空けてから叩き直してください」と出す。**

**既定の構成では、常駐がこのポートを掴んでいることは無い**（常駐は `server.port` を使い、既定は null）。
**`server.port: 8931` と書いた利用者では衝突する。**上のエラーがそれを出す。
...
**一つ。install の前の説明を出す段が、そこにしか無い。**
**上の表の「install の前」の列は、いちばん怖い1行**（その App が、選んだリポジトリの issue へ書けるようになる）**を運ぶ。**
**人間が「installするときも同様」と名指しで求めたのは、この画面である。**

**二つ。`state` を運ぶ場所が、そこにしか無い。**
**`request_oauth_on_install: true` の流れでは、認可のリクエストを組み立てるのは install の URL を開いた側である。**
**continuo が組み立てて渡さないと、`code` は `state` 無しで戻る。**
**`state` を必須にした実装は正規の redirect を弾き、setup が二度と完了しない。**
**必須にしない実装は、上で塞いだ穴をそのまま開ける。**
...
**ただし、名前が既に使われていたときは1つだけ入力が要る。**
**GitHub App の表示名は GitHub 全体で一意なので、
manifest に載せた名前が使われていれば、GitHub の作成画面はその場でエラーを出す。**

**送る名前に持ち主のアカウント名を混ぜる。**`continuo-<アカウント名>` とする
（`gh api user --jq .login`。[internal/tracker/ghuser.go:38](../../internal/tracker/ghuser.go#L38)）。
**それでも衝突しうるので、押す前の説明へ1行入れる。**

> **この名前が既に使われていたときは、GitHub の画面で別の名前を打つように言われます。**
> **好きな名前で構いません。**continuo は名前を1文字も見ないので、何でも動きます。

**continuo が使うのは `client_id` と更新用のトークンだけである**（3-82b）。**名前も slug も見ない。**
**この1行を落とすと、2人目以降の利用者が「入力する欄は1つもない」という案内と食い違う画面に出る。**

**manifest に載せる項目。**

| キー | 何を入れるか |
| --- | --- |
| `name` | `continuo-<アカウント名>` |
| `url` | `https://github.com/maimuzo/continuo` |
| `redirect_url` | `http://127.0.0.1:8931/github-app/created?state=<値>`。**`state` はここへ載せる。**manifest の中へ書いても redirect には載らない |
| `callback_urls` | `["http://127.0.0.1:8931/github-app/authorized"]` |
| `request_oauth_on_install` | `true`（install と認可を1回の流れにする） |
| `public` | `false` |
| `default_permissions` | `{"issues": "write", "metadata": "read"}`（3-82b） |
| `default_events` | `[]` |

**`hook_attributes` は入れない。**2026-09-08 の実測で、`127.0.0.1` を入れると GitHub が
...
**言いたいこと。****印が無いコメントを2種類にしない。**
「人間が書いた」と「機械が書いたが印を付け損ねた」が混ざると、**見分けるという目的そのものが崩れる。**
**付けると決めた設定で動いているなら、トークンが取れないときは投稿せずにエラーで止まる。**

**人間の決定のうち「doctor でも検証する」は、そのままでは果たせない。**
**果たせるのは、資格情報が揃っているかと、認可した人が `gh` の持ち主と同じかまでである。**
**「トークンが実際に取れるか」を doctor で見ると、更新用のトークンが回り、
doctor が continuo を起動不能にしうる**（下の表）。
**そこは起動時の検査が受け持つ。**
...
**設定はこれだけである。**
    ...
    tracker:
      comments:
        github_app_attribution: false   # true にすると、機械の投稿に GitHub App の印が付く

**既定は `false`。**書かない利用者の continuo は、いままでどおり動く。**印は付かないが、1つも壊れない。**

**そのためには、組み込みの指示書を設定で分岐させる必要がある**（3-82j）。
...
| **この設計文書の 5-2 の設定例**（`tracker.comments` の下。**行番号は書かない**） | **雛形だけに足すと、実装者が「どの階層に置くキーか」をここから読めない。**[test/internal/scaffold/design_template_test.go:37-40](../../test/internal/scaffold/design_template_test.go#L37-L40) が、5-2 の `yaml` ブロックと雛形の front matter のキー集合を突き合わせている。**片方だけ直すと3本のテストが落ちる**（実測した） |
| **[internal/config/types.go:71-76](../../internal/config/types.go#L71-L76) の `TrackerCommentsConfig`** | いまは `Marker` と `SelfMarker` の2つしか持たない。**足さずに雛形へ書くと、[internal/config/config.go:157](../../internal/config/config.go#L157) の `yaml.Strict()` が未知のキーとして拒み、`continuo init` が置いた WORKFLOW.md を continuo 自身が読めなくなる** |
...
（[docs/FAQ.md:726-730](../FAQ.md#L726-L730) が共有の使い方を案内している）。
**2人目以降は App を作らず認可だけを行うが、そのために `client_id` と `client_secret` を渡す経路が要る。**
**WORKFLOW.md へは書けない**（commit されるため）**ので、渡し方が無い。**
**既定の `false` のままなら起きない。**対象にするなら、渡し方を決めてから対象にする。
...
**起動時の検査は、`Adapter` のメソッドを呼ぶ。**関数を直に呼んではならない。
**`runStartupChecks` は `deps` を受け取る**（[internal/daemon/checks.go:43-49](../../internal/daemon/checks.go#L43-L49)）**ので、そこから届く。**

**なぜ関数ではだめか。**使い回しの置き場所は `Adapter` である（3-82i）。
**関数の側にキャッシュは無い。**
**起動時の検査が関数を直に呼ぶと1回転し、最初の `PostComment` で `Adapter` がもう1回転する。**[internal/daemon/daemon.go](../../internal/daemon/daemon.go) では
Adapter を組み立てる段が起動時の検査より前にあるので、**素直に書くと起動のたびに更新用のトークンが2回回る。**
**開発中は実行ファイルを差し替えるたびに再起動するので、回数は少なくない。**
...
**投稿のときは、run 1件では済まない。**
**「run を人間へ渡す」ことは、issue へ通知を1件書くことで果たされている。**
**その通知も同じ2本目のクライアントを通るので、同じ理由で落ちる。**
[internal/orchestrator/lifecycle.go:1161](../../internal/orchestrator/lifecycle.go#L1161) は
**`Warn` を1行出して戻るだけである。**
**その直前の `UpdateStatus` はカンバン用の別のトークンなので通り**（3-82d）**、issue は `failure_state` へ動く。**
**人間の手元に残るのは「`Blocked` へ動いた issue」だけで、
...
**どの issue の、どの投稿が、なぜ落ちたか。**そのリポジトリ名も入れる。
**あわせて worktree のパスと、会話の記録のパスも出す。**
**落ちるその場に両方在る**（[internal/orchestrator/lifecycle.go:1163-1164](../../internal/orchestrator/lifecycle.go#L1163-L1164) の
`snap.WorktreePath` と `snap.TranscriptPath`）。
**ログはローカルなので、絶対パスを縮める処理は要らない。**
**「そのリポジトリへ App が install されていますか」の一言を添える。**

**ダッシュボードへは出さない。**
**既定では開かず**（[internal/config/default.go:253-255](../../internal/config/default.go#L253-L255) の `Port: nil`）**、
開いていても終わった run の欄は消える**
（[internal/server/view.go:36-38](../../internal/server/view.go#L36-L38) が
「**終わった run は印から外れて消える**」と書いている）。
**投稿に失敗して人間へ渡った run は、まさにその「終わった run」である。**
...
**だから既にある枠組みへ嵌める。**差し替え口を `Options` へ2つ足す。

| 何を足すか | なぜ |
| --- | --- |
| **資格情報のファイルを読む口** | 本物の `~/.continuo/` を読まずに済ませる |
| **`gh api user` を叩く口** | **いま `internal/doctor` はこれを1度も呼んでいない**（`internal/orchestrator` だけが呼ぶ）。**足さないと、doctor のテストが本物の `gh api user` を叩き、検査結果が「テストを走らせたマシンで誰がログインしているか」で変わる。**[internal/tracker/ghuser.go:37](../../internal/tracker/ghuser.go#L37) の `RunGHAPIUserLogin` と同じ形の関数型で足せる |

**あわせて、**`Label` の定数と i18n のキーも足す**（[internal/doctor/report.go:20](../../internal/doctor/report.go#L20) が全項目を定数で持ち、`Run` は同じ数を返す）、
...
**カンバンに新しいリポジトリの issue が載った日、そこへの投稿だけが権限不足で落ち、上の決まりで run が止まり、ログに理由が出る。**
**先回りしても、人間はどのみちログを読む。**
**同じ「投稿だけが落ちる」形は、更新用のトークンが切れた・secret を作り直した・install を外した、でも起きる。**
**1つだけ先回りしても、残りは捕まらない。**

### 3-82h. 漏れたときに何ができるか。何で止めるか

**言いたいこと。****秘密鍵を置かなくても、漏れたら危ない。**
更新用のトークンと `client_secret` が揃えば、**約6か月のあいだ、App の権限の範囲で何でもできる。**
**だから守りの本体は権限を最小にすることであって、秘密鍵を置かないことではない。**
...
**`Issues` だけなら、漏れても「issue へ勝手にコメントが付く」だけである。
`Pull requests: write` を足していたら、レビューを通していない pull request をマージされうる。**

**漏れたときの復旧手順は [docs/FAQ.md](../FAQ.md) へ置く。**設計には置かない
（[.claude/rules/plan-file.md](../../.claude/rules/plan-file.md) の「同じことを2箇所に書かない」）。

**FAQ へ書くときに、次の3つを落としてはならない。**

| 何 | なぜ落とせないか |
| --- | --- |
| **client secret は作り直すだけでは止まらない。古いほうを削除する** | GitHub App は client secret を複数本持てる。**作り直して安心する人が出る** |
| **古いほうを消すと、自分の continuo も同時に止まる** | 手元のファイルに入っているのも古いほうである。**認可をやり直すまで投稿は全部落ち、run は全部人間へ渡る** |
| **既に配られたアクセストークンは、secret を消しても止まらない** | **8時間で切れるのを待つか、App の install を外す** |

**`continuo doctor` の「資格情報が在るか」は、消したことを検知できる。**
**その1行を、FAQ から辿れるようにする。**


### 3-82i. 投稿する GraphQL のクライアントを2本に分ける

**言いたいこと。**書かないと実装者がその場で判断することになる。**外すと印が付かないか、run が失われる。**
**`github_app_attribution` が `false` のときは、2本目を作らず、いままでの1本で書く。**
**この節は `true` のときの話である。**
...
| **足す1本** | **`PostComment` の全部** | **同じ Go の関数を直に呼んで取る**（`continuo github-app token` が使うものと同じ実体）。**exec しない。**exec すると、動いている トークンが `ps` の一覧に見え、PATH の解決先にも依存する |
...
**トークンを取れなくなると入札も投稿できないので、その機械はその巡回では issue を取らない**（3-82g）。

#### 三、トークンの持ち回しと、実装で外さないこと
...
**continuo 本体が issue へ書く口は12箇所ある**（`o.postComment(` / `o.postOwnMarkedComment(` /
`o.postCommentWithMarker(` を `internal/` の下で数えると14行返り、
**うち2行は [internal/orchestrator/comment.go:528](../../internal/orchestrator/comment.go#L528) と
[544行](../../internal/orchestrator/comment.go#L544) の委譲なので、引いて12である**）。**巡回のたびに複数が走る。**
**使い回せば、本体の側の危険は8時間に1回まで下がる。**

**下がるのは本体の投稿だけである。**エージェント側は新しく投稿するたびに回る（3-82c で3回前後と数えた）。
...
**それでも関数を渡す形を採る。**別の口で渡す案は、`Adapter` が2つの経路で状態を持つことになり、
**テストが「どちらの口から入ったか」で分岐する。**42箇所のうち38はテストなので、そこが複雑になるほうが高い。
...
**`internal/cli/cli.go` にも触る**（`switch args[0]` へ `github-app` を足す）。
...
**[test/internal/redact/single_choke_point_test.go:87](../../test/internal/redact/single_choke_point_test.go#L87) が `PostComment` という名前ちょうどだけを見ている。**
**別名のメソッドを足すと、この検査は1件も落とさない。**
**だから足さない。**足すなら、同じ turn でその検査も直す。
**縮め損ねると、利用者の絶対パスが公開の issue へ出る。**編集履歴は消せない。

### 3-82j. 組み込みの指示書を、設定で分岐させる
...
**掛ける先は issue のコメントだけである。**指示書の書き込みは9本あり、
**pull request への2本には掛けてはならない**（[internal/prompt/builtin.md:241](../prompt/builtin.md#L241) と同 [274行](../prompt/builtin.md#L274)）。
**App の権限は `Issues` だけなので**（3-82b）**、`ghu_` のトークンで叩くと権限不足で落ちる。**
**pull request が作られず、run が死ぬ。**

| 何 | 何本か | 掛けるか |
| --- | --- | --- |
| **issue のコメント**（新規の投稿） | **5本** | **掛ける。**形が2通りあるので、下の表で分ける |
| **issue のコメント**（既存への書き足し） | **2本** | **掛けない。**理由はこの表の下 |

| どの形か | 何本 | 何に気をつけるか |
| --- | --- | --- |
| `--body-file` の1行 | 2本 | 上の見本がそのまま当たる |
| **複数行の `--body`** | **3本** | **行頭を1桁も動かさない**（下の注意） |
| pull request（作成とコメント） | 2本 | **掛けない** |

**複数行の `--body` の3本には、2つの検査が掛かっている。**
...
**設計 5-3 の ```markdown ブロックと builtin.md を1行ずつ比べている。**
**片方だけ直すと赤になる。**

**書き足し（`PATCH`）に掛けない理由。**
**`PATCH` では印は付きも消えもしない**（3-82 の実測の表の3行）。
**掛けても画面の表示は1文字も変わらない。**
...
| 何 | どうなるか |
| --- | --- |
| **入れ子が1つ浅くなる** | 掛けると `{{if}}` ＋ `case "$TOKEN"` ＋ `case "$APP"` ＋ `case "$OLD"` の4重（7-2 は5重）になる。**掛けなければ 5-3 が3重、7-2 が4重で収まる** |
| **更新用のトークンの回転が減る** | **進捗報告は2回目以降が書き足しである**（[internal/prompt/builtin.md:392-393](../prompt/builtin.md#L392-L393)）。**18時間の run 1本で18回ではなく、3回前後になる** |

**トークンを取るのは、段2b（新しく1件投稿する）へ落ちたときだけである。**

**印の無いコメントへ書き足させない。**指示書の 5-3 と 7-2 は、既にあるコメントへ `PATCH` で書き足す。
**印は作成のときに決まり、編集では付かない**（3-82 の実測）。
...
**既存の `OLD=$(gh api … --jq .body)` は1文字も変えない。**
**印を見る呼び出しを、`{{if}}` の中に入れて、その手前へ足す。**
...
**`--jq` は `null` を返し、`// ""` が空文字にするので、`APP` は必ず空になる。**
**門を無条件に置くと、既定の `false` で動いている全利用者の進捗報告が毎回新規投稿になる。**
**18時間の run で18件積まれる。**
[internal/prompt/builtin.md:391](../prompt/builtin.md#L391) の
「**コメントは増やさないでください。何十件も並ぶと、issue を開いても本題が読めなくなります**」が、
**まさにそれを防いでいる。**
**3-82g の「既定は `false`。1つも壊れない」が成り立たなくなる。**

**`false` のときは `APP=skip` を入れて、門を素通りさせる。**空でなければ書き足すので、いままでどおり動く。
**別の App の印と見分ける必要は無い。**
**書き足す先は、段1 が `viewerDidAuthor` と2つの印で既に絞っている**
（[internal/prompt/builtin.md:395-401](../prompt/builtin.md#L395-L401)）。
**他人の App が付けた印のコメントは、そもそも候補に入らない。**

**`// ""` を落としてはならない。**印の無いコメントに対して
**`null` という4文字が標準出力へ出るので、空かどうかで見ると必ず真になる。**
**`.id` まで辿って `// ""` を付けると、印が無いときにちょうど空文字になる。**
**この罠は、指示書が [internal/prompt/builtin.md:423-425](../prompt/builtin.md#L423-L425) で既に塞いでいる。**
...
| どこ | 何行目 | 何をしているところか | 印を見る門が要るか |
| --- | --- | --- | --- |
| **5-3** | [internal/prompt/builtin.md:411](../prompt/builtin.md#L411) | **進捗報告へ書き足す**（`--method PATCH` は414行） | **要る** |
| **7-2** | [internal/prompt/builtin.md:662](../prompt/builtin.md#L662) | **読むだけ。**`printf '%s\n' "$OLD"` で表示して終わる | **要らない** |
| **7-2** | [internal/prompt/builtin.md:702](../prompt/builtin.md#L702) | **グループの成果報告へ書き足す**（`--method PATCH` は705行） | **要る** |

**書き込むのは2箇所である。**上の表の「既存の `case` の中の `PATCH` | 2本」と同じ数になる。

**落ちた先の文面も、同じ turn で直す。**
**新しい門は「本文は読めたが、印が無い」で落ちるのに、
いま用意されている文面は「読めませんでした」しか言わない。**
...
**足すと、実装者が「なぜここに要るのか」を読み取れない。**
**`gh api` を2回叩く形を、1回へまとめてはならない。**
**`--jq '{app: …, body: …}'` の1本にすると `$OLD` が JSON のオブジェクトになり、
`-f body="$OLD"` がコメントの本文を JSON で上書きする。**
**印が消えると continuo は進捗報告を見つけられなくなり、18時間の時計が担当を取った時刻まで巻き戻る**
（[internal/prompt/builtin.md:428-430](../prompt/builtin.md#L428-L430)）。

#### 二、テンプレートへ足す変数

**最上位の名前を2つ足す**（`github_app_attribution` と、`continuo` の下に `command`）。
...
最上位の名前を直書きしているので、足さないとその場で落ちる。
**理由は [internal/prompt/prompt.go:614-615](../../internal/prompt/prompt.go#L614-L615) と
[639-641行](../../internal/prompt/prompt.go#L639-L641) が持っているので、ここには写さない。**
...
絶対パスを取り、絶対パスでなければ起動を止めている。**

**囲うのは `RenderData` が値を入れる時点である。**テンプレートの側では囲えない。
**hook のコマンド行は既にそうしている**（[internal/orchestrator/settings.go:352](../../internal/orchestrator/settings.go#L352) の
`shellQuote(o.continuoPath)`）。

**その関数を、共通の場所へ移して export する。**
**いまの `shellQuote` は [internal/orchestrator/settings.go:425](../../internal/orchestrator/settings.go#L425) の小文字始まりで、
`internal/orchestrator` の外から呼べない。**
**`RenderData` は `internal/prompt` にあり、`internal/orchestrator` が `internal/prompt` を import しているので、
逆向きに import すると循環する。**
**写しを作ってはならない。**hook 側を直したときに、プロンプト側が取り残される。
その3行上のコメントが、なぜ囲うかを書いている。

> **引用せずに繋ぐと、パスに空白が1つ入るだけでコマンド行が別の引数へ割れ**

**囲わないと、パスに空白を含む利用者**（`/Users/<名 姓>/…` など）**の `$TOKEN` が空になる。**
**`case` のガードが空を弾き、3-82g の「投稿せずエラーで止める」に落ちる。**
**`true` にしたその日から、その人の run は1件も完了できない。**
**症状は「トークンを取れませんでした」で、原因が空白なので当人には結び付かない。**

**送る文面に、ホーム配下の絶対パスが初めて入ることになる。**
いまの `RenderData` が渡すのは `issue` / `push_branch` / `attempt` / `progress_interval_minutes` の4つで、
**local のパスは1つも無い。**
**縮める仕組み（`redact.Paths`）は `PostComment` の中にあるので、
エージェントが `gh` で直接書くコメントには効かない。**
**だから指示書へ2行足す。**

| 何を書かせるか | なぜ |
| --- | --- |
| **「投稿が失敗したとき、`GH_TOKEN=` を外して投稿し直さないでください。投稿せずに `blocked` で返してください」** | **これが無いと、失敗した `Bash` を前にしたエージェントの、いちばん自然な直し方が「`GH_TOKEN=` を落として叩き直す」になる。**`Bash` は引数を限定せずに許可されている（[internal/config/default.go:171-182](../../internal/config/default.go#L171-L182)）。**印の無い機械のコメントが1件できた瞬間、「印が無い＝人間が書いた」がその issue で成り立たなくなる** |
| **「失敗したときは『トークンを取れませんでした』とだけ書き、コマンド行と `$TOKEN` の中身を書かないでください」** | **指示書の別の場所が「何が問題なのかを書いてください」と求めており、素直に書くと絶対パスが公開の issue へ出る。**縮める仕組みは `PostComment` の中にあるので、エージェントが `gh` で直接書くコメントには効かない |

**2つ目は「お願い」で塞ぐ形なので、完全ではない。**
**それでも採る。**パスを pane の環境変数へ移す案は、
**`continuo prompt --show` が実物と同じ文面を見せられなくなる**（環境変数は文面に現れない）。
**[internal/prompt/prompt.go:631-637](../../internal/prompt/prompt.go#L631-L637) が
「ずれた瞬間、`--show --url` は『送られる文面』ではないものを見せる」と書いている。**
**止まったエージェントが「`/Users/alice/.local/bin/continuo github-app token` が失敗しました」と書くのは、
指示書の「何が問題なのか」を素直に書いた結果である。**編集履歴は消せない。

**`RenderData` の署名を変える段が要る。**いまは issue と2つのスカラーしか受け取らず、
**実行ファイルのパスを持っていない。**
**`continuo prompt --show` にも、実物と同じ値を渡す。**
[internal/prompt/prompt.go:631-637](../../internal/prompt/prompt.go#L631-L637) が
**「別々に組み立ててはならない。ずれた瞬間、`--show --url` は『送られる文面』ではないものを見せる」**
**と書いている。**設定と実行ファイルのパスは Orchestrator が無くても引けるので、そこから渡す。

**`Validate` は、この枝の中まで届かない。**
[internal/prompt/prompt.go:614-615](../../internal/prompt/prompt.go#L614-L615) が「値そのもので分かれる枝の中までは届かない」と書いている。
**`.github_app_attribution` を `.attempt` と同じように2通りへ振る段を足す。**
**足さないと、`true` にした利用者だけが起動時に落ち、開発側では再現しない。**


### 3-82k. 認可した人と `gh` の持ち主を突き合わせる
...
| いつ | 何をするか |
| --- | --- |
| **認可が終わった直後** | **そのトークンで `viewer` を引き、`gh api user` の返り値と突き合わせる。**通ったら、そのログイン名を資格情報のファイルへ `authorized_login` として書く |
| **違っていたら** | **その場で画面へ出す。**「`gh` は A、認可したのは B です」と両方を並べる |
| **起動時の検査**（3-82g） | **同じ突き合わせを行い、違っていたら起動しない** |
| **`continuo doctor`** | **`authorized_login` と `gh api user` を突き合わせる。****トークンは1度も取らない** |

**doctor が `viewer` を引いてはならない。**
**引くにはアクセストークンが要り、取れば更新用のトークンが回る**（3-82c）。
**3-82g が「doctor は回さない」と決めているので、回す実装は矛盾する。**
**だから認可のときに引いた名前をファイルへ持ち、doctor はそれを読む。**
...
`gh api user` と GraphQL の `viewer` を比べている（3-77f）。**そこでは WARN を1行出すだけで、起動は止めない。**
**ここでは止める。**理由は、ずれの性質が違うためである。
...
**起動時の検査で、GitHub との往復は1回増える**（`viewer` を引くぶん）。
**トークンは 3-82g が「起動時に1回だけ取る」と決めた1回をそのまま使う。**
...
| 遷移 | 誰が | いつ | 手段 |
| --- | --- | --- | --- |
| **（カンバンに載る）** → `Ice Box` | **continuo の外**（人間、または人間に代わって働く道具） | **issue を作ったあと、カンバンへ載せて `Ice Box` を付けるとき。**continuo はカンバンに載っていない issue を見ない。**continuo が起動したエージェントは、ここを触らない**（5-3 の `CONTINUO-STATUS:` の1行だけを書く） | GitHub の画面／GraphQL |
...
**人間の決定（2026-08-18）。Priority は使わない。カンバンの並び順だけで順序を決める。**
...
**並べ替えが要るときは、continuo の中ではなくカンバンを書き換える**（3-30）。
**中だけで並べ替えると、人間がカンバンで見る順序と、実際に処理される順序が食い違う。**
...
| **dispatch の直前に、issue ごとに「対象リポジトリが信頼済みか」を検査する** | `~/.claude.json` の `projects` を**読み取って**確認する。**対象リポジトリの集合はカンバンを読むまで確定しないので、起動時には検査できない**（3-6） |
...
API が返す順序に一切反映されない（4-2）。**したがって、カンバンで見えている順序が、そのまま continuo の実行順序になる。**
...
**費用は多くて「2リクエスト・計 8 point」である**（104件のカンバン。3-31 の式から計算した値であって、実測ではない）。
...
**「カンバンに載っていません」とだけ言わない。**`FetchIssueByIdentifier` が偽を返す理由は5通りあり、
**Status が未設定なのは本番のカンバンでも104件中4件ある通常の状態である**
（[internal/tracker/query.go](../../internal/tracker/query.go) の実測）。
**`Bootstrap` を通していないので、`tracker.provider.status_field` の綴りがカンバンとずれていると
...
### 5-3q. 計画そのものを issue へ書かせる

**言いたいこと。**判断票だけを書かせても、何を直そうとしていたかが残らない。
**計画を、実装の前に issue のコメントへ書かせる。**
**その印は判断票と別にする。**同じにすると、レビューを1回もせずに CI を通せる。

**書かせるもの。**

| 何を | なぜ |
| --- | --- |
| **何が原因か**（ファイル名と行番号つき） | 根拠の無い計画は、レビュワーが当たりを付けられない |
| **どのファイルをどう直すか** | **範囲が先に出る。**あとから広がったことが分かる |
| **決まっていないこと** | 空欄にすると、決めていないことを決めたつもりで実装へ入る |

**印を3つに分ける。**

| 何を貼るか | 1行目 | 2行目 |
| --- | --- | --- |
| **計画** | `<!-- continuo:agent -->` | **`<!-- continuo:plan -->`** |
| **設計のレビューの判断票** | `<!-- continuo:agent -->` | `<!-- design-review-result -->` |
| **実装のレビューの判断票** | `<!-- code-review-result -->` | — |

**計画に `<!-- design-review-result -->` を付けさせない。**
**付けられると、レビューを1回も受けていないコメントが CI に数えられる。**
数える側は本文を1文字も見ない（5-3p）。

**`<!-- continuo:plan -->` が要る理由は、成果の報告と見分けるためである。**
[internal/orchestrator/comment.go](../../internal/orchestrator/comment.go) の `hasRunComment` は、
**`<!-- continuo:agent -->` で始まるコメントを「この run が成果を書いた」と数える。**
**計画は run の最初に書かれるので、印が無いと判定はほぼ必ず外れる。**
外れると、turn が途中で終わった run に「何をしたか」を書かせ直す経路（3-25 の段9）が飛ぶ。
**進捗報告（`<!-- continuo:progress -->`）も同じ理由で除く。**そちらは以前から同じ穴を持っていた。

**渡し方は `--body-file` にする。**
**計画にはファイル名と行番号を書くので、backtick とドルの記号が混ざる。**
`--body "…"` の二重引用符の中では、それが実行される。

**既定のレビュワーは general-purpose である**（7-4）。
**雛形の `レビューを頼む subagent` を消しても、レビューは起きる。**
消して止まるのは「どれに頼むか」の指定だけである。
...
**GraphQL は `data` と `errors` を同時に返す。**カンバンから消えた item の ID を混ぜて
...
**`status_field` の綴りが違うときのエラーまで消える**（カンバンを読めていないのに、
...
| **後片付けが止められていた** | 終わった run の pane を閉じる処理が、シャットダウンの ctx に引きずられて中断していた。**カンバン上は終わった issue の pane が残る** |
...
| GitHub の GraphQL / `gh` | 叩かない | 認証と本番のカンバンが要る。`httptest` で形は固定できる |
...
| **park のあと止まると板が戻らない** | カンバンは書き換わったのに「何も消していません」としか出ず、issue が置き去りになる | Status がその値のまま残ることを1行で言う。**戻しはしない**（3-37） |
| **計画表示の Status が古い** | カンバンを1回しか読まないので、park の**前**の値が出る | 書き込みが通ったら持ち回っている値も更新する |
...
**言いたいこと。**カンバンに `In Progress` と `AI In Progress` が並んでいると、
...
**カンバンを読んだときの応答を使い回すので、リクエストは1本も増えない**
...
`Done` を含む（a-b-a-n-**d-o-n-e**-d）。カンバンに `Done` と `Abandoned` を並べるのはごく普通で、
...
| --- | --- | --- |
| `Adapter.Bootstrap` | 設定に書いた名前がカンバン側に在るか | **カンバンに紛らわしい別の選択肢が並んでいても通る** |
...
| 確かめる対象が0件 | [internal/doctor/checks.go](internal/doctor/checks.go) の `checkClone` / `checkTrust` | カンバンが空だと `clone` と `信頼登録` は `!`。終了コードに影響させない |
...
**`!` だけのときを「問題があります」と書かない**（対象が0件のカンバンもここへ来るため）。
...
| `カンバンから消えたissue` | issue がカンバンに見えるかを見る | [internal/orchestrator/lifecycle.go](../../internal/orchestrator/lifecycle.go) の `refreshIssue` |
...
（カンバンは1回しか読まないので、park が読めていれば読み直しは起きない）。
...
| **owner を1つしか持てない** | 複数リポジトリを1枚のカンバンで回す運用と衝突する |
...
カンバンに載っているかも、Status の名前も見ない。**そのぶん、消える危険と残る危険がはっきり分かれる。
...
| **カンバンに載っていない issue へ書けない** | `gh issue comment <番号>` はカンバンを見ない。Status が動かないことは、いままでどおり `noteSignalTargetsMissing` が代表へ1件書いて人間へ知らせる |
| **Status の名前を変えたカンバンで、書き込みだけ黙って止まる** | エージェントが見るのは自分の表明の値（`review` / `blocked`）だけで、**`status_signal_map` の遷移先も `failure_state` も1文字も読まない。**対応表の**手前**の値しか使わない。**突き合わせるより強い** |
...
| **3** | トラッカーのアダプタ（GraphQL 直叩き） | カンバンから issue を取り、ID 指定で取り直せる。起動時の選択肢名の照合ができる |
...
| 短縮名 | continuo はどうするか |
| --- | --- |
| **実行順序に Priority を使わない** | カンバンの並び順だけを使う |
...
**continuo。**カンバンの並び順だけを使い、返ってきた配列の順にそのまま dispatch する（4-2）。
...
| **落ちている間の通知の取り戻し** | 再起動直後にカンバンを1回取り直す（3-19） | turn の終わりの通知は投げっぱなしで一度きり。**落ちている間のものは再送されない** |
