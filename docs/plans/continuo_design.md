...
level=INFO msg="身元ファイルのセッションへ復帰しないで、新しいセッションで始めます" identifier=octocat/hello-world#188 復帰しなかったセッション=b52c16af-… 新しいセッション=e1f2… 記録の置き場所=/…/.claude/projects worktree=/…/worktrees/…
...
**着手が段9 の途中で落ちると、会話が1度も作られていない UUID が身元ファイルに残る。**
...
**なぜ投げる前に見るのか。着手が段6 より先で落ちると、身元ファイルには会話が1度も作られていない
UUID が残るためである。**段6 で身元ファイルへ UUID を書いたあと、段7（before_run）・段8（pane を引く・
pane の rename）・段9（`agent.start`）のどこで落ちても同じ状態になる。**worktree と身元ファイルは
失敗しても残す**（次の巡回に委ねられる場合があるため。3-22）ので、**次の再着手がその UUID を読む。**

**二重の失敗は要らない。**3-3b の「復帰に失敗したら新しいセッションで始め直す」が採り直した UUID を
身元ファイルへ書いたあと、立て直しの `agent.start` も失敗した場合も同じ状態になるが、**それは例の1つである。**
**新規の着手はそもそも立て直しを通らない**（`resumeUUID` が空なので分岐に入らない）。
...
**根の内側かどうかも見ない。**`hasTranscriptFor` は**そのパスを1バイトも読まないし、返しもしない。**
...
**RUCM の経路は増えない。**起動フラグを決める段（B20）は条件ステップではないので、
記録の有無で枝が分かれることはない。**`RUCM-CFG-SHA256` を手で書き換えてはならない**
（[CONTRIBUTING.md:108-121](../../CONTRIBUTING.md#L108-L121) の「触らないもの」）。

**この変更で古くなるのは、基本フローの段20 の文である。**「身元ファイルに前回のセッション UUID があれば
復帰つきの起動フラグを使う」と書いてあるが、**UUID があることは、もう十分条件ではない。**
**段17 は身元ファイルを読む段なので、1文字も古くならない。**直すかどうかは、仕様を再生成する側が決める。
...
5b. どのセッションで起動するかを決める（3-3b）
   ...
   → **ただし、その session_uuid の会話の記録が1件も無ければ --resume を渡さない**（3-3b の表の3列目）。
