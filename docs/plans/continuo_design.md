...
**「他の役割と重なるとき」は別で、そちらは起動を断る**（3-82k）。
...
| **4** | **空きスロットが無い**（`agent.max_concurrent_agents`） | **Debug 1行。**人間の決定で、専用の知らせは作らない（下） |
...
**そのときは 3-82j の手順の段2 が要る。**利用者向けの案内にも、その但し書きを書く。
...
| **`Direct Chat` を他の役割に書いている人は、上げただけで起動しなくなる** | 設定の検査が重なりを見つけると**起動を断る**（3-82k）。**既定が非空になる以上、この機能を1度も頼んでいない人にも当たる。****しかもエラーは、その人の WORKFLOW.md に1行も書いていないキーの名前を出す。****エラーの文面へ「このキーを書いていない場合は既定値です」を入れる。****破壊的変更として [docs/upgrading.md](../upgrading.md) へ書く** |
...
**起動を断る理由。**重なったまま起動すると、**上の表の害が黙って起きる。**
人間がカードを direct chat へ動かした瞬間に、その issue が2つの扱いを同時に受ける。
**ログにも issue にも「重なっているせいだ」と書かれないので、気づく手立てが無い。**

**既定値を持つので、この検査はこの機能を頼んでいない利用者にも当たる。**
`Direct Chat` という名前を他の役割に書いている人は、**版を上げた瞬間に起動しなくなる。**
**それは破壊的変更として [docs/upgrading.md](../upgrading.md) へ書く**（3-82j）。
**破壊的変更を避けるために設計を歪めない。**

#### `automated_state_rewrite` のキーとの重なりは、この節では見ない

**既存の検査が先に弾く。**`direct_chat_state` は `KnownStates` に入るので、
...
| **選んだ Status が他の役割と重なるとき** | **その場で選び直させる。**WORKFLOW.md へ書かない。**重なりを見る相手7つは、上の「重なりを見る相手は7つ」の一覧をそのまま読む**（別に持たない） |

**選び直させる理由。****setup はその場に人間が居る。**重なりを伝えれば直してもらえる。
**警告だけ出して書き込むと、その人は起動して初めて断られる。**setup を叩いた意味が消える。

**他の5つの役割でも、同じ重なりは起きうる。**そちらは既存の検査が持っている。
**この役割だけを外さない。**
...
| **設定に書いた名前が、他の役割と重なっている** | **`!` を出し、重なった相手のキー名を出す。**相手7つは、上の一覧をそのまま読む |

**既存の「紛らわしい組」の検査へ相乗りさせてはならない。**あれは
**「カンバンの列名と設定の綴りが似ているか」**を見るもので、**「設定どうしが同じ名前か」**は見ていない。
**別の判定なので、別に置く。**相乗りさせると、片方を直したときにもう片方が壊れる。

**2つ目を足す理由。**重なっていると continuo は起動しない。
**doctor は「なぜ起動しないか」を調べるために叩かれる道具である。**
**そこで何も出ないと、人間は設定のどこが悪いのかを自分で探すことになる。**
