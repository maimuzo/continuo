...
**入れないと、読む人はどの pull request の話かを見分けられません。**
...
**この形にすると、支障1・支障2・支障4 は起きなくなる。**エージェントが `gh issue comment` を
直に叩くので、continuo の Status の書き込みの結果も、ボードに載っているかも、Status の名前も見ないためである。
**支障3 と支障6 は残る。**下の表がその内訳である。

| 何 | どうなるか |
| --- | --- |
| **Status が動かなかったとき**（支障1） | **起きない。**`postStatusMove` の `if !move.Wrote` を1度も通らない。代表が `In Review` と `Blocked` を往復しても、表明のたびに段1〜段3 が走る |
| **ボードに載っていない issue**（支障2） | **コメントは書ける。**`gh issue comment <番号>` はボードを見ない。Status が動かないことは、いままでどおり `noteSignalTargetsMissing` が代表へ1件書いて人間へ知らせる |
| **Status 名が設定で変わる**（支障4） | **起きない。**エージェントが見るのは自分の表明の値（`review` / `blocked`）だけで、**`status_signal_map` の遷移先も `failure_state` も1文字も読まない。**対応表の**手前**の値しか使わないので、列名を変えたボードでも壊れない。**突き合わせるより強い** |
| **continuo が単独で `Blocked` にする経路**（支障3） | **残る。**4-1 の遷移表の「`max_dispatch_turns` 到達・stall 検知でリトライを使い切ったとき」では、**エージェントは表明を1行も書かない。**7-2 は1度も走らず、**代表以外には何も残らない。承知のうえで受け皿を作らない**（continuo はグループの構成を知らず、知る手段が「エージェントが表明した対象」しか無い） |
| **番号を書き間違えたとき**（支障6の隣） | **残る。**continuo 側には「ボードに載っていなければ捨てる」門があるが、**エージェントの `gh issue comment` には門が無い。**無関係な issue へ「まとめて直しました」が公開の形で残りうる。**issue のコメントは編集履歴が残るので取り消せない。承知のうえで門を置かない**（門を置くにはエージェントにボードを引かせることになり、7-2 が API の呼び出しをさらに増やす） |

**実機で1件通しても、支障3 は見つからない。**通るのは表明を書けた run だけだからである。
