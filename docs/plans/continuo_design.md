...
**効くのは1つの場面だけである。**`dispatchAllowed` が偽になる理由は3つあるが、
**この例外が救うのは「Status の選択肢名の照合に失敗した」ときだけである。**
そのとき候補は取れている（選択肢の写しはまだ生きている）ので、pane は用意できる。
...
**巡回がその1手で印を外す**ので、戻せば pane ができる。**FAQ にそう書く。**
...
**この一覧に無い門は、1つも通らない。**とくに次の4つは、**意識して通さない。**
...
**バックオフ中の run は巡回が Status を取り直さないので、カードを direct chat へ戻しても気づかない。**
**`In Review` か `Blocked` なら、巡回が1手で印から外す。**
**FAQ にそう書く。**
...
**自分が担当者になっていない run は、担当の確かめ直しで止めない。**
...
**印を立てるのは2箇所である。**direct chat の用意が済んだときと、`enterDirectChatMode` が真を返したとき。
**下ろさない。**一度でも direct chat を通った run は、以後ずっと免除する。
**復元が引き取るときは、身元ファイルではなく `AdoptedRun` で引き継ぐ**（下の代償）。
