...
**対象は「表明の行に対象を書いた issue」だけである。**いま作業している issue は 3-7 で書くので含めない。
**含めると、代表にコメントが2件付く。**別のリポジトリの issue も含めない（直していない）。
...
**対象の上限は `maxSignalsPerTurn` の10件である。**1件につき段1 で `gh issue view` が1回、
段2a なら `gh api` の GET と PATCH で2回、段2b なら `gh issue comment` が1回。
**最大で 10 × 3 = 30 回／turn。**GitHub の REST の枠は毎時5000なので、
`maxSignalsPerTurn` を置いた理由（枠を使い切ると他の run のトラッカー操作まで巻き添えで失敗する）に触れない。

#### なぜ第6節へ置いたか

**3-26 の中へ置くと、設計文書の行番号を指すリンクが14本ずれる。**うち9本は
`.claude/` の下にあり、**この worktree で作業するエージェントには編集の権限が無い。**

**行を足す位置で決まる。**設計文書を指す行番号リンクのうち、`.claude/` の下から指されている
いちばん後ろの行は 8728 で、全体でも 9900 である。**9900 より後ろへ足せば1本もずれない。**
