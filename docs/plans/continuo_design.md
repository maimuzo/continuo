...
| **雛形にある設定項目が全部書かれているか** | **雛形（`internal/scaffold/template.go`）の front matter のキーが `WORKFLOW.md` に全部あるかを見る**（3-63） | **記号は `!`。**ボードを1バイトも読まない。**版を上げて増えた項目は、リリースノートを読まないかぎり存在に気づけない。**足す差分と、それを当てるコマンドの2つを出す |
...
### 3-63. 版を上げて増えた設定項目を、doctor が差分にして見せる
