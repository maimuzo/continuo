...
- いま担当している issue と関係のない外部への書き込み: 他のリポジトリへの push、パッケージの公開、外部サービスへの投稿
  ...
  リポジトリ <owner>/<repo> の issue、pull request、release、リポジトリの設定への書き込みと削除、
...
| **担当先の文と、担当先の外への免除が衝突したらどちらが勝つか** | **担当先の文が勝つ。**「リポジトリ X が相手のときは、この段落の扱いが下に続く免除より優先する」と書く | **担当先の文は merge を断り、下の免除は「merge …は、この免除に含めない。それらは、いまの作業と関係があるかどうかで、この条件のとおりに判断する」と書く。****担当先が相手なら「関係がある→通す」へ倒れるので、この1文だけがそれを断る側へ固定している。****落とすと、担当先の `gh pr merge` と `gh release create` とラベルの付け外しが通る。**「下に続く」が本当に下にあることも、検査で固定する（雛形の順序を入れ替えると、上にある免除を「下に続く」と呼ぶ） |
