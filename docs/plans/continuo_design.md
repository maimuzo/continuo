...
**言いたいこと。**印は本文の先頭に置くただの文字列で、**issue にコメントできる人なら誰でも書ける。**
印だけで「continuo の側が書いた」と決めると、第三者のコメントで成果の確認をすり抜けられる。
**「continuo が使う gh の持ち主」を起動時に1回取り、印と併せて見る。**

**何を見るか。**

| 印 | いままでの判定 | これからの判定 |
| --- | --- | --- |
| `tracker.comments.marker` | 本文の先頭がこの文字列 | 左に加えて**投稿者が持ち主**（`Comment.IsAgent`） |
| `tracker.comments.self_marker` | 本文の先頭がこの文字列（結果から外す） | 左に加えて**投稿者が持ち主**のものだけを外す |

**持ち主の取り方。**`gh api user --jq .login` を1回だけ実行し、`octocat` のような1行を得る
（[internal/tracker/ghuser.go](../../internal/tracker/ghuser.go) の `RunGHAPIUserLogin`）。
**エージェントの `gh issue comment` は、この同じアカウントで残る**
（設計 2-2: エージェントのコメントは人間のアカウントで残る）。

| 何を | 内容 |
| --- | --- |
| **誰がいつ取るか** | orchestrator が最初の巡回（`Tick`）で1回。**取り直さない**（毎巡回で外部プロセスを起こさない） |
| **どこに持つか** | `Orchestrator.selfLogin string`（メモリ上だけ。**ファイルにも設定にも書かない**） |
| **判定に使う場所** | `FetchComments`（[internal/tracker/adapter.go](../../internal/tracker/adapter.go)）。orchestrator は `hasRunComment` から渡す |

**取れなかったらどうするか。****起動も巡回も止めない。**`selfLogin` は空文字のままにし、
**いままでどおり印だけで判定する**（`Comment.WrittenBy("")` は true を返す）。
**理由。**`gh api` に一時的に届かないだけで continuo が動かなくなるほうが害が大きい。
**そのときは警告を1回だけログに残す**（取得が1回きりなので、警告も1回きりになる）。

```text
level=WARN msg="gh の持ち主を取れませんでした（コメントの印だけで判定します。…）" error=…
```

**設定に持たせない理由。**持ち主は「いま `gh` が誰でログインしているか」であり、
**設定ファイルに書くと、`gh auth switch` の後に黙って食い違う。**
食い違った値は「第三者が書いた」と読まれるので、**成果を書いた run が人間へ渡り続ける。**

**`token_source: env` で別のアカウントのトークンを使うと、continuo 自身の投稿だけは
`gh` の持ち主と一致しない。**そのとき self_marker のコメントは結果から外れなくなるが、
**判定は変わらない**（`hasRunComment` が数えるのは marker だけである）。

**採らなかった案。****`gh auth status` の出力から名前を拾う案**は採らない。
出力の書式が版で変わるうえ、同じホストに複数のアカウントがあるときの読み分けが要る
（設計 3-32 でその読み分けを既に1つ抱えている）。**`gh api user` はいま有効なアカウントを1つだけ返す。**
