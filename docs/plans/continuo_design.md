...
**書き戻してから標準出力へ出す。**順序を逆にすると、使えるトークンを配ったあとに次回が壊れる。
...
**`~/.continuo/` の中に専用のロックを1本取ってから段2〜3を行う。**
**ロックを待つ上限は30秒。**超えたら、待っている側は取り直さずに終了コード 1 を返す。
...
**呼ぶ側は、返ってきた文字列の形を確かめてから使い**（3-82f）**、空なら投稿を止めない。**
いままでのトークンで投稿して WARN を1行残す。**止めると成果報告が投稿できず、
