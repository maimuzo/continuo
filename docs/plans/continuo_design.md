...
**時計を進めるのは、上限の判定を行うたびである。**判定は枠待ちの標識が立っている run について
巡回のたびに行うので、**起点は厳密には「最初に枠待ちと判定した時刻」になる。**
**「最初に満杯を見た時刻」との差は、最短で `claude.turn_timeout_ms`（既定1時間）である。**
**この差は許す。**満杯を見た全部の run について巡回のたびに時計を持つと、
**枠待ちに入らない run の分まで持ち続けることになる。**

**枠待ちの標識を外すときは、この時計も一緒に消す。**
**消さないと、何日か普通に動いたあと1週間の枠がもう一度満杯になったときに、
...
| 順 | 何をするか | 答えが出なかったら |
| --- | --- | --- |
| **0a** | **画面の版が `claude.turn_timeout_ms` のあいだ増えていないか**（`agent.get`） | **手放さない。**枠待ちの標識も立てない |
| **0b** | **まだ自分が担当か**（issue の担当者と hold のコメントを読み直す） | **手放さない。**次の巡回でやり直す |
...
| 段 | 何が起きるか | なぜその段が要るか |
| --- | --- | --- |
| **0a** | 画面が動いていれば手放さない | **枠待ちの条件その2（hook の無音）は、「枠を待っている」と「長い1つの仕事をしている」を区別できない。**hook はツールが終わってから飛ぶので、**1時間を超える1回のツール呼び出しの最中は1件も来ない。**そこへ `weekly_scoped` が100%だと条件が両方そろい、**正常に走っている run を殺す。**`weekly_scoped` は使っていないモデルでも返る（3-77 の「最初から返ってくる」） |
| **0b** | 担当が移っていれば 3-77h の「担当が移った」へ倒す（`after_run` を走らせず、担当者にも触らず、worker を止めて run の登録から外す） | **枠待ちのあいだ、担当は自分の意思と無関係に外れる。**`idle_timeout_ms`（既定18時間）は「担当者の最後の進捗報告から」で数え、**枠待ち中は hook が来ないので進捗のコメントも増えない。****3-77c は「担当を外された機械は、その branch へ push してはならない」と決めている** |
...
**段0a の窓は `claude.turn_timeout_ms`（既定1時間）である。巡回の間隔（既定30秒）ではない。**
**「前の巡回から版が増えたか」だけで決めてはならない。**この段の根拠は
「1時間を超える1回のツール呼び出し」なので、**30秒動かない窓を1つ拾っただけで手放すと、
90分のビルドの最中に殺すことになる。**`agent.get` は「最後に動いていた時刻」を進めるので、
**静止が続いた長さがそのまま窓になる。**`claude.turn_timeout_ms` が0以下なら窓を掛けない
（**その設定でも上限は効かせると決めている**）。
...
> **mac-studio が1週間の枠を待つ上限（300分）を超えたので、担当を手放しました。**
