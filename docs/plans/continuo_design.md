...
**印の綴りと置き場所。**[internal/config/aimarker.go](../../internal/config/aimarker.go) の
`AIMarker`。**設定キーにしない。**`ProgressMarker` と同じで、機械ごとに違う値にできると
**「印が無ければ人間」が成り立たなくなる。**

**足す位置は「本文の先頭から連続する HTML コメントの行の、いちばん後ろ」である**（`WithAIMarker`）。
**既にある印を1つも動かさない。**

**先頭へ置かない理由。本文の先頭から読む判定が、本番に11ある。**
**下の表で数えた**（10が「先頭が特定の印で始まるか」、1つが「先頭に並ぶ印の中に進捗の印があるか」）。
**内訳は、`FetchComments` が2・持ち回りが3・組み込みの `jq` が2・CI が3で、合わせて10である。**
...
**`design-review-skipped` にだけは付けない。**この検査だけ「目印の直後に空白でない文字が1つあるか」も
見ており、**理由を書いたかどうかをそこで数えている。**印を足すと、理由の無い断りが通る。

**印が言うのは「この本文を打鍵したのは機械である」ことだけである。**

| 印が言わないこと | なぜ |
| --- | --- |
| **書いてある内容が誰の決定か** | **人間の決定を AI が記録したコメントにも、この印は付く。**だから組み込みの指示書は、記録するときに本文で名乗らせる（5-5） |
| **偽れないこと** | 誰でも書ける文字列であり、認証ではない（3-65）。issue #245 が求めたのは「見分けられること」である |

**だから 6-1（命令として扱ってよい立場）は変えない。**印で `OWNER` の指示の重みを下げると、
**裏の取れない文字列1つで信頼を判定する初めての場所になる。**

**過去のコメントへは遡らない**（2026-09-06、人間の決定）。
**印が無いことが「人間が書いた」を意味するのは、この仕組みが入ったあとに作られたコメントだけである。**
