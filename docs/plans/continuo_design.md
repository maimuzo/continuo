...
**残る未達は、このリポジトリの `CLAUDE.md` に規則を1行足せば埋まる**（文面は [docs/FAQ.md](../FAQ.md) にある）。
...
**下2つは、このリポジトリ自身の門である。**CLAUDE.md が「この規則は機械で止める。3箇所で止まる」と書いた3つのうちの2つで、
...
**採らないのは、読む側の探し場所が2箇所になるからである。**
組み込みの指示書は「本文の先頭に並ぶ印の中に出ます」の1本で読み方を教えており、
**末尾も見よと言い足すと、その1本が使えなくなる。**
