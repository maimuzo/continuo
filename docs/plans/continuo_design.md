...
| 取り直した値 | 書き込みの mutation | 戻り値 | ログ |
| --- | --- | --- | --- |
| `blockedStates` に入っている | 送らない | `(false, nil)` | 書いてはいけない状態に入っていました |
| item がもう見えない | 送らない | `(false, nil)` | item がもう見えません |
| `targetState` と同じ | **送らない** | **`(true, nil)`** | **Status は既にその値でした（書き込みを省きました）** |
| `targetState` と違う | 送る | `(true, nil)` | Status を書き込みました |

**戻り値の意味を「書き込みの API を呼んだか」から「目的の Status になっているか」へ変える。**
呼び出し側（`startRun` / `failRun` / `abandonRunClaimed`）は戻り値で先へ進むかどうかを決めているので、
**同じ値だったときに `false` を返すと、着手も失敗の記録もできなくなる。**
