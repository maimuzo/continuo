...
**104件のカンバンなら1リクエストで取れる**（cost 1〜4。3-31）。
...
### 3-77-0. 同じ GitHub アカウントを複数の機械で使うことは、サポートしない

**言いたいこと。**持ち回りは**機械ごとに別の GitHub アカウント**を使う前提で作る。
**同じアカウントを複数の機械で使う運用は、サポートしない。**
**必要な人には、別のアカウントを用意してもらう。**
...
| 何 | 扱い |
| --- | --- |
| **同じ GitHub アカウントを、複数の PC で使う** | **サポート外。**別のアカウントを使ってもらう |
| **同じ PC で continuo を複数動かす** | **サポート外。**検証のためだけの使い方である |

#### なぜサポートしないか

**担当者（assignee）だけでは、2台を見分けられないためである。**

同じアカウントなら、どちらが担当しても assignee は同じ値になる。
**見分けるには hold のコメントの `host`（`os.Hostname()` の値）に頼るしかないが、これは重複しうる。**
...
**アカウントを分けてもらうほうが、確実で、説明も短い。**

#### 重複したときに何が起きるか（サポートしない根拠）

| 何 | 中身 |
| --- | --- |
| 2台目が入札を書かなくなる | `HasBidBy` が1台目の入札を「自分のもの」と読む |
| **2台とも勝者になる** | 勝者の判定は `winner.Host` と自分の名前の照合1本 |
| 次の巡回でも止まらない | 「別の機械の担当」に落ちる条件も `host` の一致 |
| **同じ branch を押し合う** | branch 名は issue から導くので、2台とも同じになる |

#### `--id` でも解けない

**`--id <名前>`（3-17b）が分けるのは二重起動を止めるロック1本だけで、持ち回りの `host` は分かれない。**
**1台で2つ動かしても、両方が同じ `os.Hostname()` を名乗る。**
**入札はこの場面では効かない。**受け持つのは issue #143（持ち回りで機械を見分ける値が
os.Hostname() だけで決まり、重複しても検知しない）である。
**`--id` は、開発時に本番を止めずにテスト版を動かすための機能であり、
そのとき見張るカンバンは本番と別である**（3-17b）。**同じカンバンを2本で見張ることは想定しない。**

#### 書く場所

| どこ | 何を |
| --- | --- |
| [docs/FAQ.md](../FAQ.md) | **「同じアカウントで複数台を動かしたい」の節。**別のアカウントを用意する手順 |
| この節 | 決定と、その理由 |
...
> カンバンの並び順で良い。ただし、**カンバンの並び順を AI が変更でき、GitHub のポイントも問題なさそうな場合に限る。**
...
**組み込みを言語ごとに持つかどうかは、まだ人間が決めていない**（5-3e）。
...
| **組み込みを言語ごとに持つか** | 組み込みは日本語だけである。`language` に合わせて英語版を持つかどうか。**持たないなら、英語圏の利用者は送られる指示書の大半を日本語のまま受け取る** | **日本語だけを持つ** |
...
| `LatestHoldFor` / `LatestReleased` | どれがいちばん新しいか | 古い hold を1文字直すだけで、担当している機械の名前が入れ替わる |
...
| 印に加えて `tracker.provider.comments.marker` も見る | **採らない** | あちらは機械ごとに変えられる。**片方が変えた瞬間、もう片方から進捗報告が見えなくなる** |

### 5-3i. PR はエージェントが出す

**言いたいこと。**`review` を出す前に PR を作らせる。**置き場所は組み込みのプロンプトで、雛形ではない。**
雛形が持つのは「draft にするか」「base はどれか」だけである。
...
| **`## この issue に紐づく PR も読むこと` の一覧から選ばせない** | **あの一覧は「この issue を閉じる PR」ではなく「この issue に言及があった PR」を返す。**実測（2026-09-03）: issue #60 の timeline は PR #112 を返すが、PR #112 が閉じるのは issue #87 であり、本文に `#60` は0件である。**その branch へ push させると、別の issue で動いているエージェントの作業が消える。**`state` の綴りも取り方で `OPEN` と `open` の2通りになる |
...
| **本文へ `Closes #<番号>` を入れさせる** | `## この issue に紐づく PR も読むこと` の1つ目のコマンドが `closingIssuesReferences` で探す。**入れないと、次の turn がレビューの指摘を読む先を失う** |
...
| 根拠 | 中身 |
| --- | --- |
| **組み込みが既に PR の存在を前提にしている** | `## この issue に紐づく PR も読むこと` が読む相手を、誰も作っていなかった |
