...
| 何 | 識別子の出どころ | なぜそこか |
| --- | --- | --- |
| **入札** | **コメントの投稿者**（GitHub が付ける `author.login`） | **同じ事実の出どころを2つ作らない。**本文にも書くと、自分で名乗った値と投稿者の2つができる。**本文は第三者にも書けるので、他の continuo の名前を騙られると、騙られたほうは `HasBidBy` が真になって、その回は入札しない**（`bid_window_ms: 0` では、hold も released も現れないかぎり回が変わらないので、その issue では以後も入札しない） |
...
**判定スコアがいちばん大きければ、同点にならなくても勝つ。**同点になった場合も、
**同点の3段目（3-77d の「判定スコアも投稿の時刻も同じ」の行）では、空文字がどのログイン名にも勝つので、必ずそうなる。**
...
**古い版は、入札の本文に機械の名前を要求する。**だから**新しい版が書いた入札を1件も数えない。**
**2台が同じ巡回で締め切りを跨ぐと、両方が別々の勝者を選び、両方が自分を担当者に加える。**
アカウントが別なので**担当者が2人になる。**
**そのまま両方が hold を書いて着手する。**branch の名前は issue から決まるので同じになり、**片方の push が通らない。**

**走り出した2つの run は、そのあとも止まらない。**`verifyHandoff` は担当者の一覧に自分が入っていれば
「担当は自分のまま」と読むので、**2人のうち片方が自分である限り真にならない。**
**止まるのは、次の巡回からの新しい着手だけである**（`GateReasonManyAssigneesWithSelf` の枝へ入り、issue へは1バイトも書かない）。
**人間が担当者を外すまで戻らない。**

**だから全機械を同時に上げる。**段ごとの説明は [docs/upgrading.md](../upgrading.md) の
...
**3段目まで来た場合も、空文字はどのログイン名よりも小さいので必ず勝つ。**
