...
**`--id <名前>`（3-17b）が分ける5つに、持ち回りの `host` は入っていない。**
**1台で2つ動かしても、両方が同じ `os.Hostname()` を名乗る。**
**これも「同じ PC で複数動かすのはサポート外」の根拠である。**
...
**これが無いと、次の1時間後に段1が見つけられず、コメントがもう1件増えます。**
...
| **PR の決まり** | draft にするか・base にする branch・付けるラベル。**「PR を作らない」もここに書く** | **組み込みの指示のとおりに作られる**（5-3i） |
...
**プロンプトの変数に時刻は無く**（5-3 の9つ）、**continuo も turn の途中では何も送らない**（5-4）。
...
**設定のキーは作らない。**この印はエージェントだけが読み書きし、**Go 側は1バイトも見ない。**
...
**`lastActivityOf` だけを「`createdAt` と `updatedAt` の新しいほう」に変える。**残りは据え置く。
...
**`updatedAt` へ移すのは `lastActivityOf` の1箇所だけである。**残りは投稿の順番を決めている。
...
**残る危なさ。****担当者が古いコメントを1文字直しただけでも、18時間の時計は振り出しに戻る。**
**いまも「新しいコメントを1件書く」で同じことが起きる**ので、同じ種類の危なさである。
**進捗報告の印が付いたものだけ `updatedAt` を見る形は採らない。**
`lastActivityOf` は「進捗のコメントも hold のコメントも、どちらもその機械が生きている証拠」として
同じに扱うと決めており、**編集も同じ証拠である。**分けると
[internal/handoff](../../internal/handoff) がエージェント側の取り決めを知ることになる（いまは知らない）。
...
**採る形。**[internal/prompt/builtin.md](../../internal/prompt/builtin.md) の後半、
`## 長くかかるときは、途中でも状況を書くこと` と `## 終わったらやること` のあいだに
`## PR を出すこと` を置く。

| 何を書くか | なぜ |
| --- | --- |
| **本文に「PR を作らない」と書いてあれば作らない** | **雛形は調査だけ・レビューだけを頼む運用を用意している**（`## 何をする作業か` の HTML のコメント）。**commit する成果が無い run は PR を作れず、無条件にすると `blocked` で止まる。**打ち消しの口を1つだけ開け、それを組み込みの側が決める |
...
| **本文は前半と後半のあいだに挟まる**（5-3c） | 後半に置けば、本文に何を書いても最後に読まれる。**打ち消しを受け付けるかどうかを、組み込みの側が決められる**（受け付ける口は「PR を作らない」の1つだけである） |

**雛形に既定を置かない。**draft にするか・base にする branch・付けるラベルは project ごとに違う。
...
| **設定のキーで作る／作らないを切り替える** | **採らない** | **本文が既にその口である**（上の表の1行目）。**キーを足すと、同じことを front matter と本文の2通りで言えることになり、食い違ったときにどちらが勝つかを決める話が増える** |
