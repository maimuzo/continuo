...
| **hold** | **本文の `assignee`** | **利用者がこの欄で分かれ方を確かめる**（[docs/FAQ.md](../FAQ.md) の「別のアカウントを用意する手順」）。投稿者へ寄せると、issue の画面から担当の分かれ方を読めなくなる |
...
**両方が別々の勝者を選び、両方が自分を担当者に加える。**アカウントが別なので**担当者が2人になる。**
**2人になった issue は、人間が担当者を外すまで両方が止まる**（3-77b の「担当者が2人以上」の行）。
**だから全機械を同時に上げる。**手順は [docs/upgrading.md](../upgrading.md) にある。
