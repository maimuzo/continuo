...
**何が起きたか。**エージェントが PR を作った3秒後に自動化が Status を書き、
その29秒後の巡回が worker を止めた（利用者の環境での実測。全体の流れは
[docs/agent_life_cycle.md](../agent_life_cycle.md) にある）。
...
| 誰が動かしたか | `actor.__typename` | 判定 |
| --- | --- | --- |
| 組み込みの自動化 | **`Bot`** | **自動化** |
| **continuo 自身**（`gh auth token` の持ち主） | `User` | 人間あつかい |
| 人間 | `User` | 人間あつかい |

**`wasAutomated` だけでは見分けられない。**組み込みの自動化でも `false` を返す（設計 2-6）。
**同じ応答に載っているので OR に混ぜるのはただで済み、**GitHub が将来直せば自動で効く。

**採る手順**（[internal/orchestrator/unknownstate.go](../../internal/orchestrator/unknownstate.go) の
`claimAutomatedRewrite` と `rewriteAutomatedState`）。3-50 の分岐の**手前**に入る。

| 何が起きたか | どうするか |
| --- | --- |
| 自動化が動かし、対応表に戻す先がある | **止めない。**その Status へ書き戻し、**戻した先で判定し直す** |
| 自動化が動かしたが、対応表に無い | いままでどおり止める。**足す2行を issue のコメントに書く** |
| 自動化が動かしたが、書き戻す回数が上限（3回）に達した | 書き戻さずに止める。**「押し合っている」ことを issue に書く**（同じ行を足させない） |
| 人間が動かした | **いままでどおり。**猶予を置いて止める |

**なぜ書き戻すのか。**止めないだけだと、人間の列（`In Progress`）に continuo が担当中の issue が
居座る。**人間はボードを見て状況を判断するので、列を分けた意味が消える。**
**なぜ上限を置くのか。**書き戻した直後に自動化がまた動く組み合わせがあると、
continuo とボードが同じ issue を押し合い続けるからである（`maxAutomatedRewrites`）。

**上限を数えるのは、ボードが実際に動いたときだけである。**通信の失敗・item が見えない・
既にその値だった、のいずれでも枠を返す（`releaseAutomatedRewrite`）。
**返さないと、書き込みが3回失敗しただけで押し合いと同じ扱いになり、worker が止まる。**

**書き戻したら、戻した先の Status で「終わりかどうか」を判定し直す**（`rewriteAndDecide`）。
**終わりの判定は書き戻しの前に済んでおり、書き戻しはそのあとで Status を変える。**
判定し直さないと、**終わった issue へ次のプロンプトを送る。**

**巡回から書き戻すときは、`beginTerminal` で印を取ってから書く**（`rewriteAutomatedStateAsync`）。
**書き込みが飛んでいる間に turn ループが失敗して run を手放すと、印が消えたあとに
「作業中」の Status がボードへ書かれる。**次の巡回はそれを候補として拾い直し、
**同じ worktree に2本目の Claude Code を立てる。**書き終えたら印を返す（`endTerminal`）。
...
**設定の検査は5つを見る**（`config.Validate` の `validateAutomatedStateRewrite`）。
...
| **値が `active_states` の外** | 戻した先で run が終わるか、worktree が消える |
| **大文字小文字だけが違うキーが2つ** | どちらに当たるかが map の反復順で決まり、実行のたびに変わる |

**値（戻す先）は起動時にボードとも照合する**（`requiredStatesForBootstrap`）。綴りの誤りをそこで落とす。
**戻す先を集める処理は `config.AutomatedRewriteTargets` の1箇所である。**
起動時に照合する集合と、実行時に知っている Status の集合（`knownStates`）を食い違わせない。

**リクエストは増えない。**ID 指定の取り直し（`nodes(ids:)`）の `... on Issue` に
`timelineItems` を足すだけで、Status の値と同じ1リクエストで返る（設計 2-6 の実測）。
**候補の取得（100件返る）と識別子での照合には足さない。**1件ずつにしか意味の無い値である。
**Status を書く前の取り直しにも足さない**（`byIDsWithoutTimelineQueryTemplate`）。
書き込みの経路は取り直した `State` しか見ない。
**`project.number` で自分のボードへ絞る**（複数のボードに載っていると両方返る。設計 2-6）。
**窓は `last: 50` である。**`timelineItems` にボードで絞る引数が無いので、
**絞るのは返ってきたあとである。**別のボードで Status が何度も動くと、自分のボードの
イベントが窓から押し出され、「誰が書いたか分からない」になって書き戻しが効かなくなる。

**書き戻しに失敗しても run は止めない。**次の巡回で拾い直す。
**書き戻したら「何から何へ動かしたか」を issue に1件残す**（3-29）。
...
18本のまま据え置き、CFG のテストパスも814本のままである。
...
| `失うものを調べられない` | 足さない（実装に分岐が無い） | 基本フローのステップ27 とステップ32。`Inspect` は git の失敗をエラーにせず直線で通す |
...
**CFG は再生成しても1バイトも変わらない。**テストの印も貼り直していない。
