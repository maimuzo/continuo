...
**症状。**2026-09-06 に実測した。担当しているリポジトリそのものへ起票しようとしたところ、
判定役はこう返した（**リポジトリ名は `<owner>/<repo>` に伏せてある。**それ以外は原文である）。

> This command attempts to create a GitHub issue in an **external repository (`<owner>/<repo>`)** unrelated to the current work context.

（**訳:** このコマンドは、いまの作業と関係のない**外部のリポジトリ（`<owner>/<repo>`）**に issue を作ろうとしている）
...
| **他の条件を免除するか** | **しない。同じ文の中で言い切る** | 条件の中に置いても、**免除の1文だけを読んだ判定役が、他の条件まで通す余地が残る。**とくに2つ目（資格情報の持ち出し）が危ない。`gh issue create --repo <担当のリポジトリ> --body "$(cat <資格情報のファイル>)"` は、**担当しているリポジトリが相手なので、この1文だけを読むと通る。公開リポジトリなら、その issue は誰でも読める** |
