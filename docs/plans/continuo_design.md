...
| **`blocked`** | `止まりました: <どこまで見て、なぜ止まったか>`。**行頭に「止まりました:」を付けてください。**
**付けないと、上に「pull request: …」が並んでいる issue では、読む人が「直った」と読みます。**
**pull request の URL を書かないでください。**直していないので、指す先がありません |

**そのうえで書き換えます。`$URL` から置き直してください。**
**上の塊で置いた変数は、ここへ引き継がれません。**道具は塊ごとに別のシェルで走ります
（**実測。**この指示書を書いた Claude Code の Bash の道具で、
1つ目の塊で変数と関数を置き、2つ目の塊で読み直しました。**どちらも残っておらず、`$$` も別の値でした**）。
...
**2件以上になったことは、代表へ書く成果報告にも1行添えてください。**
...
**`review` を出した issue には、こう書きます。**
    ...
    <段2a が落ちたときだけ、ここに「前に書いた分は読めませんでした。」を1行足す>
...
| いつ | なぜ2件になるか |
| --- | --- |
| **段1 が URL を返したのに、段2a が読み取りに失敗した** | 書き換える先が分からないので、段2b で新しく1件投稿させる。**比べているのは「今回の分が1文字も残らない」と「前回の分と重ねて残る」で、後者のほうがましである。****そのとき前の分は書き直させない**（読めていないので、書き直すと同じ話が2回並ぶ）。**1行目に「前に書いた分は読めませんでした」と書かせ、今回の分だけを書かせる。****2件になったことは、代表へ書く成果報告にも1行添えさせる** |
