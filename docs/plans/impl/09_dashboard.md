...
| `LastSeenAt` | hook のほか、turn を送った・枠待ちを外した・`agent_status` が `working` だったのを見たとき | 打ち切りの判定（3-21）。JSON では `stall_clock_at` として出す |
...
**採り直すと、台帳の項目に入っている `session` と食い違うので、差分ではなく全額が足される**
（鍵そのものは issue の識別子のままで、変わらない）。**失うのは台帳の項目1件ぶんのメモリだけである。**

**そこへ `forgetTokenLedger` を足すなら、`ws.Cleanup` の戻り値を必ず見ること。**
`reconcileWorktrees` は `result.Removed` をログに出すためだけに使っているので、
**`if result.Removed` の中へ置かないと、見送られたのに落とす形になる。**
