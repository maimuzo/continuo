...
上限に達すると落ちるのは古い側である**（[internal/tracker/adapter.go:1235-1267](../../../internal/tracker/adapter.go#L1235-L1267)）。
...
（[internal/tracker/adapter.go:1264-1267](../../../internal/tracker/adapter.go#L1264-L1267)。
...
| どこ | どうするか |
| --- | --- |
| [internal/tracker/adapter.go:1225](../../../internal/tracker/adapter.go#L1225) の `fetchCommentNodes` | 戻り値に `truncated bool` を足す。**`keep` で抜けたときは偽**（狙って止めたので、切れていない） |
...
| [internal/orchestrator/handoff.go:108](../../../internal/orchestrator/handoff.go#L108) と [:716](../../../internal/orchestrator/handoff.go#L725) | 111 は `truncated` を使う。716（担当を確かめ直す経路）は `_` で捨てる |
...
その機械は止まらない（[internal/orchestrator/handoff.go:708-720](../../../internal/orchestrator/handoff.go#L708-L720) が
...
（[internal/orchestrator/handoff.go:482-510](../../../internal/orchestrator/handoff.go#L482-L510)）ので、
...
| 主張 | 根拠 |
| --- | --- |
| **`FetchAllComments` は2000件までは落とさない** | [internal/tracker/adapter.go:1155](../../../internal/tracker/adapter.go#L1155) が `fetchCommentNodes(ctx, issueNodeID, maxCommentsPerFetch, 0)` を呼ぶ。`keep` が0なら `keep` では打ち切らない（[internal/tracker/adapter.go:1253](../../../internal/tracker/adapter.go#L1253) の `if keep > 0 && unmarked >= keep`）。**ページ数では打ち切る**（[internal/tracker/adapter.go:1235](../../../internal/tracker/adapter.go#L1235) の `for page := 0; page < maxCommentPages; page++`。`maxCommentPages` は20、`maxCommentsPerFetch` は100） |
| **上限で落ちるのは古い側である** | [internal/tracker/query.go:253](../../../internal/tracker/query.go#L253) が `orderBy: { field: UPDATED_AT, direction: DESC }` で取り、[internal/tracker/adapter.go:1270-1273](../../../internal/tracker/adapter.go#L1270-L1273) が最後に反転して古い順へ戻す。**打ち切りは新しい側を読み終えた時点で起きる** |
| **上限に達したことはログに出るが、戻り値からは分からない** | [internal/tracker/adapter.go:1264-1267](../../../internal/tracker/adapter.go#L1264-L1267) が `Warn("コメントが多すぎるので途中まででやめました（古いコメントは読めていません）", …)` を出すだけで、`FetchAllComments` の戻り値は `([]Comment, error)` のままである（[internal/tracker/adapter.go:1150-1158](../../../internal/tracker/adapter.go#L1150-L1158)）。**だから戻り値に真偽値を1つ足す**（7-1） |
| **件数では切れを当てられない** | 打ち切りは [internal/tracker/adapter.go:1235](../../../internal/tracker/adapter.go#L1235) の `for page := 0; page < maxCommentPages; page++` を、続きの cursor を持ったまま抜けたかどうかで決まる。**`len(nodes)` は1ページの件数が100に満たなくても増えないので、2000未満のまま切れることがある** |
