...
| `Status の名前` | 設定に書いた Status と紛らわしい選択肢がカンバンに無いか（記号は `!` だけ。設計 6-14） |
| `対応表のキー` | `tracker.automated_state_rewrite` のキーがカンバンの Status の選択肢にあるか（記号は `!` だけ。設計 3-57） |
| `自動化` | カンバンの自動化が1つでも有効なのに `tracker.automated_state_rewrite` が空でないか（記号は `!` だけ。設計 3-54。issue #209） |
...
**`<owner>/<repo>` は設定に無い。**doctor が**カンバンを1回読んで**、返ってきた issue の `nameWithOwner` を集める（設計 3-32）。
...
| ファイル | 何が入っているか |
| --- | --- |
| [internal/doctor/doctor.go](../../../internal/doctor/doctor.go) | `Options` / `Run`（見出し語の呼び出し順と依存の適用）／カンバンから対象リポジトリを集める処理 |
...
| --- | --- |
| 設定ファイル | `config.Load` |
| 片付けの状態 | `config.CleanupStatesOutsideTerminal`（**起動時の警告と同じ関数を呼ぶ。**カンバンは読まない） |
| herdr | `herdr.Client.CheckProtocol`（**`herdr status` の CLI は使わない**） |
| gh の認証 | `tracker.CheckGHAvailable` / `tracker.CheckGHProjectScope` |
| カンバン | `tracker.ResolveToken` → `tracker.Adapter.Bootstrap` → `FetchIssuesByStates` |
| Status の名前 | `tracker.Adapter.StatusOptionNames`（カンバンを読んだときの応答を使い回す。リクエストは増えない） |
...
| 決めたこと | なぜ |
| --- | --- |
| **`カンバン` の検査は Bootstrap と候補の取得の両方を含む** | 対象リポジトリはカンバンを読まないと決まらない（3-32）。どちらで落ちても「カンバンを読めなかった」であり、下流は同じく `!` になる |
...
| **集計の行は `✗` が0件なら「問題があります」と書かない** | 対象リポジトリが0件（カンバンが空）でも `!` が2件出る。**カンバンが空なのは設定の誤りではない**（3-32）ので、最後の1行を読んで問題があると読めてはならない |
...
設定ファイル ─┬─ herdr（設定の protocol と照合する）
              └─ gh の認証 ── カンバンを読める ─┬─ clone（対象リポジトリが決まる）
                                                └─ 信頼登録（clone のパスが要る）
...
**本番のカンバンへは1リクエストも送らない**（`httptest.Server` のテスト用GraphQL mock）。
