...
| パッケージ | 何を |
| --- | --- |
| `cmd/continuo`（`doctor` サブコマンド） | **全項目**を検査して結果を出す（下記の見出し語で出す） |
| `internal/doctor` | 各検査の実装 |

**検査する項目と、出力に出す見出し語。**この語で固定する（設計 3-32）。
**いくつあるかはここに書かない。**正は [internal/doctor/report.go](../../../internal/doctor/report.go) の `Label` 定数である。
...
**言いたいこと。**`continuo doctor` は `internal/doctor` にあり、全項目を検査してから
...
| ファイル | 何が入っているか |
| --- | --- |
| [internal/doctor/doctor.go](../../../internal/doctor/doctor.go) | `Options` / `Run`（検査の呼び出し順と依存の適用）／ボードから対象リポジトリを集める処理 |
| [internal/doctor/checks.go](../../../internal/doctor/checks.go) | 各項目の検査 |
