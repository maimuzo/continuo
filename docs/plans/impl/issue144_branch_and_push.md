...
| `issueNumberFromSlug` に渡す owner/repo | **pane の label から取った issue の owner/repo を先に使う。**label が無ければ**置き場所の owner/repo で試す**（下） |
...
**label が無いときに「渡さない」ではなく「置き場所の owner/repo で試す」のはなぜか。**
**コードのリポジトリが issue のリポジトリと同じなら、置き場所の owner/repo が正しい値だからである**
（リンクを張っていない worktree は全部そうである）。
渡さない形にすると、**いままで復元できていた worktree が pane を失った瞬間に復元できなくなる。**
**間違った番号を切り出す危険は無い。**`issueNumberFromSlug` は
テンプレートを描画して固定部分と突き合わせるので、**cross-repo では突き合わせが外れて 0 を返す。**
**番号を切り出せたときの owner/repo は `PathClue` に控え、`Identifier()` はそれで組み立てる。**
...
`data` に足さない側である**（13 の「push 先を分けても片付く」）。
...
| 何 | どうする |
| --- | --- |
| 印の置き場所 | **メモリだけ**（3-68）。`o.notified`（[internal/orchestrator/orchestrator.go](../../../internal/orchestrator/orchestrator.go)）と同じ形の `o.repoNotices` |
...
**印は `gate.go` の `gated` とは別に持つ。**#134（ダッシュボードに「着手できずに止まっているもの」を出す）が
[internal/orchestrator/gate.go](../../../internal/orchestrator/gate.go) に同じ数え方（3回・60秒）を既に持っているが、
**あちらの記録は `clearGate` が「関門より前で飛ばした」ときに数え直す。**
[internal/orchestrator/dispatch.go](../../../internal/orchestrator/dispatch.go) は
`preflight` が偽を返した直後に `clearGate` を呼ぶので、
**`preflight` の中で止まるこの経路をあちらに載せると、回数が3に届く前に必ず 0 へ戻り、
案内が一度も書かれない。**
**数え方（`noticeMinCount` / `noticeMinAge`）はあちらの定数をそのまま使う。**二重に作るのはその外側だけである。
...
**issue へ書く本文（文面1・文面3）は `internal/i18n` を通さない。**
[internal/orchestrator/prompt.go](../../../internal/orchestrator/prompt.go) の `buildGatedComment` が
「`internal/orchestrator` の人間向けの文言はまとめて資源へ移す」と決めており、
**この issue だけ先に移すと、同じ画面に2つの流儀が混ざる。**同じ形で Go の文字列として組み立てる。

**文面2 は `internal/i18n` へ入れる。**あれは
[internal/workspace](../../../internal/workspace) が返すエラーであり、
そちらは初めから資源を通している。**入れたら `en.json` の `_source_sha256` を入れ直す。**
3本目は 11f。
...
**この文書が、その1つを採る。**3-69 の節は決定済みに書き換える（未決定の表を消す。13 の「push 先を分けても片付く」）。
...
**貼り先2。**「push 先を分けても片付く」で入った「別の名前へ push するとき」の下に、
リンクされた branch を **push 先の候補として名前で出す**行を積む。**既定は動かさない。**
...
**言いたいこと。**3つの表は**全部で30行**であり、30ファイルではない。**1本の PR にしない。**
**それぞれ、それだけで筋の通った状態になるように切る。**順に出す（3つ目は 13b。出し方は 13c）。

**数えた単位。****行数であってファイル数ではない。**
`git.go` / `prepare.go` / `dispatch.go` / `query.go` / `template.go` / `continuo_design.md` /
`internal/i18n` / `docs/FAQ.md` は
**2つ以上にまたがるので重複して数えている。**重複を除くと**20行**になる。
...
**「push 先を分けても片付く」（6行）。**これだけで「通常」と「1つの issue で PR を複数」が閉じる。

| ファイル | 何を |
| --- | --- |
| [internal/scaffold/template.go](../../../internal/scaffold/template.go) | 12 の貼り先1と2（**`{{.push_branch}}` を含む行はここでは入れない。**下） |
| [internal/workspace/cleanup.go](../../../internal/workspace/cleanup.go) | 9 の4段 |
| [internal/workspace/git.go](../../../internal/workspace/git.go) | `gitRemoteRefContainsHead`（`for-each-ref --count=1 --contains HEAD`）。**「リンクした branch を起点にする」でも触る** |
| [docs/plans/continuo_design.md](../continuo_design.md) | 3-9 の手順2b、5-3 の本文、5-3b の1行、**3-69 を決定済みに書き換える**（12）。**「リンクした branch を起点にする」「issue とコードを別のリポジトリに置ける」でも触る** |
...
**「push 先を分けても片付く」で入れる本文は、変数を使わない形である。**
...
**変数化は「issue とコードを別のリポジトリに置ける」で、`prompt.go` に `.push_branch` を足すのと同じ PR で行う**（12 の末尾の形へ差し替える）。

**「リンクした branch を起点にする」（+10行）。**「既存 branch の続き」が閉じる。
...
| [internal/workspace/prepare.go](../../../internal/workspace/prepare.go) | `resolveBase` の5段、段4 の fetch。**「issue とコードを別のリポジトリに置ける」でも触る** |
| [internal/workspace/git.go](../../../internal/workspace/git.go) | `gitFetchBranch`。**「push 先を分けても片付く」でも触る** |
| [internal/config](../../../internal/config) | `workspace.fetch_timeout_ms`（`types.go` の `WorkspaceConfig` / `default.go` / validate） |
| [docs/plans/continuo_design.md](../continuo_design.md) | **5-2 の front matter に `fetch_timeout_ms`**（11c）。**「push 先を分けても片付く」「issue とコードを別のリポジトリに置ける」でも触る** |
| [internal/orchestrator/dispatch.go](../../../internal/orchestrator/dispatch.go) | `toIssueRef` の写し。**「issue とコードを別のリポジトリに置ける」でも触る** |
| [internal/i18n](../../../internal/i18n) | 11e の文面1と2（keys / ja / en）。**「issue とコードを別のリポジトリに置ける」でも触る** |
| [docs/FAQ.md](../../FAQ.md) | **fork を使うボードでは `herdr.worktree.base` を null にする**（11a）。**「push 先を分けても片付く」「issue とコードを別のリポジトリに置ける」でも触る** |

---

## 13b. 「issue とコードを別のリポジトリに置ける」と、受け入れの通し方

**言いたいこと。**3つ目で「既存 OSS への PR」が閉じる。
**`verifiedRepo` は1行も触らない。**信頼を確かめる相手だけがコード側へ移る。

**「issue とコードを別のリポジトリに置ける」（+14行）。**

| ファイル | 何を |
| --- | --- |
| [internal/workspace/layout.go](../../../internal/workspace/layout.go) | `Locate` が使う host / owner / repo をコード側へ |
| [internal/workspace/prepare.go](../../../internal/workspace/prepare.go) | `ghqList`（[:153](../../../internal/workspace/prepare.go#L153)）と、段0 の `CheckWorktreeUsable` が使う `clonePath`（[:450](../../../internal/workspace/prepare.go#L450)）の**両方**をコード側へ。`clonePath` の内部キャッシュの鍵（[internal/workspace/repo.go:127](../../../internal/workspace/repo.go#L127) の `key := owner + "/" + repo`）もコードのリポジトリで引かれる。**「リンクした branch を起点にする」でも触る** |
...
| [internal/tracker/query.go](../../../internal/tracker/query.go) | **`Dispatchable` を判定する相手をコード側へ**（下）。**「リンクした branch を起点にする」でも触る** |
| [internal/orchestrator/prompt.go](../../../internal/orchestrator/prompt.go) | 11d の変数 |
| [internal/scaffold/template.go](../../../internal/scaffold/template.go) | 12b の段落と6本の差し替え、12 の貼り先2を `{{.push_branch}}` の形へ。**「push 先を分けても片付く」でも触る** |
| [docs/plans/continuo_design.md](../continuo_design.md) | 5-3 の本文を同じ内容へ。**「push 先を分けても片付く」「リンクした branch を起点にする」でも触る** |
| [internal/orchestrator/dispatch.go](../../../internal/orchestrator/dispatch.go) | 信頼（`CheckTrust`）の相手をコード側へ（下）。**「リンクした branch を起点にする」でも触る** |
| [internal/i18n](../../../internal/i18n) | 11f の文面（keys / ja / en）。**「リンクした branch を起点にする」でも触る** |
| [docs/FAQ.md](../../FAQ.md) と [docs/upgrading.md](../../upgrading.md) | 身元ファイルを手で書いて復元する手順（11c のサンプルを載せる）と、pane も身元ファイルも無い cross-repo の worktree の片付け方（8c2）。**「push 先を分けても片付く」「リンクした branch を起点にする」でも触る** |
...
決めている。**3つ目は、開発者の環境の fork（`<ACCOUNT>/oss-project`）と
テスト用のボード（project #10（実データを持たない検証用のボード））で通す。**
**本番のボード（project #3（AI自動進行管理。実データが入っている））では試さない。**

---

## 13c. 3つを別々に出す — 出し方と、まとめて出さない理由

**言いたいこと。**「push 先を分けても片付く」を先に1本出し、
**「リンクした branch を起点にする」と「issue とコードを別のリポジトリに置ける」は
一緒に1本で出す。****後ろの2つを分けると穴が出る。**

**出し方。**

| 何を出すか | いつ出すか |
| --- | --- |
| **「push 先を分けても片付く」** | **先に単独で出す。**片付けの判定だけで閉じるので、他の2つを待たせない |
| **「リンクした branch を起点にする」＋「issue とコードを別のリポジトリに置ける」** | **一緒に1本で出す** |

**後ろの2つを分けない理由。**「リンクした branch を起点にする」だけを入れると、
**リンクの先が別のリポジトリだった場合に置き場所と clone が食い違う。**
そこを埋めるには「着手せずに1回知らせる」入口を新しく作ることになり、
**次の版でそれを消すことになる。**使われない入口を1つ増やして、すぐ捨てる形である。

**「issue とコードを別のリポジトリに置ける」だけを先に出すこともできない。**
コードのリポジトリを知る手立てが Development のリンクしか無く、
それを読むのは「リンクした branch を起点にする」の側だからである。

**この2つが一緒だと何が重くなるか。**復元（3-49）・`continuo abandon`・信頼の関門の3つが、
**照合する相手をコードのリポジトリへ移す。**どれも**壊れても静かに壊れる**ところであり、
実機で1件通しても踏まない経路が多い。**だから受け入れは実機の1件では足りない**
（13b の受け入れの表と、テストの側の分岐を両方通すこと）。
