...
| **効果の根拠** | **無い。**「差分だけにして周回が減った実測」は、**4箇所で探して見つかっていない**（[../../review-loop-efficiency.md:475](../../review-loop-efficiency.md#L475)、[05:711](05_industry_tools_blogs.md#L711)、[06:510](06_academic.md#L510)、[04:728](04_anthropic_official.md#L728)）。採る根拠は「Bugbot が既定でそうしている」だけで、その出典は WebFetch の要約モデル経由である |
...
| **未検証の前提** | **`/code-review` に2つの SHA を渡せるかを、誰も試していない**（[../../review-loop-efficiency.md:498](../../review-loop-efficiency.md#L498)）。このセッションの skill の一覧に出る `code-review` の説明文は「Review the current diff, or a **PR number/branch/path target**」で、**commit の範囲を受け取るとは書いていない。**計画でいちばん構造を変える決めごとが、いちばん確かめられていない前提の上に乗っている |
...
| **費用** | **PR 1本につき1周分×10本。**しかも「出た Critical と High は直して回し、その周も10回に数える」（[../../review-loop-efficiency.md:463](../../review-loop-efficiency.md#L463)）。**2026-09-05 の「収まったら最大1周」を、測る10本のあいだだけ超えることを、計画自身が0節の決めごとにしている** |
