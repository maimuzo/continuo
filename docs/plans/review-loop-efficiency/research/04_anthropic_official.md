...
| **なぜ困るか** | 1周ごとにレートリミットと時間を使う。直しが新しい欠陥を持ち込む機会も増える（[CLAUDE.md:593](../../../../CLAUDE.md#L593) の実例: 8周目の Critical が7周目の直しから生まれた） |
...
- [CLAUDE.md:502](../../../../CLAUDE.md#L502) の「手順」の段1（対応表を書く前）に段を足す。Critical と High の指摘1件ごとに検証役を立て、指摘の文と PR の意図だけを渡す。返させるのは「再現できる file:line と、壊れる筋書き」である。返せなかった指摘は「直さない」とし、理由欄に「検証役が再現できなかった」と書く。
...
`git grep -n 'Critical' -- CLAUDE.md .claude/rules .claude/skills`（HEAD `df36f9d7`）で23行が返った。定義らしい行は [CLAUDE.md:521](../../../../CLAUDE.md#L521) の「Critical / High / Medium / Low / Info」だけで、各レベルに何が入るかは書かれていない。
...
**効く観点。**レビュワー側。ループの制御にも効く。[CLAUDE.md:548](../../../../CLAUDE.md#L548) の「収まっている」は Critical と High の件数だけで決まるので、何を High と呼ぶかがぶれると判定もぶれる。
**制約。**REVIEW.md は `/code-review` に届かない（3-4）。Agent のレビュワーには渡せる。

**当てはめうる場所（案）。**

- [CLAUDE.md:516](../../../../CLAUDE.md#L516) の「対応表の列」の「レベル」に、日本語の規則文書と Go のコードそれぞれで Critical と High に入る種類を列挙した定義を足す。
  文書の例: 「書かれたとおりに従うと、利用者か AI が壊れる操作をする」だけを High 以上にする。この例は worker が作ったもので、公式の記述ではない。
- 「この PR より前から在る」を別の印にし、[CLAUDE.md:548](../../../../CLAUDE.md#L548) の件数から外すかを検討する（3-7）。

### 3-6. 2回目以降のレビューでは、新しい軽微な指摘を出させず、件数に上限を置く

**何をするか。**1回目のあとは Important（直してからマージ）だけを出させる。Nit の件数には上限を置く。
**いまの規則と同じ向きか。**同じ向き。2026-09-05 の人間の指示（[CLAUDE.md:555](../../../../CLAUDE.md#L555)）が、Medium と Low について近いことを決めている。
...
- [CLAUDE.md:555](../../../../CLAUDE.md#L555) の節を「2周目からは Critical と High だけを出させる」へ広げる。
...
- [CLAUDE.md:548](../../../../CLAUDE.md#L548) の「収まっている」の件数から「この PR より前から在る」指摘を外し、CLAUDE.md が既に決めている follow-up の扱いに回す。
...
- Academy の「修正が大きな変更に育ったら再レビュー」を、[CLAUDE.md:555](../../../../CLAUDE.md#L555) の周回の判断に取り込めるか検討する。いまは Critical か High を直したら、必ず次の周を回す。
...
- 範囲外の指摘は Critical と High に数えない。これは [CLAUDE.md:548](../../../../CLAUDE.md#L548) の定義を変えることになる。
...
**効く観点。**ループの制御。
**制約。**無い。
**当てはめうる場所（案）。**[CLAUDE.md:595](../../../../CLAUDE.md#L595) と [:645](../../../../CLAUDE.md#L645) の「3・6・9回目」の段に、次の条件を足す。

> Critical と High の件数が2周続けて減らなかったら、その時点で修正を止め、対応表から学んだことを入れた設計で、新しい文脈の書き手に書き直させる。

いまの6段（[CLAUDE.md:731](../../../../CLAUDE.md#L731) 以下）には「実装を止める → 設計を敵対的レビューする → 実装し直す」が既にある。**入る時点を早めるだけで足りるかを検討する。**
...
- レビューループに関わる規則を「消したら AI が間違えるか」で1行ずつ見直す。対象は [CLAUDE.md:486](../../../../CLAUDE.md#L486) 以下、[.claude/rules/design-review.md](../../../../.claude/rules/design-review.md)、[worker-briefing 2-5〜2-7](../../../../.claude/skills/worker-briefing/SKILL.md#L183)、[pr-review-and-merge 段2〜4](../../../../.claude/skills/pr-review-and-merge/SKILL.md#L93)。
