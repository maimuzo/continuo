...
| 止まる箇所 | 打つ手 |
| --- | --- |
| **権限の確認** | `--permission-mode auto`（既定）で起動する。**判定役が会話を読んで決めるので、人手の入力を待たない** |
...
**`auto` が既定である。**判定役が会話を読んで可否を決めるので、人手の入力を待たない。**遮断が続いたときに確認方式へ戻るかは、実機で観測できていない。**
...
| **`Bash` 以外の確認で herdr が `blocked` を返すか** | **`dontAsk` では権限の確認が出ない**（既定の `auto` では、判定役の遮断で確認が出うる）**。**確認を出すには権限モードを変える必要があり、**それは continuo の運用と違う条件になる** | **拾えない確認があれば、その turn は時間切れで人間へ渡る。**止まったまま残ることはない |
