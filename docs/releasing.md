...
1. **目印がコメントの先頭にある。**本文の途中や、引用・説明の中に出てきただけのもの
   （例:「◯◯のコメントに目印が無い」という指摘そのもの）は数えない
2. **投稿者が、このリポジトリの OWNER / MEMBER / COLLABORATOR のいずれかである。**
   このリポジトリは PUBLIC なので、通りがかりの人が貼った目印まで数えると、
   誰でもその PR を「レビュー済み」にできてしまう

本文に `code-review` と書いただけのコメントも数えない。それを数えると、「レビューの話をしただけ」の PR が通ってしまう。

**外部の人（Contributor など）が `/code-review` の結果を貼っても数えられない。**
2つ目の条件を満たさないためである。そのときは、OWNER / MEMBER / COLLABORATOR のいずれかが
結果の内容を確認したうえで、先頭に目印を置いたコメントとして貼り直す。
