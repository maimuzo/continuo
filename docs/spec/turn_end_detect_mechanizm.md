...
**いまの実装は、この値を判定に使っていない。**
[internal/orchestrator/reconcile.go:651-658](../../internal/orchestrator/reconcile.go#L651-L658) が
`agentInfo` の応答から `agent.AgentStatus` を取り出しているが、**ログの1行に載せているだけである。**
判定に使っているのは同じ応答の `agent.Revision` で、**そちらは continuo の pane では動かない。**

**つまり、正しい値を手に持ったまま、動かない値で判定している。**`agent.get` を追加で叩く必要は無い。
