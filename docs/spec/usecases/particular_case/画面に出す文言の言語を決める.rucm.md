...
- `docs/plans/continuo_design.md` の「3-35g. symlink にも hardlink にもできない」（`en.json` を `ja.json` の複製として持つ理由）
...
- `internal/cli/cli.go` の `runTrust` / `runDoctor` / `runAbandon` / `runMain` / `runPrompt` / `runSetup`
...
| `continuo prompt` | 読む（`--builtin` を除く） | `internal/cli/cli.go` の `runPrompt` |
| `continuo setup` | 読む | `internal/cli/cli.go` の `runSetup` |
| `continuo init` / `continuo hook` / `continuo version` / `continuo allow-keychain-access` | 読まない | 環境変数から決めた言語のまま出す |

**`continuo prompt --builtin` だけは読まない。**組み込みの指示書は実行ファイルの中にあるので、
設定が壊れている利用者でも読める。**そのために設定を1バイトも読まずに返す。**

**`continuo trust` と `continuo setup` は決め直す場所が違う。**この2つは自分で設定を読んでから決め直す。
**残りは `useLanguageFromConfig` に読ませ、読めなければ黙って戻る**
（読めないこと自体は、それぞれのコマンドが自分の文言で報告する）。

**読めなかったときに止まるかどうかは、コマンドごとに違う。**
`continuo trust` はそこで終わる。**`continuo setup` は止まらない。**
`continuo setup` は、`continuo init` が `gh` から値を引けずプレースホルダの残った `WORKFLOW.md` に対しても走るコマンドであり、
止めると、その利用者が Status の割り当てを1回も終えられなくなる。**止まらないが、読めなかった理由は出す。**

**`continuo setup` が読むのは、書き換える `WORKFLOW.md` があると確かめたあとである。**
それより前に出る文言は、環境変数から決めた言語のまま出る。
引数の誤り・`--help` の使い方と、`scaffold.CheckUpdatable` が返す失敗の全部
（`WORKFLOW.md` が無い・symlink である・ディレクトリでない・Status のキーが無い・キーの値を書き換えられない）が当たる。
