...
WHEN 使い切っている枠に1週間の枠が含まれ、その枠が明けるまでの待ち時間が weekly_wait_limit_minutes を超える場合
1. システムは herdr に画面の版を要求する。
2. IF 画面の版が前より増えている THEN
3.   システムは担当を手放さない。
4.   RESUME STEP 12
5. ENDIF
6. システムは issue の担当者を読み直す。
7. IF 担当者が自分でない THEN
8.   システムは workspace_hooks.after_run を実行しない。
9.   システムは herdr の pane を閉じる。
10.   ABORT
11. ENDIF
12. システムは workspace_hooks.after_run を実行する。
13. システムは issue の担当者から自分を外す。
14. システムは担当を外したことを issue のコメントに書く。
15. システムは herdr の pane を閉じる。
16. システムは worktree を残す。
17. ABORT
...
| 枠待ちをやめて担当を手放す | 使い切っている枠に1週間の枠が含まれ、待ち時間が weekly_wait_limit_minutes を超える。かつ画面の版が増えていない。かつ担当者がまだ自分である | after_run を走らせ、担当者から自分を外し、pane を閉じる。worktree と Status はそのまま |
| 画面が動いているので手放さない | 上の条件に当たるが、agent.get の版が前より増えている | 何もしない。枠待ちを続ける |
...
flowchart TD
    ...
    B12 -. "待つ上限を超えた: WHEN 1週間の枠で、待ち時間が上限を超える場合" .-> G2S1
    ...
    subgraph GAF2 ["GLOBAL ALTERNATIVE FLOW 待つ上限を超えた / BRANCH FROM BASIC FLOW 12"]
        G2S1["1. 画面の版を要求する"] --> G2S2{"2. IF 版が前より増えている"}
        G2S2 -- 真 --> G2S3["3. 担当を手放さない"] --> G2S4["4. RESUME STEP 12"]
        G2S2 -- 偽 --> G2S5["5. ENDIF"] --> G2S6["6. 担当者を読み直す"] --> G2S7{"7. IF 担当者が自分でない"}
        G2S7 -- 真 --> G2S8["8. after_run を実行しない"] --> G2S9["9. pane を閉じる"] --> G2S10["10. ABORT"]
        G2S7 -- 偽 --> G2S11["11. ENDIF"] --> G2S12["12. after_run を実行する"] --> G2S13["13. 担当者から自分を外す"] --> G2S14["14. 担当を外したことをコメントに書く"] --> G2S15["15. pane を閉じる"] --> G2S16["16. worktree を残す"] --> G2S17["17. ABORT"]
    end
