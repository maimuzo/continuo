...
| 48 | ステップ21 の条件を反転させたこと | 「branch が実在する」から「branch が実在しない」に変えて IF-ELSEIF-ELSE の3分岐にした | **`cleanup.delete_branch` を見る分岐を入れる場所が要る。**実在しないときは設定に関わらず「消す対象が無かった」であり、設定の判定より先に置かないと、元から無い branch を「設定で残しました」と報告することになる | `internal/workspace/cleanup.go` の `Cleanup` の branch の switch（`branchAbsent` を先に見て、次に `!DeleteBranch`） | 95% |
| 49 | ステップ23 と24 を足したこと | ELSEIF 設定の cleanup.delete_branch が偽である THEN、branch を残し、残ったものとして利用者に伝える | **設定で消さないと決めた branch を、消したかのように報告してはならない。**実装は残ったものの一覧へ積んでいるのに、RUCM にその状態が無かった | `internal/workspace/cleanup.go` の `case !m.cfg.Cleanup.DeleteBranch`、`internal/i18n/messages/ja.json` の `workspace.leftover.branch_disabled` | 95% |
| 50 | 基本フローの事後条件を書き換えたこと | 「branch は無い」から「cleanup.delete_branch が真であれば無い。偽であれば残っており、残ったものとして利用者に伝えている」へ | **元の事後条件は設定が偽のときに必ず偽になる。**テストが到達すべき状態として使えない | `internal/workspace/cleanup.go` の `CleanupResult.Leftovers` | 95% |
| 51 | 壊れた ref の始末を設定の判定より前に置いたこと | GLOBAL ALTERNATIVE FLOW `壊れたref` の BRANCH FROM はステップ21 のまま | 壊れた ref は実在の検査が「無い」と答える。**そこが分岐元であり、設定の判定はその先にある。**設定が偽なら `gitBranchDelete` を呼ばないので、壊れた ref のファイルも消えない | `internal/workspace/cleanup.go` の `brokenRefPolicyFor`（`default:` の枝からしか呼ばれない） | 85% |
| 52 | 起動時の孤児 branch の掃除をこのユースケースに書かなかったこと | 掃除の手順は `再起動して実行中の issue を引き継ぐ` の基本フローに置き、ここからは参照だけにした | **契機が違う。**このユースケースの主アクターは巡回タイマーであり、掃除は起動の手順の一部である。同じ手順を2箇所に書くと必ずずれる | `internal/orchestrator/sweep.go` の `SweepOnStartup`（`Restore` のあとにだけ呼ばれる） | 85% |
