...
- `internal/doctor/checks.go` の `checkConfig` / `configRemedies` / `filesystemRemedies` / `writeRemedies` / `checkCleanupStates` / `checkClaude` / `checkRuntimeDir` / `checkClaudeHome` / `checkWorkspaceRoot` / `checkHerdr` / `checkGHAuth` / `checkBoard` / `boardFailure` / `checkClone` / `checkTrust` / `checkCredentials` / `checkKeychainCredentials`
