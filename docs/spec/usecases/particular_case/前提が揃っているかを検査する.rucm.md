...
- `internal/doctor/checks.go` の `checkConfig` / `configRemedies` / `filesystemRemedies` / `writeRemedies` / `checkClaude` / `checkRuntimeDir` / `checkClaudeHome` / `checkWorkspaceRoot` / `checkHerdr` / `checkGHAuth` / `checkBoard` / `boardFailure` / `checkClone` / `checkTrust` / `countDetail` / `checkCredentials` / `checkKeychainCredentials`
