...
- `internal/orchestrator/unknownstate.go` の `knownStates`、`handleUnknownState`、`stopForUnknownStateAsync`、`finishRunUnknownState`、`unknownStateReason`、`claimAutomatedRewrite`、`rewriteAutomatedState`、`automatedStateHint`
...
1. IF その Status を書いたのがボードの自動化であり、対応表に戻す先があり、書き戻した回数が上限に達していない THEN
2.   システムはボードの issue の Status に戻す先の選択肢を書く。
3.   システムは Status を動かした記録を issue にコメントする。
4.   ABORT
5. ENDIF
6. システムは VALIDATES THAT turn の終わりを待つ猶予が残っていない。
7. システムは issue に動いた先の Status と止めた理由と戻し方のコメントを1件書く。
8. システムは workspace_hooks の after_run を実行する。
...
flowchart TD
    ...
    subgraph GAF2 ["GLOBAL ALTERNATIVE FLOW 知らないStatus / BRANCH FROM BASIC FLOW 2"]
        G2S1{"1. IF 自動化が書き、対応表に戻す先があり、上限に達していない"}
        G2S1 -- 真 --> G2S2["2. Status に戻す先を書く"] --> G2S3["3. Status を動かした記録をコメントする"] --> G2S4["4. ABORT"]
        G2S6{"6. VALIDATES THAT turn の終わりを待つ猶予が残っていない"}
        G2S6 -- 真 --> G2S7["7. 止めた理由と戻し方をコメントする"] --> G2S8["8. after_run を実行する"] --> G2S9["9. pane を閉じる"] --> G2S10["10. 印を外す"] --> G2S11["11. ABORT"]
    end

    subgraph SAF5 ["SPECIFIC ALTERNATIVE FLOW turnの猶予 / RFS 知らないStatus 6"]
        F5S1["1. worker を止めずに turn の終わりを待つ"] --> F5S2["2. 待っていることを記録に残す"] --> F5S3["3. ABORT"]
    end
