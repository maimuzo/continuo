...
- `docs/plans/continuo_design.md#3-55`（戻す先は `active_states` に限る。終端へは書き戻さない）
- `docs/plans/continuo_design.md#3-56`（書き戻しが止まらなくなる形を、4つとも塞ぐ）
- `docs/plans/continuo_design.md#3-57`（対応表のキーはボードに実在しなくてよい）
- `docs/plans/continuo_design.md#3-57b`（止めたときの案内は、貼ってもそのまま起動する直し方だけを1つ出す）
- `docs/plans/continuo_design.md#3-58`（書き戻しの印と「終わらせる処理」の印を分ける）
- `internal/orchestrator/unknownstate.go` の `handleUnknownState`、`stopForUnknownStateAsync`、`finishRunUnknownState`、`unknownStateReason`、`claimAutomatedRewrite`、`rewriteAutomatedStateAsync`、`rewriteAutomatedState`、`automatedStateHint`、`rewriteTargetSuggestion`
- `internal/orchestrator/runstate.go` の `rewriteClaim`、`claimAutomatedRewrite`、`noteAutomatedRewriteFailure`、`expireAutomatedRewriteFailures`、`noteAutomatedRewriteHandoff`、`beginTerminal`、`claimTerminal`、`endTerminal`、`beginRewrite`、`endRewrite`
- `internal/config/validate.go` の `validateAutomatedStateRewrite`
- `internal/config/states.go` の `KnownStates`、`NamedStates`
...
- `internal/tracker/adapter.go` の `UpdateStatus`、`requiredStatesForBootstrap`、`missingRewriteKeys`
- `internal/doctor/status_names.go` の `configuredStates`
...
1. システムは VALIDATES THAT その Status を書いたのがボードの自動化ではない、または対応表に戻す先が無い、または書き戻した回数が上限に達している、または戻せなかった回数が上限に達している。
...
| 戻せなかった回数 | その Status について上限（3回）に達していない |

**1つでも欠ければ、いままでどおり猶予を置いて止める。**
**書き戻しに失敗しても、その場では止めない。**次の巡回で拾い直す。
...
**そのかわり、「戻せなかった」は別に数える。**回数を戻すだけだと、
**毎回失敗する書き込みを巡回のたびに永久に打ち続けることになる**
（猶予の時計はその手前で戻ってしまうので始まらない）。
**上限に達したら書き戻しをやめ、猶予を置いて利用者へ渡す。**
これが起きるのは、**利用者がボードから戻す先の選択肢を消したとき**である。

**戻す先は `active_states` に入っていなければならない**（設定の検査が起動前に要求する）。
`terminal_states` や `cleanup.on_states` を戻す先にすると、
**書き戻した瞬間に run が終わるか worktree が消える。**
**したがって、書き戻せたときの行き先はいつも「次の turn へ」である。**

**書き戻せなかったときは、書き込みが取り直した Status で判定し直す。**
利用者が `terminal_states` へ動かしていると書き戻しは断られるので、
**その値で判定しないと、終わった issue へ次の指示を送る。**
...
flowchart TD
    ...
    subgraph GAF2 ["GLOBAL ALTERNATIVE FLOW 知らないStatus / BRANCH FROM BASIC FLOW 2"]
        G2S1{"1. VALIDATES THAT 自動化ではない、または戻す先が無い、または書き戻した回数が上限、または戻せなかった回数が上限"}
...
sequenceDiagram
    ...
    opt Status が設定に名前の無い選択肢へ動く
        ...
        alt 自動化が書き、対応表に戻す先があり、書き戻す回数も戻せなかった回数も上限に達していない
