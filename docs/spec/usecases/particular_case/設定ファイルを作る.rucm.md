...
19. ENDIF
20. システムは VALIDATES THAT 書き出す先に指定されたパスがある。
21. システムは VALIDATES THAT 書き出す先に指定されたパスがディレクトリである。
22. システムは VALIDATES THAT 書き出す先の WORKFLOW.md が symlink ではない。
23. システムは VALIDATES THAT 書き出す先に WORKFLOW.md がない。
24. システムは書き出す先に WORKFLOW.md を作る。
25. システムは WORKFLOW.md に雛形の全文を書き込む。
26. システムは利用者に作成した WORKFLOW.md の絶対パスを応答する。
27. システムは利用者にキーごとの値の決め方を応答する。
POSTCONDITION: 書き出す先のディレクトリに WORKFLOW.md がある。WORKFLOW.md の tracker.provider.owner と tracker.provider.project_number に決まった値がある。終了コード 0 が返っている。
...
flowchart TD
    ...
    B11 --> B20{"20 書き出し先のパスがあるか"}
    B18 --> B20
    B20 -- いいえ --> ND1["書き出し先のパス不在 1 から 2 ディレクトリを先に作る案内を応答する"]
    ND1 --> NDEND(["書き出し先のパス不在 3 ABORT 終了コード 1"])
    B20 -- はい --> B21{"21 パスがディレクトリか"}
    B21 -- いいえ --> NF1["書き出し先がディレクトリではない 1 から 2 受け取るのはディレクトリだと応答する"]
    NF1 --> NFEND(["書き出し先がディレクトリではない 3 ABORT 終了コード 1"])
    B21 -- はい --> B22{"22 WORKFLOW.md が symlink ではないか"}
    B22 -- いいえ --> SL1["書き出し先がsymlink 1 から 2 symlink を消す案内を応答する"]
    SL1 --> SLEND(["書き出し先がsymlink 3 ABORT 終了コード 1"])
    B22 -- はい --> B23{"23 書き出す先に WORKFLOW.md がないか"}
    B23 -- いいえ --> EX1{"既存の設定ファイル 1 force オプションの指定があるか"}
    ...
    EX4 --> B27
    B23 -- はい --> B24["24 書き出す先に WORKFLOW.md を作る"]
    B24 --> B25["25 WORKFLOW.md に雛形の全文を書き込む"]
    B25 --> B26["26 作成した絶対パスを応答する"]
    B26 --> B27["27 キーごとの値の決め方を応答する"]
    B27 --> BEND(["基本フロー完了 終了コード 0"])
    B25 -. 書き込み中の中断 WHEN 利用者が実行を中断する .-> INT1["書き込み中の中断 1 書き込みの途中で終了する"]
