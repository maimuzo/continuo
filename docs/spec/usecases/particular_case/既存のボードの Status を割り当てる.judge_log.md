...
| 26 | 二重割り当てのときに打ち切らない理由 | RESUME STEP 8 で同じ役割を尋ね直す | 打ち間違いは利用者が即座に直せる。打ち切ると、それまでの回答をすべて入れ直させることになる。**終端の判定基準（参照フローの残りの段を実行するか）で判定し直しても RESUME STEP 8 のままである。**参照フローは基本フローであり、**8へ戻って同じ役割の説明から続きを行う。**基本フローの8以降を実行する | - | 90% |
| 27 | 該当する選択肢が無いときに ABORT する理由 | 対話を打ち切り、GitHub の画面で選択肢を足すよう案内する | 5つの役割はどれも continuo の動作に必要で、欠けたまま書いた WORKFLOW.md は起動時の検証で落ちる。選択肢を API で足すことは禁じられている（`updateProjectV2Field` は全件置き換えとして扱われ、設定済みの Status が全部消える）ので、システム側で補うこともできない。**終端の判定基準（参照フローの残りの段を実行するか）で判定し直しても ABORT のままである。**参照フローは基本フローであり、**基本フローの12以降を実行しない。**このユースケースはここで終わる | CLAUDE.md、docs/plans/continuo_design.md#4-1、docs/plans/continuo_design.md#3-34 | 85% |
...
| 48 | 代替フローに ABORT と RESUME STEP のどちらを使うか | 入力の打ち間違いは RESUME STEP、前提が欠けているものは ABORT | 利用者が同じ画面で直せるものは尋ね直し、GitHub の画面での作業やコマンドの引数が要るものは打ち切る、という基準で揃えた。**ただし終端を決める基準はこの使い分けではない。**基準は「参照フローの残りの段を実行するか」の1本だけである（`rucm_structure.md` の「`ABORT` と `RESUME STEP` の選び方」）。**この行の使い分けは、その基準へ当てはめ直しても9本すべてで同じ答えになったので残している。****新しい代替フローを足すときは、この行ではなく基準のほうを見ること** | `rucm_structure.md` の「`ABORT` と `RESUME STEP` の選び方」 | 90% |
...
| 63 | 代替フロー『WORKFLOW.mdが無い』の終端 | ABORT | 参照フローは基本フローである。**基本フローの3以降を実行しない。**このユースケースはここで終わる | - | 95% |
| 64 | 代替フロー『ボードを読めない』の終端 | ABORT | 参照フローは基本フローである。**基本フローの5以降を実行しない。**このユースケースはここで終わる | - | 95% |
| 65 | 代替フロー『選択肢が足りない』の終端 | ABORT | 参照フローは基本フローである。**基本フローの6以降を実行しない。**このユースケースはここで終わる。選択肢を足すのは GitHub の画面で人間が行う操作であり、この実行の中に戻り先が無い | - | 90% |
| 66 | 代替フロー『番号が範囲外』の終端 | RESUME STEP 8 | 参照フローは基本フローである。**8へ戻って同じ役割の説明から続きを行う。**基本フローの8以降を実行する | - | 95% |
| 67 | 代替フロー『書き換える対象のキーが無い』の終端 | ABORT | 参照フローは基本フローである。**基本フローの17以降を実行しない。**このユースケースはここで終わる | - | 95% |
| 68 | 代替フロー『書き換えると読めなくなる』の終端 | ABORT | 参照フローは基本フローである。**基本フローの19以降を実行しない。**このユースケースはここで終わる | - | 95% |
| 69 | 代替フロー『中断』の終端 | ABORT | 参照フローは基本フローである。Ctrl+C で実行そのものが終わるので、**基本フローの16以降を実行しない。**このユースケースはここで終わる | - | 95% |
