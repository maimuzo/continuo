...
| 58 | ステップ17 と18 に分けたこと | 「全文を組み立てる」と「組み立てた全文の front matter を読み直せる」を別のステップにした | **書く前に自分の組み立てを読み直すのが要点である。**1ステップにまとめると、読み直しがいつ起きるのか（書く前か後か）が記述から決まらない | `internal/scaffold/update.go` の `UpdateStatuses`（`writeAtomically` の直前） | 95% |
