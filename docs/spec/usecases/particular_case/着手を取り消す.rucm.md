...
SPECIFIC ALTERNATIVE FLOW 手を離させていないのにpaneが生きている:
RFS BASIC FLOW 25
...
| pane の生死の検査 | 手を離させる書き込みを行ったか | pane を確かめずに通す | pane が1つも無いか `--force` が要る |
...
`worktreeが無い` と `候補から外したworktreeがある` の末尾がそれで、真のときだけ1行応答して
`ABORT` へ進むだけだった。**分岐にすると、そこへ至る経路がすべて2本に割れる**
（この2箇所で4本増えていた）。
...
| 入らなかった（`active_states` に入っていなかった） | 待たずに、pane の生死の検査へ落とす（代替フロー「手を離させていないのにpaneが生きている」の分岐元） |

## 手を離させていないときこそ pane を確かめる理由

**手を離させる書き込みが入らない理由は2つある。**継続監視が動いていなかったか、
動いてはいるがボードの Status が `tracker.active_states` の外だったかである。
**どちらの実行もステップ25 へ落ちる**ので、この検査は「継続監視が動いていない」を
前提にした文言を出してはならない（i18n の `abandon.err_pane_alive`）。
...
flowchart TD
    ...
    subgraph SAF12 ["SPECIFIC ALTERNATIVE FLOW 手を離させていないのにpaneが生きている / RFS BASIC FLOW 25"]
