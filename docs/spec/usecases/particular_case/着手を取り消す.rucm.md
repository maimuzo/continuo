...
## 同じ入力で2度分岐しない — 継続監視と dry-run は入り口で1度だけ判定する
...
| 手離し先の Status の読み取り | 手を離させる段を通らないか | ボードを読まずに通す | ボードから Status と project item の ID を引く |
...
| pane が消えるのを待つ | 手を離させる書き込みを行っていないか | 待たずに通す | pane が期限内に無くなるまで待つ（`--force` で越えられる） |
| 実行したら動かす Status の予告 | 継続監視が動いていて dry-run を指定しているときだけ | 実行したときの park の先を1行で予告する | 何も応答しない |
...
| 片付けたあとの Status の値の指定 | worktreeが無い / 候補から外したworktreeがある | 真のときだけ「指定された Status へ動かしていない」と1行応答する |

**別のフローに同じ条件が出るのは畳む対象ではない。**1回の実行が2つのフローを通ることは無いので、
`IF` の組み合わせは増えない。畳むのは、**同じフローの中で同じ入力を何度も見ている**ときである。
...
使う形は2つで、**条件が偽なら何もしない「〜ときだけ〜する」**と、
**条件が真なら素通しになる「〜か、〜」の選言**である。
**その段が実際に何をするかは、上の表の「条件が真のとき」「条件が偽のとき」が正である。**

**テスト側で埋めるもの。**畳んだ条件は**真偽の両側を通すテストで押さえる。**
どの経路にどの条件が畳まれているかは上の表が正であり、
