...
BRIEF DESCRIPTION: 利用者が間違えて着手した issue の URL を渡す。システムは継続監視が動いているかと利用者が dry-run を指定しているかを最初に1度だけ判定し、以降の段はその判定を条件として持ち回る。システムは手を離させる段を通るときだけ Status を park の値へ動かして pane が消えるまで待つ。システムは身元ファイルの issue_url で worktree を1つに絞り、置き場所のパスで裏を取る。システムは失うものを見せてから worktree と pane と herdr の workspace と branch を消す。システムは worktree が1つも無いときも、規則から組み立てた branch が残っていないかを見て片付ける。システムは片付けたあとの Status を、利用者が指定したときだけ動かす。
...
19. システムは VALIDATES THAT 手を離させる書き込みを行っていないか、worktree を作業ディレクトリに持つ pane が期限内に無くなるか、利用者が force を指定している。
20. システムは VALIDATES THAT worktree が置き場所の内側にあり身元ファイルを読める。
21. システムは利用者に消すものと、失うものと、git が答えられずに調べられなかったものの一覧を応答する。
22. システムは継続監視が動いていて利用者が dry-run を指定しているときだけ、利用者に実行したときに手を離させるために動かす Status の値を応答する。
23. システムは VALIDATES THAT 利用者が dry-run を指定していない。
24. システムは VALIDATES THAT 失うものが無く調べ切れているか、利用者が force を指定している。
25. システムは VALIDATES THAT 手を離させる書き込みを行ったか、worktree を作業ディレクトリに持つ pane が1つも無いか、利用者が force を指定している。
...
2. システムは Status を park の値へ動かしているときだけ、利用者に Status を park の値のまま残すことを応答する。
3. ABORT
...
WHEN pane が消えるのを待っているあいだに利用者が SIGINT または SIGTERM で実行を中断した場合
...
## 同じ入力を2度見ない — 継続監視と dry-run は入り口で1度だけ判定する

**言いたいこと。**継続監視が動いているか・dry-run か・park を実際に書いたかは、
**基本フローの入り口で1度だけ判定し、以降は分岐にしない。**
以降のステップは、その判定を文の中の条件として持ち回る。
...
| 手を離させる書き込みを行う | ステップ16 | 手を離させる段を通っていて、ボードの Status が `tracker.active_states` に入っている |

**以降のステップは、この2つを文の中で見る。**

| ステップ | 文の中の条件 | 条件が偽のとき |
| --- | --- | --- |
| 13 | 継続監視が動いていないか | park の値を確かめずに通す |
| 14 | Status の指定も継続監視も無いか | ボードへ問い合わせずに通す |
| 15 | 手を離させる段を通らないか | ボードを読まずに通す |
| 17 | 手を離させる書き込みを行わないか | ボードへ書かずに通す |
| 18 | 手を離させる書き込みを行ったときだけ | 何も応答しない |
| 19 | 手を離させる書き込みを行っていないか | pane を待たずに通す |
| 22 | 継続監視が動いていて dry-run を指定しているときだけ | 何も応答しない |
| 25 | 手を離させる書き込みを行ったか | pane が1つも無いか force が要る |

**なぜ分岐にしないか。**`IF` で書くと、後ろの段ほど**そこへ至る `IF` の組み合わせを全部引き継ぐ。**
同じ入力を4箇所で見ていたときは、基本フローを完走するだけで288通りになり、
**その大半は現実には起こらない組み合わせだった**（継続監視が動いていないのに
手を離させる書き込みを済ませている、など）。文の中の条件にすれば、
**表現できる場合分けは1つも減らないまま**、経路は1本になる。

**分岐のまま残したもの。**その分岐でしか決まらない入力を見ているものは残す。

| 残した分岐 | 見ている入力 | なぜ残すか |
| --- | --- | --- |
| ステップ28・29 | 片付け切れずに残ったものと branch の実在 | 片付けを実行して初めて分かる。応答の中身が3通りに分かれる |
| ステップ37 | 片付けたあとの Status の値の指定 | 真のときだけボードを読んで書く。代替フローが2本ぶら下がる |
| 「worktreeが無い」のステップ4・7 | branch の実在と `cleanup.delete_branch` | `cleanup.delete_branch` はここでしか見ない。消すか消さないかで事後条件が変わる |
...
| --- | --- |
| 入った（Status を park の値へ動かした） | 消えるまで待つ（ステップ19）。時間切れは `--force` で越える |
| 入らなかった（`active_states` に入っていなかった） | 待たずに、動いていないときと同じ検査へ落とす（ステップ25） |
...
**pane が期限内に消えないとき（ステップ19）も同じである。**片方だけ越えられないのは
...
flowchart TD
    ...
    B19{"19. VALIDATES THAT 手を離させる書き込みを行っていないか、pane が期限内に無くなるか、force を指定している"}
    B20{"20. VALIDATES THAT 置き場所の内側にあり身元ファイルを読める"}
    B21["21. 消すものと失うものと調べられなかったものの一覧を応答する"]
    B22["22. 継続監視が動いていて dry-run を指定しているときだけ、実行したら動かす Status の値を応答する"]
    B23{"23. VALIDATES THAT dry-run を指定していない"}
    B24{"24. VALIDATES THAT 失うものが無く調べ切れているか、force を指定している"}
    B25{"25. VALIDATES THAT 手を離させる書き込みを行ったか、pane が1つも無いか、force を指定している"}
    ...
    subgraph SAF6 ["SPECIFIC ALTERNATIVE FLOW paneが期限内に消えない / RFS BASIC FLOW 19"]
    ...
    subgraph SAF8 ["SPECIFIC ALTERNATIVE FLOW 失うものがある / RFS BASIC FLOW 24"]
        S8["1. force が無い限り消さないことを応答する"] --> S8C["2. Status を park の値へ動かしているときだけ、park の値のまま残すことを応答する"] --> S8E["3. ABORT"]
    end

    subgraph SAF12 ["SPECIFIC ALTERNATIVE FLOW 動いていないのにpaneが生きている / RFS BASIC FLOW 25"]
        S12["1. 残っている pane の ID とロックファイルのパスを応答する"] --> S12E["2. ABORT"]
    end

    subgraph SAF9 ["SPECIFIC ALTERNATIVE FLOW 片付けの失敗 / RFS BASIC FLOW 27"]
        S9["1. 片付けを見送った理由の一覧を応答する"] --> S9C["2. Status を park の値へ動かしているときだけ、park の値のまま残すことを応答する"] --> S9E["3. ABORT"]
    end
