...
8. システムは利用者に継続監視が動いているかどうかを応答する。
...
19. システムは VALIDATES THAT 手を離させる書き込みを行っていないか、herdr が pane の一覧に答えたうえで worktree を作業ディレクトリに持つ pane が期限内に無くなるか利用者が force を指定している。
...
41. ELSE
42.   システムは利用者に Status を動かしていないことを応答する。
43. ENDIF
...
RFS BASIC FLOW 19
1. システムは利用者に、残っている pane の ID と待った上限と force で越えられることか、herdr が pane の一覧に答えられない理由を応答する。
...
1. システムは pane が消えるのを待つのと、git と herdr とボードへの問い合わせをやめる。
2. システムは利用者に中断されたことと、pane が消えるのを待っていたときだけ残っている pane の ID を応答する。
3. システムは手を離させる書き込みを行ったときだけ、利用者に Status を park の値のまま残すことを応答する。
4. ABORT
POSTCONDITION: worktree は残っている。branch は残っている。issue の Status は、手を離させる書き込みを行ったなら park の値のまま、行っていなければ変わっていない。終了コードは 1 である。
```
...
| pane が消えるのを待つ | 手を離させる書き込みを行っていないか | 待たずに通す | pane が期限内に無くなるまで待つ（時間切れは `--force` で越える。**herdr が答えなければ越えられない**） |
...
上の8つは、真でも偽でも同じ段を通り、**その段が何をするかだけが変わる。**
...
`失うものがある`・`片付けの失敗`・`片付け後のStatusを読めない`・`片付け後の書き込み失敗`・
`実行の中断` の末尾がそれである。
...
| 5 | 片付けたあとの Status を動かす（ステップ37〜43） | **どこへ置くかは人間だけが知っている。**指定が無ければ動かさない |
...
| --- | --- |
| 入った（Status を park の値へ動かした） | 消えるまで待つ（ステップ19）。時間切れは `--force` で越えるが、herdr が答えなければ越えられない |
...
環境変数で決まる（`CONTINUO_RUNTIME_DIR` / `XDG_RUNTIME_DIR` / `TMPDIR`）。launchd から
起動した継続監視と端末から叩いた `continuo abandon` で食い違えば、**動いている継続監視を
「動いていない」と判定して、生きた pane ごと worktree を消す。**
...
**ステップ19 の pane 待ちは、越え方が1つだけである。**

| どこで | herdr が pane の一覧に答えない | pane が残っている |
| --- | --- | --- |
| pane 待ち（ステップ19） | **`--force` を見ずに止まる** | 上限まで待ってから `--force` で越える |
| pane の生死の検査（ステップ25） | `--force` で越える | `--force` で越える |

**ステップ19 の `--force` は「上限まで待っても閉じなかったら越える」である。**
herdr が答えなければ、その待ち自体を1度も行えていない。**待たずに越えるのは、
手を離させたばかりの pane を、閉じる暇も与えずに消すことである。**
ステップ25 は手を離させていない実行しか通らないので、待つ意味がそもそも無い。

**越え方が違うので、止まる文言の鍵も分ける。**ステップ19 は `abandon.err_pane_list`、
ステップ25 は `abandon.err_pane_list_check` である。**同じ鍵を使い回すと、
`--force` で越えられることをステップ19 の側にも書いてしまう**（越えられないのに）。
...
**1つだけ例外がある。****pane 待ちの最中に herdr が答えなくなったときは、`--force` を見ずに止まる**
（ステップ19）。理由は上の「手を離させていないときこそ pane を確かめる理由」に書いた。
...
Status が park の値のまま残ったことを誰も言わない**（代替フロー `片付け後のStatusを読めない` と
`片付け後の書き込み失敗` の末尾がその1行である）。
...
flowchart TD
    ...
    B19{"19. VALIDATES THAT 手を離させる書き込みを行っていないか、herdr が pane の一覧に答えたうえで pane が期限内に無くなるか force を指定している"}
    ...
    subgraph SAF6 ["SPECIFIC ALTERNATIVE FLOW pane待ちを終えられない / RFS BASIC FLOW 19"]
        S6["1. 残っている pane の ID と待った上限と force で越えられることか、herdr が pane の一覧に答えられない理由を応答する"] --> S6B["2. Status を park の値のまま残すことを応答する"] --> S6E["3. ABORT"]
    end
    ...
    subgraph SAF10 ["SPECIFIC ALTERNATIVE FLOW 片付け後のStatusを読めない / RFS BASIC FLOW 38"]
        S10["1. project item の ID を引けないことと理由を応答する"] --> S10B["2. 手を離させる書き込みを行ったときだけ、Status を park の値のまま残すことを応答する"] --> S10E["3. ABORT"]
    end
    ...
    subgraph GAF1 ["GLOBAL ALTERNATIVE FLOW 実行の中断 / BRANCH FROM BASIC FLOW 19"]
        G1["1. pane が消えるのを待つのと、git と herdr とボードへの問い合わせをやめる"] --> G1B["2. 中断されたことと、pane が消えるのを待っていたときだけ残っている pane の ID を応答する"] --> G1C["3. 手を離させる書き込みを行ったときだけ、Status を park の値のまま残すことを応答する"] --> G1E["4. ABORT"]
    end
```
    ...
    alt 一致する worktree が1つも無く、候補から外したものがある
        ...
        opt 手を離させる段を通る
            ...
            opt 手を離させる書き込みを行う
                ...
                break 期限内に pane が消えず force も指定していない、または herdr が pane の一覧に答えない
                    S-->>U: 残っている pane の ID と待った上限と force で越えられることか、herdr が答えられない理由を応答する
                    Note over S: ABORT 終了コードは 1
                end
            end
        end
