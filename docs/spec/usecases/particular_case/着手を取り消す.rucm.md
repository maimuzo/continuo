...
10. システムは身元ファイルを読めない worktree と、身元ファイルの issue_url が渡された issue と一致するが置き場所のパスから取り出した owner とリポジトリ名が食い違う worktree を候補から外し、外したことを利用者に応答する。
...
35. ENDIF
36. システムは herdr に worktree と pane と herdr の workspace の削除を要求し、実体が残っていれば worktree のディレクトリを消して git の worktree の登録を掃除する。
37. システムは VALIDATES THAT worktree が置き場所から消えている。
38. IF branch が消えていて片付け切れずに残ったものが無い THEN
39.   システムは利用者に worktree と branch を消したことを応答する。
40. ELSE
41.   システムは利用者に worktree を消したことと、片付け切れずに残ったものを1件ずつ応答する。
42. ENDIF
...
| git（`git worktree remove`。ステップ36） | worktree のディレクトリを自分で消し、`git worktree prune` で登録を落とす |
...
flowchart TD
    ...
    B9["9. 身元ファイルを読む"]
    B10["10. 読めない身元ファイルと置き場所のパスと食い違う worktree を候補から外して応答する"]
    ...
    B35["35. ENDIF"]
    B36["36. worktree と pane と workspace の削除を要求し残っていれば自分で消す"]
    B37{"37. VALIDATES THAT worktree が消えている"}
    B38{"38. IF branch が消えていて残ったものが無い"}
    B39["39. worktree と branch を消したことを応答する"]
    B40["40. ELSE"]
    B41["41. worktree を消したことと残ったものを1件ずつ応答する"]
    B42["42. ENDIF"]
