...
10. システムは身元ファイルを読めない worktree と、身元ファイルが1つも無いディレクトリと、身元ファイルの issue_url が渡された issue と一致するが置き場所のパスから取り出した owner とリポジトリ名が食い違う worktree を候補から外し、外したことを利用者に応答する。
...
2. IF 身元ファイルを読めないか身元ファイルが無いか置き場所と食い違うために候補から外したディレクトリが1つも無い THEN
...
16.   ENDIF
17. ENDIF
18. IF 利用者が片付けたあとの Status の値を指定している THEN
19.   システムは利用者に指定された Status へ動かしていないことを応答する。
20. ENDIF
21. ABORT
...
**候補から外したディレクトリが1つでもあれば、この道へ進まない**（ステップ2）。
身元ファイルを読めなかったものも、**身元ファイルが1つも無いものも**同じ扱いである。
...
flowchart TD
    ...
    B9["9. 身元ファイルを読む"]
    B10["10. 読めない身元ファイルと身元ファイルが無いものと置き場所のパスと食い違うものを候補から外して応答する"]
    ...
    subgraph SAF2 ["SPECIFIC ALTERNATIVE FLOW worktreeが無い / RFS BASIC FLOW 11"]
        S2["1. 一致する worktree が無いことを応答する"] --> S2B{"2. IF 候補から外したディレクトリが1つも無い"}
        S2B -- 真 --> S2C["3. branch 名を組み立てて clone を宛先にする"] --> S2D{"4. IF branch 名と clone を決められて branch が実在する"}
        ...
        S2O --> S2P["17. ENDIF"]
        S2B -- 偽 --> S2P
        S2P --> S2Q{"18. IF 片付けたあとの Status の値を指定している"}
        S2Q -- 真 --> S2R["19. 指定された Status へ動かしていないことを応答する"] --> S2S["20. ENDIF"]
        S2Q -- 偽 --> S2S
        S2S --> S2T["21. ABORT"]
    end
    ...
    subgraph SAF6 ["SPECIFIC ALTERNATIVE FLOW paneが期限内に消えない / RFS BASIC FLOW 24"]
        S6["1. 残っている pane の ID と待った上限を応答する"] --> S6B["2. Status を park の値のまま残すことを応答する"] --> S6E["3. ABORT"]
    end
