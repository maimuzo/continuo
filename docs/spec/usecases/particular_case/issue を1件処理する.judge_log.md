...
| 37 | フロー `無音の打ち切り` の種別と BRANCH FROM | GLOBAL ALTERNATIVE FLOW。BRANCH FROM BASIC FLOW 18 | 無音はどの時点でも起こりうる。**分岐元はステップ18（Stop hook を待っている時点）である。**ここは待ち時間がいちばん長く、エージェントの作業が最も進んだ状態で失われる | `docs/plans/continuo_design.md#3-21`、`internal/orchestrator/reconcile.go` の `checkStalls` | 80% |
