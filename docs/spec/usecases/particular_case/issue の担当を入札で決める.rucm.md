...
BRIEF DESCRIPTION: 巡回タイマーが巡回を起こす。システムは候補の先頭の issue の担当者を読み、担当者がいなければ枠の余裕値から判定スコアを出して入札のコメントを1件書く。システムは締め切りまで待って届いた入札をすべて読み、判定スコアがいちばん大きい機械が自分であれば自分を担当者に加えて hold のコメントを1件書く。システムは担当者が自分の issue には入札せず、そのまま着手と引き継ぎへ渡す。
...
23. ENDIF
24. システムは入札の印と hold の印が先頭に付いたコメントを、エージェントへ渡す入力から外す。
25. システムは先頭の issue を、この機械が着手または引き継ぎを行う相手として次の段へ渡す。
POSTCONDITION: 先頭の issue の担当者はこの機械の投稿者1人である。担当者が1人もいなかった issue には、入札のコメントが1件と hold のコメントが1件増えている。担当者がこの機械の投稿者だった issue には、コメントが1件も増えていない。issue の Status は変わっていない。エージェントへ渡す入力に入札の印と hold の印のコメントは1件も入っていない。
...
4. システムは外した担当者の名前と、担当者の最後のコメントの時刻を記録に残す。
5. RESUME STEP 6
POSTCONDITION: 先頭の issue に担当者は1人もいない。issue にコメントは1件も増えていない。issue の Status は変わっていない。branch は残っている。
...
| 担当者が他人1人＋hold あり＋期限切れ | `他人の担当` のステップ3〜5 | 担当を外して入札をやり直す |
...
| どの枠かの使用率が `rate_limit.pause_above_percent` を超えた | `枠の使い過ぎ` | 新規の着手を止めている機械が担当を取ると、取ったまま動かない |
...
## 入札と hold のコメントを、エージェントに読ませない

**言いたいこと。**入札はコメントに書くが、**エージェントへ渡す入力には混ぜない**（ステップ24）。
外すのは `<!-- continuo:bid -->` と `<!-- continuo:hold -->` の2つだけで、
...
```
<!-- continuo:hold -->
{"host":"<機械の名前>","assignee":"<担当者のログイン名>","branch":"continuo/octocat/hello-world/188","at":"2026-08-29T09:45:00Z"}
```
...
flowchart TD
    ...
    B23["23. ENDIF"]
    B24["24. 入札の印と hold の印のコメントをエージェントへ渡す入力から外す"]
    ...
    subgraph SAF2 ["SPECIFIC ALTERNATIVE FLOW 他人の担当 / RFS BASIC FLOW 5"]
        ...
        A2S2 -- 真 --> A2S3["3. 担当者を issue から外す"] --> A2S4["4. 外した担当者と最後のコメントの時刻を記録に残す"] --> A2S5["5. RESUME STEP 6"]
    end
...
sequenceDiagram
    ...
    S->>S: 入札の印と hold の印のコメントをエージェントへ渡す入力から外す
