...
4. システムは外した担当者の機械の名前と branch の名前と時刻を書いた released のコメントを、released の印を先頭に置いて issue に1件書く。
...
**<機械の名前> がこの issue の担当に立候補しています。**上の JSON は、その機械にレートリミットの枠がどれだけ残っているかです。
...
**この issue の担当は <機械の名前> に決まりました。**入札した機械のうち、レートリミットの枠の余裕がいちばん大きい機械です。
...
```
<!-- continuo:released -->
{"from":"<外した担当者の機械の名前>","branch":"continuo/octocat/hello-world/188","at":"2026-08-30T09:00:00+09:00"}
...
**引き継ぐ機械の名前は、この段では書かない。**外すのは入札をやり直す前なので、
