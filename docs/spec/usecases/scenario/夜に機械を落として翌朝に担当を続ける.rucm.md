...
- `docs/plans/continuo_design.md#3-77b`（期限は担当者の最後の進捗報告から数える。`idle_timeout_ms` の既定は18時間）
...
2. 利用者は機械の電源を落とす。
3. ほかの機械は夜のあいだ、担当者がほかの機械の投稿者ではなく hold の印のコメントが1件以上あり担当者の最後の進捗報告から idle_timeout_ms を過ぎていないので、この issue に触らない。
...
WHEN 電源が落ちているあいだに担当者の最後の進捗報告から idle_timeout_ms を過ぎた場合
...
flowchart TD
    ...
    B3 -. "期限をまたぐ: WHEN 担当者の最後の進捗報告から idle_timeout_ms を過ぎた場合" .-> G1S1
...
sequenceDiagram
    ...
    loop 夜のあいだの巡回
        ...
        alt 担当者の最後の進捗報告から idle_timeout_ms を過ぎていない
            M->>M: この issue に触らない。入札もしない
        else 担当者の最後の進捗報告から idle_timeout_ms を過ぎた
