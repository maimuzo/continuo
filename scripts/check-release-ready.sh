...
for p in ${prs}; do
	# **gh の失敗（認証切れなど）と、成功して0件だった場合を区別する。**
	# 以前は gh の標準エラーを捨てていたため、gh が失敗したときも出力が空になり、
	# 「0件（レビュー無し）」と見分けが付かなかった。認証が切れているだけなのに
	# 「レビューを回し直せ」と出て、原因の特定を誤らせる（2026-08-30、レビューで指摘）。
	# **gh の終了コードで判定する。**失敗したときは理由を画面に出したうえで
	# 「確かめられなかった」として扱い、**危険側（release を止める側）へ倒す**
	# （0件のときと同じく ng を増やす。「レビュー結果=無し」とは出さない —
	# レビューを回し直しても直らない問題を、回し直せと案内してしまうため）。
	if r_raw="$(review_of "${p}" 2>&1)"; then
		# **数として比べる。**`review_of` の出力に余計な行が混ざったり（`gh` の警告など）、
		# 空文字になったりすると、`= "0"` という文字列比較では「レビュー有り」側へ
		# 誤って倒れる（2026-08-30、レビューで指摘）。**末尾の数字だけを取り出し、
		# 取り出せなければ 0 として扱う。**
		r="$(printf '%s\n' "${r_raw}" | grep -oE '[0-9]+' | tail -n1)"
		r="${r:-0}"
		if [ "${r}" -eq 0 ]; then
		...
		echo "PR #${p}  レビュー結果=確かめられなかった  ← gh が失敗しました。理由:"
		printf '%s\n' "${r_raw}" | sed 's/^/    /'
		ng=$((ng + 1))
	fi
