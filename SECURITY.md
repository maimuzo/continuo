...
| **危ない道具の呼び出しを断らせる** | `claude.tool_gate.mode` は既定で `off` です。`public_only` にすると、公開リポジトリの issue のときだけ `Bash` の呼び出しを実行の前に検査します（`on` ならいつでも）。**コメントで許可を出しても通りません**（`auto` の判定役も、この検査も、issue のコメントを読みません） |
...
| **Have dangerous tool calls refused** | `claude.tool_gate.mode` defaults to `off`. Set it to `public_only` to inspect `Bash` calls before they run when the issue is in a public repository (`on` for always). **Granting permission in a comment does not get past it** (neither the `auto` classifier nor this check reads issue comments) |
