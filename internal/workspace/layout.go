...
// EnsureRoot は workspace.root が無ければ 0700 で作り、シンボリックリンクを解決した
// 絶対パスを返す（3-20 の段1・段2）。
//
// **既にあるときは権限を変えない**（os.MkdirAll は既存のディレクトリに触らない）。
// 利用者が先に 0755 で作っていれば、worktree の中身は他人から読める。**黙って締めると
// 利用者の意図を壊すので、ここでは変えず、doctor（08）が警告する。**
//
...
type Location struct {
	...
	Path string
	// Branch は変数展開・正規化した branch 名である。
	Branch normalize.SafeName
}

// Locate は issue から worktree の置き場所を組み立てる（3-22）。
//
// branch 名の変数展開・正規化と、置き場所の各階層の組み立てをまとめて行う。
...
// 戻り値の3つ目: branch 名の変数展開に失敗した場合のエラー。
