...
type Deps struct {
	...
	// **空なら常駐している側と同じ関数から決める**（daemon.ResolveLockFilePath）。
	LockPath string
...
//
// cfg: 検証済みの設定。
// inst: `--id` から導いたロックの置き場所。**常駐している側と同じ Layout である**（3-17c）。
...
func (d Deps) resolve(
	...
	if d.LockPath == "" {
		// **常駐している側と同じ関数から決める**（3-17c）。
		// **ここが1バイトでもずれると、動いている continuo を「動いていない」と
		// 判定して worktree を消しにいく。**
		d.LockPath = daemon.ResolveLockFilePath(cfg, inst)
		if err := daemon.EnsureLockFileDir(inst, d.LockPath); err != nil {
			return d, err
		}
	}
	if d.NewTracker == nil {
