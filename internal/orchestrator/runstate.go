...
type runState struct {
	...
	//
	// **知らない Status になった issue へ「元は何だったか」を書くために持つ**（設計 3-49）。
	...
	// （設計 3-49）。ゼロ値なら、いまは知っている Status である。
	//
...
// setLastWrittenState は continuo がボードへ書いた Status を控える（設計 3-49）。
//
...
// turnLoopActive は turn ループの goroutine が走っているかを返す（設計 3-49）。
//
...
// turn ループへ「待つのをやめろ」と伝える（設計 3-50）。
//
...
//
// **turn ループはこれを見て、自分のせいの失敗を WARN にしない**（設計 3-50）。
//
// 戻り値: 止めていれば true。
...
func (rs *runState) beginAttempt() int {
	...
	// **「止めた」の合図も作り直す**（設計 3-50）。前の世代のものを使い回すと、
