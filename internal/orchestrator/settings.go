...
//
// **fork への push と本家への pull request を名指しする。**免除を issue とコメントだけの
// 列挙にすると、**列挙に無いこの2つが断られる側へ落ちる。**
// `docs/spec/usecases/particular_case/本家のリポジトリへ PR を出す.rucm.md` の代替フロー
// 「公開のリポジトリ」は、判定を掛けたうえで道具の呼び出しが通ることを求めている。
//
// **塞げない穴が残る。**注入された文を、運用者の名前で任意のリポジトリへ出せる。
// **塞ぐには本文と題名の中身で切るしかなく、それは正当なバグ報告ごと止める**
...
const toolGateExemptionNote = "\n  相手のリポジトリを問わず、いま担当している作業の中で気づいたことを書く形なら、" +
	...
	"この免除も、上の条件を免除しない。書き込む中身が鍵・トークン・資格情報・環境変数のときは、" +
