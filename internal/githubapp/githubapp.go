...
// 漏れたときの被害は `Issues` の権限の範囲で止まるが、**期間では止まらない**（3-82b）。
// **更新用のトークンは回すたびに期限が約181日へ戻り、回せるのは continuo だけではない。**
// 漏れたら、待つのではなく client secret を作り直すこと（docs/FAQ.md の「資格情報が漏れたかもしれないとき（GitHub App）」）。
//
...
type Credentials struct {
	...
	// 組で、人間の代理として動くトークンを作り放題になる**（3-82b）。
	// **期間では止まらない。**更新用のトークンは回すたびに期限が約181日へ戻る。
	ClientSecret string
	// RefreshToken は更新用のトークン（`ghr_` で始まる）である。**1回使うと無効になる。**
	RefreshToken string
	// RefreshTokenExpiresAt は更新用のトークンの期限である。
	//
	// **ゼロ値は「期限を知らない」である。「未認可」ではない。**
	// 認可していないときもゼロだが、**認可済みでもゼロになりうる**
	// （GitHub が新しい更新用のトークンを返しながら期限を返さなかったとき。
	// `applyToken` が古い期限を捨てる）。
	// **未認可かどうかは HasRefreshToken で見ること。**
	//
