...
//
// **front matter（設定）と本文（その project でだけ効く指示）の2つを持つ**（設計 5-3d）。
// **仕組みの説明は continuo の実行ファイルの中にある**（internal/prompt/builtin.md）。
// 本文は、その組み込みの真ん中へ挟まって送られる（設計 5-3c）。
// **仕組みの説明を本文へ書き写してはならない。**書き写すと continuo が説明を直しても、
// 既に配った WORKFLOW.md には二度と届かなくなる。
//
...
<!-- ここから下が本文です。continuo が Claude Code へ送る指示書の一部になります。 -->
<!-- 組み込みの指示書の後半（表明の1行の書き方）の直前に差し込まれます。 -->
<!-- 送られる全文は continuo prompt --show で読めます。 -->
<!-- 組み込みの側だけを読みたいときは continuo prompt --show --builtin です。 -->
<!-- チームで共有するものです。git 管理下に置いてください。 -->
<!-- 要らない節は消してかまいません。全部消しても continuo は動きます。 -->
...
<!-- この節は、下のどの節よりも先に置いてください。 -->
<!-- 何語で書くかを先に決めておかないと、下の節が書かせるコメントの言語が揃いません。 -->
...
<!-- PR を作ること自体は、組み込みの指示に入っています。 -->
<!-- 何を作らせるかは continuo prompt --show --builtin で読めます。 -->
<!-- 何も書かなければ、組み込みの指示のとおりに作られます。 -->
<!-- ここには、このリポジトリでだけ効く決まりを書いてください。 -->
<!-- 例: draft で作ってからレビューを通して外す / base にする branch / 付けるラベル。 -->
<!-- 作らせたくないときは、この節に「PR を作らない」と書いてください。 -->
<!-- 上の「## 何をする作業か」を調査だけ・レビューだけに書き換えたなら、あわせてそう書いてください。 -->
<!-- commit する成果が無い運用では、PR を作れずに止まります。 -->
...
<!-- subagent にレビューさせない運用なら、この節は消してかまいません。 -->
<!-- 消しても PR は作られます。作らせない指示は、上の「## PR の決まり」に書いてください。 -->
<!-- 組み込みの指示が、本文の「PR を作らない」を読んで従います。 -->
`
