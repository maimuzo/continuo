...
tracker:
  ...
  provider:                                 # ここから下は GitHub Projects v2 に固有の設定
    ...
    handoff:                                # 同じカンバンを複数の機械で見張るときの取り決め。担当は issue の担当者で持つ
      ...
      progress_interval_ms: 3600000         # エージェントに進捗報告を書かせる間隔。3600000 なら1時間。
                                            # この値は continuo が測るのではなく、送る文面へ分に直して埋める。
                                            # 60000（1分）以上にすること。59999 までは「0分以上黙らない」になる。
                                            # 上の idle_timeout_ms より短くすること。長いと、エージェントが
                                            # 指示どおりに書いていても書く前に担当が外れる
...
<!-- ここから下が「4-4. このプロジェクトの決まり」の中身です。 -->
<!-- 組み込みの指示書の、その位置へそのまま差し込まれます。 -->
<!-- 送られる全文は continuo prompt --show で読めます。 -->
<!-- この形式のコメント（行頭が半角の <!-- で始まる行）は、送る文面からは取り除かれます。 -->
<!-- 送りたい説明は、バッククォート3つのコードブロックで囲んでください。囲んだ中は取り除かれません。 -->
...
issue のコメント・pull request の本文・commit メッセージ・コードのコメントは、すべて日本語で書いてください。
技術用語は英語のまま使ってください（worktree / branch / commit / pane / hook など）。日本語に直訳しないでください。
...
<!-- pull request を作ること自体は、組み込みの 3-5 に入っています。何も書かなければそのとおりに作られます。 -->
<!-- ここには、このリポジトリでだけ効く決まりを書いてください。 -->
<!-- 例: draft で作ってからレビューを通して外す / base にする branch / 付けるラベル。 -->
<!-- 同じディレクトリに置かれた continuo-ci.yaml を .github/workflows/ へ移し、 -->
<!-- その2つの検査を branch protection の必須の検査に入れたなら、 -->
<!-- draft で作る決まりは要りません。レビューを貼るまでマージが機械で止まります。 -->
<!-- 入れていないなら、draft で作らせるかどうかをここで決めてください。 -->
