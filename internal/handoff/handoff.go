// Package handoff は、同じボードを複数の continuo が見張るときに
...
// 騙られたほうは `HasBidBy` が真になって以後1度も入札しなくなる。**
type Bid struct {
	...
	// `bid_window_ms: 0` では書いた巡回でそのまま勝敗を決めるため、担当者が永久に決まらない。
	Author string `json:"-"`
