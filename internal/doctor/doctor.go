...
//
// **前提はいくつもあり、どれが欠けても continuo は静かに失敗する。**機械的に検査して、
// 足りないものと直し方を人間に出すのがこのパッケージの仕事である。
//
// 検査するものと見出し語は次のとおりである（report.go の Label 定数）。
//
...
// 戻り値: 見出し語ごとの検査結果。**エラーは返さない。**検査に失敗したこと自体が結果である。
