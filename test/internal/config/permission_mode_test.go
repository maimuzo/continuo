...
//
// **既定は `auto` である。**判定役が実行の前に確かめるので、
// `.claude/` 配下と `.mcp.json`（保護対象パス）へも書ける。
// **判定役は issue のコメントを読まない**（判定役への要求から道具の結果は取り除かれる。設計 3-11）。
...
//
// **`auto` では、任意のコード実行を許す広いルールが落とされる**（落とされるものの一覧は、
// 公式文書の permission modes のページにあり、設計 3-11 が原文を引いている）。**`"Bash"` はその1つである。**
