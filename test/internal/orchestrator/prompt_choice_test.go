...
// TestResumeBackoff_再dispatchはUUIDを採り直して1回目の本文を送る は、
// バックオフ明けの再開を確かめる。
//
// 目的: 設計 3-3 の「**セッション UUID は起動のたびに新しく採番する**（一度使った UUID は
// 再利用できない）」と、設計 5-3 の「`{{if .attempt}}` に試行回数が入る」を示す。
// **UUID を採り直した時点で会話履歴を持たない別のセッションになるので、継続の指示（5-4）
// だけを送ると、どの issue を何のためにやるのかが1文字も伝わらない。**
//
// 与える情報: 1回目の turn が stall で打ち切られ、バックオフが明けた run。
// 時計は `testClock` で手で進める（実時間では待たない）。
// 成功条件:
//   - 2回目の `agent.start` の `--session-id` が1回目と違う（新しく採番している）
//   - 2回目に送る本文が1回目の本文（5-3）の描画結果であり、
//     **「2 回目の試行です」が入っている**
func TestResumeBackoff_再dispatchはUUIDを採り直して1回目の本文を送る(t *testing.T) {
	clock := newTestClock()
	fx := newFixture(t, fixtureOptions{
		Now: clock.Now,
	...
	fx.Orc.Tick(context.Background())
	waitFor(t, 10*time.Second, "再 dispatch で2回目の turn が送られる", func() bool {
		return len(prompts()) >= 2
	})

	// **セッション UUID を採り直している**（一度使った UUID は再利用できない）。
	starts := startSessionIDs(fx)
	if len(starts) < 2 {
		t.Fatalf("再 dispatch で agent を起動し直していない: %v", starts)
	}
	if starts[0] == starts[1] {
		t.Fatalf("同じセッション UUID を使い回している: %v", starts)
	}
	if starts[1] == "" {
		t.Fatalf("2回目の起動に --session-id が無い: %v", starts)
	}
	...
	if strings.Contains(got, "続けてください") {
		t.Fatalf("会話履歴の無い新しいセッションへ継続の指示（5-4）だけを送っている: %q", got)
	}
}
...
//
// **`--session-id` が無い起動（`--resume` での復元）は空文字で並びに入る。**
//
// fx: 対象の fixture。
// 戻り値: セッション UUID の並び。
