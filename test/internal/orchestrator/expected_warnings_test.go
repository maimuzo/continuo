...
// 実運用で出た欠陥（カンバンから消えた issue が1件混ざるだけで、走行中の全 issue の
...
var expectedWarnings = map[string][]string{
	...
	"TestRUCM_P026_入力を受け付けないまま期限を過ぎたら人間へ渡す":             {"run を諦めてリトライを積みました"},
	"TestRUCM_P029_paneが使えないまま期限を過ぎたら人間へ渡す":             {"リトライの回数を使い切りました", "着手に失敗しました"},
	"TestResumeBackoff_再着手はセッションに復帰して1回目の本文を送る":         {"run を諦めてリトライを積みました"},
	"TestRunViews_セッションに復帰した再着手でトークンを二重に数えない":           {"run を諦めてリトライを積みました"},
	...
	"TestExternalFailure_カンバンを読めなくても巡回は止まらない":           {"候補の取得に失敗しました"},
	...
	"TestSignal_カンバンに載っていない対象はコメントに残して捨てる":                              {"表明が指す issue がカンバンに載っていません"},
