...
// TestQuota_枠を読めなければ新規のdispatchも止める は、入札の判定と新規着手の判定が
// 1つに揃っていることを確かめる（設計 3-77j）。
//
// 目的: **枠を読めないとき、入札は「黙る」、新規 dispatch は「止めない」で
// 逆を向いていた。**入札が先に効くので後ろは一度も効かず、
// **ボードが1件も進まないのに出るのは `Debug` の1行だけ**だった。
// **「分からないなら止まる」に揃えたことを示す。**
//
// 与える情報: usage API が 500 を返す（枠を読めない）。`Ready` の issue が1件。
// 成功条件: 新規の dispatch が1件も起きないこと。
func TestQuota_枠を読めなければ新規のdispatchも止める(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
	...
	for _, v := range fx.Orc.RunViews() {
		if v.Identifier == "octocat/hello-world#190" {
			t.Fatalf("枠を読めないのに新規を dispatch している: %+v", v)
		}
	}
	// **止まったことが人間に見えなければ、直したことにならない。**
	// **これが `Debug` の1行だけだったので、ボードが何時間も進まない理由を
	// 誰も説明できなかった。**
	assertNewWorkBlockedLine(t, fx, "枠を読めない")
}

// TestQuota_マージンのほうが先に効いて新規着手が止まる は、実際の閾値が
// `rate_limit.pause_above_percent` ではないことを確かめる（設計 3-77j）。
//
// 目的: **新規着手が止まる本当の使用率は `100 − マージン` である。**
// 既定（`pause_above_percent: 95` / マージン10）では **90%** であって 95% ではない。
// **この違いを検査で固定しないと、「95% まで動くはず」という思い込みが実装へ戻ってくる。**
//
// 与える情報: 枠が 92%（`pause_above_percent` の 95 には届いていない）。
// マージンは既定の 10（＝余裕値は `100 − 92 − 10 = −2` でマイナス）。`Ready` の issue が1件。
// 成功条件: 新規の dispatch が1件も起きないこと。
func TestQuota_マージンのほうが先に効いて新規着手が止まる(t *testing.T) {
	endpoint, _ := newUsageServer(t, []map[string]any{
		{"kind": "session", "percent": 92, "resets_at": nil, "severity": "normal"},
	})
	...
	for _, v := range fx.Orc.RunViews() {
		if v.Identifier == "octocat/hello-world#190" {
			t.Fatalf("余裕値がマイナスなのに新規を dispatch している: %+v", v)
		}
	}
	assertNewWorkBlockedLine(t, fx, "余裕値がマイナス")
	// **実際の閾値を出す。**`pause_above_percent` の 95 ではなく 90 である。
	if got := fx.Logs.String(); !strings.Contains(got, "新規着手が止まる使用率=90") {
		t.Fatalf("実際の閾値（90）をログに出していない:\n%s", got)
	}
}

// assertNewWorkBlockedLine は、新規着手を止めたことが `Info` で1行出ていることを見る。
//
// **理由も一緒に出ていること**を確かめる。理由が無いと、人間は次に何をすればよいか
// 分からない。
//
// t: 呼び出し元のテスト。
// fx: 対象の一式。
// reason: 出ているはずの理由（`handoff.SkipReason.String()` の値）。
func assertNewWorkBlockedLine(t *testing.T, fx *stubFixture, reason string) {
	...
	if !strings.Contains(got, reason) {
		t.Fatalf("止めた理由 %q を出していない:\n%s", reason, got)
	}
}

// TestQuota_source_noneなら枠を読めなくても新規に着手する は、逃げ道が残っていることを
// 確かめる（設計 3-27 / 3-77j）。
//
// 目的: **`rate_limit.source: none` は「読めなかった」ではない。**
// 運用者が「枠で判定しない」と決めた状態なので、そこで止めると
// **その機械は1件も処理しなくなる。**
//
// 与える情報: `rate_limit.source: none`。`Ready` の issue が1件。
// 成功条件: その issue が dispatch されること。
func TestQuota_source_noneなら枠を読めなくても新規に着手する(t *testing.T) {
	fx := newStubFixture(t, stubFixtureOptions{
		Mutate: func(cfg *config.Config) {
