...
//
// **免除の範囲を、綴りで数え上げずに種類で閉じていること**（設計 3-64f）。
// 「担当している作業そのもの」という宣言は**リポジトリ X に掛かっていて、操作の種類を1つも区別しない。**
// **閉じる文が無いと、リポジトリ X が相手なら merge も release も「関係がある→通す」へ倒れる。**
//
// **そのうえで、肯定の一覧から承認・取り込み・却下・close を1句で外していること。**
// **`gh pr review --approve` は、道具の形としては pull request へのコメントの投稿である。**
// **肯定の一覧に入ると「いま挙げた以外で」が種類の外へ出すので、閉じる文が届かない。**
//
// **名指しは3つであること。**既定の branch へ直に送る push と、branch や tag を消す push と、
...
func TestToolGate_担当しているissueを判定役へ渡す(t *testing.T) {
	...
	for _, want := range []string{
		...
		// **肯定の一覧から、承認・取り込み・却下・close を外していること。**
		// **`gh pr review --approve` は、道具の形としては pull request へのコメントの投稿である。**
		...
		"コメントの書き込みに数えない",
		// **push の肯定を、既定の branch 以外の branch へ限っていること。**
		...
		// **`HEAD:<別の branch 名>` は肯定の側に残る。**2本目の pull request を出すときに要る。
		"そしてリポジトリ octocat/hello-world の、既定の branch（`main`）以外の branch への push は",
		// **免除の範囲を、肯定した形そのもので閉じていること。**
		// 「担当している作業そのもの」という宣言は**リポジトリ X に掛かっていて、操作の種類を1つも区別しない。**
		// **閉じる文が無いと、リポジトリ X が相手なら merge も release も「関係がある→通す」へ倒れる。**
		//
		// **種類で閉じる形は採らない。**承認・却下・close は、その瞬間にコードも配布物も設定も
		// 1バイトも変えないので、**どの種類にも当たらない。**
		"リポジトリ octocat/hello-world への書き込みのうち、いま挙げた形に当たらないものは",
		"担当先であることを理由に「関係がある」として通してはならない",
		// **ref を消す push を、肯定した形から外していること。**「への push」に当たると読めるためである。
		// **断る条件の1つ目が挙げているのは force push だけで、ref を消す push は入っていない。**
		// **並行して走っている別の run の branch を remote から消せると、その成果が別の機械から見えなくなる。**
		"branch や tag を消す push",
