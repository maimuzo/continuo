...
func TestToolGate_担当しているissueを判定役へ渡す(t *testing.T) {
	...
	for _, want := range []string{
		...
		//
		// **カンバンも閉じる。**GitHub Projects v2 のカンバンはリポジトリではないので、
		// 「リポジトリの設定」だけでは `updateProjectV2Field` に届かない。
		// **あれは選択肢を全件置き換えるので、設定済みの Status の値が全部消える。**
		"いま挙げた以外でコードや配布物を変える操作と、リポジトリやカンバンの設定を変える操作には及ばず",
		// **送り先を名指しする push を、免除から外していること。**
		// **この文は「リポジトリ X への push は当たらない」と、送り先も操作の向きも限定していない。**
		...
		// **ref を消す push も外していること。**断る条件の1つ目が挙げているのは force push だけで、
		// **ref を消す push は入っていない。**
