...
func TestTurn_blockedで引き渡すときサブエージェントの記録も案内する(t *testing.T) {
	...
	for _, want := range []string{
		...
		// **この issue は RepoIsPrivate を立てていないので、公開として扱われる。**
		// 公開リポジトリでは「コメントで許可を書く」を案内しない。
		// 判定役が読む会話には issue のコメントが載るので、
		// **その案内を公開の場所へ書くと、読んだ第三者が同じ文を書けるためである。**
		"auto",
	...
	// **公開リポジトリへ攻略法を書かない**（issue #259）。
	// この文面は issue のコメントとして投稿されるので、公開なら誰でも読める。
	if strings.Contains(body, "コメントに「その操作を許可します」と書いてください") {
		t.Errorf("公開リポジトリの引き渡しに、コメントで許可を出す案内が入っている:\n%s", body)
	}
}
