...
// redactWarning は、home を引けずに縮められなかったときのログの目印である
// （internal/orchestrator/comment.go の `postComment`）。
const redactWarning = "手元の絶対パスを縮められませんでした"
...
// TestHandoff_homeを引けなければ警告を1行残す は、
// **縮められなかったことが、あとから辿れる形で残る**ことを確かめる。
//
// 目的: Unix の `os.UserHomeDir` は `$HOME` しか見ない。**環境を絞って起こされると
// home が引けず、絶対パスがそのまま公開の issue へ出る。**
// **issue のコメントは編集履歴が残るので取り消せない。**黙って素通りさせてはならない。
// 与える情報: `HOME` を空にした状態で1周させた Tick。
// 成功条件: 警告が1行出ること。**投稿そのものは止まらないこと**（止めると人間は
// 「なぜ止まったのか」を知る手立てを失う）。
func TestHandoff_homeを引けなければ警告を1行残す(t *testing.T) {
	fx := newFixture(t, fixtureOptions{Mutate: func(cfg *config.Config) {
		cfg.Agent.MaxDispatchTurns = 1
	}})
	// **newFixture のあとに空にする。**先に空にすると、テスト用のリポジトリを作る
	// git が利用者の設定を読めなくなる。
	t.Setenv("HOME", "")
	fx.AllowLog(redactWarning)
	...
	waitFor(t, 15*time.Second, "引き渡しの通知が投稿される", func() bool {
		for _, c := range fx.Tracker.CommentsOf("I_node195") {
			if strings.Contains(c.Body, "人間へ引き渡しました") {
				return true
			}
		}
	...
	if !strings.Contains(fx.Logs.String(), redactWarning) {
		t.Fatalf("縮められなかったことが警告として残っていない:\n%s", fx.Logs.String())
	}
}
