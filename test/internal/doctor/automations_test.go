...
//
// 与える情報: 有効な自動化を2件持つ偽カンバンと、対応表が空の既定の設定。
...
// 外れたときに静かに取りこぼす。
//
// 与える情報: `Auto-add` で始まる自動化だけを有効にした偽カンバンと、対応表が空の既定の設定。
...
// 与える情報: `Item added to project` だけを有効にした偽カンバンと、対応表が空の既定の設定。
...
// 与える情報: 自動化を切ってある偽カンバンと、対応表が空の既定の設定。
// 成功条件: `自動化` が `✓` で、説明が「1つも有効ではありません」であること。
...
// 与える情報: 有効な自動化を1件持つ偽カンバンと、`Ice Box` → `Ready` の対応表。
...
// 与える情報: `workflows` を応答から落とした偽カンバン。
