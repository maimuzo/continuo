...
func TestTemplate_雛形はblockedを出す前にもpushを求める(t *testing.T) {
	...
	if len(found) == 0 {
		t.Fatalf("雛形の本文に push を求める文がありません（%q を含む行が1行もない）。"+
			"人間へ渡す前に push させないと、worktree の片付けで作業が失われます", pushDemandMarker)
	}
	if len(found) > 1 {
		t.Fatalf("雛形の本文に push を求める文が %d 行あります。"+
	...
	if !strings.Contains(body, pushCommand) {
		t.Errorf("雛形の本文に push のコマンド %q がありません。"+
