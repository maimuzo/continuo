...
func TestRenderData_返す名前がSampleDataと一致する(t *testing.T) {
	got := prompt.RenderData(tracker.Issue{}, nil, 0, false, "/usr/local/bin/continuo", "<!-- continuo:self -->")
...
func TestRenderData_渡した値がそのまま変数になる(t *testing.T) {
	...
	// 1回目（attempt は nil）。
	got := prompt.RenderData(issue, nil, 3600000, false, "/usr/local/bin/continuo", "<!-- continuo:self -->")
	if got["attempt"] != nil {
	...
	// やり直し（attempt に回数が入る）。
	n := 3
	got = prompt.RenderData(issue, &n, 60000, false, "/usr/local/bin/continuo", "<!-- continuo:self -->")
	if got["attempt"] != 3 {
...
func TestRenderData_組み込みの文面を実際に展開できる(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			out, err := frag.Render(prompt.RenderData(issue, c.attempt, 3600000, false, "/usr/local/bin/continuo", "<!-- continuo:self -->"))
			if err != nil {
