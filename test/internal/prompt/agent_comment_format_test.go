...
func TestTemplate_組み込みのプロンプトはコメントの節の形を決めている(t *testing.T) {
	...
	// **計画のコメント（3-2）を対象に含める。**
	// **図を書かせるのも、前提を書かせるのも、同じ計画のコメントである。**
	// 対象から落ちると、図だけが入って前提の無いコメントが届く。
	if !strings.Contains(section, "計画                       3-2（issue へ）") {
		t.Errorf("%q の節が、計画のコメントを対象に含めていません。"+
			"図を書かせるのと同じコメントなので、落とすと前提の無い図が届きます", commentFormatHeading)
	}
