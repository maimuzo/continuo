...
func TestTemplate_組み込みのプロンプトはコメントの節の形を決めている(t *testing.T) {
	...
	//
	// **`sectionUntilNextChapter` は `# ` でも `## ` でも切る。**
	// **いまは `sectionOf` でも同じ範囲になる。**5-5 の次が `## 5-6.` だからである
	// （[test/internal/prompt/ai_marker_test.go](ai_marker_test.go) は `sectionOf` を使っている）。
	// **同じままにしておくのは、5-5 が 6章の直前へ戻ったときに備えるためである。**
	// そのとき `sectionOf` は `# 6. セキュリティ` を越えて `## 6-1.` まで拾う。
