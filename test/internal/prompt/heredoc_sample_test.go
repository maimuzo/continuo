...
// 目的: 本文に終わりの語だけの行を作るな、という注意が、自由に書く本文を持つ見本の節に在ることを固定する（設計 5-3t）。
//
// **なぜ要るか。**本文に `PLAN` だけの行があると、そこで本文が切れ、後ろの行がシェルのコマンドとして実行される。
// **見本を囲みごと引用すると入る。**continuo 自身を直すエージェントは、この指示書の見本を読み、判断票で引く。
// 3-5・3-6・3-7 は「3-2 と同じ理由です」で 3-2 を指すので、3-2 に在れば届く。
// 7-2 の「そこを変えた理由」「前に書いていない分」も自由に書く行なので、7-2 にも要る。
//
// 与える情報: prompt.Builtin() の 3-2 と 7-2 の節。
// 成功条件: 両方の節に、終わりの語を変えよという注意があること。
func TestTemplate_本文に終わりの語だけの行を作らせない(t *testing.T) {
	body := prompt.Builtin()
	for _, heading := range []string{
		...
		if !strings.Contains(section, "だけの行を作らないでください") {
			t.Errorf("%s に、本文に終わりの語だけの行を作るなという注意がありません", heading)
		}
	}
}
