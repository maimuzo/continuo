// 組み込みの指示書が見せるシェルの見本の形の検査である（設計 5-3s）。
//
...
// 目的: ヒアドキュメントの見本が、全部コード囲みの中で、行頭の終わりの行で閉じることを固定する（設計 5-3s）。
//
...
// 目的: 組み込みの指示書が、PR の題名を二重引用符の中へ直に書かせないことを固定する（設計 5-3s）。
//
...
// 目的: 組み込みの指示書が、本文のファイルを worktree の中に作らせないことを固定する（設計 5-3s）。
//
