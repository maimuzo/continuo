...
func TestTemplate_blockedのissueには直したという記録を書かせない(t *testing.T) {
	...
	for _, want := range []struct {
		...
		{"どこまで見たか", "`blocked` のときに書かせる本文が無いと、" +
			"エージェントは自分で書式を作ることになり、どこで止まったのかが読み取れなくなります"},
		{"なぜ止まったか", "止まった理由を書かせないと、" +
			"人間はその issue を開いても、何を決めればよいのかが分かりません"},
		// **先頭の1文は共通である。**分けると、`blocked` で書いた issue が
		// あとから `review` になったとき、**行を足すだけの段2a では先頭が嘘のまま残る。**
		{"**「まとめて直しました」や「直していません」を先頭に書かないでください。**",
			"先頭を表明ごとに分けると、逆の表明で行を足したときに先頭だけが嘘になります"},
		...
		// **`review` の側の行も名指しで守る。**`blocked` だけを守ると非対称になり、
		// **表の `review` の行を消しても、段2b の本文が `pull request: ` を持つので通ってしまう。**
		{"（pull request: <PR の URL>）",
			"書き足す行に pull request の URL を入れさせないと、読む人はどの変更の話かを辿れません"},
		// **読めなかったときに前の分を書き直させない。**書き直させると同じ話が2回並び、
...
func TestTemplate_書き換えは印を確かめる門の中で行わせる(t *testing.T) {
	...
	// **門は2つある**（読む塊と、書き換える塊）。**どちらも印を確かめてから動く。**
	// 読む塊の門を外すと、`gh api` が返したエラーの JSON を「前の本文」として見せるので、
	// **エージェントは「前に何も書いていない」と読み、次の書き換えで全文を足す。**
	if n := strings.Count(section, `*"`+groupMarker+`"*)`); n < 2 {
		t.Errorf("%q の節に %q の枝が %d 個しかありません。2個以上にしてください"+
			"（前の本文を読む塊と、書き換える塊の両方が、印を確かめてから動くこと）。"+
			"**枝を増やす直しは止めない。**減らす直しだけを止める",
...
func TestTemplate_グループの成果報告は対象を絞りStatus名を書かない(t *testing.T) {
	...
	if n := strings.Count(section, "段2b で新しく1件投稿します"); n < 5 {
		t.Errorf("%q の節に %q が %d 個しかありません。5個以上のはずです"+
			"（段2a の echo が4つと、段2b の条件文の引用が1つ）。"+
			"合図の文言がずれると、エージェントは段2b へ行く合図を受け取れません。"+
			"**分岐を増やす直しは止めない。**文言をずらす直しだけを止める",
