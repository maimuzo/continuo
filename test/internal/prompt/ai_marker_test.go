...
func TestTemplate_印の抜けを自分で点検させる(t *testing.T) {
	...
	for _, want := range []struct {
		...
		{"抜けを出す検索", `test("(?m)^` + config.AIMarker + `")) | not)`},
		...
		{"改行の綴りを合わせる", `if ($h | test("\r\n$")) then "\r\n"`},
		// **過去分は放置する**（2026-09-06、人間の決定）。
		{"前の run は直さない", "前の run のコメントは直しません"},
		// **二重に付けない。**行頭が印で始まる行が1つでもあれば何もしない。
		{"二重に付けない守り", `if ($h | test("(?m)^` + config.AIMarker + `")) then .`},
		// **先頭に並ぶ印の中だけを見る。**本文の途中で引用しただけの報告を飛ばさない。
		{"先頭の並びだけを見る", `capture("^(?<h>((<!--[^\n]*-->[^\n]*|[ \t]*)`},
		// **飛ばす断りの例外を、締めでも言い直す。**
		{"飛ばす断りの例外の再掲", "点検のときも付けないでください"},
