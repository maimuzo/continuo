...
// 目的: 印の抜けを、エージェント自身に直させることを固定する（設計 3-82e。issue #245）。
//
// **continuo はこの印を1つも数えていない。**落としても何も止まらないので、
// **落ちたことに気づけるのはエージェント自身だけである。**
//
// **手順は決めない。**`jq` の手順を置いたら、実装レビューの3周ともそこから Critical が出た。
// **見るのは、守ることの3点と例外の1点だけである。**
// **叩けないコマンドを載せると、エージェントは自分で書き足す。**そこに守りが1つも無い形を書く。
//
// 与える情報: prompt.Builtin() の全文。
// 成功条件: 節があり、守ることの3点と、飛ばす断りの例外が書いてあること。
func TestTemplate_印の抜けを自分で直させる(t *testing.T) {
	body := prompt.Builtin()
	for _, want := range []struct {
		what  string
		piece string
	}{
		{"節そのもの", "### 印を落としたら、自分で直します"},
		{"id の調べ方", "#issuecomment-"},
		// **確かめずに書くと、取得に失敗したときのエラーの JSON が本文になる。**
		{"書く前に確かめる", "で始まっていることを確かめてから書く"},
		// **既にある印を1つも動かさない。**
		{"置く場所", "先頭に並ぶ印の、いちばん後ろへ入れる"},
		// **過去分は放置する**（2026-09-06、人間の決定）。
		{"この run だけ直す", "この run で自分が投稿したものだけ"},
		// **飛ばす断りには付けない**（設計 3-82c）。
		{"飛ばす断りの例外", "の断りには、ここでも付けません"},
	} {
		if !strings.Contains(body, want.piece) {
			t.Errorf("印を直させる節に%sがありません: %q", want.what, want.piece)
		}
	}
}
