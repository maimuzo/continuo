...
//
// **`jq` の手順は置かない。**4度目の突き合わせで敵対的レビュワーが「いらない」と判定した。
// **拾える集合が `IsAgent` の集合と定義上一致し、人間ができる推論が1つも増えない。**
// **守るのは3点だけである。**
//
// 与える情報: prompt.Builtin() の全文。
// 成功条件: 節があり、この run だけを直すことと、飛ばす断りの例外が書いてあること。
func TestTemplate_印の抜けを自分で直させる(t *testing.T) {
	...
	for _, want := range []struct {
		...
		{"直す手立て", "gh api --method PATCH"},
		// **過去分は放置する**（2026-09-06、人間の決定）。
		{"この run だけ直す", "この run で自分が投稿したものだけです"},
