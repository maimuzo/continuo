...
//
// **落とせるのは、名前で書かれた形である。**次の書き方を構文木から名前で見つけて落とす。
//
...
//	os.OpenFile の flag の数値      … 引数に直接書いた数値と、名前の定数に混ぜた数値
//
// どれも書いている途中で落ちると、元の内容が失われる。
//
// **数値へ逃がされた形は落ちないことがある。**この検査は構文木しか見ず、式の型を解かない。
// 次の書き方は素通りする。**塞ぐには型を解く仕組みが要るので、別の issue に切り出してある。**
//
//	名前付き定数へ退避する（`const truncWrite = 0x601` を flag に渡す）
//	`|=` で足す（`flags |= 0x400`）
//	関数値に入れて呼ぶ / 数値を返す関数を flag に渡す
//	os.Remove で消してから作り直す
//
// **`Truncate` は受け手の型を見ずに名前だけで落としている。**別の型の `Truncate` があれば
// 誤検知しうる。**いま `internal/` と `cmd/` に `.Truncate(` の呼び出しは1つも無い**ので、
// 実際には起きていない。受け手の型を見る仕組みも、同じ別の issue に切り出してある。
//
